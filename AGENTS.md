...
- **제목을 바꾸면 답변도 같이 바꿔야 한다 — 가드가 그 둘을 묶어 놨다**(2026-09-08): `aeoAnswer.test.js`는 **제목 콜론 앞의 핵심어가 `seoAnswer`에 실제로 있는지**를 발행 글 전체에서 파생 검사한다. 검색어 재편으로 10편 제목을 바꿔 보니 그중 9편이 여기서 걸렸다(예: "전환율 개선"으로 갈았는데 답변엔 "퍼널 이탈"만 있음). 우연이 아니라 맞는 강제다 — 새 타겟 질의가 답변 블록에 없으면 그 질의로 인용되지 않는다. **제목 개정의 실제 범위는 "제목 ×2 로케일"이 아니라 "제목·h1·답변 ×2"**이고, 콜론 없는 제목은 제목 전체가 핵심어가 되므로 앞머리를 고정어로 두고 콜론을 넣을 것. 감사에서 뽑은 GSC 문구가 제목에서 빠지면 `contentRegistry.test.js`가 잡으니 그 문구는 description으로 옮긴다.

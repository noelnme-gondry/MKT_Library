...
- ✅ 검증 하네스: `npm run test:all` **328파일·2670 통과**(1 skipped) · eslint 0 errors/0 warnings · `next build` ✓ (2026-08-25 실측). **수치를 적을 땐 실제로 돌려서 적을 것**.
...
- ⚠ **공개 도구는 현재 20개**(2026-08-25 `ROUTES.filter(isRoutePublished)` 실측 — 표에 5-28이 빠져 있던 것도 이때 드러났다. 도구 수는 세지 말고 파생해 확인할 것). 승격 이력(PR #696): 5-18 안의 분석 다섯이 개별 도구로 승격되고 허브(5-18)가 `subtool`로 내려갔다. **`routeMap`의 `component`가 파일 이름이 아닌 경우가 생겼다** — `5-18-trend`의 `MarketingResponseTrend`는 파일이 없고 `PageClient`가 `routeId`로 디스패치한다. 라우트에서 컴포넌트 파일을 찾는 코드·테스트는 디스패치에서 파생할 것.

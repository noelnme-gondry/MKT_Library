...
| 5-3 | 예산 배분 시뮬레이터 (절대 CPR/ROAS 가중 + 한계효용 그리디 — 그리디는 지금 배분 출발안과 비교) | 효율 CSV 공유 |
...
- **고정 상수로 환산한 금액은 고지가 없으면 거짓 숫자다**(2026-09-01): `USD_KRW_RATE = 1400`이 어디에도 고지되지 않았고, 5-18 예측은 그 환율로 예산을 되돌려 **배분 제약에 고정 환율이 들어갔다**. "통화" 토글도 두 의미(5-2·5-18 **환산** / 5-4·5-23 **단위 선언**)로 갈려 있었다. 지금은 `ds/FixedRateNote`가 환산이 실제로 일어날 때만 고지하고, 선언 전용 토글은 `data-currency-scope="declare"`를 **DOM에** 남긴다. 가드는 **"통화를 바꾸는 컨트롤을 가진 파일"** 에서 파생한다(환산은 형제 탭에서 일어나 호출부로 세면 토글 바가 빠진다).
...
- **데모 픽스처가 표준키를 헤더로 쓰면 자동매핑은 검사된 적이 없다**(2026-08-18): 5-27을 실제 App Store Connect export로 올리니 필수 4개 중 2개가 `__ignore__`였는데, 픽스처 헤더가 표준키(`source`·`installs`) 그대로라 스모크·골든이 전부 통과했다. **자기가 만든 헤더로만 검사하면 실사용 CSV의 실패는 영원히 안 보인다** — 새 도구는 실제 플랫폼 export 컬럼명으로 `buildMappingContract`를 돌리는 케이스를 함께 둘 것(`storeConsoleMapping.test.js`).
...
- **커버리지 가드가 손으로 쓴 배열을 돌면 가드가 아니다 — 그리고 고친 파일 옆에서 그대로 재발한다**: `toolOg.test.js`를 파생으로 고친 뒤에도 옆의 `routeSeo.test.js`가 같은 형태로 같은 두 도구(5-25·5-26)를 놓치고 있었고, 파생(`ROUTES.filter(isRouteIndexable)`)으로 바꾸자 검사된 적 없던 초과 2건이 더 나왔다. **교훈을 적용할 땐 같은 패턴의 파일을 전부 grep해 한 번에 고칠 것**, "완료" 문구도 적기 전에 grep으로 셀 것.
...
- **숫자 표기 해석은 `utils/parseNumeric` 하나가 소유한다 — 폴백만 소비처가 고른다**(2026-09-16 감사): 엔진 13곳이 각자 콤마를 벗겨 같은 셀이 도구마다 다른 숫자가 됐다(`"1.000,25"`가 5-2에서 0, 5-21·5-3에서 1.00025 — 1,000배 축소가 조용히 통과). 해석은 공유하되 **폴백은 통일하지 말 것**(strict=null · orZero=0 · orNaN). `parseNumericSingleSource.test.js`가 소스에서 파생해 막고(주석 먼저 제거), 예외는 `PARSE_NUMERIC_EXEMPT` 표식+사유로만.
...
- **엔진만 읽고 "화면이 말하지 않는다"고 단정하면 오탐이 난다 — 한 세션에 2회**(2026-09-17 자기 정정): ① 중복 매핑이 무경고라 했는데 `CsvUploader`가 이미 분석 CTA를 막고 있었다(재현 스크립트가 UI 게이트를 우회) ② 모드 C가 곡선을 무시한다 했는데 `getAllocationEvidenceLimits`가 관측 최대·∩형 꼭짓점으로 상한을 건다. **틀린 진단으로 화면 문구까지 배포했다**(§8). 결함을 보고하기 전에 값이 **화면에 닿는 경로를 끝까지**(호출부·게이트·인자 출처) 따라갈 것.
...
- **부호 하나로 문장을 고르면 같은 부호의 다른 경로에서 거짓이 된다**(2026-09-17): `mix = (cpāᵢ − C̄)·Δs`의 양수 경로는 (비싼 것 × 비중↑)과 (싼 것 × 비중↓) 둘인데 `classifyNarrative`가 부호 하나로 "비중이 늘었고 비싼 편"을 고정해 후자에서 **방향·가격 둘 다 반대**로 썼다. 곱으로 만든 지표의 서술은 **두 축에서 각각** 파생할 것. `simpsonParadox.test.js`가 네 사분면을 돌고 반대말이 들어가면 실패시킨다.
...
- **"계산 불가"를 좋은 등급으로 접지 말 것**(같은 감사): `creativeMath.vif`는 적합 실패를 `r2=0`으로 떨어뜨려 **VIF=1(완전히 깨끗)**로 표시했다 — 식별 불가를 문제 없음으로 뒤집는 방향의 실패다. `satMath.classify`도 `satIndex==null`(미상)을 "포화"로 단정했다. 미상은 미상 버킷(null)으로. 거울상도 있다: 결과 작업대 VIF 그림이 ∞(완전 공선 = 가장 심각)를 JSON 안전 변환(`safeNumber`)에서 null로 잃어 **"계산 불가"로** 그렸다 — ∞는 `isInfinite`로 따로 싣는다.
...
- **같은 이름의 엔진이 두 벌이면 통합이 아니라 분기 감지가 답일 수 있다 — 먼저 "안전 추출 가능분"을 실측할 것**(2026-09-01): `mmmMath.js`와 동결 스냅샷 `mmmMathPr416.js`(classic·prism)는 핵심 알고리즘 17개가 의도적으로 갈려 있고 안전 공유분은 225/6,718줄뿐이었다. 통합 대신 `mmmEngineDivergence.test.js`가 공유 심볼의 한쪽-만-변경을 감지하고, 의도적 포크는 **사유와 함께** 목록에 올린다(목록 항목이 실제로 두 파일에 있고 다른지도 단언 — 낡은 예외는 예외가 아니다).
...
**렌더·UI**
- **디자인 규칙은 CSS가 아니라 렌더된 박스로 지킨다**(2026-09-24): "AI 티" 다섯 증상(글자 겹침·상자 안 상자·형제 가장자리 어긋남·제목 위 작은 라벨·굵기 난립, + 왼쪽 색 막대 안내 상자)을 `e2e/design-rules.spec.js`가 정적 화면과 **예시 결과 화면**을 3개 폭에서 잰다. 첫 실행에서 리뷰로 안 보이던 것이 나왔다(가이드 15편이 폰에서 본문 28px 폭 · 늘 떠 있는 absolute 팝오버 등). 스캐너는 줄 단위(`getClientRects`)·잘림 교차·고정 요소 분리로 거짓 겹침을 피한다. 예외는 `data-design-exempt="규칙: 사유"` DOM 표식으로만.
- **AI 티 두 번째 묶음은 "고정된 것이 떠 보이는" 문제였다**(2026-09-24): 고정 표면의 그림자·장식 그라디언트·hover 들림·누를 수 없는 알약·토큰 밖 글자색을 `e2e/support/designRules.js`(렌더)와 `app/hoverLift.test.js`(선언)가 막는다. **그라디언트를 단색으로 바꾸면 상자 안 상자가 새로 생긴다** — 장식 바탕은 `transparent`로. 빈 상태는 **빈 목록만** 줄일 것(작업 탭까지 걷었다가 `주간 성과 비교` 입구가 막혔다).
...
- **분석이 다르면 그림도 달라야 한다 — 같은 막대 하나로 그리면 "무슨 분석인지"가 사라진다**(2026-09-25): 결과 작업대가 PVM·포화도·예산·VIF·소재 피로도·생존을 전부 같은 가로 막대로 그렸다. 지금은 그림 사양(질문·variant)을 `lib/assistant/coreFigures.js`가 소유하고 어댑터와 도구 화면(`ToolCoreFigure`)이 **같은 사양·같은 부품**(`assistant/ResultCharts.jsx`)을 쓴다 — `visualizationVariants.test.js`가 variant 없는 막대를 막는다. 짝 교훈 셋: ① 샘플을 채널 하나로 좁히면 **채널을 비교하는 분석이 전부 "비교 대상 1개"로 빈다**(홈 미리보기·결과·주간 리뷰는 같은 범위, `sampleJourney.test.js`). ② 두 값을 비교할 땐 **같은 모형에서 나온 값끼리**(5-3 "지금 배분"도 `predictSafeCpr`로 예측) — 그렇게 비교하자 0원부터 쌓는 그리디가 지금보다 **나쁜** 안을 내는 경우가 드러났다(샘플 8,782→6,072건). `calculateAllocationModeB({currentSpends})`가 지금 배분에서 출발해 합계가 늘어나는 이동만 받은 안(`improveAllocationFromCurrent`, 구성상 지금보다 나빠질 수 없다)과 비교해 많은 쪽을 돌려주며, 결과 화면·도구 화면·frontier·What-if가 같은 경로를 쓴다. 상한은 그리디와 같게(∩형 꼭짓점 뒤는 단가가 고정돼 선형 이득처럼 보인다). ③ 범위 밖 이름을 돌려주는 헬퍼(`return fallback` in 다른 함수)는 **값이 빈 행에서만** ReferenceError로 터진다 — 빈 성분 픽스처를 둘 것. ④ HTML 그림의 PNG는 `utils/figureImage`(계산 스타일 인라인 → SVG foreignObject → canvas, 라이브러리 없음)가 만든다 — 이미지 안은 웹 글꼴 대신 대체 글꼴이라 **글자만 담은 요소에 페이지에서 잰 폭을 박으면 이름이 접혀 겹친다**(폭 속성은 빼고 막대 같은 빈 요소만 유지). 실제 PNG를 열어 봐야 보인다(`e2e/tool-core-figure-download.spec.js`, `FIGURE_SAVE_DIR`).
...
- **결과 옆에 신뢰도·방법을 늘어놓지 말 것 — 문제가 있을 때만 빨간 `!` 하나**(2026-09-24 사용자 결정): 분석 범위·신뢰도·실행 정보·주의 배지·/start 승인 단계를 사용자가 "쓸 수 없는 허들"로 지목했다. 지금은 `ds/IssueMark`가 확인할 점이 있을 때만 뜨고(누르면 어느 기간·열·칸인지), 재현 정보는 XLSX `00_README`·`04_SCOPE`가 소유한다. 그림 이모지(상태 기호 ✓ ⚠ ↗ ✕는 허용)·CSS 대문자·숫자 고정폭도 금지. **예시만으로 결론까지 안 가는 도구는 실제 입력을 밟는 결과 스캔을 둘 것.**
...
- **컨테이너에 건 font-size는 요소 셀렉터에 진다**(2026-09-03): 전역 `p { font-size:14px }`(0,0,1)가 `.blog-prose { font-size:17px }`의 **상속**을 이겨 **문단 14px vs 목록 17px**이 됐다(테스트 2876개 통과, CSS만 보면 정상). **크기를 컨테이너에만 걸었으면 그 안의 요소 셀렉터 전역 규칙을 함께 확인할 것.** `app/proseTypeScale.test.js`가 전역 규칙의 존재와 `inherit` 복구를 함께 단언한다(근거가 사라지면 예외도 무너지게).
- **"복잡하다"는 지적에 접는 것으로 답하지 말 것**(2026-09-05): 사이드바 자동 접힘을 넣었더니 "닫히기만 하고 바뀐 게 없다"는 답이 왔다. 감추는 것은 치우는 것이 아니다 — **접는 시점은 사용자가 정하고, 내용은 실제로 덜어낸다**. 실측하니 탐색 축이 넷 공존했고 영문 대문자 부제는 아무 역할도 없었으며, 랜딩에서 같은 라벨을 고칠 때 사이드바를 grep하지 않은 것이 원인이었다.
...
- **차트 축이 무엇인지 안 말하면 눈금은 그냥 숫자다**(2026-09-15): 차트를 그리는 파일 28개 중 13개가 축 제목 0건이었고 하필 5-2 대시보드 탭 전부였다(형제 5-3·MMM은 이미 패턴이 있었다 — 형제를 안 보고 만든 자리는 설계 결정처럼 보이지만 누락이다). `app/chartAxisTitle.test.js`가 `scales:` 선언 파일에서 파생해 강제하고, 예외는 `CHART_AXIS_TITLE_EXEMPT` 표식 + 사유로만.
...
- **판정을 흐림(opacity) 하나로만 표시하면 다른 파일이 취소했을 때 판정이 통째로 사라진다**(2026-09-16): `/start`의 `is-dim`을 `library-workspace.css`(특이도 0,3,0)가 `opacity:1`로 되돌려 **20개 중 5개만 가능한데 화면 차이가 0**이었다. 취소 규칙의 사유 주석도 사실이 아니었다 — **사유 주석이 사실인지도 확인할 것**. 지금은 글자가 말하고 `app/toolIndexAvailability.test.js`가 두 파일을 함께 읽는다. 측정이 세 번 틀렸고 셋 다 실제 파일·DOM을 다시 열어서야 잡혔다.
...
- **semantic 토큰에 라이트 오버라이드가 있어도 raw hex를 쓰면 소용없다**(2026-08 감사): 다크 전용 리터럴 51곳이 라이트에서 대비 1.9:1이었고, 바로 옆 규칙은 토큰을 쓰고 있었다. `ds/`는 다른 도구가 베끼는 기준이라 전파된다. 차트는 `CHART_THEME.colors`·`.danger`·`.warning`·`.success` getter(리터럴 hex라 `+ "AA"` 알파 접합 가능, canvas는 `var()`를 못 읽으므로 **데이터셋엔 반드시 getter**). remap Map은 **각 토큰의 다크·라이트 값 + 과거 리터럴을 전부 키로** 가져야 한다.
...
- **초안을 병합한 레코드를 이력의 기준으로 쓰면 아직 관측이 아닌 값이 관측으로 굳는다**(2026-09-16, v11): `WeeklyReview`의 `records`는 초안(`drafts`)을 병합한 값이라, 구세대 폴백이 입력 중인 `actual`을 기존 관측으로 오인해 두 번 쌓았다. **이력의 기준은 저장된 레코드**(`storedRecordOf(id)`) — "값이 같으면 건너뛴다"는 정정 관측을 삼키므로 답이 아니다. 골든은 못 봤고 스모크가 잡았다(§6.1). 같은 기능의 두 경로가 서로 다른 소스를 읽으면 한쪽만 이 함정에 걸린다.
...
- **빌드 타임에 외부에서 받아오는 자산은 배포를 남의 네트워크에 건다**(2026-09-02): OG 카드용 Google Fonts 응답이 잘리면 satori가 `RangeError`로 **빌드 전체를 죽였다**(try/catch는 깨진 바이트를 못 잡는다). 지금은 저장소 폰트로 미리 그린 `public/og-card.png` 한 장. 30여 페이지가 가리키던 그 파일이 실제로는 없고 라우트가 대신 그리고 있었다 — **화면 밖에서만 드러나는 자산은 파일 실재와 규격을 테스트가 볼 것**(`app/ogCard.test.js`).
...
- **내부 링크는 앵커 모양이 아니라 연결성을 세야 한다**(2026-09-08): 앵커 텍스트 품질(전부 키워드 앵커)을 재고 "이미 됐다"고 보고했는데, 예산 클러스터는 글끼리 상호링크가 0건이었다 — 프록시를 재고 다른 속성을 말한 것이다. 형제·필라를 한 번도 안 가리키던 글이 25건이었고, 화면의 클러스터 링크 부품은 삭제돼 **내부 링크는 전적으로 본문이 진다**. `topicClusters.test.js`가 클러스터 정의에서 파생해 강제한다(링크 하나를 지워 실패 확인).
...
- **`docs/product-ssot.md` — 제품 계약 SSOT**(§2.12): 대외 사실·한계(F/L), 도구 카탈로그 정의(도구 수는 `publishedToolIds()` 파생 — 적지 말 것), UX·접근성 계약, 품질 게이트, 실측 부채 백로그(D-xx). 공개 카피·상태 문구·a11y 판단이 걸린 작업은 **여기 먼저**.
...
- `docs/aeo-prompt-checklist.md` — **생성물**. AEO 측정용 프롬프트 목록(KO/EN 같은 수 — 수는 생성 결과에서 읽을 것). 손으로 고치지 말고 `node scripts/aeo-prompts.mjs`로 재생성 — 원본은 `toolSearchContent`의 `question`/`answer`와 `compareContent`다. 월별 기록은 `docs/aeo-runs/`에 사본을 떠서 한다.
...
- ✅ **제품 계약 SSOT 신설**(2026-08-19) — `docs/product-ssot.md`. 외부 검토 4건을 코드 대조해 확정(제품명 `Growth Opt Playbook` 단일화 결정).

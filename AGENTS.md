...
- **지표 사슬 공식은 단위 배수를 숫자로 검산할 것 — 눈으로 읽으면 1,000배가 조용히 빠진다**(2026-08-17, 수정 완료): 발행된 용어집·블로그가 `CPC = CPM ÷ CTR`, `CPA = CPM ÷ (CTR × CVR)`로 적고 있었다. CPM은 **1,000회** 노출 단가라 실제는 `(CPM ÷ 1,000) ÷ CTR`이고, 구 표기는 1,000배 틀렸다(CPM 5,000·CTR 1% → 500원인데 50만 원). 카피에 "≈"·"연결된다"를 붙여도 등식으로 읽히므로 **거짓 숫자(§8)**다. 옆 문단의 **예시 숫자는 처음부터 맞아서**(CPM 2,000·CTR 1% → CPC 200원) 공식과 예시가 서로를 반증하고 있었다 — 산문에 등식이 있으면 한 조합을 대입해 예시와 맞는지 확인할 것.
  - **`÷`는 멀티바이트라 `grep -E "[÷/]"` 문자클래스가 조용히 안 잡는다**: 바이트 단위 클래스로 쪼개져 KO 라인 4건을 놓쳤고, 그 결과 "6곳"이라 보고한 뒤 실제로는 7곳(`performance-marketing-metrics.md` 추가)이었다. 비ASCII 기호는 **교체(`(÷|/)`)로** 검색할 것. 최종 수정은 KO/EN 용어집·블로그 7곳.

...
- **스모크 테스트는 "이 컴포넌트가 렌더된다"를 검사하지 "이 컴포넌트가 쓰인다"를 검사하지 않는다**(2026-08-24 감사): PR #712가 `ToolAssistRail`을 KO/EN `PageClient` 양쪽에서 제거하면서 **같은 커밋에서 본체 122줄을 수정**했다. 413줄 컴포넌트와 자체 스모크가 그대로 남아 영원히 초록인데, 도구 화면의 "지난 결정 검토" 진입점은 사라졌다(`AnalysisEligibilityList`도 같은 커밋에서 같은 상태가 됐다). 테스트가 컴포넌트를 직접 `render()`하면 마운트 여부와 무관하다 — **화면에 있어야 하는 부품은 라우트 디스패치에서 파생한 마운트 검사를 따로 둘 것**(감사 스윕은 `docs/analysis-route-ux-audit.md` R3).
...
- `docs/analysis-route-ux-audit.md` — **유저 분석 루트 UX 감사**: 여정 단계(진입→업로드→매핑→게이트→결과→다음→재방문) × 이탈 조건 단위 방법론 + 재실행 스윕 R1~R7 + 실행 로그. 표면 단위인 `automateaudit.md`가 못 보는 **이음매** 결함(추천↔도구 상태 불일치·다음 단계 재업로드·죽은 부품)이 여기 잡힌다.
...
- ⚠ **공개 도구 수 19**(2026-08-24 실측: `routeMap`에서 preview·subtool·legacy 제외한 5-x·9-x. #696에서 18, #712의 5-28로 19): 5-18 안의 분석 다섯이 개별 도구로 승격되고 허브(5-18)가 `subtool`로 내려갔다. **`routeMap`의 `component`가 파일 이름이 아닌 경우가 생겼다** — `5-18-trend`의 `MarketingResponseTrend`는 파일이 없고 `PageClient`가 `routeId`로 디스패치한다. 라우트에서 컴포넌트 파일을 찾는 코드·테스트는 디스패치에서 파생할 것.

...
- **`hidden` 속성은 명시적 `display`를 이기지 못한다**(같은 날): 가드를 부러뜨리려 `<span hidden>`을 붙였는데 요소가 그대로 보였다 — `globals.css`의 `.dashboard-sticky-context__rows { display: inline-flex }`와 브라우저 기본 `[hidden] { display: none }`이 **특이도 동률(0,1,0)이라 소스 순서로 갈리기** 때문이다. 이 저장소엔 `[hidden]{display:none!important}` 보강이 없으므로, `el.hidden = true`로 숨기려는 자리는 클래스에 `display`가 걸려 있으면 전부 안 숨겨진다. 숨김이 필요하면 조건부 렌더로 빼거나 `display` 선언을 함께 거둘 것.

...
- **가드가 "지금 값"을 그대로 적으면 그 순간부터 버그를 지킨다**(2026-09-02): 말풍선 꼬리가 틀린 방향이었는데 스모크가 그 `rotate(-45deg)`를 단언하고 있었다. **근거**(왜 그 각도인가)를 적고 반대값을 함께 금지할 것. 정착 전 **중간 상태**를 단언하는 스모크도 DOM이 조금만 무거워지면 깨진다.
- **테스트 안의 `foo?.()`는 검사를 조용히 삭제한다**(2026-08-18): 없는 이름 `setAnalyzed?.()`로 게이트를 연 척해 결과가 안 그려졌고, 그 안의 조건부 단언까지 무효였다. 셋업이 상태를 바꿨으면 **그 사실을 단언**하고(`expect(isGroupAnalyzed(id)).toBe(true)`), `if` 안에 단언을 넣지 말 것.
...
- **목록에서 "빠진 것"과 "의도적으로 뺀 것"은 코드에서 구분돼야 한다**(2026-08-14 감사): `/start` 추천 목록이 한 도구는 명시 제외, 두 도구는 계약이 없어서 빠져 있었다(후자는 사고였다). 제외는 사유 달린 명시 제외로, 커버리지는 SSOT 파생 테스트로 고정할 것.
...
- **이름을 줄이면 검색이 같이 줄어든다**(2026-08-18): 도구 이름을 짧게 하자 ⌘K에서 "VIF"로 5-25가 안 나왔다(제목 일치 시 다른 검사를 건너뛰는 구조). seoTitle을 제목 코퍼스에 넣어 해결 — **표시 이름과 검색 코퍼스는 별개로**. 테스트는 이름을 적지 말고 레지스트리에서 뽑을 것(`toolIndexEntry(id).name`).
...
- **저장 레코드에 세대를 적고, 구세대는 파일을 살리되 굳은 해석만 버릴 것**(2026-09-17): IndexedDB 원본은 다시 파싱되지만 `mapping`은 저장 시점 어휘로 굳는다. `WORKSPACE_DATASET_SCHEMA_VERSION`이 구세대(표식 없음 포함)면 매핑을 버리고 `buildMappingContract`로 재인식하되, 0건이면 옛 매핑을 유지한다. 짝인 의미 바인딩(V2)도 함께 비운다.
...
- **절단 사후분포의 평균을 점예측으로 쓰면 적합선이 통째로 뜬다**(2026-08-15): 5-18 Bayesian이 `fitted`를 비음수 절단 draw의 평균으로 만들어 R² 0.74→−6.91이 됐다. 사후 draw는 불확실성 전용이고 점예측은 MAP. **탐지 신호: 학습 오차가 OOS 오차보다 나쁘면 적합값이 아니다** — 이 부등식을 골든으로 둘 것.
...
- **공용 프로파일러의 날짜·숫자 판정은 서로를 오염시킨다**(2026-08-25, 5-29): `new Date("1")`이 유효해 맨 정수 코드가 `date`로, `parseFloat("2026-07-01")=2026`이라 날짜가 `numericRate` 1.0으로 온다. 가르는 신호는 **값이 순수한 숫자 표기인가**뿐이다. 자유 텍스트 판정도 `.some(길다)`가 아니라 "대체로 길다"로(한글은 30자면 문장).
...
- **체험용 우회는 판정을 덮지 말고 판정 옆에 길만 낼 것**(2026-09-25): 샘플 모드가 모든 도구를 "되는 분석"에 넣어, 효율 도구 넷이 자동 매핑 충돌(`channel`·`source` 헤더가 둘 다 channel)로 **계산되지 않는** 버그가 안 보였다. 표준 필드명과 정확히 같은 헤더가 하나면 그게 이긴다(`resolveExactNameConflicts`, 둘 이상·0개면 사람이 고른다). 자격은 그대로 두고 "예시로 열기"만 붙였다.
- **분석이 다르면 그림도 달라야 한다**(2026-09-25): PVM·포화도·예산·VIF·피로도·생존을 같은 가로 막대로 그리면 무슨 분석인지가 사라진다. 그림 사양은 `lib/assistant/coreFigures.js`, 부품은 `assistant/ResultCharts.jsx` 하나를 결과 작업대·도구 화면(`ToolCoreFigure`)이 함께 쓴다(`visualizationVariants.test.js`). 짝 교훈: ① 샘플을 채널 하나로 좁히면 채널 비교 분석이 전부 빈다(`sampleJourney.test.js`) ② 두 값은 **같은 모형에서 나온 값끼리** 비교 — 그러자 0원부터 쌓는 그리디가 지금보다 나쁜 안을 내는 게 드러나, `calculateAllocationModeB({currentSpends})`가 지금 배분 출발안(`improveAllocationFromCurrent`)과 비교해 많은 쪽을 돌려준다 ③ 범위 밖 이름을 돌려주는 헬퍼는 **빈 행에서만** ReferenceError — 빈 성분 픽스처를 둘 것 ④ HTML 그림 PNG(`utils/figureImage`)는 대체 글꼴로 그려지므로 **글자 요소에 잰 폭을 박지 말 것** — 실제 PNG를 열어 봐야 보인다(`FIGURE_SAVE_DIR`).
- **결과가 나왔으면 매핑은 결과 위에 없어야 한다 — 접기가 아니라 이름 있는 편집 창으로**(2026-09-24): 폰에서 결론이 3,000px 아래였다. `ds/MappingEditorDialog`(한 줄 요약 + `데이터·매핑 편집`). 매핑을 바꾸면 게이트가 닫히므로 **창 표시를 게이트로 판정하지 말 것**(`isCollapsed = open || 분석됨`). 스캐너 부수 교훈: 전 쌍 비교는 글자 수천 개에서 150초(세로 스윕으로) · `<table>` 안 `.sr-only`는 폭이 안 먹어 가로 넘침(`div`로 감쌀 것) · `:is(> x)`는 무효라 규칙 전체가 조용히 버려진다.
...
- **버튼이 안 보이는 원인은 색이 아니라 캐스케이드일 수 있다**(2026-08-17): `.blog-prose a`(0,1,1)가 CTA 버튼(0,1,0)의 글자색을 배경과 같은 `--primary`로 덮었다 — 대비 계산기로는 못 잡는다. 컨테이너 요소 셀렉터는 `a:not([class])`로 원고 링크만 한정하고, `--primary` 배경 위 글자는 `--on-primary`로 통일(`app/buttonContrast.test.js`).
...
- **그리드 자식을 `display:none`으로 빼면 남은 자식이 첫 트랙(0px)으로 간다**(2026-09-04): 사이드바를 접자 본문 폭이 0이 됐고 단위 테스트·lint·build는 전부 통과, e2e만 잡았다. 접힌 상태엔 `grid-template-columns`를 다시 선언할 것. **레이아웃 변경은 박스 크기를 재는 검사가 있어야 잡힌다.**
- **하한 가드는 종류를 안 세고, 종류 가드는 하한을 안 본다 — 둘 다 필요하다**(2026-09-15): 12px 하한만 걸어 둔 사이 px 폰트가 27종→29종으로 늘었고, 스케일(`--fs-*`)은 이미 있는데 소비처가 3곳·경쟁 계열(`--type-*`)까지 있었다. **이름이 셋이면 스케일이 아니다** — 하나로 합치고 `app/typeScale.test.js`가 스케일 밖 px를 막는다.
...
- **위계를 "테두리 개수"로 세면 틀린 진단이 나온다**(자기 정정): `border` 145곳을 근거로 "위계 없음"이라 했는데 실제 최상위 표면은 결론 카드·본문 블록·마감 영역으로 위계가 있었다(145곳은 칩·입력·표 셀). 프록시가 아니라 실제 표면을 읽을 것 — 진짜 결함은 재질이 다른 소수 하나(`.dashboard-support-tools`)였다.
- **브레이크포인트를 필요할 때마다 새로 적으면 그 사이 폭은 아무도 안 본 화면이 된다**: 13종을 8종(480·560·640·720·768·860·1000·1100)으로 접고 `min-width`는 경계+1(`app/layoutScale.test.js`). `max-width:760px`은 태블릿 768px에 안 걸린다.
...
- **semantic 토큰에 라이트 오버라이드가 있어도 raw hex를 쓰면 소용없다**(2026-08 감사): 다크 전용 리터럴 51곳이 라이트에서 대비 1.9:1이었고, 바로 옆 규칙은 토큰을 쓰고 있었다. `ds/`는 다른 도구가 베끼는 기준이라 전파된다. 차트는 `CHART_THEME.colors`·`.danger`·`.warning`·`.success` getter(리터럴 hex라 `+ "AA"` 알파 접합 가능, canvas는 `var()`를 못 읽으므로 **데이터셋엔 반드시 getter**). remap Map은 **각 토큰의 다크·라이트 값 + 과거 리터럴을 전부 키로** 가져야 한다.
  ...
  - **역으로, 토큰화하면 안 되는 리터럴이 더 많다**: ① 브랜드색 ② **영구 다크 표면 위 텍스트**(`.sidebar`는 라이트 재정의가 일부러 없다 — 토큰화하면 다크 위 다크 글자) ③ Chart.js 데이터셋(`CHART_THEME`) ④ 토큰 없는 색과 짝지은 값. **정확히 같은 값의 토큰이 있는 순수 텍스트 색만** 치환할 것.
  - **`:root`에서 `var(--다른토큰)`으로만 선언된 별칭은 라이트 재정의가 닿지 않는다**(2026-09-16): 값이 `:root`에서 확정돼 원본을 밝게 바꿔도 별칭은 다크로 굳는다(최악 1.04:1). grep으론 안 보이고 별칭을 한 단계 따라가야 드러난다 — 사용처를 열어 보고 말할 것(`app/themeAliasTokens.test.js`).
...
- **`title` 단독은 어포던스가 아니다 — 커버리지는 셀렉터가 아니라 대상 수로 선언할 것**(2026-08-19): 좁은 셀렉터를 적고 "전부를 덮는다"고 쓴 순간 나머지가 안 보인다. 전부 금지하지는 않고 **title이 유일한 경로인 경우만** 잡는다(`app/titleAffordance.test.js`, 나머지는 `docs/product-ssot.md` D-04).
- **같은 부품이 구현 두 벌이면 한쪽만 고치고 끝난다**(2026-09-02): 말풍선 꼬리가 CSS 회전·SVG 두 벌이라 방향 수정이 한쪽을 비켜갔고, SVG는 `Z`로 닫혀 밑변 stroke가 테두리를 가로질렀다. 가드는 **디렉터리 전체에서 파생**할 것(`assistant/*` 꼬리 전수).
...
- **화면에 보이는지는 `checkVisibility()`로만 판정할 것**(2026-09-16): `getBoundingClientRect()`는 닫힌 `<details>` 안에서도 0이 아닌 박스를 준다 — 그 실측으로 "중복"을 지웠다가 유일하게 보이던 자리를 지워 되돌렸다(`e2e/data-shape-repetition.spec.js`, 0도 실패). 문구 스캐너 짝 함정: 텍스트 노드 단위는 JSX가 쪼갠 `{16}{"행"}`을 놓치고, `\b`는 한글 경계에서 성립하지 않는다.
- **"없다"를 말하기 전에 그 화면이 실제로 그리는 것을 읽을 것**(2026-09-16 감사): grep 하나로 "검토함이 검증 도구로 못 보낸다"고 보고했는데 바로 옆에서 `decisionReviewFollowUpMode`가 후속 안내를 렌더하고 있었다. `primary` CTA 11개도 정규식 오탐이었다. **의심은 전수 출력으로 확인한 뒤에 말할 것.**
...
- **"무관해 보이는" 변경에 테스트가 깨지면 이분법으로 좁힐 것 — 단독 통과는 무관의 증거가 아니다**(2026-09-16): ① 베이스라인 전체 ② 변경 전체 ③ 의심 파일만 원복한 전체, 세 번으로 판정한다. 원인을 못 밝혔고 실익이 작으면 변경을 버리는 편이 낫다.
...
- **JS가 벗겨 주기를 기다리는 숨김에는 브라우저 레벨 failsafe와 시간 예산을 함께 둘 것**(2026-09-05): 청크가 거부가 아니라 **지연**되면 `.catch`가 안 돌아 콘텐츠가 투명하게 남는다(로컬·CI 재현 불가). 숨김 규칙에 `animation: … forwards`로 최종 상태를 넣고, `MOTION.armBudget`(600ms)을 넘기면 연출을 버린다. **연출보다 콘텐츠.**
...
- **사람이 넣는 마커에 렌더를 걸면, 안 넣은 곳에서는 그 기능이 없는 것과 같다**(2026-09-03): 본문 중간 CTA가 마커 있는 14/48편에만 나왔다. 마커가 없으면 위치를 **파생**하고 발행 글 전체 가드가 강제한다. 짝: `X ? id : "5-2"` 같은 폴백은 **무엇이 폴백으로 떨어지는지 세는 검사**를 같이 둘 것.
...
- **매핑을 사용자에게 미루면 첫 화면이 폼이 된다**(2026-08-25, 5-29): 기본값은 **"다 잡아서 다 돌리고, 틀린 것만 고친다"** — 정직성은 ① 무엇으로 읽었는지 한 줄 ② 애매한 컬럼은 확인 목록 ③ 사용자가 손대면 자동 중단으로 지킨다. 계산이 이미 할 수 있는 일(여러 축 랭킹)을 UI가 막고 있지 않은지 먼저 볼 것.
...
- **서식은 글마다 갈리는 게 아니라 계약이 없어서 갈린다**(2026-08-18): 본문 `# 제목`(페이지 h1과 중복, 섹션 크기로 강등)·손번호 `2)`·볼드 소제목이 섞여 있었다. `blogFormatting.test.js`가 파일에서 파생해 강제한다.
- **KO 용어사전은 전문용어에서 이기고 기초 용어에서 진다**(GSC 가중평균 11.6위 vs 65.2위): 확장은 한국어 문서가 얇은 전문용어(SKAN·컨버전 밸류…)로. 신규 용어 1개 = KO+EN 파일 + `EN_GLOSSARY_SLUGS` + `GLOSSARY_PRIMARY_TOOL`(`glossary.test.js`가 길이 강제).
- **"전부에 강제하지 않는다"는 주석은 커버리지 구멍의 알리바이가 된다**(2026-08-18): FAQ 형태만 검사하는 사이 기초 용어 19편이 FAQ 0건으로 FAQPage를 못 내보냈다(빠진 블록은 화면·테스트 어디에도 안 보인다). 예외를 선언하려면 셀 수 있어야 한다 — 채운 뒤 파생 검사로 고정했다.
...
- **AEO 답변은 제목의 핵심어를 담아야 인용된다**(`aeoAnswer.test.js`, 하나라도 겹치면 통과하는 최저선이다 — 판정식을 끝까지 읽고 말할 것). `getBlogEditorial`은 frontmatter `answer`를 레지스트리보다 먼저 쓰므로 **둘을 함께** 고친다.
...
- **제목·설명 SSOT는 `blogSeo.js`지 frontmatter가 아님**: `.md` title을 고쳐도 화면은 안 바뀐다 → `KO_TITLES`/`EN_TITLES`에서. 화면 h1은 별개 맵(`KO_H1`/`EN_H1`, 없으면 제목 상속, 같은 값 금지). 길이 한도는 description만(KO 80·EN 160) — 제목은 **검색어를 앞에**. `updated`는 로케일별이라 KO만 고쳤으면 EN 날짜를 올리지 말 것.
- **제목을 바꾸면 답변도 같이 고칠 것**(가드가 아니라 AEO가 이유): 콜론 없는 제목은 앞머리를 고정어로, 어미가 핵심어가 되는 말("단계란"·"총정리")은 제목에서 뺀다.
...
- **새 페이지는 sitemap에 넣었다고 사이트에 붙은 게 아니다**: `/compare`가 인바운드 링크 하나로 혼자 놓여 있었다. 체크리스트: ① 관련 도구 화면(역인덱스 **파생**) ② 푸터 ③ ⌘K(개별 항목으로) ④ 사이드바. `compareContent.test.js`가 모든 비교가 최소 한 도구에서 도달 가능함을 강제한다.
...
- **레이아웃 전제를 CSS 주석에만 적어 두면 데이터가 늘 때 조용히 깨진다**(2026-08-19): 갈래가 6→7이 되자 마지막 갈래가 혼자 줄을 차지했다. 개수를 고정하지 말고 `:last-child:nth-child(3n+1)`처럼 **혼자 남는 경우만** 처리하고, 열 수가 바뀌는 브레이크포인트마다 다시 건다.
...
- **안 찍은 이벤트는 영영 답할 수 없는 질문이 된다**(2026-09-17): 프로젝트 저장은 GA4에 성공 이벤트가 없었고 표면마다 다른 이름으로 갈라 셌다. 지금은 **단일 통과 지점**에서 `project_review_saved`/`_save_failed`(사유는 범주형 코드만)를 찍고 합산 금지를 `docs/ga4-product-events.md`에 둔다. **제품 질문이 생기면 답할 이벤트가 있는지 먼저 셀 것.**
- **폼이 안 읽는 스키마 필드는 그 필드가 없는 것과 같다**(2026-09-15): 도구의 기록 폼이 `goalMetric`·`guardrail*`을 선언하지 않아 도구에서 저장된 결정이 전부 채점 불가였고, 자유 텍스트 목표는 방향 어휘 밖이었다. 지금은 `lib/decisionGoals.js`가 선언하고 `publishedToolIds()` 파생 가드가 막는다.
...
- **신호 미배선이 여섯 번 반복됐다**(§7 메타 3): 정확검정 판정기·`metricRegistry`·5-3 채널 신뢰도·MMM 커버리지 비율·`regularized` 플래그·`findingsByGroup`이 전부 계산만 되고 화면이 안 읽었다. 결론을 publish하는 도구는 `findingProducers.TOOL_KIND`의 넷이다.

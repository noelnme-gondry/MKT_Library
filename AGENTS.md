...
- **글 마감 영역은 `.blog-post-outro` 한 줄 + 글쓴이 + FAQ뿐**(2026-09-03): 예전엔 연결툴·글쓴이·검토출처·검색의도·토픽클러스터·구독·관련가이드·관련용어·FAQ 아홉 블록이 세로로 이어져 본문보다 길었다(사용자 지적). 지금은 연결툴과 구독을 `.blog-post-outro` 2열에 나란히 두고, 글쓴이·FAQ만 남긴다. **FAQ 아코디언은 지우지 말 것** — `buildPostJsonLd`가 FAQPage JSON-LD를 따로 내보내므로 화면에서만 없애면 "화면에 없는 내용을 구조화 데이터로 내보내는" 상태가 되어 리치결과 자격과 §8 정직성을 함께 잃는다. `TopicClusterLinks`·`RelatedGuideList`·`RelatedGlossaryList`는 이때부터 용어사전에서만 쓰이거나(EditorialTrust·SearchIntentLinks) 프로덕션 미사용이 됐다 — 지우기 전에 §7 전수 grep.

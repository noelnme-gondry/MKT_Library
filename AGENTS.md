...
- **CSS**: 의미적 토큰(`--bg-1`·`--text-muted`·`--border`). 인라인 style은 일회성만, 공용은 클래스(`chart-container` 등). **글자 크기는 `--fs-xs`~`--fs-3xl` 8단, 모서리는 `--radius-sm`~`--radius-pill` 5단에서만 고른다 — px 리터럴은 CSS·JSX 양쪽에서 가드가 막는다**(스케일을 바꿔야 하면 토큰을 먼저 고칠 것).
...
### 이 절을 관통하는 네 가지 (사례마다 다시 적지 않는다)

1. **가드는 파생이어야 한다.** 손으로 쓴 배열을 도는 검사는 가드가 아니고, **가드가 있다는 사실이 가드가 없다는 사실을 가린다.** 스캐너에는 규모 단언(`toBeGreaterThan(0)`)을 같이 두고, 무엇을 셌는지 표본으로 눈으로 확인한다. 만들었으면 **한 번 부러뜨려** 실패를 확인할 것.
2. **가드는 값이 아니라 근거를 고정한다.** 지금 값을 그대로 적으면 그 순간부터 버그를 지킨다. 예외는 주석이 아니라 **코드 표식 + 사유**로만 통과시키고(문자열 검사는 자기 주석에 속는다 — 주석을 먼저 제거할 것), 예외의 근거 규칙이 아직 있는지도 함께 단언한다.
3. **신호를 만들었으면 읽는 화면을 같은 작업에서 배선한다.** 플래그·점수·저장소·스키마 필드를 계산만 해 두면 없는 것과 같다. 거울상도 같다 — 안 찍은 이벤트는 영영 답할 수 없는 질문이 된다.
4. **같은 것을 두 곳에서 계산하면 반드시 갈린다.** 지표·정규화·목록은 SSOT에서 파생하고, 결함을 보고하기 전에 그 값이 **화면에 닿는 경로를 끝까지** 따라갈 것(엔진 하나가 아니라 호출부·게이트·인자 출처까지).
...
- **숫자 표기 해석은 `utils/parseNumeric` 하나가 소유한다 — 폴백만 소비처가 고른다**(2026-09-16 감사): 엔진 13곳이 각자 `String(v).replace(/,/g,"")` 류를 갖고 있어 **같은 CSV 셀이 도구마다 다른 숫자가 됐다**. `"1.000,25"`(유럽식, 실제 1,000.25)가 5-2에서 0, 5-21·5-3에서 1.00025 — 둘 다 틀렸고 서로 달랐다. 콤마만 벗기는 파서는 소수점 구분자를 판별할 수 없어 **1,000배 축소된 값을 조용히 통과**시킨다. 해석은 공유하되 **폴백은 통일하지 말 것**(strict=null · orZero=0 · orNaN=결측0/오염NaN) — 묶으면 읽을 수 없는 값이 0으로 뭉개지거나 멀쩡한 도구가 통째로 멈춘다. `parseNumericSingleSource.test.js`가 소스에서 파생해 막고, 주석을 먼저 제거하며(§16), 예외는 `PARSE_NUMERIC_EXEMPT` 표식+사유로만 통과한다. **이 가드가 내가 놓친 3곳을 잡았다** — 손으로 목록을 썼으면 그대로 넘어갔다.
- **분모 가드는 `den ?`가 아니라 `den > 0`**(같은 감사): 노출·클릭·설치·결제는 정의상 음수가 될 수 없는데 falsy 가드가 음수를 통과시켜 CPM −2,000원·CPI −50원이 떴다. 음수 효율은 **정렬에서 최상위로 올라와 가장 좋은 채널로 읽힌다** — "계산 불가"가 "아주 저렴함"으로 뒤집히는 방향의 실패다(§8.6).
- **모르는 미래 값은 건너뛰지 말고 거절할 것**(같은 감사, 3곳): `persistMigrate`가 미래 버전의 저장 동의를 그대로 믿었고(v2~v4→v5 확대는 앞 방향만 막혀 있었다 — `main` 자동 배포라 롤백은 실제 경로다) · `readStoredTable`이 모르는 `transform`을 조용히 건너뛰어 **변환 안 된 표가 정상처럼** 나갔으며(바로 다음 줄의 헤더 수 검증은 변환 전 헤더와 비교하므로 통과한다) · 복원이 `Promise.all`이라 한 건 실패가 멀쩡한 나머지를 다 가져갔다. **못 읽는 건 못 읽는다고 말하는 편이 틀린 표를 그리는 것보다 낫다.**
- **엔진만 읽고 "화면이 말하지 않는다"고 단정하면 오탐이 난다 — 한 세션에 2회**(2026-09-17 자기 정정): ① 중복 매핑이 무경고라고 보고했는데 `CsvUploader`에 이미 `mappingConflicts → analysisBlocked`가 있어 **분석 CTA가 렌더조차 안 되고** 있었다. 재현 스크립트가 `mapRowsToStandard`를 직접 불러 UI 게이트를 우회한 것이다(§7 "셋업의 상태 주입이 진입 경로를 우회한다"의 재발). ② 예산 모드 C가 곡선을 무시한다고 보고했는데 `maxSpends`가 어디서 오는지 안 봤다 — `getAllocationEvidenceLimits`가 관측 최대치와 **∩형 꼭짓점**으로 상한을 건다(곡선은 상한으로 반영되고, 비율에만 안 쓰인다). **더 나쁜 건 그 틀린 진단으로 화면 문구를 써서 배포한 것**이다(§8 위반). **결함을 보고하기 전에 그 값이 실제로 화면에 닿는 경로를 끝까지 따라갈 것** — 엔진 함수 하나가 아니라 호출부·게이트·인자 출처까지.
- **저장 레코드에 세대를 적고, 구세대는 파일을 살리되 굳은 해석만 버릴 것**(2026-09-17): IndexedDB 데이터셋은 원본 blob을 다시 파싱해 드리프트에 강한데 `mapping`(원본헤더 → 표준키)만 저장 시점 어휘로 굳는다 — 표준키가 바뀌면 90일 전 파일이 **열리기는 하고 숫자만 빠진다**. `WORKSPACE_DATASET_SCHEMA_VERSION`을 적고 복원 시 구세대면 매핑을 버리고 `buildMappingContract`로 다시 인식하며, 재인식이 0건이면 저장본을 유지한다(빈 매핑보다 옛 매핑이 낫다). 표식이 **없는** 레코드도 구세대로 본다. 짝인 의미 바인딩(V2)은 새 매핑과 안 맞으므로 함께 비운다.
- **부호 하나로 문장을 고르면 같은 부호의 다른 경로에서 거짓이 된다**(2026-09-17): `classifyNarrative`가 `mix >= 0`(=전체를 악화시켰나) 하나만 보고 "비중이 늘었고 평균보다 비싼 편"을 고정 문구로 썼다. `mix = (cpāᵢ − C̄)·Δs`라 양수 경로는 **둘**이다 — (비싼 것 × 비중↑)과 (싼 것 × 비중↓). 후자에서 문장이 방향·가격 **둘 다 반대**로 나갔다(비중 98.9%→55.6%인데 "늘었고", 최저가인데 "비싼 편"). 곱으로 만들어진 지표의 서술은 **곱의 부호가 아니라 두 축에서 각각** 파생할 것 — 축 값을 새로 넘길 필요도 없다(부호에서 되찾힌다). 마침 소비처가 0곳이라 화면 피해는 없었지만, 배선하는 순간 거짓이 배포되는 잠복 함정이었다. 가드는 네 사분면을 전부 돌고 **반대말이 들어가면 실패**시킨다(`simpsonParadox.test.js`).
- **분해의 무잔차는 "귀속이 맞다"를 보장하지 않는다 — Simpson 픽스처로 따로 볼 것**(같은 날): 합이 맞아도 구성 변화와 단위 내부 변화를 반대로 귀속하면 화면이 엉뚱한 캠페인을 범인으로 지목한다. "전 캠페인 CPA 개선 + 전체 CPA 악화"(싼 쪽에서 비싼 쪽으로 물량 이동) 픽스처로 **각 항의 부호와 크기**를 단언할 것 — 5-21·5-29 둘 다 실측 결과 정확했지만, 그걸 지키는 검사가 없었다. 변형검사(복제·스케일·순서·리네임)도 5-2에만 걸려 있어 형제 엔진으로 넓혔다(`engineMetamorphic.test.js`).
- **판별 근거가 없으면 규칙을 바꾸지 말고 사유를 노출할 것**(같은 감사): `03/04/2026`은 데이터만으로 M/D·D/M을 가를 수 없다. 읽는 규칙(M/D 고정)은 그대로 두고 `ambiguous` 플래그를 함께 돌려줘 화면이 "무엇으로 읽었는지" 말하게 했다 — 기존 `isoDate`·`canonical` 계약은 추가만 하고 건드리지 않는다. 중복 매핑도 같다: 덮어쓰기는 그대로 두고 **어느 컬럼이 겹쳤는지 이름으로** 알린다(어느 쪽이 맞는지는 사용자만 안다).
- **게이트 `requiresAny` 키는 정규키와 정확히 일치(단/복수)**: `["click"]` vs `clicks` 불일치 → 데모인데 영구 잠김(silent). 키는 추측 말고 복붙.

- **저장 집계는 잘라 쓸 원본이 아니다**: 부분 주로 나누거나 다른 통화·전환 기준과 비교하지 않는다. 평소 변동 이력은 현재 기간뿐 아니라 과거 기간끼리의 겹침도 제외한다(시작일만 검사하면 중복 관측). 같은 배열의 실제 기간·값을 근거로 표시하고, 다음 업로드·새로고침 후 재방문을 모두 검증한다(#812, 2026-09-13).
...
**렌더·UI**
- **디자인 규칙은 CSS가 아니라 렌더된 박스로 지킨다**(2026-09-24): 사용자가 "AI 티"로 지적한 다섯 증상(글자 겹침·상자 안 상자·형제 상자 가장자리 어긋남·제목 위 작은 라벨·굵기 난립 + 왼쪽 색 막대 안내 상자)을 `e2e/design-rules.spec.js`가 정적 화면과 **예시 결과 화면**을 3개 폭에서 재서 막는다. 이 스캐너가 처음 돌자마자 코드 리뷰로는 안 보이던 것들이 나왔다 — **가이드 15편 전부가 폰에서 본문 28px 폭**(`.with-toc` 2열을 접는 규칙이 아예 없었다) · 768px 규칙이 뒤쪽 기본 규칙에 져서 근거 줄이 한 글자씩 세로로 쌓임 · `details`를 항상 열린 `section`으로 바꾼 뒤 absolute 팝오버가 결과 위에 **상시** 떠 있음 · 홈 버튼이 손으로 준 grid 칸 번호 때문에 카드 위에 얹힘. 스캐너 자체의 함정도 셋이었다: 여러 줄 글자를 합친 상자로 재면 옆 줄과 거짓 겹침, overflow로 잘린 글자, 스크롤한 스티키 헤더 — 줄 단위(`getClientRects`)·잘림 교차·고정 요소 분리로 푼다. 예외는 `data-design-exempt="규칙: 사유"` DOM 표식으로만.
- **AI 티 두 번째 묶음은 "고정된 것이 떠 보이는" 문제였다**(2026-09-24): 고정 표면의 그림자·장식 그라디언트·hover 들림 19곳·누를 수 없는 알약·토큰 밖 글자색. `e2e/support/designRules.js`가 shadows·gradients·pills·offPalette를 렌더 결과로 재고(떠 있는 층·누르는 컨트롤은 제외), `app/hoverLift.test.js`가 선언을 막는다. **그라디언트를 단색으로 바꾸면 section이 채워진 상자가 되어 상자 안 상자가 새로 생긴다** — 장식 바탕은 단색이 아니라 `transparent`로. 짝 교훈: 도구로 넘길 때 효율 슬라이스를 재매핑 사본으로 바꾸므로 뒤로 오면 매핑을 다시 물었다 — `DochiResultWorkspace`가 넘긴 사본(`handoffs`)을 기억해 원본(`sourceData`)으로 되돌아간다. 빈 상태는 **빈 목록만** 줄일 것 — 빈 `내 프로젝트`를 만들며 작업 탭까지 걷었다가 저장한 결정 없이도 쓰는 `주간 성과 비교` 입구가 막혀 e2e 5개 스펙이 깨졌다.
- **샘플이 "다 된다"고 말하는 동안 실제로 계산된 건 하나였다 — 체험 우회가 판정 버그를 가렸다**(2026-09-25): 결과 화면이 샘플 모드에서 모든 도구를 "지금 이 파일로 되는 분석 20"에 넣고 있어서, 효율 도구 넷이 **자동 매핑 충돌로 확인 대기에 빠져 계산되지 않는** 버그가 한 번도 안 보였다(`channel` 헤더와 `source` 헤더가 둘 다 channel로 잡힘 — source를 쓰지 않는 도구 스코프에서 `source`가 channel 별칭이 된다). 표준 필드명과 정확히 같은 헤더가 하나면 그 헤더가 이기게 고쳤다(`scoreMappingCandidates.resolveExactNameConflicts`, 둘 이상·0개면 지금처럼 사람이 고른다). **체험용 우회는 판정을 덮지 말고 판정 옆에 길만 낼 것** — 자격은 그대로 두고 "예시로 열기"만 붙였다.
- **분석이 다르면 그림도 달라야 한다 — 같은 막대 하나로 그리면 "무슨 분석인지"가 사라진다**(2026-09-25): 결과 작업대가 PVM·포화도·예산·VIF·소재 피로도·생존을 전부 같은 가로 막대/꺾은선으로 그렸다. 지금은 어댑터가 `options.variant`로 핵심 그림을 선언하고(`components/assistant/ResultCharts.jsx`: mix-rate 다리·평균↔한계 단가·지금↔바꾼 안·VIF 기준선·상태 띠·KM 계단), `lib/assistant/visualizationVariants.test.js`가 어댑터 소스에서 파생해 variant 없는 막대를 막는다. 짝 교훈 둘: ① 샘플을 채널 하나로 좁혀 두면 **채널을 비교하는 분석이 전부 "비교 대상 1개"로 빈다** — 샘플은 그 화면이 보여 줄 분석이 실제로 돌 모양이어야 한다(홈 미리보기·결과·주간 리뷰는 같은 범위를 쓴다, `sampleJourney.test.js`). ② 두 값을 비교할 땐 **같은 모형에서 나온 값끼리** — 5-3 "지금 배분"을 관측값으로 두면 모형 오차가 배분 효과로 보인다(`predictSafeCpr`로 둘 다 예측). 그렇게 비교하자 탐욕 배분이 지금보다 **나쁜** 안을 내는 경우가 드러났다(0원부터 쌓으면 규모가 커져야 싸지는 채널의 첫 칸을 못 고른다, 샘플 8,782→6,072건). 지금은 결과 화면이 `improveAllocationFromCurrent`(지금 배분에서 출발해 합계가 늘어나는 이동만 받음 — 구성상 지금보다 나빠질 수 없다)와 그리디 중 예상 성과가 많은 쪽을 쓴다. 모드 B·C 원 함수와 도구 화면은 불변.
- **결과가 나왔으면 매핑은 결과 위에 없어야 한다 — 접기가 아니라 이름 있는 편집 창으로**(2026-09-24): 폰에서 결론 카드가 3,000~4,900px 아래(5-22는 선택상자 20개 뒤)였다. SSOT §4가 접기·펼치기를 금지하므로 `ds/MappingEditorDialog`(한 줄 요약 + `데이터·매핑 편집`)로 풀었다. **편집 창은 게이트가 닫혀도 유지할 것** — 매핑을 바꾸는 순간 분석 게이트가 닫히는데 그걸로 창 표시를 판정하면 고치는 도중 창이 사라진다(`isCollapsed = open || 분석됨`). 스캐너 부수 교훈 셋: 전 쌍 비교(n²)는 글자 수천 개 화면에서 150초를 넘긴다(세로 정렬 스윕) · `<table>`은 폭을 무시해 `.sr-only` 1px이 안 먹고 가로 넘침을 만든다(`div.sr-only`로 감쌀 것) · `:is(> x)`처럼 관계 선택자로 시작하는 인자는 무효라 **규칙 전체가 조용히 버려진다**.
- **결과 옆에 신뢰도·방법을 늘어놓지 말 것 — 문제가 있을 때만 빨간 `!` 하나**(2026-09-24 사용자 결정): '실제 분석 범위·분모 확인'·'신뢰도·방법'·'실행 정보(엔진 버전·필터 JSON)'·'●주의해서 해석' 배지·/start의 '상세 품질 아직 미검사 → 승인하고 계속'을 사용자가 "쓸 수 없는 허들"로 지목했다. 지금은 `ds/IssueMark`가 확인할 점이 있을 때만 결론 옆에 뜨고(누르면 어느 기간·열·칸인지), 없으면 아무것도 그리지 않는다. 재현 정보는 XLSX `00_README`·`04_SCOPE`가 소유한다. 같은 날 그림 이모지(✓ ⚠ ↗ ✕ 같은 상태 기호는 허용)·CSS 대문자·숫자 고정폭 글꼴도 없앴고 `e2e/design-rules`가 잰다. **예시 버튼만으로 결론까지 안 가는 도구(5-3·5-4·5-23)는 실제 입력을 밟는 결과 스캔을 둘 것** — 입력 화면만 재던 동안 결과 화면의 상자 안 상자·13px 제목·색 막대가 전부 숨어 있었다.
- **"예시가 결론까지 안 간다"의 원인은 도구마다 달랐다 — 발행 도구 전체를 한 번에 재야 보인다**(2026-09-24): 5-3은 기본 예산(최근 일예산 합)이 관측 상한을 넘어 **실행안 자체가 없었고**(실사용자도 같은 기본값을 받는다), 5-23은 계획 기간 미선언, 5-4는 첫 탭이 계산기라 예시 길이 없었고, 5-18 유입 변화맵은 버튼 이름만 달랐다. 도구별로 고치며 손으로 목록을 쓰는 대신 `publishedToolIds()`에서 파생한 e2e 하나가 21개 중 5개를 한 번에 드러냈다.
- **굵기는 400·600·700 세 단**(`app/fontWeightScale.test.js`) — `strong`의 `bolder`는 굵은 부모 안에서 선언 없이 900을 만든다(grep으로 안 보인다).
...
- **하한 가드는 종류를 안 세고, 종류 가드는 하한을 안 본다 — 둘 다 필요하다**(2026-09-04 → 2026-09-15): px 폰트 12px 하한을 걸어 뒀는데 하한은 종류를 세지 않아 1년 뒤 912곳·27종이 1,084곳·29종으로 **늘어** 있었다. 더 나쁜 건 스케일이 이미 있었다는 것이다 — `--fs-*`가 "이 토큰에서 고른다"는 주석까지 달고 정의돼 있는데 소비처가 3곳이었고, 옆에 `--type-*` 두 번째 계열이 따로 살아 있었다. **이름이 셋이면 다음 사람은 아무거나 고르고, 그래서 스케일이 스케일이 아니게 된다.** 지금은 `--fs-*` 하나로 합치고 `app/typeScale.test.js`가 스케일 밖 px를 아예 못 적게 막는다(CSS·JSX 양쪽).
- **차트 축이 무엇인지 안 말하면 눈금은 그냥 숫자다**(2026-09-15): 외부 글("바이브 코딩 대시보드가 형편없어 보이는 10가지 이유")을 코드와 대조하다 실측하니 차트 62개 중 축 제목이 40건뿐이었고, **차트를 그리는 파일 28개 중 13개가 0건**이었다 — 하필 제일 많이 보는 5-2 대시보드 탭(스코어카드·페이싱·LTV·퍼널·이상탐지·코호트·시즈널리티)이 통째로 거기 있었다. 형제 도구(5-3·MMM)는 이미 `title:{display:true,text:tr(ko,en)}` 패턴을 갖고 있었다 — **새 화면을 낼 때 형제를 안 보고 만든 자리는 나중에 설계 결정처럼 보이지만 사실은 누락이다**(§7). `app/chartAxisTitle.test.js`가 `scales:`를 선언한 파일에서 파생해 강제하고, 예외는 주석이 아니라 `CHART_AXIS_TITLE_EXEMPT` 표식 + 사유로만 통과한다(§16 — 표식을 주석에 두면 가드가 주석에 속는다).
- **위계를 "테두리 개수"로 세면 틀린 진단이 나온다**(같은 날, 자기 정정): `border: 1px solid var(--border)`가 145곳인 걸 근거로 "표면 위계 없음"을 P3로 올렸는데, 실제 도구 화면의 최상위 표면을 읽어 보니 결론 카드(3px 컬러 상단선·`--radius-xl`)·본문 블록(테두리 없음)·마감 영역(`--operator-line`/`--work-surface`)으로 **위계가 이미 있었다** — 145곳은 대부분 칩·입력·표 셀이다. 프록시를 재고 결론을 말한 것이다(§7 "내부 링크는 앵커 모양이 아니라 연결성을 세야 한다"와 같은 오류). 진짜 결함은 하나였다: §12.30이 재질을 정해 둔 `.dashboard-support-tools`만 결론 카드와 **같은** `--border`/`--surface-container-lowest`를 쓰고 있었다(다수가 맞으면 소수의 예외가 안 보인다, §16).
...
- **전역 규칙이 있어도 개별 규칙이 취소하면 없는 것과 같다 — 개수가 아니라 캐스케이드를 볼 것**(2026-08-19): 전역 `:focus-visible{outline:2px solid var(--primary)}`를 **12곳이 `outline:none|0`으로 취소**하고 1px 색 변화로 대체하고 있었다. `outline:none`을 grep하면 14곳이지만 **유효한 취소는 12곳**이다 — 전역보다 앞에 선언된 기본 규칙은 소스 순서로 지고 `input`은 `!important`가 따로 덮는다. **취소가 실제로 이기는지는 특이도와 선언 순서로 갈리므로 grep 개수로는 판정할 수 없다.** `app/focusVisible.test.js`가 두 갈래를 CSS에서 파생해 막는다.
- **판정을 흐림(opacity) 하나로만 표시하면 다른 파일이 취소했을 때 판정이 통째로 사라진다**(2026-09-16): `/start`가 되는 도구를 가려 `is-dim`까지 붙이는데 `library-workspace.css`가 `.app`을 앞에 붙여 특이도 0,3,0으로 `opacity: 1`을 되돌려, **20개 중 5개만 가능한 상태에서 화면 차이가 한 픽셀도 없었다**(히어로는 "지금 실행 가능한 분석만 보여줍니다"라고 말하는 채로). 취소 규칙엔 사유 주석까지 있었지만("자격은 카드가 설명한다") 카드의 `필요 데이터`는 정적 계약이라 자격을 설명한 적이 없다 — **사유 주석이 사실인지도 확인할 것**. 지금은 글자가 말하고 `app/toolIndexAvailability.test.js`가 두 파일을 함께 읽는다. 이걸 찾는 동안 측정이 세 번 틀렸다(업로드 안 된 화면을 쟀고, 규칙이 걸린 자식 대신 부모를 쟀고, 스캐너가 0건을 돌려줬다) — **셋 다 실제 파일·실제 DOM을 다시 열어 보고서야 잡혔다.**
- **판정을 만든 화면을 떠나면 그 판정을 다시 못 쓴다 — 컴포넌트 안의 지역 함수는 그 화면 전용이다**(같은 날): "이 CSV로 뭐가 되나"를 `AssistantWorkspace` 지역 함수가 계산해서, 도구로 들어가는 순간 같은 답을 낼 방법이 없었고 "이 데이터로 뭘 더 할 수 있나"가 화면에서 사라졌다. `lib/assistant/csvEligibility.js`로 빼자 도구 화면 하단(`ToolContinuityIndex`)이 같은 부품·같은 문구로 흐름을 잇는다. **화면 두 곳이 같은 질문에 답해야 하면 답하는 코드는 컴포넌트 밖에 있어야 한다.**
- **자격 판정기가 두 벌이면 포매터도 두 벌이다**(같은 날): `lib/analysis-router/evaluateEligibility`(`blockers[].fields`)와 `lib/assistant/evaluateAnalysisEligibility`(`blockers[].alternatives`)가 이름도 역할도 비슷한데 결과 모양이 다르다. router 쪽 `formatEligibilityBlocker`를 assistant 결과에 끼워 `undefined.join`으로 `/start`가 통째로 죽었다(스모크가 잡았다). 공용 문구는 `lib/assistant/blockerText.js` 한 곳이고 골든이 두 모양을 다 받는다. **"같은 이름의 엔진이 두 벌" 함정은 엔진만이 아니라 그 결과를 읽는 모든 것에 번진다.**
- **semantic 토큰에 라이트 오버라이드가 있어도 raw hex를 쓰면 소용없다**(2026-08 감사): `globals.css`에 다크 전용 리터럴이 51곳 있었고 **바로 옆 규칙은 `var(--danger)`를 쓰고 있었다**(한 블록 안에서 규칙이 갈림). 라이트 배경에서 대비 1.9:1(AA 미달). `ds/` 컴포넌트에도 같은 혼재가 있었는데 거긴 다른 도구가 베끼는 기준이라 전파된다. 차트 팔레트도 하드코딩하면 `refreshMountedChartThemes` 대상에서 빠진다 → `CHART_THEME.colors`·`.danger`·`.warning`·`.success`(판정 색 getter). remap Map은 **각 토큰의 다크·라이트 값 + 과거 리터럴을 전부 키로** 가져야 한다 — 하나라도 빠지면 그 데이터셋만 옛 색으로 굳는다. getter 반환값은 리터럴 hex라 `CHART_THEME.danger + "AA"` 알파 접합이 그대로 되고(§7 hex-알파 함정은 `var()`에만 해당), canvas는 `var()`를 못 읽으므로 **데이터셋엔 반드시 getter**를 넘길 것.
  ...
  - **`:root`에서 `var(--다른토큰)`으로만 선언된 별칭 토큰은 라이트 재정의가 닿지 않는다**(2026-09-16): `--bg-3: var(--surface-container)`는 값이 `:root`에서 확정되므로 `body.light-mode`가 원본을 밝게 바꿔도 별칭은 다크로 굳는다 — 배경만 어두운 채 글자색은 라이트로 뒤집혀 대비가 무너진다(e2e `@light-en` 5건이 깨졌다). **grep으로는 안 보인다** — 원본을 찾으면 라이트 정의가 멀쩡히 나오고 별칭을 한 단계 따라가야 드러난다. 전수로 세니 별칭 16개 중 4개가 그랬고 사용처 6곳 중 4곳이 AA 미달이었다(최악 **1.04:1**). **"hover 배경이라 괜찮다"고 처음 적은 건 틀렸다 — 사용처를 열어보지 않고 말했다.** `app/themeAliasTokens.test.js`가 정합을 막는다.
...
- **계측 스크립트에 환경 가드가 없으면 개발자가 곧 트래픽이다**(2026-08-19): GTM·GA4·AdSense가 조건 없이 실려 `npm run dev` 화면 확인이 전부 운영 속성에 쌓였다. `page_view`만이 아니라 `window.gtag` 하나를 **제품 이벤트 수십 종이 공유**하므로 퍼널 지표까지 부풀려진다(통로가 하나라 게이트도 한 곳이면 된다는 뜻이기도 하다). **빌드타임 env로 가르면 안 된다** — 정적 프리렌더라 같은 HTML이 localhost와 운영에 함께 나가고 `build && start`가 프로덕션으로 잡혀 그대로 샌다. 갈리는 값은 호스트뿐이라 `lib/analyticsHost.js`(정확일치 — 접미사 비교면 `…com.evil.example`이 통과한다)로 판정한다. 광고는 정확도가 아니라 **정책** 문제다. `noscript` iframe은 게이트가 클라이언트라 옮길 수 없다 — 옮기면 JS 꺼진 **운영** 방문자에게서도 사라진다.
- **정적 대비 검사는 "라이트 토큰 × 영구 다크 표면" 조합을 구조적으로 못 본다**(2026-09-05): 슬림 푸터(전 도구 페이지)가 라이트 모드에서 3.07:1이었다 — 배경은 영구 다크(`#10131a`)인데 글자색만 테마 토큰이라 라이트에서 어두운 글자가 어두운 배경에 얹혔다. 전체 푸터는 같은 이유로 이미 리터럴을 쓰는데 슬림 변형만 빠져 있었다. **토큰 쌍을 읽는 가드로는 안 나온다 — 배경과 글자를 실제로 합성해야 나온다.** 그리고 텍스트를 `opacity`로 흐리게 하면(블로그 태그 칩 `opacity:.6` → 2.36:1) 토큰은 멀쩡한데 실효 대비가 무너져 같은 이유로 안 보인다. `e2e/light-mode-contrast.spec.js`가 라이트 모드로 주요 라우트를 렌더해 axe로 잡는다 — 기존 e2e는 base가 `colorScheme:"dark"`라 **라이트 모드를 보는 것이 EN `/start` 하나뿐이었다.**
- **Playwright `webServer`는 `reuseExistingServer`라 포트에 남은 옛 서버를 그대로 쓴다**(같은 날): 수동으로 띄운 `next start`(옛 빌드)가 3100에 살아 있어서, 고친 코드가 반영 안 된 채 "여전히 위반"으로 나왔다. 실측 전에 `fuser -k 3100/tcp`로 포트를 비울 것 — 안 그러면 자기 수정을 자기가 못 본다.
...
- **화면에 보이는지는 `checkVisibility()`로만 판정할 것 — 박스 크기로 세면 접힌 내용이 보이는 것으로 잡힌다**(2026-09-16): `getBoundingClientRect()`는 `content-visibility`를 반영하지 않아 닫힌 `<details>` 안에서도 0이 아닌 박스를 돌려준다(71×18.59). 그 틀린 실측 위에서 "중복" 두 곳을 지웠는데 하필 **항상 보이던 유일한 두 자리**라 대시보드가 규모를 0번 말하는 회귀가 났다(되돌렸다). 지금은 `e2e/data-shape-repetition.spec.js`가 `checkVisibility({checkVisibilityCSS:true})`로 세고 **0도 실패**시킨다. 문구를 세는 스캐너의 짝 함정 둘: 텍스트 **노드** 단위로 세면 JSX가 쪼갠 `{16}{"행"}`을 놓치고(요소 `textContent`로 보되 조상 중복은 가장 안쪽만 남긴다), `\b`는 단어문자를 `[A-Za-z0-9_]`로 보므로 **한글 경계에서 성립하지 않는다**.
- **한 파일만 grep하고 "없다"고 단정하면 바로 옆에 렌더되는 배선을 못 본다**(2026-09-16 감사): 검토함이 검증 도구로 못 보낸다고 보고했는데 근거는 `sourcePath` grep 하나였다. 실제로는 그 링크 바로 옆에서 `decisionReviewFollowUpMode`가 네 모드를 판정해 후속 안내를 렌더하고 있었고(`WeeklyReview.jsx:677`), 자동 판정 불가 목표는 `decisionGoals`의 `rerun:` 접두사가 이미 선언적으로 가르고 있었다. **"이 기능이 없다"를 말하기 전에 그 화면이 실제로 그리는 것을 읽을 것** — 있는 설계 위에 추측 기반 레이어를 얹을 뻔했다. 같은 감사에서 `primary` CTA가 11개라는 보고도 오탐이었다(정규식이 `sidebar-primary-nav__item`까지 잡았다). **의심은 전수 출력으로 확인한 뒤에 말할 것.**
- **초안을 병합한 레코드를 이력의 기준으로 쓰면 아직 관측이 아닌 값이 관측으로 굳는다**(2026-09-16, v11): `WeeklyReview`의 `records`는 `storedRecords.map(r => ({...r, ...drafts[r.id]}))`라 화면의 `record.actual`이 **입력 중인 초안**이다. 관측 이력을 붙이며 그 레코드를 넘겼더니, 구세대 폴백(`episodes`가 없으면 `actual`을 첫 관측으로 세움)이 초안을 기존 관측으로 오인해 같은 값을 두 번 쌓았다. **이력의 기준은 저장된 레코드**(`storedRecordOf(id)`)로 못 박을 것 — "값이 같으면 건너뛴다"는 방어는 정정 관측(같은 값을 다시 적는 경우)을 삼키므로 답이 아니다. 골든 10건은 이걸 못 봤다(엔진만 본다). **스모크가 잡았다** — §6.1이 "순수함수 밖은 스모크로 보강"이라 적어 둔 그 자리다. 같은 화면에서 스토어를 직접 읽는 경로(5-18 예측의 `getState().decisionRecords.find`)는 병합이 없어 안전했다 — 같은 기능의 두 경로가 서로 다른 소스를 읽으면 한쪽만 이 함정에 걸린다.
- **테스트가 "무관해 보이는" 변경에 깨지면 이분법으로 좁힐 것 — 단독 통과는 무관의 증거가 아니다**(2026-09-16): 한 줄을 지우자 import 관계도 없는 스모크 4건이 **전체 스위트에서만** 깨졌다. 그 파일만 단독 실행하면 3회 다 통과해서 "무관"이라 보고했는데, 의심 파일만 원복하니 전부 통과했다 — 인과였다(단독 실행은 부하·순서가 달라 취약 테스트를 살려 준다). 판정은 **① 베이스라인 전체 ② 변경 전체 ③ 의심 파일만 원복한 전체** 세 번으로. 깨진 단언은 비동기 사슬의 **중간 상태**라 DOM이 조금만 달라져도 갈린다. 메커니즘은 못 밝혔고 실익이 작아 **변경을 포기**했다 — 원인 모를 결합을 안고 가느니 얻는 게 작은 변경을 버리는 편이 낫다.
- **`hidden` 속성은 명시적 `display`를 이기지 못한다**(같은 날): 기본 `[hidden]{display:none}`과 클래스의 `display`가 특이도 동률이라 소스 순서로 갈린다(이 저장소엔 `!important` 보강이 없다). 숨김이 필요하면 조건부 렌더로 빼거나 `display` 선언을 함께 거둘 것.
- **부러뜨리기 스크립트에 단언이 없으면 "가드가 못 잡는다"는 틀린 결론이 나온다**(같은 날): 치환이 조용히 실패했는데(셸 따옴표 섞임) 테스트는 **원본 그대로** 돌아 통과했고, 나는 그걸 "가드 버그"로 읽어 멀쩡한 가드를 세 번 더 고쳤다. §7의 "테스트 안의 `foo?.()`는 검사를 조용히 삭제한다"와 같은 종류다 — **상태를 바꿨다면 바뀐 사실 자체를 단언할 것**(`assert old in s` · 치환 후 `s2 != s`).
...
- **JS가 벗겨 주기를 기다리는 숨김에는 브라우저 레벨 failsafe와 시간 예산을 함께 둘 것**(2026-09-05, 제보 2회): 랜딩 진입 모션이 먼저 숨기고 청크를 await하는데, 청크가 **거부가 아니라 지연·중단**되면 프라미스가 영영 settle되지 않아 `.catch`의 disarm이 안 돌고 콘텐츠가 자리만 차지한 채 투명하게 남는다(느린 모바일에서 흔하고 로컬·CI에선 재현되지 않으며 콘솔 에러도 없다). JS 워치독만으로는 부족하다 — 숨김 규칙 자체에 `animation: … Xs forwards`로 최종 상태를 넣어야 브라우저가 보장한다. 지금은 `MOTION.armBudget`(600ms)이 예산을 넘기면 연출을 버리고 정적 확정하며 늦게 온 모션은 무시한다. **연출보다 콘텐츠.**
...
- **빌드 타임에 외부에서 받아오는 자산은 배포를 남의 네트워크에 건다 — 그리고 화면 밖 자산은 없어도 앱이 멀쩡히 돈다**(2026-09-02): OG 카드를 `next/og`로 그리며 Google Fonts에서 한글 폰트를 받아 왔는데, 200으로 응답하고 본문만 잘려도 satori 파서가 `RangeError`를 던져 **빌드 전체가 죽었다**(`try/catch`는 네트워크 오류만 잡고 깨진 바이트는 못 잡는다). 정작 도구 카드는 폰트를 안 넘겨 한글이 그려진 적도 없었다. 지금은 저장소 폰트로 미리 그린 `public/og-card.png` 한 장을 전 페이지가 쓴다. 더 위험한 건 **30여 페이지가 그 경로를 가리키는데 public에 파일이 없었다**는 점 — 라우트가 대신 그려 주고 있어 라우트만 지웠으면 전 페이지가 조용히 404가 된다. 화면 밖에서만 드러나는 자산은 **파일 실재와 규격을 테스트가 봐야 한다**(`app/ogCard.test.js`).
- **사람이 넣는 마커에 렌더를 걸면, 안 넣은 글에서는 그 기능이 없는 것과 같다**(2026-09-03): 본문 중간 CTA가 `<!-- CONTENT_ACTION -->`를 못 찾으면 패널이 통째로, 조용히 사라졌다 — KO 48편 중 **14편에만** 있었다. 커버리지 가드는 손으로 쓴 3편만 돌고 있었다(§7 메타 1의 재발). 지금은 마커가 없으면 본문 블록 경계에서 위치를 **파생**하고(마커가 있으면 마커가 이긴다) 발행 글 전체 파생 가드가 강제한다. 짝 사고: `TOOL_COPY[id] ? id : "5-2"` 폴백이 카피 없는 도구 3개를 가려 **글 7편+용어 3편이 엉뚱한 도구로** 가고 있었다 — 폴백을 쓸 거면 **무엇이 폴백으로 떨어지는지 레지스트리에서 파생해 세는 검사**를 같이 둘 것.
- **내부 링크는 앵커 모양이 아니라 연결성을 세야 한다 — 프록시를 재고 "이미 됐다"고 보고했다**(2026-09-08): 본문 내부링크 108개가 전부 `[A/B 테스트 방법]` 식 키워드 앵커라는 걸 확인하고 클러스터 재배선을 "우선순위 낮음"으로 보고했는데, 정작 **예산 클러스터는 글끼리 상호링크가 0건**이고 소재는 1건이었다. 앵커 텍스트 품질과 클러스터 연결성은 다른 속성인데 앞엣것을 재고 뒤엣것을 말한 것이다. `topicClusters` 파생으로 세니 형제·필라를 한 번도 안 가리키는 글이 **25건**이었다(`clusterLinksFor`가 만드는 링크는 화면 컴포넌트가 삭제돼 아무 데도 안 그려지므로, 내부 링크는 전적으로 본문이 진다). 지금은 `topicClusters.test.js`가 클러스터 정의에서 파생해 "모든 글이 형제나 필라를 본문에서 최소 한 번 가리킨다"를 강제한다 — **가드를 만들면 실제로 깨지는지 한 번 부러뜨려 볼 것**(링크 하나를 지워 실패를 확인했다).
...
- **같은 문장을 제목과 sr-only 라벨에 두 번 쓰면 보조기술이 질문을 두 번 읽는다**(2026-09-17): 서베이 입력칸에 이름을 주려고 `<label className="sr-only">`에 제목과 같은 문구를 넣었더니 스크린리더가 질문을 두 번 읽고, 화면 검사는 제목을 둘로 세어 `getByText`가 통째로 실패했다(테스트 3건). 이름은 `aria-labelledby`로 **이미 있는 제목을 가리키면** 되고, 설명은 `aria-describedby`로 잇는다 — 시각 사용자에게 보이는 문장이 곧 접근 가능한 이름이 되는 쪽이 항상 낫다.
...
5. **자동 종합 해석**: 통계 지식 없이도 결론 읽게(빨강 부정 / 초록 긍정 / 회색 무유의) + `쉽게 말하면` 평어 콜아웃(그림 이모지 없이).
...
### 12.1 새 분석 도구 추가
1. store `IA`에 `{ id:"5-N", title, desc }` → 2. `routeMap.js`에 `{id, slug, component}` → 3. `PageClient` 디스패치(SOP 폴백 가드 확인) → 4. `TOOL_REQUIRED/OPTIONAL_FIELDS` 정의 → 5. `toolGroups.js`에 CSV 그룹 → 6. `utils/toolGuide.js:TOOL_GUIDE[id]` + `lib/decisionGoals.js`(목표·가드레일 선언 — 파생 가드가 막는다) → 7. `demoData` 픽스처 → 8. 컴포넌트(`ds/*` 상속, §12.21) → 9. `sitemap.js` → 10. 골든+**스모크** → PR.
...
### 12.8 데모 모드
`demoData` 픽스처(`seededNoise`, `Math.random` 금지)는 자동으로 싣지 않고 '예시 데이터로 결과 바로 보기'로 부른다. 조립은 `lib/toolDemo.buildToolDemo` 하나(도구 예시 버튼·홈 샘플 공용). **예시는 곧장 결과다** — 매핑 확인·분석하기·안내 창 없이 결론 카드까지(데모 안내 창은 삭제, 되살리지 말 것). 사람의 선언이 필요한 입력(5-23 기간·시점, 5-28 이벤트 정의)은 예시 데이터가 함께 싣는다. `e2e/example-straight-to-result.spec.js`가 발행 도구 전체에서 파생해 강제한다.
...
- **블로그 넛지는 A~E 다섯 부품이 한 세트다**(2026-09-24, ARCHITECTURE.md "블로그 본문 내 전환 경로"): 중간 자리는 A 예시 결과(엔진 결과 사본 `lib/blogExamples/data.json`, 신선도 테스트가 강제) 또는 D 30초 점검, 글 끝은 모든 글에 B 상황 확인, 읽는 중엔 C 하단 한 줄 바, 도구 도착 시 E 출처 한 줄. 예시 숫자는 **엔진이 실제로 낸 값만** 쓴다(§8) — 문장도 방향(오름/내림/그대로)에서 파생한다. 가운데 팝업(도치 브리지)·요약 옆 CTA·outro 도구 패널은 제거됐다 — 되살리지 말 것.
- **글 마감 영역은 `.blog-post-outro`(구독) + 글쓴이 + FAQ뿐**(2026-09-03 → 09-24): 아홉 블록이 본문보다 길었다(사용자 지적). 도구 연결은 B 상황 확인이 맡는다. **FAQ 아코디언은 지우지 말 것** — `buildPostJsonLd`가 FAQPage JSON-LD를 따로 내보내므로 화면에서만 없애면 "화면에 없는 내용을 구조화 데이터로 내보내는" 상태가 되어 리치결과 자격과 §8 정직성을 함께 잃는다. `TopicClusterLinks`·`RelatedGuideList`·`RelatedGlossaryList`는 소비처 0으로 삭제됐다(2026-09-03) — 그래서 **클러스터의 내부 링크는 이제 전적으로 본문이 진다**(2026-09-08).
...
- **제목·설명 SSOT는 `blogSeo.js`지 frontmatter가 아님**: `blog.js`가 `seo?.title || data.title`로 덮어써 `<title>`·OG·JSON-LD·sitemap·RSS가 전부 레지스트리 값을 쓴다. **`.md`의 title을 고쳐도 화면은 안 바뀐다**(발행글 대부분이 이미 divergent) → 제목 수정은 `KO_TITLES`/`EN_TITLES`에서. **화면 h1은 별개 맵(`KO_H1`/`EN_H1`, 2026-09-08 신설)** — SERP 제목은 질의와 겹치라고 "키워드: 부제"로 쓰고 h1은 도착한 사람이 읽는 첫 문장이라 질문형이 자연스럽다. 맵은 성기고(없으면 제목 상속) **따로 적었다는 것 자체가 의도의 표식**이라 같은 값을 적으면 가드가 막는다. 길이 한도는 description만 강제(KO 80자·EN 160자) — 제목 상한은 검색어를 못 담게 만들어 뺐으니, 잘려도 핵심이 남게 **검색어를 앞에 세울 것**. `updated`는 로케일별 날짜 Set → sitemap `lastmod`+`dateModified`이므로 KO만 고쳤으면 EN 날짜를 올리지 말 것.
- **제목을 바꾸면 답변도 같이 봐야 한다 — 단, 가드는 최저선만 지킨다**(2026-09-08): `aeoAnswer.test.js`는 제목 핵심어가 `seoAnswer`와 **하나라도 겹치면 통과한다**(`if (!hit.length)`). "전부 있어야 한다"로 읽고 더 엄격한 사본을 만들어 "10편 중 9편이 터진다"고 잘못 보고했다(실제 2편). **가드를 근거로 말하기 전에 판정식을 끝까지 읽을 것** — 재현이 원본보다 엄격하면 없는 위험을 만든다. 그래도 답변은 같이 고치는 게 맞다(가드가 아니라 AEO가 이유다). 콜론 없는 제목은 앞머리를 고정어로 두고 콜론을 넣고, 어미가 핵심어가 되는 말("단계란"·"총정리")은 답변에 못 담기니 제목에서 뺀다. frontmatter `answer:`가 있는 글은 레지스트리보다 그쪽이 이기므로 **둘을 함께** 고친다.

### 12.24b 결정 스키마에 필드를 더할 때 (v10 가드레일 → v11 관측 이력)
평면 CSV 컬럼(`DECISION_REVIEW_COLUMNS`)을 유지한 채 목록형 필드를 더하는 방법. 두 세대가 이 패턴을 공유한다.
- **문자열 하나에 담는다**: 항목은 `|`, 목록은 `;`. `parse`/`serialize` 쌍을 두고 **serialize는 재조립으로 한 번 더 거른다**(직렬화 입력이 검증됐다고 가정하지 않는다, §12.29).
- **자유 텍스트 조각은 퍼센트 인코딩**: 가드레일은 지표명·연산자·숫자라 구분자가 안 섞이지만 관측은 사람이 쓰는 문장이라 `CPA 1,200원; 목표 미달|재검토`가 그대로 찢어진다. `encodeURIComponent`가 `;`·`|`·`,`·줄바꿈을 전부 `%XX`로 바꿔 구분자도 CSV도 안전하다. 한글 1자가 9자가 되므로 `FIELD_LIMITS`를 넉넉히.
- **구세대 폴백을 목록 함수 안에 둔다**: `decisionGuardrailList`가 `guardrailMetric` 단수를 첫 항목으로 세우듯 `decisionEpisodeList`는 v10의 `actual`을 첫 관측으로 세운다. **단, 새 필드가 있으면 미러를 다시 더하지 말 것** — 최신 값이 두 번 세어진다.
- **미러로 소비처를 지킨다**: `actual`·`learning`을 최신 항목의 사본으로 유지하면 읽는 13곳이 그대로 돈다. 이력이 필요한 화면만 목록 함수를 부른다.
- **쌓아 놓고 읽는 곳을 같은 작업에서 배선**(§16) — 관측 2회 이상이면 이력 접기를 노출한다.
...
`LandingPage` = ① **1단 중앙 히어로**(헤드라인+한 줄 데크+목적 CTA 3열 균등+통합 신뢰 1줄) ② `ds/ToolIndex` 전체 도구 인덱스 ③ 주간 결정 루프 3단계 ④ 블로그 | SOP 허브. **목적 선택(②)이 개념 설명(③)보다 앞**(첫 화면에서 바로 도구를 찾게, 스모크가 순서 강제). 구 `ConnectedToolJourney`는 인덱스와 **같은 갈래·같은 도구**를 카드로 또 그려서 삭제됐다 — 되살리지 말 것.
...
- **전면 오버레이는 모바일 검색에 불리하다**(구글 인터스티셜): 닫기 상시 노출·ESC·바깥클릭에 더해 **≤720px에서는 하단 시트**로 내려 위쪽 콘텐츠가 비쳐 보이게 한다. 서버 스냅샷을 항상 "닫힘"으로 두면 프리렌더 HTML에 오버레이가 없어 크롤러는 가려진 화면을 보지 않는다.
...
- **기존 사용자도 노출하되 마지막 단계부터 시작**: 기기 저장이 켜졌으면 IndexedDB 작업 목록 복원이 끝날 때까지 열지 않는다. 저장된 작업이 확인되면 인사를 건너뛰고 데이터 전달 단계부터 연다. 가져오기를 시작한 뒤 오버레이를 닫거나 건너뛰어도 준비 작업과 결과 화면 이동은 중단하지 않는다.
...
- **`ds/ToolIndex`의 실제 소비처는 `/start` 하나다**(2026-09-16 grep 실측 — "홈·사이드바에도 심는다"고 적혀 있었으나 홈은 다른 부품을 쓴다). 밀도는 `compact`·`full`·`grid` 셋이고 발행 화면이 쓰는 것은 `grid`뿐이다.
...
- **파일을 올렸으면 자격이 정렬 축이다**(2026-09-16): "할 수 있는 분석"은 우리가 제공하는 목록이 아니라 *지금 이 CSV로 되는 것*이다. `ToolIndex density="grid"`는 되는 것을 위에·안 되는 것을 아래에 묶고(개수 병기) 갈래는 버튼 안 작은 글씨로 내린다. 파일이 없을 때만 갈래가 다시 축이다. 세로 목록은 도구 20개에서 4,661px이었고 격자는 1,115px이다.
  **상세는 누른 버튼 바로 다음 줄 전체**(`grid-column: 1 / -1`)에 연다 — 격자 밖(맨 아래)에 두면 15개짜리 묶음에서 다섯 줄 밑에 열려 화면 밖으로 나가고, 버튼 사이에 좁게 끼우면 같은 줄의 뒤 버튼이 밀린다. 줄 전체를 쓰면 누른 버튼과 그 앞은 제자리다.
  **안 되는 것에는 브릿지를 여러 갈래로**: 빠진 컬럼을 버튼에서 바로 읽히게(3개+"외 N개" — 전부 적으면 버튼이 목록이 된다) · 판정이 준 구체적 사유 · 그 컬럼이 든 템플릿 받기 · 그래도 열어 보기. 길을 막지는 않는다(막으면 도구를 구경조차 못 한다). 내부 키(`cost`)가 아니라 `fieldLabels`로 사람이 읽는 라벨을 쓴다.
...
- `docs/pitfalls.md` — 함정 상세 / `docs/backlog.md` — 백로그 + MMM 스펙(§B) / `docs/harness-detail.md` — 본 파일의 PR별 상세본
- ⚠ **이 목록은 `docs/` 전부가 아니다** — 여기 적힌 건 의사결정에 상시 쓰는 것뿐이고, 나머지 수십 편의 지난 스펙·감사 기록도 **없는 게 아니다**. 특정 주제를 찾을 땐 `ls docs/`로 먼저 셀 것(개수는 적지 않는다 — 적자마자 낡는다, §7).
...
- **낡은 규칙은 우연히 발견되지 않는다 — 압축할 때 실측으로 훑을 것**(2026-09-18에 이 방법으로 3건을 잡았다): ① 하네스가 이름을 적은 **파일·경로가 실재하는지** 전수 확인 ② 소비처가 0인 기능을 전제한 절이 있는지(`grep`으로 실제 사용처를 센다 — VOC 구글폼은 링크가 0건인데 레시피가 남아 있었다) ③ **단계 수·px 경계처럼 코드가 소유한 값**을 다시 읽는다(도치 인사는 "4단계"라 적혀 있었지만 `DOCHI_WELCOME_STEPS`는 2개였고, "≤700px"은 실제 720px이었다 — §7이 경고한 그 오차다) ④ 삭제한 절을 가리키는 **참조가 남았는지**. 값을 소유한 코드가 있으면 개수를 적지 말고 **거기서 읽으라고 쓸 것**.
...
- **안 찍은 이벤트는 영영 답할 수 없는 질문이 된다 — 신호 미배선의 거울상**(2026-09-17): "프로젝트 리뷰 저장을 쓰는 사람이 몇인가"에 답할 수단이 없었다. 저장은 IndexedDB 전용이라 서버에 안 남고(§2.2, 옳다), GA4에는 `ReviewSaveDialog`의 저장 성공 이벤트가 **아예 없었다** — 나가는 건 `review_calendar_downloaded` 하나뿐이고, 표면별 `decision_record_added`·`weekly_decision_saved`는 같은 저장을 세 이름으로 갈라 세고 있었다. 실패는 한 건도 안 찍혔다. 지금은 모든 저장이 지나는 **단일 통과 지점**에서 `project_review_saved`/`_save_failed`를 찍고, 합산 금지를 `docs/ga4-product-events.md`에 못 박았다. 실패 사유는 열거된 범주형 코드만 싣는다(원문엔 사용자 데이터가 섞인다). **제품 질문이 생기면 그 답을 낼 이벤트가 있는지 먼저 세어 볼 것.**
- **폼이 안 읽는 스키마 필드는 그 필드가 없는 것과 같다**(2026-09-15, §7 메타 3): `decisionReview` v9의 `actionKind`·`goalMetric`·`guardrail*`을 스코어러는 채점할 수 있는데 도구 화면의 기록 폼(`ds/DecisionReview`의 `createDraft`)이 **하나도 선언하지 않아** 도구에서 저장된 결정이 전부 `no_terms_recorded`였다(채우는 곳은 `WeeklyReviewScreen` 하나뿐). 게다가 목표가 자유 텍스트라 방향을 정규식이 추측했는데 도구가 실제로 제안하던 목표 12개 중 **8개가 그 어휘 밖**이었다 — 도구는 자기 목표의 좋은 방향을 아는데 자유 텍스트로 넘기면서 버려지고 있었다. 지금은 `lib/decisionGoals.js`가 선언하고 `publishedToolIds()` 파생 가드가 커버리지를 막는다.
- **목표가 하나면 "이겼지만 졌다"를 못 잡는다 — 성공 정의 1개 + 가드레일 N개**(같은 날): 잠식 결정에서 "오가닉 전환 ↑"만 걸면, 광고를 끈 뒤 어트리뷰션이 오가닉으로 옮겨가 **총 전환이 줄어도 성공으로 읽힌다**. 목표를 여러 개 걸고 "하나라도 맞으면 성공"은 다중 탐색(§8.10)이라 답이 아니다 — 목표는 하나로 두고 나머지는 깨지면 실패인 가드레일로. 스코어러는 **하나라도 못 재면 합격시키지 않는다**(잰 것만 보고 통과시키면 가드레일을 더 걸수록 판정이 후해지는 뒤집힌 유인이 생긴다).
- **신호 미배선이 여섯 번 반복됐다**(§7 메타 3): `shouldPreferExactTest`는 소비처가 2곳뿐이라 **A/B 대량 검정이 근사 p로 유의를 선언**했고(n=50/50, 전환 0 vs 5 → Fisher 0.0563 vs z 0.0218로 판정이 뒤집힌다) · `metricRegistry`가 파생식 SSOT인데 `dashboardVerdict`가 공식을 다시 적어 **같은 화면에 라벨이 같은 두 CVR·ROAS**가 떴다 · 5-3은 채널 신뢰도를 계산해 칩까지 띄우면서 배분엔 안 썼고 · MMM은 커버리지 비율을 계산해 두고 sparse 판정은 절대 주 수만 봤으며 · `REG_STATS.ols`의 `regularized` 소비처는 1곳 · `store.findingsByGroup`은 `rankFindings()`까지 갖췄는데 소비처가 0곳이었다(2026-09-04에 `/weekly-review`가 그 소비처가 됐다). 결론을 publish하는 도구는 전부가 아니라 `findingProducers.TOOL_KIND`의 넷이다.

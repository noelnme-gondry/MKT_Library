...
- **글 마감 영역은 `.blog-post-outro` 한 줄 + 글쓴이 + FAQ뿐**(2026-09-03): 예전엔 연결툴·글쓴이·검토출처·검색의도·토픽클러스터·구독·관련가이드·관련용어·FAQ 아홉 블록이 세로로 이어져 본문보다 길었다(사용자 지적). 지금은 연결툴과 구독을 `.blog-post-outro` 2열에 나란히 두고, 글쓴이·FAQ만 남긴다. **FAQ 아코디언은 지우지 말 것** — `buildPostJsonLd`가 FAQPage JSON-LD를 따로 내보내므로 화면에서만 없애면 "화면에 없는 내용을 구조화 데이터로 내보내는" 상태가 되어 리치결과 자격과 §8 정직성을 함께 잃는다. `EditorialTrust`·`SearchIntentLinks`는 용어사전 전용이 됐고, `TopicClusterLinks`·`RelatedGuideList`·`RelatedGlossaryList` 셋은 소비처가 0이 되어 2026-09-03에 삭제했다(CSS 포함). **그 결과 `clusterLinksFor`·`guidesForPost`는 자기 테스트에서만 불린다** — 즉 아래 "레지스트리 6곳"의 `topicClusters` 등록은 지금 아무 화면도 그리지 않는다(가드는 계속 강제한다). 등록을 계속 요구할지, 레지스트리째 걷어낼지는 미결이다 — **없앨 땐 내부 링크가 그만큼 줄어든다는 걸 SEO 판단으로 먼저 확인할 것.**
...
- ✅ 검증 하네스: `npm run test:all` **364파일·2876 통과**(0 skipped) · eslint 0 errors/0 warnings · `next build` ✓ (정적 301페이지) — 2026-09-02 실측. `playwright` **25/25**(mobile-320·tablet-768·desktop-1440·light-EN)는 2026-09-01 실측값. **수치를 적을 땐 실제로 돌려서 적을 것**(돌리지 않은 항목은 언제 잰 값인지 함께 남긴다).

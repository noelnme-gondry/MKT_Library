...
- **이름은 10자 이내·괄호 금지·결과를 가리키는 명사구**(평균 19.6자→6.8자). 내부 id·slug는 불변.
  **예외는 마케터가 이름으로 찾는 고유명사뿐**(2026-08-19 `채널 기여도 (MMM)`): 목록을 눈으로 훑을 때 "MMM"이라는 글자가 화면에 없으면 있는 도구를 못 찾는다(⌘K로는 seoTitle 경유로 잡혀서 더 안 보인다). 괄호를 열면 **이름을 정규식으로 쓰는 검사가 조용히 어긋난다** — `new RegExp("채널 기여도 (MMM)")`은 캡처 그룹이 되어 다른 문자열을 찾는다. 이름 규칙을 바꿀 땐 이름을 소비하는 검사부터 확인할 것.
...
- ✅ 검증 하네스: `npm run test:all` **327파일·2669 통과**(1 skipped) · eslint 0 errors/0 warnings · `next build` ✓ (2026-08-25 실측). **수치를 적을 땐 실제로 돌려서 적을 것**.

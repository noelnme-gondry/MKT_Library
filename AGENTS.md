...
- **크기를 안 정하면 앱은 제일 작은 쪽으로 수렴한다 — 하한만 걸면 "몇 종을 쓰는가"는 아무도 안 본다**(2026-09-04 → 2026-09-15): 2026-09-04에 px 폰트가 912곳·27종인 걸 실측하고 12px 하한 가드를 걸었는데, 하한은 종류를 세지 않는다 — 1년 뒤 다시 재니 **1,084곳·29종으로 오히려 늘어 있었다**(모서리도 18종). 교훈을 적었다는 사실이 교훈이 안 지켜진다는 사실을 가렸다. 더 나쁜 건 **스케일이 이미 있었다는 것**이다: `--fs-xs`~`--fs-2xl` 7단이 `globals.css:81`에 "신규·수정 화면은 이 토큰에서 고른다"는 주석까지 달고 정의돼 있는데 소비처가 **3곳**이었고, 옆에 `--type-title/body/meta/display-lg`라는 **두 번째 계열**이 소비처 5곳으로 따로 있었다(§16 "신호를 만들어 놓고 읽는 곳을 안 배선한 자리"). 이름이 셋이면 다음 사람은 아무거나 고르고, 그래서 스케일이 스케일이 아니게 된다. 지금은 `--fs-*` 하나로 합치고(히어로 fluid clamp용 `--fs-3xl` 추가) 반경은 기존 `--radius-*` 5단으로 스냅했으며, `app/typeScale.test.js`가 **스케일 밖 px를 아예 못 적게** 막는다(CSS·JSX 인라인 양쪽). **하한 가드는 종류를 안 세고, 종류 가드는 하한을 안 본다 — 둘 다 필요하다.** 그리고 **가드가 리터럴 px를 적으면 크기를 고치는 순간 테스트가 먼저 반대한다**: `typographyFontContract`가 `--type-body: 14px`·`thead th … 11px`을 그대로 박아 두어 분열을 고정하고 있었고, `typographyFloor`의 스캐너 셋은 토큰 이관 직후 **리터럴이 없어 공허하게 통과**했다(지금은 토큰을 값으로 풀어 스캔한다). **단, 그때 "`--radius-full`이 12px이라 pill 자리가 각진다"고 진단한 것은 틀렸다**: 조건 없는 `:root`가 여러 벌이라 실효값은 999px이었고 나는 앞 블록을 읽고 판단했다 — 바로 이 §7이 "마지막 정의를 볼 것"이라 적어 둔 함정이다. **이름이 값과 다르면 다음 사람이 속고, 정의가 두 벌이면 도구도 사람도 앞엣것을 읽는다.**
...
- **판정을 흐림(opacity) 하나로만 표시하면 다른 파일이 취소했을 때 판정이 통째로 사라진다**(2026-09-16): `/start`는 올린 CSV로 되는 도구를 정확히 가려 `is-dim`까지 붙이는데, `library-workspace.css`가 `.app`을 앞에 붙여 특이도 0,3,0으로 `opacity: 1`을 되돌려 놓아 **20개 중 5개만 가능한 상태에서 화면 차이가 한 픽셀도 없었다**(히어로는 "지금 실행 가능한 분석만 보여줍니다"라고 말하는 채로). 취소 규칙엔 사유 주석까지 있었는데("자격은 카드가 설명한다") 카드의 `필요 데이터`는 올린 CSV와 비교한 값이 아니라 정적 계약이라 자격을 설명한 적이 없다 — **사유 주석이 사실인지도 확인할 것**. 지금은 묶음 제목과 글자(`.tool-index__avail`→`tool-index__count`)가 말하고 `app/toolIndexAvailability.test.js`가 두 파일을 함께 읽어 취소를 막는다(흐림 규칙의 **존재**도 같이 단언한다 — 근거가 사라지면 가드도 무의미하다). 이걸 찾는 동안 측정이 세 번 틀렸다: 파일 input을 잘못 골라 업로드 안 된 화면을 쟀고, 규칙이 걸린 자식(`a`) 대신 `li`의 opacity를 쟀고, `document.styleSheets` 스캐너가 0건을 돌려줘 "배포본에 규칙이 없다"고 볼 뻔했다(CSS 원문에는 있었다). **셋 다 실제 파일·실제 DOM을 다시 열어 보고서야 잡혔다.**
...
- **semantic 토큰에 라이트 오버라이드가 있어도 raw hex를 쓰면 소용없다**(2026-08 감사): `globals.css`에 다크 전용 리터럴이 51곳 있었고 **바로 옆 규칙은 `var(--danger)`를 쓰고 있었다**(한 블록 안에서 규칙이 갈림). 라이트 배경에서 대비 1.9:1(AA 미달). `ds/` 컴포넌트에도 같은 혼재가 있었는데 거긴 다른 도구가 베끼는 기준이라 전파된다. 차트 팔레트도 하드코딩하면 `refreshMountedChartThemes` 대상에서 빠진다 → `CHART_THEME.colors`·`.danger`·`.warning`·`.success`(판정 색 getter). remap Map은 **각 토큰의 다크·라이트 값 + 과거 리터럴을 전부 키로** 가져야 한다 — 하나라도 빠지면 그 데이터셋만 옛 색으로 굳는다. getter 반환값은 리터럴 hex라 `CHART_THEME.danger + "AA"` 알파 접합이 그대로 되고(§7 hex-알파 함정은 `var()`에만 해당), canvas는 `var()`를 못 읽으므로 **데이터셋엔 반드시 getter**를 넘길 것.
  ...
  - **`:root`에서 `var(--다른토큰)`으로만 선언된 별칭 토큰은 라이트 재정의가 닿지 않는다**(2026-09-16): `--bg-3: var(--surface-container)`는 값이 `:root`에서 확정되므로, `body.light-mode`가 `--surface-container`를 밝게 바꿔도 `--bg-3`은 다크(`#202632`)로 굳는다. 배경만 어두운 채 글자색은 라이트로 뒤집혀 대비가 무너진다 — 새 규칙 하나로 e2e `@light-en` 5건이 깨졌다. **grep으로는 안 보인다**: `--surface-container`를 찾으면 라이트 정의가 멀쩡히 나오고, 별칭을 한 단계 따라가야 드러난다. 전수로 세니 별칭 16개 중 4개(`--bg-3`·`--bg-4`·`--surface-hover`·`--border-strong`)가 그랬고, 사용처 6곳 중 4곳이 실제로 AA 미달이었다(최악 `.date-range-presets button:hover` **1.04:1** — 사실상 안 보임). **처음엔 "hover 배경이라 글자색이 안 짝지어져 괜찮다"고 적었는데 틀렸다** — 사용처를 열어보지 않고 말했다. 지금은 넷 다 라이트에 선언하고 `app/themeAliasTokens.test.js`가 별칭↔라이트 정합을 막는다. **대비를 계산할 땐 글자색도 "마지막 정의"로** — `--warning`을 다크값으로 재서 한 건을 8.47:1로 잘못 읽었다(실효 3.23:1, 고친 뒤 4.03:1이며 700·20px이라 large text 3:1 기준이다).
...
- **화면에 보이는지는 `checkVisibility()`로만 판정할 것 — 박스 크기로 세면 접힌 내용이 보이는 것으로 잡힌다**(2026-09-16): 외부 글의 "같은 정보 반복" 지적을 대조하려 대시보드의 숫자를 `getBoundingClientRect()`로 재고 "행 수가 세로 460px 안에 네 번 보인다"고 보고했는데, 실제로는 **다섯 중 셋이 닫힌 `<details>` 안**이었다. 이 API는 `content-visibility`를 반영하지 않아 접힌 내용도 0이 아닌 박스를 돌려준다(71×18.59가 나왔다). 그 틀린 실측 위에서 "중복" 두 곳을 지웠는데 하필 **항상 보이던 유일한 두 자리**라 대시보드에서 규모를 0번 말하는 회귀가 났다(둘 다 되돌렸다 — 실측해 보니 기사가 지적한 수준의 반복은 우리에게 없었다). 지금은 `e2e/data-shape-repetition.spec.js`가 `checkVisibility({checkVisibilityCSS:true})`로 세고 **0도 실패**시킨다(덜어내기가 과했는지도 같이 지킨다). 같은 작업에서 스캐너 함정 둘이 더 나왔다 — ① 텍스트 **노드** 단위로 세면 `{16}{"행"}`처럼 JSX가 쪼갠 문구를 통째로 놓친다(요소 `textContent`로 보되 조상 중복은 가장 안쪽 일치만 남겨 거른다) ② `\b`는 단어문자를 `[A-Za-z0-9_]`로 보므로 **한글 경계에서 성립하지 않는다**(`행\b`가 늘 실패). **셋 다 "0건"으로 조용히 통과할 수 있었는데 하한 단언(`toBeGreaterThan(0)`)이 잡았다** — 규모 단언은 스캐너의 생명줄이다. 다만 §7이 함께 요구한 "무엇을 셌는지 표본으로 눈으로 확인"은 세 번 고치도록 안 했고, 출력해 보자 한 번에 드러났다.
...
- **테스트가 "무관해 보이는" 변경에 깨지면 이분법으로 좁힐 것 — 단독 통과는 무관의 증거가 아니다**(2026-09-16): `ds/AnalysisSetupBar`에서 한 줄을 지우자 `AssistantWorkspace.smoke`의 4건이 전체 스위트에서만 깨졌다. 그 컴포넌트는 `AssistantWorkspace`가 import하지도 않고 지운 문구를 참조하는 테스트도 없다. **해당 파일만 단독 실행하면 3회 모두 통과**해서 "무관"이라 보고했는데, 베이스라인 전체 실행이 3,894건 깨끗이 통과하고 그 한 파일만 원복하니 다시 전부 통과했다 — 인과였다. 단독 실행은 부하·순서가 달라 취약 테스트를 살려 주므로 무관의 증거가 못 된다. 판정은 **① 베이스라인 전체 ② 변경 전체 ③ 의심 파일만 원복한 전체** 세 번으로 좁힐 것. 깨진 단언은 §7이 이미 지목한 `"완료" 4건`(비동기 사슬의 **중간 상태**)이라 DOM이 조금만 달라져도 갈린다. 메커니즘은 끝내 못 밝혔고, 그 한 줄의 실익이 작아 **변경을 포기**했다 — 원인 모를 결합을 안고 가느니 얻는 게 작은 변경을 버리는 편이 낫다.
...
- **글 마감 영역은 `.blog-post-outro` 한 줄 + 글쓴이 + FAQ뿐**(2026-09-03): 예전엔 연결툴·글쓴이·검토출처·검색의도·토픽클러스터·구독·관련가이드·관련용어·FAQ 아홉 블록이 세로로 이어져 본문보다 길었다(사용자 지적). 지금은 연결툴과 구독을 `.blog-post-outro` 2열에 나란히 두고, 글쓴이·FAQ만 남긴다. **FAQ 아코디언은 지우지 말 것** — `buildPostJsonLd`가 FAQPage JSON-LD를 따로 내보내므로 화면에서만 없애면 "화면에 없는 내용을 구조화 데이터로 내보내는" 상태가 되어 리치결과 자격과 §8 정직성을 함께 잃는다. `TopicClusterLinks`·`RelatedGuideList`·`RelatedGlossaryList`는 소비처 0으로 삭제됐다(2026-09-03) — 그래서 **클러스터의 내부 링크는 이제 전적으로 본문이 진다**(2026-09-08).
...
- **제목을 바꾸면 답변도 같이 봐야 한다 — 단, 가드는 최저선만 지킨다**(2026-09-08): `aeoAnswer.test.js`는 제목 콜론 앞 핵심어를 뽑아 `seoAnswer`와 대조하는데 **하나라도 겹치면 통과한다**(`if (!hit.length)`). "전부 있어야 한다"로 읽고 가드를 재현하려다 더 엄격한 사본을 만들어 "10편 중 9편이 터진다"고 잘못 보고했다(실제로 걸린 건 배치 2 기준 2편). **가드를 근거로 말하기 전에 판정식을 끝까지 읽을 것** — 재현 스크립트가 원본보다 엄격하면 없는 위험을 만든다. 그래도 답변은 같이 고치는 게 맞다: 새 타겟 질의가 답변 블록에 없으면 그 질의로 인용되지 않는다(가드가 아니라 AEO가 이유다). 콜론 없는 제목은 제목 전체가 핵심어가 되므로 앞머리를 고정어로 두고 콜론을 넣고, 어미가 그대로 핵심어가 되는 말("단계란"·"고르기"·"총정리")은 답변에 자연스럽게 못 담기니 제목에서 뺄 것. 감사에서 뽑은 GSC 문구가 제목에서 빠지면 `contentRegistry.test.js`가 잡으니 그 문구는 description으로 옮긴다. **`apple-search-ads-guide`처럼 frontmatter `answer:`가 있는 글은 레지스트리보다 그쪽이 이기므로 둘을 함께** 고친다.
...
- **폼이 안 읽는 스키마 필드는 그 필드가 없는 것과 같다**(2026-09-15): `decisionReview` v9가 `actionKind`·`goalMetric`·`guardrail*`을 갖고 `weekly-review/decisionScore`가 실행 검증·목표·가드레일까지 채점할 수 있었는데, 도구 화면의 기록 폼(`ds/DecisionReview`의 `createDraft`)이 그 필드를 **하나도 선언하지 않아** 도구에서 저장된 결정이 전부 `no_terms_recorded`로 판정 불가였다. 채우는 곳은 `WeeklyReviewScreen` 하나뿐. 게다가 목표는 자유 텍스트라 방향을 `decisionMetricDirection` 정규식이 추측했는데, 도구들이 실제로 제안하던 목표 12개 중 **8개가 그 어휘 밖**이었다(`강한 잠식 후보`·`최대 VIF`·`추정 증가분`…) — 도구는 자기 목표의 좋은 방향을 아는데 자유 텍스트로 넘기면서 버려지고 있었다. 지금은 `lib/decisionGoals.js`가 도구별 목표·가드레일을 **선언**하고 `publishedToolIds()` 파생 가드가 커버리지를 막는다. **스키마 필드를 더할 땐 그 필드를 쓰는 폼을 같은 작업에서 배선할 것** — 스코어러만 있으면 판정은 영영 안 붙는다.
...
- **신호를 만들어 놓고 읽는 곳을 안 배선한 자리가 여섯 번 반복됐다**: (2026-09-16 감사에서 둘 더 — `shouldPreferExactTest`가 정확검정 라우팅을 판정하는데 소비처가 2곳뿐이라 **A/B CSV 대량 검정이 근사 p로 유의를 선언**했고(n=50/50 전환 0 vs 5 → Fisher 0.0563 vs z 0.0218, 수동 화면과 판정이 반대) · `metricRegistry`가 파생식 SSOT인데 `dashboardVerdict`가 공식을 다시 적어 **같은 화면에 라벨이 같은 두 CVR·ROAS**가 떴다. **같은 지표를 두 곳에서 계산하면 반드시 갈린다 — 파생식은 레지스트리에서 가져올 것.**) 5-3은 채널 신뢰도를 R²·점 수 두 축으로 계산해 칩까지 띄우면서 배분엔 안 썼고(가장 불확실한 곡선으로 예산을 옮기던 상태) · MMM은 커버리지 비율을 계산해 두고 sparse 판정은 절대 주 수만 봤으며(200주 중 25주가 통과) · `REG_STATS.ols`의 `regularized` 소비처는 1곳뿐이었고(§7) · `store.findingsByGroup`은 `rankFindings()` 3중 정렬까지 갖췄는데 **소비처가 0곳**이었다(2026-09-04에 `/weekly-review`가 그 소비처가 되고 `detectFindingConflicts`가 얹혔다). **플래그·점수·저장소를 만들었으면 읽는 화면을 같은 작업에서 배선할 것** — 계산과 표시만 있는 신호는 있으나 마나다. 참고로 결론을 publish하는 도구는 전부가 아니라 `findingProducers.TOOL_KIND`의 넷(5-2·5-21·5-22·5-3)이다.

...
await writeFile(path.join(project, 'design.md'), `# Product tutorial design\nFast four-action tutorial: 24 seconds; actual UI evidence first. One focus zoom per chapter, then a readable hold. Rule: ui-focus-zoom servo settle plus stat-bars-and-fills chapter progress. Use real local UI captures as the focal evidence, with synthetic-data label. Pretendard matches the application; no second display face. Palette: #eef1f6 background, #1e293b text, #315be8 accent. Each frame has top title, actual UI in a 1000×560 crop displayed at 844×472.64, right step explanation, and bottom chapter progress. Minimum caption size 24px. Silent for in-app viewing. Short captions, decisive chapter cuts and a single anchored focus move replace nine-second static holds.\n`);
...
for (const tutorial of VIDEO_TUTORIALS) for (const locale of ['ko', 'en']) {
  ...
  const duration = TUTORIAL_STEP_SECONDS * tutorial.steps.length;
  manifest.push({ id, duration, checks: [0.3, ...tutorial.steps.map((_, index) => (index + .5) * TUTORIAL_STEP_SECONDS), duration - .1] });
  ...
  const styles = `@font-face{font-family:Tutorial;src:url('${font}') format('woff2');font-weight:100 900}html,body{margin:0;width:100%;height:100%;background:#101827;color:#f8fafc;font-family:Tutorial,sans-serif}*{box-sizing:border-box}#root{width:100%;height:100%;position:relative;overflow:hidden}.slot{position:absolute;inset:0}.scene{width:100%;height:100%;padding:34px 36px;display:flex;flex-direction:column;gap:24px;background:#101827}.top{display:flex;justify-content:space-between;align-items:center;gap:24px}.top h1{font-size:32px;font-weight:750;margin:0;letter-spacing:-.03em}.top span{font-size:18px;color:#cbd5e1}.work{display:grid;grid-template-columns:844px minmax(0,1fr);gap:28px;align-items:center;flex:1;min-height:0}.shot{width:844px;height:472.64px;border:1px solid #c6cfdf;border-radius:10px;overflow:hidden;background:#f8fafc;box-shadow:0 10px 24px #13243a14}.shot img{display:block;width:100%;height:100%;object-fit:contain}.text h2{margin:14px 0 22px;font-size:30px;line-height:1.4;letter-spacing:-.035em;word-break:keep-all}.text p{margin:0;font-size:24px;line-height:1.65;word-break:keep-all}.number{display:block;font-size:48px;font-weight:800;color:#a5b4fc}.bottom{display:flex;align-items:center;justify-content:space-between;gap:20px;color:#cbd5e1;font-size:18px}.progress{height:4px;display:block;background:#a5b4fc;transform-origin:left center;width:100%;position:absolute;bottom:0;left:0}`;
  await writeFile(path.join(dir, 'styles.css'), styles);
  let vtt = 'WEBVTT\n\n';
  for (let i = 0; i < tutorial.steps.length; i++) {
    const step = tutorial.steps[i], text = step[locale], scene = `${id}-step-${i}`;
    const focus = JSON.parse(await readFile(path.join(project, 'assets', `${step.frame}-${locale}.focus.json`), 'utf8'));
    ...
    await writeFile(path.join(dir, 'compositions', `${scene}.html`), `<!doctype html><html><body><template><style>#${scene}{width:100%;height:100%}</style><div id="${scene}" data-composition-id="${scene}" data-width="1280" data-height="720" data-duration="${TUTORIAL_STEP_SECONDS}"><section class="scene"><header class="top"><h1>${escape(tutorial[locale])}</h1><span>GROWTH OPT PLAYBOOK</span></header><div class="work"><div class="shot"><img src="assets/${step.frame}-${locale}.png" alt="${escape(text.title)}"></div><div class="text"><span class="number">${String(i + 1).padStart(2, '0')} / 04</span><h2>${escape(text.title)}</h2><p>${escape(text.body)}</p></div></div><footer class="bottom"><span>${en ? 'Actual app · synthetic demo · account actions simulated locally' : '실제 앱 화면 · 가상 데이터 · 계정 동작은 로컬 시연'}</span><span>${en ? 'Pause to follow along' : '잠시 멈추고 따라 해보세요'}</span></footer></section></div><script>const tl=gsap.timeline({paused:true});tl.fromTo('#${scene} .text',{y:14,opacity:0},{y:0,opacity:1,duration:.28,ease:'power2.out'},0);tl.fromTo('#${scene} .shot img',{scale:.90},{scale:1,transformOrigin:'${focus.x}% ${focus.y}%',duration:.55,ease:'power2.inOut'},.75);window.__timelines['${scene}']=tl;</script></template></body></html>`);
    vtt += `${i + 1}\n${ts(i * TUTORIAL_STEP_SECONDS)} --> ${ts((i + 1) * TUTORIAL_STEP_SECONDS)}\n${text.title}\n${text.body}\n\n`;
    storyboard.push(`## Frame ${id}-${i + 1}\nstatus: outline\nsrc: ${id}/compositions/${scene}.html\nRule: stat-bars-and-fills (continuous elapsed chapter progress at parent). UI focus zoom: establish 0–0.75s, settle 0.75–1.3s, hold through ${TUTORIAL_STEP_SECONDS}s; hard cut to the next action.\n${text.title}: ${text.body}\n`);
  }

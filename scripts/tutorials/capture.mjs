...
for (const locale of ['ko', 'en']) {
  ...
  // Local fixture: starts as an authenticated account without a trial. Consent
  // to its selected synthetic memo activates the trial through the real UI.
  let trial = false;
  ...
  await page.addInitScript(() => { localStorage.setItem('mkt-library-theme', 'light'); localStorage.setItem('gop:sidebar', 'collapsed'); });
  const snap = async (name, selector) => {
    ...
    const x = Math.max(0, Math.min(box.x - 16, 1280 - 1000));
    const y = Math.max(0, Math.min(box.y - 16, 800 - 560));
    await page.screenshot({ path: path.join(out, `${name}-${locale}.png`), clip: { x, y, width: 1000, height: 560 }, animations: 'disabled' });
    console.log(`${locale}: ${name}`);
  };
  const upload = async () => {
    const uploader = page.locator('.csv-uploader[data-hydrated="true"]').first();
    await uploader.locator('input[type="file"][accept*="csv"]').first().setInputFiles({ name: 'tutorial-campaigns.csv', mimeType: 'text/csv', buffer: source });
  ...
  await page.getByRole('dialog').getByRole('button', { name: en ? 'Not now' : '나중에', exact: true }).click();
  ...
  await page.goto(`${base}${prefix}/weekly-review`);
  await snap('review-empty', '.project-review-workspace');
  const reviewUpload = await upload();
  await reviewUpload.getByRole('button', { name: en ? 'Analyze data' : '데이터 분석하기', exact: true }).click();
  ...
  await dialog.getByRole('button', { name: en ? 'Create project and save' : '프로젝트 만들고 저장', exact: true }).click();
  ...
  await snap('preview', '[role="dialog"]');

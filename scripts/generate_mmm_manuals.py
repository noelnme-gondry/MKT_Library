...
def korean_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    s += [
        ...
        callout(
            "중요한 범위",
            "기본 효과 추정은 채널별 one-at-a-time profile 평균입니다. 다만 변환 불확실성이 큰 최대 2개 채널에 대해서는 상위 후보를 최대 4개 조합으로 추가 점검합니다. 이 제한적 joint 진단은 계수를 자동 교체하지 않으며 전체 채널 joint posterior를 의미하지 않습니다.",
            styles,
            "red",
        ),
        heading("7.5 제한적 baseline knot 선택", 2, styles),
        body(
            "유효 주차가 78주 이상이면 knot 0개·1개·2개 후보를 비교합니다. 각 후보를 적합하고 BIC가 최소 6 이상 개선될 때만 자연수요 baseline에 hinge knot을 적용합니다. 개선이 작거나 불안정하면 기본 추세·Fourier·step 구조를 유지합니다. knot을 많이 허용하면 자연수요 변화가 광고 효과를 흡수할 수 있으므로 후보 수를 제한합니다.",
            styles,
        ),
        heading("7.6 실험 prior의 mROI 표시", 2, styles),
        body(
            "실험 prior는 변환 feature 계수 단위로 모델에 적용되지만, 화면은 현재 주간 spend에서 Hill 미분을 곱해 추가 spend 1단위당 기대 KPI인 mROI도 표시합니다. 이 값은 기존 prior의 단위 변환 참고값이며 별도의 ROI posterior 샘플링 결과가 아닙니다.",
            styles,
        ),
        heading("7.7 반응곡선과 정상상태", 2, styles),
        body(
    ...
    s += [
        ...
        data_table(
            ...
            [
                ...
                ["reach/frequency", "계수 모델에는 넣지 않고 후속 진단으로 보류", "도달·빈도 정의와 중복률이 안정적일 때 확장"],
                ["ROI/mROI prior", "실험 계수 prior를 현재 spend점 mROI로 표시·단위 변환", "비용·매출 단위와 ROI estimand를 엄격히 정렬"],
                ["knot baseline", "78주 이상에서 0/1/2 knot BIC 선택만 제한 적용", "장기간 비선형 baseline과 충분한 데이터"],
                ["joint transform averaging", "불확실성 상위 2개 채널·최대 4조합만 진단", "전체 joint posterior는 오프라인 MCMC/SMC"],
...
def english_story(styles: dict[str, ParagraphStyle]) -> list:
    ...
    s += [
        ...
        heading("7.5 Limited baseline-knot selection", 2, styles),
        body(
        ...
        heading("7.6 Experiment-prior mROI display", 2, styles),
        body(
        ...
        heading("7.7 Steady-state response curves", 2, styles),
        body(

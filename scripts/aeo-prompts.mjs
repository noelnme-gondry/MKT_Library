...
- **언급률** = 언급된 응답 수 ÷ 실제 AI 답변을 받은 수. 실행률·오류·AI 답변 미노출은 따로 기록한다.
- **인용률** = 우리 도메인을 인용한 응답 수 ÷ 실제 AI 답변을 받은 수. 답변이 없으면 미측정이며 0%가 아니다.
- 플랫폼·로케일·실행 조건·응답 원문과 실제 인용 URL을 함께 보관한다. 상세 계약은 \`docs/aeo-runs/README.md\`를 따른다.
- 두 수치 모두 절대값보다 **같은 목록의 월별 변화**가 정보다. 목록이 바뀌면 그 달에 표시해 둘 것.
- 제3자 언급과 자사 도메인 인용은 구분한다. 이 목록이 개선되지 않는다는 이유만으로 페이지를 늘리지 않는다.
`;

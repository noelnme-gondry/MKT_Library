...
여기서 자주 보는 실수가, 우선순위를 퍼널 순서대로(가입 1순위 → 장바구니 2순위 → 구매 3순위…) 넣는 거예요. 직관적으로는 맞아 보이는데, 우선순위는 '먼저 일어나는 순'이 아니라 '중요한 순'이라 매출 이벤트가 위로 가야 해요. ATT 미동의 유저는 우선순위가 가장 높은 이벤트 위주로만 신호가 잡히는 구조라, 구매를 아래에 두면 정작 매출 신호가 잘려나가거든요. 세팅 끝나면 우선순위를 위에서부터 한 번 다시 읽어보세요.

...
keywords: "마케팅 예산 배분, 한계효용, 한계 CPA, 응답곡선, 채널 예산 최적화, 퍼포먼스 마케팅, ROAS, 예산 재배분, 채널 믹스, 그로스 마케팅, 광고 예산 짜는 법, 예산 배분 방법, 채널별 예산 계획"
tags: ["예산 배분", "퍼포먼스 마케팅"]
draft: false
---
...
이 구분이 확 와닿는 순간이 있어요. 채널 담당자별로 성과 보고를 받을 때예요. 다들 자기 채널의 평균 ROAS를 들고 오죠. 그런데 "지금 100만 원을 더 받으면 얼마를 만들 수 있어요?"라고 물으면 답이 갈려요. 평균이 제일 좋은 담당자가 제일 머뭇거리기도 해요. 이미 부을 만큼 부었다는 걸 본인이 제일 잘 아니까요. 예산 회의에서 물어야 할 건 지난달 평균이 아니라 이 질문이에요.
...
그런데 문제는, 채널마다 응답곡선을 그리고 한계효율을 손으로 추정하는 게 꽤 번거롭다는 거예요. 엑셀로 곡선 피팅하는 것도 일이고요. 이런 계산을 매번 하기 번거로우실 테니 만들어둔 게 있습니다. [예산 배분 툴](/tools/budget-allocation)에 채널별 지출·전환 CSV를 올리거나 구글 시트를 연동하세요. 채널마다 한계효용 곡선을 추정해서 최적 배분안을 뽑아줍니다. 시트로 연동해두면 예산 짤 때마다 새로 파일 만들 필요 없이 꾸준히 최신 데이터로 돌려볼 수 있어요. (물론 실제 상황에서는 효율 좋은 채널이 한계 효율도 제일 좋은 경우도 많아요. 그럴 경우를 대비해 효율별 예산 분배도 만들어두었습니다.)
...
같은 한계효용 관점을 CPA 대신 ROAS로 풀어본 예시가 궁금하면 [ROAS 개선 글](/blog/roas-improvement)도 같이 보세요.

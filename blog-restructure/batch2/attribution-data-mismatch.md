...
이 네 가지가 한꺼번에 작동하면 격차가 꽤 커져요. 그래서 추천하는 습관이 하나 있어요. 분기에 한 번, 매체·GA·MMP·결제 DB 네 숫자를 같은 주 기준으로 나란히 놓고 **우리 계정의 '평소 격차 비율'**을 재두는 거예요. 매체가 결제 DB의 1.2배쯤이 평소라면, 어느 날 1.8배가 됐을 때 그게 이상 신호로 보이거든요(비율은 계정마다 달라요). 격차 자체가 아니라 격차의 '변화'가 진짜 정보예요.
...
참고로 GA4 숫자 자체가 이상하게 느껴진다면 그건 별도 문제예요. 세션 정의나 집계 지연 같은 GA4 고유의 함정은 [이 글](/blog/ga4-data-traps)에 정리해뒀어요.
...
숫자 기준을 정리하고 나면 다음 질문이 생겨요. "그래서 어느 채널에 얼마를 더 써야 하지?" 그 계산은 대시보드 숫자만으론 안 나오는데, 매번 스프레드시트로 돌리기 번거로우시면 저희가 만든 [무료 분석 툴](https://mktlibrary.up.railway.app)에 매체 리포트나 MMP export CSV를 올려보세요. 증분 분석과 MMM 기반 기여 분해까지 브라우저 안에서 바로 돌아가고, 데이터는 서버로 나가지 않아요.

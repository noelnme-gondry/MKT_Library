...
- **배포**: Railway (`mktlibrary-production.up.railway.app`) — `main` 자동 deploy. **커스텀 도메인 `growthoptplaybook.com`**(SEO canonical/OG/sitemap/robots/`rss.xml`[네이버 제출용, `app/rss.xml/route.js`]는 `routeMap.js` `SITE_URL` SSOT + `layout.js` 하드코딩). AdSense `ca-pub-3073450406371629`(layout Script·meta·`public/ads.txt`).
...
- **필터 옵션 생성 ↔ 필터 비교의 trim 불일치 = "선택해도 0행"**(5-2 source/paid 버그): `DashboardFilterBar` 옵션은 `String(v)` 그대로, `getMonFilteredRows`는 `.trim()`으로 비교 → `" Paid"`처럼 공백 있는 값은 옵션과 안 맞아 선택 시 전부 0. **옵션 생성·필터 비교 양쪽 trim 통일**(채널·국가·소스·플랫폼). 값 기반 필터는 항상 생성·비교 정규화를 같이 맞출 것.
- **헤더명이 아니라 "값"으로 자동매핑하는 필드는 valueVocabulary로**(source=광고/오가닉): 필드 키가 `source`면 헤더 "source"가 키명 일치(0.82)로 잡혀 값이 구글/메타여도 채널 대신 source로 오매핑. `scoreMappingCandidates`에서 `field.valueVocabulary` 있으면 **헤더/타입 점수 무시하고 값 어휘 매칭(≥0.6→0.9)만** 사용 → 값이 organic/paid일 때만 auto-map(구글/메타는 channel 유지). 컬럼 고유값은 저카디널리티(≤40)만 수집.
- **시즈널리티 다운로드·검증 = 순수 헬퍼 + 렌더층**(5-2 §12.18 연장): `annotateCalendarPeriod`(seasonalityMath, 엔진과 동일 버킷 규칙 재사용·골든 무관)로 ISO 뷰 부여 + DownloadHub 3종(원본ISO·소스채널집계·그래프데이터, BOM+CRLF §7). 추세 제외 검증 차트는 `points[].value` vs `points[].trend`(항상 계산됨) 오버레이 — 검증 축은 detrend 무관 실제값 포맷(`valueTickFormat`), detrend %는 오버레이 차트만.
...
### 12.26 분석하기 전면 광고 인터스티셜 + 광고 제외 모드 (PR #290)
분석하기 클릭 시 광고 모달(뒤 블러)→N초 카운트다운 후 닫기→닫으면 실제 분석 실행. **store 게이트 패턴**: `adGate`(휘발 open/pending/seq)+`requestAd(cb)`/`closeAd`(adFree면 cb 즉시, 아니면 모달 오픈→닫을 때 cb). `adFree`는 persist(partialize 추가 — 설정이라 §2.2 원본데이터 아님). `AdInterstitial.jsx`=body portal·backdrop-filter 블러·AdSense `ins`(key=seq로 열 때마다 리마운트+`push({})`, 중복 push는 try/catch). `AdFreeInit.jsx`=비밀 URL 파라미터(`?adfree=<토큰>`→persist, `off`로 해제, 즉시 히스토리 치환 제거; 토큰은 클라 번들 노출=숨김이지 보안 아님). 배선=전 도구 분석버튼 onClick을 `requestAd(()=>기존액션)`으로 래핑(CsvUploader 공용이 대부분 커버 + AHA/MMM/ContentElement 개별 버튼). **데모 자동로드는 게이트 없이 직접**(광고 안 뜸). **함정**: ①persistPartialize에 새 키 추가하면 `useDataStore.test.js` 불변식 골든 키 배열 갱신 필수 ②버튼 클릭하는 스모크는 `adFree:true` seed로 광고 우회 ③setState-in-effect(카운트다운 리셋)는 eslint-disable 주석. **정책(§8 정직)**: 콘텐츠 강제 게이팅은 AdSense 정책 위반 소지 — 카운트다운·라벨로 완화하나 계정 리스크 사용자에 명시(회색지대, rewarded/interstitial 전용 포맷이 정석).

### 12.25 세그먼트 나눠보기 필터 (매핑 role→토글, 5-20·5-18, PR #282)
매핑 단계에서 차원 컬럼(성별·플랫폼·국가)을 지정하면 분석을 그 값별로 나눠 봄. **엔진·게이트 불변, 행 부분집합만 필터해 재계산**. AHA: colMap `role=segment` DnD zone + `guessRole` 화이트리스트로 platform/gender/country/age_group 자동 안착(그 외 문자열은 tray, 오탐 방지) + 결과 최상단 **단일 토글 pill**([전체]+값)→그 값 행만 `computeAhaCache` 재계산(rAF 디퍼 오버레이 §7). 세그먼트=탐색 토글이라 **게이트 시그 밖**(재분석 불필요), `validSeg`로 매핑 잔상 방지. MMM: 기존 `platform` role 일반화(`mmmSegmentValues` 고유값 열거→breadcrumb pill, 태그모드와 상호배타), `effPlatformFilter` 파생값으로 잔상 방지(setState-in-effect 회피). 데모엔 세그먼트 컬럼 없어 골든/스모크 byte-동일. **함정**: `ahaParseActionWindow` D1/D7 인식은 접미(`_7d`)·접두(`d7_`)·`day7`·`_7일`까지 확장(전엔 `_d7`만) — action 비면 파싱 실패(원 동작). 일괄버튼(`mapAllAsFeature`)은 segment도 제외해야 사용자 지정 보존.

### 12.28 랜딩 히어로 + 홈 구조 (Semrush형 전환 랜딩, `components/landing/*`)
"앱 목록" 느낌 랜딩을 전환형 SaaS 히어로로. `LandingHome`(LandingPage.jsx) = ① 큰 헤드라인+서브+CTA 2개(내데이터/데모, 둘 다 `goTool("5-2")`)+프라이버시 배지 ② `ProductPreview`(브라우저창 프레임, 6개 도구 미니화면 2.8초 로테이션+제목 전환+씬 인디케이터; SVG 목업 결정론·전역store 비침습; `videoSrc` prop 주면 `<video>` 즉시 교체) ③ `ToolCarousel`(←/→ + **마우스 드래그** scroll-snap 질문 슬라이드, 드래그후 클릭 가드, 트랙 paddingTop으로 호버 짤림 방지; 카드=eyebrow+훅+"+"+목업) ④ **블로그 | SOP 나란히**(`.home-hub-card`, /blog·/guide). 목업=`ToolCardMock`(도구별 CSS/SVG `TOOL_MOCK_TYPE`) 기본+`LiveMiniChart` 1개.
- **브랜드·언어 일원화**: 로고+"Growth Opt Playbook"은 전역 `Header` 좌상단(브레드크럼 첫칸, favicon.svg), 언어전환은 Header EN 토글만 — 랜딩 자체 eyebrow/English 버튼(중복) 제거.
- **콘텐츠(9-x)→분석 흡수**: `SECTIONS.analysis`에 "09" 추가·`content` 섹션 삭제(사이드바 자동 반영), slug `/content/*` 보존. 캐러셀은 opsGroups(09 포함) 파생.
- **무주소 게이트 제거 → 뒤로가기 정상**: 랜딩 `track`(guide/content 상태화면) 삭제. 가이드는 `/guide` 실제 라우트(`GuideIndex.jsx`, routeMap `guide-index`, PageClient KR·EN 분기+SOP폴백 제외 가드, blog처럼 자체 주소). 개별 가이드는 이미 `/guide/*`.
- **정직성**: 유저수·로고 날조 금지(§8). CSS `.carousel-arrow`·`.carousel-track`·`.home-hub-card`·`.guide-index-item` globals.css. EN=`hero` 카피 `tr`+GuideIndex COPY.
- **전 페이지 헤더/셸 완전 통일**(중요): 도구·SOP·홈·**블로그·가이드** 전부 동일 셸(메인 `Sidebar`+공용 `Header`+`GlobalModals`). 예전 슬림 헤더 3종(`BlogHeader`·`EnLandingHeader`·`EnGuideSidebar`)이 홈/테마만 있어 EN토글·⌘K·CSV가 빠져 페이지마다 상단이 달랐음 → **전부 삭제**하고 공용 `Header`로 교체(`app/blog/layout`·`app/en/blog/layout`·`app/en/guide/layout`·`app/en/page`). 블로그는 routeMap 밖이라 `Header`가 `pathname`으로 직접 감지(`isBlog`)해 "블로그" 크럼+언어전환(/blog↔/en/blog) 처리. 홈은 `app is-home`(브랜드만, `is-home .breadcrumb{display:none}` 규칙 제거). 브랜드=`brand-mark`("GO")+이름, 전 페이지·양 로케일 공통. **규칙: 새 라우트/레이아웃도 반드시 이 공용 셸 사용**(슬림 헤더 재도입 금지). 미번역 EN 도구/가이드는 클릭 시 `hasEnVersion`=false→KR(구조는 동일).
- **캐러셀 드래그**: `scroll-snap mandatory`가 드래그를 매 프레임 카드경계로 잡아채 "뚝뚝" → 제거(자유 스크롤)+드래그 중 `scroll-behavior:auto`·grabbing. 시연 슬롯=KPI+차트+사이드 요약/판정칩 미니앱(그래픽 1개 X).
- **데모 안내 모달**(`DemoNoticeModal`, `demoNoticeSeen` 휘발 store·persist 제외): 도구 진입 시 데이터가 데모(fileName "demo_")면 세션 1회 "데모 사용 중, 우상단 CSV 변경" 안내. PageClient KR·EN 5-/9- 마운트. setState-in-effect는 조건부 1회라 eslint-disable.
- **"내 데이터로 시작" 게이트**(`/start`=`start-gate`, `StartGate.jsx`): 히어로 primary CTA → 이 페이지. 진입 시 `startMyData()`(store): `demoDisabled=true`(휘발) + **데모 슬라이스(fileName "demo_")만 비움**(실 업로드 보존)+csvData 미러 갱신. 도구 선택 카드 → 빈 업로드 진입. `demoDisabled`가 데모 자동로드 5곳(`CsvUploader`+Aha·ContentElement·Incrementality·MarketingResponse)의 `if(!hasData && !demoDisabled)` 가드로 억제. "데모 보기" CTA·게이트 "둘러보기" 링크는 `setDemoDisabled(false)`로 복원. 새로고침=리셋(첫인상 데모 복원). EN=`/en/start`(catch-all).

### 12.24 블로그 (SEO 마케팅 컬럼, `/blog`)
`routeMap`(도구 라우팅) **밖**의 fs 기반 MD 파이프라인 — 도구/SOP와 독립. **글 발행 = `v2-migration/content/blog/<slug>.md` 파일 추가**
- **여러 글 → 필라 통합(consolidation) = 6곳 동시 갱신**(W30 예산 4→1): ① `content/blog(-en)/`·`content/glossary(-en)/` 파일 add/삭제(EN 짝파일 필수 — `blog-en`·`glossary-en`이 실제 경로, `en/blog` 아님) ② `next.config.mjs redirects()`에 구 URL→필라 301(ko·en 각각) ③ **레지스트리 3종 정합**(`contentRegistry.test.js`가 강제): `contentToolRegistry`(BLOG_PRIMARY_TOOL·RELATED_GLOSSARY 키=발행글 slug 정확 일치)·`blogSeo`(KO_TITLES·EN_TITLES)·`localizedHref`(EN_BLOG_SLUGS) ④ 삭제글 참조하던 glossary `relatedPosts`는 필라로 재지정(body 링크는 301로 해결 OK). 내부 링크는 KR 상대경로만(렌더러가 EN 접두). zip 등 외부 SSG 프론트매터(slug 경로형·lang·translationKey·redirect_from)는 repo 스키마(bare slug·keywords 콤마문자열·tags 배열)로 **변환**해 쓸 것.(frontmatter: `title`≤40·`description`≤80·`date`(ISO)·`slug`·`keywords`·`tags`·`draft`·`ogImage`; `_TEMPLATE.md` 복사, `_`프리픽스·`draft:true`는 미발행). `src/lib/blog.js`(`getAllPosts`/`getPostBySlug`, **server 전용** — gray-matter+marked, 클라이언트 import 금지). 라우트 `app/blog/page.js`(목록·빈상태)·`app/blog/[slug]/page.js`(`generateStaticParams`+`generateMetadata` canonical/OG article). **SEO 연동**: `sitemap.js`·`rss.xml/route.js`가 `getAllPosts`로 글을 직접 포함(routeMap 밖이라 SSOT 파생 안 됨 — 블로그는 fs가 SSOT). 타이포 `globals.css` `.blog-prose`. Sidebar에 `/blog` Link 1개. **SOP(JSON)는 MD 이관 안 함** — 렌더 HTML이 SEO 대상이라 소스 형식은 SEO 무관(불필요 리스크).

...
`AdInterstitial.jsx`·`AdFreeInit.jsx`와 `adGate`·`adFree` 상태는 현재 코드에 없다. `requestAd(cb)`만 기존 분석 버튼 호출부 호환을 위해 콜백을 즉시 실행하는 no-op 래퍼로 남아 있다(`useDataStore.js`). 새 기능은 이 래퍼를 광고 게이트로 확장하거나 비밀 URL 모드를 복구하지 않는다. 수익화 변경은 사용자 이탈·AdSense 정책 데이터를 확인한 별도 의사결정으로 다룬다.

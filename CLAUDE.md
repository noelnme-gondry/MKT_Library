...
- **절대 기여(level) 선호 — "그 주 지출이 만든 유저"** (PR #91): 분해를 볼 때 centered(평균±편차)보다 **level(각 주 지출이 실제로 만든 예상 인원, 저지출=0·지출↑=양)**을 강하게 선호. "전체 기여 정해놓고 더했다 덜했다"를 거부. 단 통계적 정합성($0 외삽 금지)은 floor-기준 level로 충족(§12.11). 하네스 규칙이라도 **사용자가 방법론을 명시 지정하면 그게 우선**(§15.6) — 단 honest하게 구현(외삽 회피·캐비엇).
...
### 12.11 주별 기여 분해 — LEVEL(채널=floor 기준) + centered(비매체) (PR #61→#91)
**사용자 명시 결정(PR #91)**: 분해는 "각 주 지출이 만든 예상 유저"(level)여야지, "평균 기여 정해놓고 ±"(centered)가 아니다. centered는 $0/저지출 주에 음수가 나와 오해. → **채널 contrib = `β·(X−min_j)`**(관측 최저 지출 floor 대비 level, floor주=0·지출↑=양), **비매체(추세·계절·휴일·레짐)=`β·(X−mean)`**(모듈레이터, 평균±). `baseline = β0 + Σ_채널 β·min + Σ_비매체 β·mean`(floor 매체 수준), `baseline + Σ기여 = OLS fitted`(항등식 불변). **⚠ $0 절대기준 level은 금지** — 모델이 $0을 본 적 없어 `ln(1)=0` 외삽 → baseline 음수(예: Tinder −7138)·채널 과대귀속(Google +8825 of 3365). **floor(최저 관측)는 in-sample이라 baseline 정상(예: 2240)**. (즉 §12.11 원칙 "ln(1)=0 외삽 금지"는 유지하되, 평균-편차 대신 floor-기준 level로 사용자 의도 충족.) 스파이크 지배 드라이버는 level 절대크기 아니라 **평균 대비 편차**(`contrib−meanContrib`)로 판정(채널이 항상 최대가 되는 것 방지). 표는 채널=평균 기여(level)·비매체=변동± 2열. baseline 토글(§12.18)·툴팁 개별기여(PR #89)와 함께.

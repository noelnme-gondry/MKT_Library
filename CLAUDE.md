...
사용자 핵심: "첫 화면은 선택 창만, 사이드바도 숨김". `pageHome`을 **2단계 분기**로 — `LANDING_STATE.track`(null=STEP1 / "guide" / "analyze"). STEP1=가이드 vs 분석 2 hero 카드(`data-landing-track`), STEP2a=가이드 phase(p1/p2/p3·p4 도구 제외) `renderLandingGuide`, STEP2b=분석 버킷(`ANALYZE_BUCKETS` 4개: 시각화·모니터링 free / 효율화·실험·고급 Pro)→도구 카드 `renderLandingAnalyze`. 핸들러는 `bindCardLinks`에 `[data-landing-track]`(세팅+navigate home)·`[data-landing-back]`(track=null) 추가. **트랙 리셋**: navigate가 실제 페이지 진입 시 `LANDING_STATE.track=null`(홈 복귀 시 항상 STEP1) + `bindBrandClick`도 리셋. `is-home`(사이드바·탑바 숨김)은 기존 토글 그대로(id=home). **freemium 티어**: `TOOL_TIER`(5-2=free·나머지 pro)+`tierBadge`(무료 초록/🔒Pro), **5-2를 `AUTH_PROTECTED_PAGES`에서 제거**(무료 사용). track 카드는 `data-route` 없어 `.phase-card[data-route]` 네비 셀렉터에 안 걸림(별도 attr로 스코프). phase 라벨 개명 초기→셋업·중기→운영·후기→운영 후 분석(displayItemNumber는 위치 기반이라 무영향, 라우팅 id 불변 §12.8). render-only→골든 byte-동일. 검증 `validate_landing` 15/15(주입식). **남은(PR-6~)**: Pro 페이월 블러+잠금+키요청 팝업(인스타 DM @gondry__workshop)·다크 SaaS 비주얼·sticky 필터·⌘K·매핑 DnD 통일.

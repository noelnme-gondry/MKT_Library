...
- **⚠ `<select multiple size="1">` + `overflow-x:auto` 부모 = 드롭다운 통째 클리핑**: PR#167 커스텀 드롭다운도 `.mon-filter-inner{overflow-x:auto}`(+sticky 바)가 `position:absolute` 리스트를 잘라 안 보임. **해결: `position:fixed;z-index:9999` + 토글 시 `getBoundingClientRect()`로 top/left 계산**(부모 overflow 무관). 전역 close 리스너(click/scroll)는 `window.__monMultiselGlobalBound` 가드로 1회만(navigate 누적 방지), 토글은 `e.stopPropagation()`로 같은 클릭의 close 방지. ⚠ `preview_click`(MCP)은 추가 이벤트로 닫지만 실제 `btn.click()` 버블은 stopPropagation으로 열림 유지 — headless 클릭 결과를 곧이곧대로 믿지 말 것.

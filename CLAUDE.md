...
최소 grain(채널×캠페인×소재×일) Bennet 분해 1회(`decomposeFinest`) 후 `rollup`으로 §2 채널→§3 채널·캠페인별→§4 소재별(모드 A) 드릴다운 항등식 정합. `mix=(cpāᵢ−C̄)·(s2−s1)`(전체 평균 대비 centering, §8.8)+`rate=s̄ᵢ·(cpa2−cpa1)`, `Σcontribution=ΔCPA_total`. 표시 라벨 "CPA/CPI 영향"(=mix+rate, 먼저 볼 값) + §2 Mix·Rate 설명 토글. **§4 계층 드릴다운은 `rollup` 복합 keyFn으로**: 채널전체=`ch│cmp│cr`/채널특정+캠페인전체=`cmp│cr`/둘다특정=`cr`(단일레벨 키로 묶으면 상위 over-merge 버그). 각 그룹=단일 finest → `children[0]`로 대표 ch/cmp/cr 추출(url·CTR·New). §4 표 20행 페이지네이션(`crPage`, 필터변경 시 1리셋). New=이름 앞 컬럼·링크=영향 뒤 컬럼(라벨 인라인 X). 기간 `weekBasis`(calendar/rolling7)×`lookback`(1/2/3). ₩/$ 토글(`pvmFmtMoney`, 표시층). 소재 URL: `creative_url` 매핑 시 §4 링크 컬럼(merge는 비용 최대 변형 `crUrlMap`, `pvmSafeUrl` http/https만=XSS 차단). §0 sleek 칩(`pvmImpactChip`, 빨간점 이모지 대체)+Top-mover 카드, §1 Bridge 2줄(P1→P2=Δ%), §2 시각화 2종(`renderPvmCharts`: floating-bar waterfall+Mix·Rate 가로 누적막대). 결과 CSV(`downloadPvmCsv`, section 태그 META/SCORECARD/CHANNEL/CAMPAIGN/CREATIVE_FULL/CREATIVE_MERGED, `_mmmDownload`+CRLF+BOM). 모드 B(`crMode:"creativeOnly"`)는 의도적 비중첩. `runPvmTests()` 36/36 + 확장 harness(전체계층 Σ·비병합·페이지네이션·차트·CSV).

...
### 12.28 랜딩 히어로 (Semrush형 전환 랜딩, `components/landing/*`)
"앱 목록" 느낌 랜딩을 전환형 SaaS 히어로로. `LandingHome`(LandingPage.jsx) = ① 큰 헤드라인+서브+CTA 2개(내데이터/데모, 둘 다 `goTool("5-2")`)+프라이버시 배지 ② `ProductPreview`(브라우저창 프레임+실제 Chart.js 미니차트, 데모데이터·`pointer-events:none`·전역store 비침습; `videoSrc` prop 주면 `<video>`로 즉시 교체 — 실제 mp4 나중) ③ `ToolCarousel`(←/→ scroll-snap 질문 슬라이드, 카드=eyebrow(그룹)+훅 헤드라인+"+"+목업, 클릭→`goTool`). 카드 목업=`ToolCardMock`(도구별 CSS/SVG 미니차트 `TOOL_MOCK_TYPE` 매핑, 결정론) 기본, `liveCardId` 1개만 `LiveMiniChart`(실제 Chart.js·옵션4 비교). **정직성**: 유저수·로고 날조 금지(§8) — 프루프는 "브라우저100%·설치없음"만. CSS `.carousel-arrow`·`.carousel-track` globals.css. EN=`hero` 카피 `tr`.

...
### 12.39 운영 대시보드 8개 피드백 — Bundle A/B/C (PR #175~177, 2026-06-22)
- **Bundle A(PR#175)**: 플랫폼 필터 네이티브 → 다중선택 커스텀 드롭다운(§12.37 body portal 패턴 재사용), 코호트 탭 D7→D14/D30/D60 토글(`COHORT_EXTRA_STATE.wgtDay`), 리텐션 td `class="data tnum"` 정렬(§12.38 anchor 패턴).
- **Bundle B(PR#176)**: LTV D360 예측 = `COHORT_MATURATION` 엔진 재사용(§12.35), `monLtvCacBody`에 prediction 주입. ⓘ 정보아이콘 `mmmInfoIcon` 패턴(§12.10). D0~D360 성숙 곡선 차트(`renderLtvCurveChart`).
- **Bundle C(PR#177)**: ① 스코어카드 D30 제거(window `[7,14,28]`)+카드 클릭 드릴다운(혼합차트: 연속지표=line·절대지표=bar, front/back 색 구분). ② 퍼널 요일보정 2-bucket additive(`cvrAdj=cvr−grpMean[isWe]+dailyMean`), 표본 부족(≤3) 시 disabled 가드. ③ 페이싱 `weekdayProfile/paceWeekday`, revenue_d7 제거, 가입/구매 라벨 토글. **`pacingCacheKey`에 `MON_FILTER_KEY()` 누락 버그 수정**.
- **요일보정 합성 테스트 함정(T4)**: 2-class 순수 신호(평일0.10·주말0.20)서 "평일=가짜급락"이 발생한다고 가정했으나, `dailyMean≈0.129·sd≈0.045`라 `dailyMean−sd=0.083`이 0.10보다 낮아 실제로 플래그가 안 붙음(의도적 비-발생). **additive identity**(모든 점이 `dailyMean`으로 수렴)를 대신 검증.

### 12.40 GA4 이벤트 트래킹 — navigate 가상뷰 + 단일 위임 리스너(기존 data-* 추론) (PR #178~)
SPA(해시 라우팅 `#5-N`)에 GA 이벤트 다는 법. ① gtag.js는 `<head>` charset/viewport **직후**(SEO 메타 앞)에 삽입(측정ID `G-DK12TNR0GW`). ② **`page_view` 수동 발화 금지** — GA4 향상된 측정이 `navigate`의 `history.pushState`를 자동으로 page_view로 잡아 **이중집계**. 대신 커스텀 `tool_view`로 도구 차원만(navigate의 `!isSamePageRerender` 분기에서만 발화 → toggle/filter 재렌더 제외). ③ **단일 전역 위임 리스너**(`bindAnalytics`, init서 1회)가 기존 `data-*`에서 이벤트 추론 — `[data-tool-analyze]`→run_analysis·`[data-pngdownload]`→download(png)·dataset키 `/csv/i`→download(csv)·`a[href*=instagram.com/gondry]`→dm_click. 버튼 22개 안 건드림. 향후는 `data-ga-event` 속성만. ④ chokepoint 직접: `enterDemo`→demo_start·`handleCSVFile` complete→csv_upload·auth gate 렌더→paywall_hit. ⑤ **`track(name,params)` 안전 래퍼 필수**(`if(window.gtag)`+try/catch — 애드블록 시 no-op, 앱 안 깨짐). ⚠ **§2 보안: CSV 내용·수치·매핑값 절대 미전송** — 페이지ID·도구명·버튼종류·행수(aggregate)만. 사이드이펙트라 골든 byte-동일.
- **SOP/대시보드 세분화(PR #181~)**: ⑥ `gaContentType(id)`로 1~4-x=sop·5-x(또는 TOOL_REQUIRED_FIELDS)=tool·home 분류 → `tool_view`에 `content_type`+`group`(id 앞자리) 파라미터(GA에서 SOP vs 도구 분리 분석). ⑦ `sop_copy`=`.copy-btn` 클릭(코드/콘텐츠 복사=유용성 신호, 위임 추론). ⑧ `scroll_depth`=hash별 25/50/75/100% 임계값 1회씩(`_gaScroll` page별 fired Set, `max<200`px 짧은 페이지 스킵, rAF 스로틀, `content_type` 동반). in-place 재렌더는 hash 동일이라 재발화 안 함(정상). ⚠ **GTM IIFE 함정**: `<head>` GTM 스니펫(`d.getElementsByTagName(s)[0].parentNode.insertBefore`)이 validate 스크립트(`code+inject` 실행)서 document 스텁에 `getElementsByTagName` 없으면 throw — 스텁에 `getElementsByTagName:()=>[{parentNode:{insertBefore:noop}}]` 추가 필요(GTM/gtag inline IIFE는 new Function 실행 시 즉시 발화).
...
- **★ D1~D4 완료 (`docs/d1-d4-direction-spec.md`, 미푸시 로컬커밋, 2026-06-26)**: D1(사이드바/랜딩 그룹 통합, `GROUP_ICONS`/`groupTierBadge`)·D3(버그 no-repro, 사용자 하드리프레시 권장)·D4(`mon_tab_click` 1줄 추가)는 경미. **D2 = 신규 5-21 "성과 변동 진단"**(Free, `TOOL_GROUP["5-21"]="creative"`로 5-6 CSV 공유) — PVM(Mix/Rate) Bridge: `mix_i=cpāᵢ·(s2−s1)`+`rate_i=s̄ᵢ·(cpa2−cpa1)`로 WoW CPA 변동을 무잔차 분해(Σcontribution=ΔCPA_total 항등식, `PVM_MATH.decompose`+`window.runPvmTests()` 27/27). **함정 발견(기존 코드, D1 작업 잔재)**: `renderLandingAnalyze()`의 `IA.filter(...).map(g => { ... return \`...\`; }).join("")`에서 내부 화살표함수 닫는 `}`가 누락된 채 `return\`...\`).join("")`로 합쳐져 있어 전체 스크립트가 SyntaxError로 죽어있었음(브라우저 콘솔 미확인이면 영구 잠복). **교훈: 새 기능 작업 전 항상 Node `new vm.Script(extractedScript)` syntax check를 먼저 돌려 기존 코드 베이스가 깨져있지 않은지 확인할 것** — 이번처럼 직전 세션의 미검증 커밋이 원인일 수 있음.
...
- **★ 운영 대시보드 8개 피드백 — 완료 (PR#175~177, 2026-06-22)**: Bundle A(#175: 플랫폼 다중선택·D7 코호트토글·리텐션 td 정렬) / Bundle B(#176: LTV D360 예측모델·ⓘ 정보아이콘·D0~D360 곡선 차트) / Bundle C(#177: 스코어카드 D30제거+드릴다운 혼합차트·퍼널 요일보정 2버킷 additive·페이싱 요일예측 산식A+매출 제거). `PACING_MATH.weekdayProfile/paceWeekday`, `FUNNEL_STATE.weekdayAdj`, `SCORECARD_STATE.selectedMetric` 추가됨. **스펙 파일(`docs/operational-dashboard-fixes-spec.md`)은 완료됨**.
- `pacingCacheKey`에 `MON_FILTER_KEY()`가 누락돼 필터 변경 시 페이싱 캐시가 재계산 안 되던 버그 수정(Bundle C).

...
- **CSV를 "쓰는" 라우트는 반드시 `TOOL_GROUP`에 등록**(PR #603→#604): 읽기(`setCurrentRouteId`→sticky `activeDataGroup`)와 쓰기(`setCsvData`→`groupForRoute(currentRouteId)`)가 서로 다른 그룹을 고르면, 비효율 도구를 거친 뒤 `/start`에 업로드 → 재진입 시 미러가 빈 슬라이스를 가리켜 **방금 올린 CSV가 사라진다**. 도구가 아니어도 데이터를 쓰는 라우트면 그룹을 가져야 함. `TOOL_GROUP` 파생 상수(`TEMPLATE_FAMILY` 등)에 새 id가 딸려 들어가는지 같이 확인.
- **좁힌 업로드 스코프가 공유 슬라이스를 오염**(같은 PR): `/start`처럼 첫 업로드 화면의 매핑은 그 그룹 슬라이스에 그대로 저장돼 **사이드바로 진입한 도구가 이어 쓴다**(추천 카드 경로만 `prepareDatasetForTool` 재매핑). 스코프에서 뺀 컬럼은 하류 도구에서 영구 미매핑(퍼널·코호트 실종). 진입 화면 스코프 = **그 그룹 도구들의 필드 합집합**으로. 스코프는 오매핑 방지용이 아니라 소속 판별용 — 값 기반 오매핑은 스코어러에서 막을 것.
...
`AdInterstitial`·`AdFreeInit`·`adGate`·`adFree` 전부 삭제됨. `requestAd(cb)`만 호출부 호환용 no-op 래퍼로 잔존(`useDataStore.js`) — 광고 게이트·비밀 URL로 되살리지 말 것. 수익화는 이탈·AdSense 정책 데이터 확인 후 별도 결정.

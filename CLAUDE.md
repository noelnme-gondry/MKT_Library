...
- **KR/EN 헤더 통일**: EN 홈(`app/en/page.js`)이 슬림 `EnLandingHeader`(브랜드만·EN토글 없음)를 써 KR과 구조 divergence → KR 홈과 동일 셸(`Sidebar`+공용 `Header`+`app is-home`+`GlobalModals`). 브랜드=`brand-mark`("GO")+이름, 전 페이지·양 로케일 공통(홈은 트레일링 크럼 없이 브랜드만; `is-home .breadcrumb{display:none}` 규칙 제거). `/en/guide`는 `app/en/guide/` 폴더가 catch-all을 가려 전용 `page.js` 필요(GuideIndex, en/guide/layout 셸). `EnLandingHeader`는 `/en/guide/*` 레이아웃에서 여전히 사용.

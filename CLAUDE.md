...
최소 grain(채널×캠페인×소재×일) Bennet 분해 1회(`decomposeFinest`) 후 `rollup`으로 §2 채널→§3 채널·캠페인별→§4 소재별(모드 A) 드릴다운 항등식 정합. `mix=(cpāᵢ−C̄)·(s2−s1)`(전체 평균 대비 centering, §8.8)+`rate=s̄ᵢ·(cpa2−cpa1)`, `Σcontribution=ΔCPA_total`. 표시 라벨 "CPA/CPI 영향"(=mix+rate, 먼저 볼 값) + §2 Mix·Rate 설명 토글. 기간 `weekBasis`(calendar/rolling7)×`lookback`(1/2/3). ₩/$ 토글(`pvmFmtMoney`, 표시층). 소재 URL: `creative_url` 매핑 시 §4에 🔗(merge는 비용 최대 변형 `crUrlMap`, `pvmSafeUrl` http/https만=XSS 차단). 모드 B(`crMode:"creativeOnly"`)는 의도적 비중첩. `runPvmTests()` 36/36.

...
- **⌘K 명령 팔레트(PR#141, PR-7)**: 전역 오버레이(`#cmdk` static HTML + `.cmdk-*` CSS, body 레벨이라 **사이드바 숨김 랜딩에서도 동작**). `CMDK_STATE`+`buildCmdkCommands`(홈+IA 전 항목, 도구는 `TOOL_TIER` 태깅 FREE/🔒Pro)+`renderCmdkResults`(substring 필터·sel 하이라이트)+`openCmdk`/`closeCmdk`/`runCmdkSelected`(route→navigate·signout). `bindCmdk`(init서 1회): 전역 keydown ⌘/Ctrl+K 토글·↑↓ 이동·↵ 실행·esc 닫기, 결과 클릭(`data-cmdk-idx`), 오버레이 바깥 클릭 닫기. `bindSearch`의 기존 ⌘K(사이드바 focus)는 제거(팔레트로 일원화). Pro 도구 선택도 navigate→페이월. render/로직층만→골든 byte-동일. 검증 `validate_cmdk` 11/11(주입식, DOM 스텁은 `getElementById`로 상태 확인). **키보드 단축키 자체는 headless 불가→브라우저 확인 필요**. **남은(PR-8~)**: 다크 SaaS 비주얼·sticky 필터·매핑 DnD 통일.

...
- **render throw는 골든이 못 잡는다 → `/tmp` repro 필수**(PR #102): 골든은 순수함수만 검증. navigate가 단일 render throw에 통째로 죽어("분석하기 무반응") P0. render 추가 시 Chart 스텁(`afterDatasetsDraw` 직접 실행)으로 양쪽 분기·태그모드 패널 렌더 검증. 검증은 **주입식 harness**(`code+inject`로 내부 const 접근).
...
2. **합성 데이터 유닛 테스트**: `window.runXxxTests()`(예: `runCannibalTests` 5종, `runPvmTests` 33종). Node validate 스크립트로 통과 확인 후 commit.
...
최소 grain(채널×캠페인×소재×일) Bennet 분해 1회(`decomposeFinest`) 후 `rollup`으로 §2 채널→§3 캠페인→§4 소재(모드 A) 드릴다운 항등식 정합. `mix=cpāᵢ·(s2−s1)`+`rate=s̄ᵢ·(cpa2−cpa1)`, `Σcontribution=ΔCPA_total`. 기간 `weekBasis`(calendar/rolling7)×`lookback`(1/2/3). ₩/$ 토글(`pvmFmtMoney`, 표시층). 모드 B(`crMode:"creativeOnly"`)는 의도적 비중첩. `runPvmTests()` 33/33.

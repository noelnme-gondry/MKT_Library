...
const KO_ANSWERS = {
  ...
  "apple-search-ads-guide": "검색어 리포트에서 충분한 탭·설치와 목표 CPA를 함께 충족한 비-Exact 검색어를 Exact 후보로 올리세요. 예산을 덜 쓰면서 성과가 좋으면 CPT를 조금 높이고, 많이 쓰면서 성과가 나쁘면 낮춥니다.",
  "asa-keyword-expansion": "Discovery로 검색어를 발굴하고, 판단할 만큼 전환이 쌓이면서 목표 CPA도 만족한 검색어만 Exact로 승격합니다. 승격한 검색어는 Discovery에 네거티브로 추가해 같은 검색어에 두 캠페인이 동시에 입찰하지 않게 하세요.",
  ...
  "creative-attribute-regression": "A/B를 못 돌릴 때는 이미 집행한 소재의 속성을 열로 만들어 한꺼번에 비교합니다. 배분 알고리즘의 선택 편향이 섞이므로 방향을 좁히는 용도로만 쓰고, 확정은 실험으로 하세요.",
  ...
  "performance-marketing-analysis-order": "분석 이름부터 고르지 말고, 지금 답해야 할 질문과 가진 데이터의 단위부터 확인하세요. 일별 성과는 대시보드와 변동 분해, 지출 변화가 충분하면 포화도와 예산 배분, 대조군이 있으면 증분 분석, 52주 이상 채널 패널이 있으면 VIF를 거쳐 MMM으로 이어가는 순서가 안전합니다.",
...
const EN_ANSWERS = {
  ...
  "apple-search-ads-guide": "Promote non-Exact search terms only after they have enough taps and installs and meet target CPA. Raise CPT slightly when pacing is low and performance is good; lower it when spend is high and performance misses target.",
  "asa-keyword-expansion": "Use Discovery to find search terms, and promote to Exact only those with enough conversions to judge on and a CPA inside target. Add each promoted term as a negative in Discovery so two campaigns do not bid on it at once.",

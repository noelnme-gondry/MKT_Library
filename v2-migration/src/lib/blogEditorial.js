...
const KO_ANSWERS = {
  ...
  "ad-performance-diagnosis": "광고 성과가 하락하면 소재부터 갈지 말고 집계 기준 → 전환량 → 트래픽 품질 → 소재 순으로 좁히세요. CPA 상승은 CPM이 올랐거나 CTR·전환율이 떨어졌거나 비싼 캠페인으로 예산이 쏠린 결과라 원인마다 처방이 정반대입니다. 광고 관리자·GA4·MMP·결제 DB의 전환 수가 함께 떨어졌는지 비교하면 측정 문제인지 실제 성과 문제인지가 한 번에 갈립니다.",
  ...
  "apple-search-ads-guide": "Apple Search Ads(ASA) 키워드 최적화는 검색어 리포트에서 시작합니다 — 충분한 탭·설치와 목표 CPA를 함께 충족한 비-Exact 검색어를 Exact 후보로 올리고, 성과가 없는 검색어는 제외 키워드로 막으세요. 예산을 덜 쓰면서 성과가 좋으면 CPT를 조금 높이고, 많이 쓰면서 성과가 나쁘면 낮춥니다.",
  ...
  "store-conversion-drop-diagnosis": "앱스토어 전환율이 하락했다고 스크린샷부터 바꾸면 원인이 유입 믹스일 때 아무 일도 안 일어납니다. Search 45%·Browse 9%가 각각 그대로여도 조회 비중이 4,300·2,400에서 2,600·5,200으로 뒤집히면 전체 전환율은 32.1%에서 21.0%로 11%p 떨어집니다. 소스별 전환율이 그대로인지부터 확인하세요.",
  "store-listing-experiment": "앱스토어 A/B 테스트가 필요한 이유는, 스크린샷 변경 효과를 전후 비교로 재면 같은 기간의 광고 증액·시즌이 전부 그 몫으로 잡히기 때문입니다. 애플 PPO는 원본 외 최대 3개 안을 최대 90일까지 같은 시점 트래픽을 무작위로 갈라 비교하므로 그 혼입이 설계에서 사라집니다. 종료 조건은 반드시 시작 전에 정하세요.",
  ...
  "cpi-cpa-cpm-difference": "CPM은 노출 1,000회당 비용, CPC는 클릭당 비용, CPI는 설치당 비용, CPA는 목표 행동당 비용이고 CPC = CPM ÷ CTR로 이어집니다. 그래서 CPC가 200원에서 300원이 됐을 때 CPM이 올랐으면 경매가 비싸진 것이고 CTR이 1%에서 0.67%로 떨어진 것이면 소재 문제라, 같은 증상에 처방이 정반대입니다.",
  "creative-attribute-regression": "광고 소재를 A/B 테스트 없이 비교하려면 이미 집행한 소재의 속성을 0/1 또는 숫자 열로 만들어 한꺼번에 회귀분석으로 봅니다. '임팩트 있는 후킹' 같은 주관 열은 성과 좋은 소재에만 1이 찍혀 결과를 오염시키므로 사실만 열로 넣고, 신뢰구간이 0을 걸치는 속성은 쓰지 마세요.",
  ...
  "funnel-dropoff-analysis": "전환율 개선은 퍼널 단계별 통과율을 세워 가장 낮은 칸부터 봅니다 — 클릭의 40%가 설치까지 와도 그중 25%만 가입하면 나머지 75%가 앱을 깔고 그냥 나가는 것입니다. 다만 통과율이 가장 낮은 칸이 아니라 개선 여지 × 트래픽 규모가 가장 큰 칸이 우선순위입니다.",
  ...
  "incrementality-measurement": "증분성(인크리멘탈리티) 측정은 리포트에 찍힌 전환이 아니라 광고가 없었다면 사라졌을 전환을 재는 일입니다 — 마케팅 전환 500건 중 실제 증분은 120건이고 나머지 380건은 원래 올 손님이었을 수 있습니다. ROAS 800%가 찍혀도 증분이 낮으면 증액 근거가 되지 않으므로, 무작위 홀드아웃으로 확인하는 것이 가장 확실합니다.",
  ...
  "performance-marketing-metrics": "퍼포먼스 마케팅 지표를 총정리하면 지출 → CPI → CPA → ROAS → LTV:CAC 한 줄로 이어지고, 손대기 전에 어느 칸이 빨간지부터 찾아야 합니다. 5,000만 원으로 설치 5,000건이면 CPI는 1만 원이고 그 지출에서 매출 1.5억이면 ROAS 300%인데, CPA는 괜찮은데 ROAS만 나쁘다면 문제는 광고가 아니라 전환 이후 객단가에 있습니다.",
  ...
  "uplift-holdout-guide": "광고 업리프트 측정은 광고를 본 그룹과 홀드아웃 그룹의 전환율 차이를 재는 일입니다 — 노출군 8%, 홀드아웃 5%면 업리프트는 3%p입니다. 무작위 배정이 아니거나 두 그룹의 기간·대상이 다르면 이 차이를 광고 효과로 볼 수 없고, 구간이 0을 걸치면 '효과 없음'이 아니라 '아직 구분 안 됨'으로 읽어야 합니다.",
...
const EN_ANSWERS = {
  ...
  "cpi-cpa-cpm-difference": "CPM is cost per 1,000 impressions, CPC is cost per click, CPI is cost per install, and CPA is cost per target action, connected as CPC = CPM ÷ CTR. So when CPC moves from $0.20 to $0.30, a rising CPM means the auction got expensive while CTR falling from 1% to 0.67% means the creative stopped working — identical symptom, opposite fixes.",
  "creative-attribute-regression": "Ad creative regression lets you compare thumbnails and titles without an A/B test: turn the attributes of creatives you already served into 0/1 or numeric columns and regress them together. Subjective columns like 'strong hook' end up flagged only on winners and poison the result, so record facts only and discard any attribute whose interval crosses zero.",
  ...
  "funnel-dropoff-analysis": "Conversion rate optimization starts by laying out funnel pass-through rates and looking at the weakest step — if 40% of clicks install but only 25% of those sign up, three quarters of installers leave without an account. Prioritise by room for improvement × traffic volume, though, not simply by the lowest rate.",
  ...
  "incrementality-measurement": "Incrementality measurement asks not how many conversions your report shows but how many would disappear without the ad — of 500 marketing conversions, only 120 may be incremental while 380 were coming anyway. A reported ROAS of 800% is not a scaling case if lift is low, and a randomized holdout is the only design that settles it.",
  ...
  "uplift-holdout-guide": "Advertising uplift is the conversion-rate gap between an exposed group and a holdout — 8% exposed against 5% held out is 3pp of uplift. Without random assignment, or when the two groups differ in period or audience, that gap is not ad effect, and an interval crossing zero means 'not yet distinguishable', not 'no effect'.",

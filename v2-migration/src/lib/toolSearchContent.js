...
const CONTENT = {
  ...
  "5-28": {
    ko: {
      eyebrow: "도구 사용 가이드",
      title: "핵심 액션이 끊기는 시점과 우선 확인할 군을 찾습니다",
      question: "정의한 핵심 액션은 얼마나 유지되고, 언제 이탈·종료 위험이 커지나요?",
      answer: "중도절단을 반영한 생존곡선과 구간별 위험률로 액션이 끊기는 시점과 관측기간 반복 가치를 확인합니다.",
      outputs: ["생존곡선과 95% 신뢰구간", "기간별 이탈·종료 위험", "관측기간 RMST·반복 가치"],
      lead: "핵심 액션 생존·이탈 분석은 한 개체의 액션 관측 에피소드별 기간과 이탈·종료 여부를 사용해, 관측 종료까지 액션을 유지한 사례도 중도절단으로 올바르게 반영합니다. 구독은 한 가지 입력 예시일 뿐이며, 개별 이탈을 예측하거나 세그먼트 차이의 원인을 확정하지는 않습니다.",
      detailsLabel: "계산 조건과 해석 한계 보기",
      sections: [
        ["언제 써야 하나", "활성화·재방문·반복 구매·핵심 기능 사용처럼 정의한 액션이 어느 기간에 끊기는지 보고 개입 시점을 정할 때 씁니다. 1행은 한 개체의 액션 시작부터 이탈·종료 또는 관측 종료까지의 에피소드여야 합니다."],
        ["무엇을 계산하나", "Kaplan–Meier 생존율과 Greenwood 95% 구간, 구간별 조건부 이탈 위험, 중앙 생존기간과 관측 범위 안의 제한평균 생존기간(RMST)을 계산합니다. 기간당 반복 가치와 마진을 입력하면 관측기간 가치도 함께 봅니다."],
        ["결과를 어떻게 쓰나", "위험이 커지기 전의 온보딩·리마인드·제품·가격 개입 가설을 만들고 A/B 또는 홀드아웃으로 검증하세요. 세그먼트 차이는 관측된 연관일 뿐 원인이 아니며, 관측 범위 밖 생존율과 가치를 외삽하지 않습니다."],
      ],
      faq: [
        { q: "관측 종료까지 액션을 유지한 사례도 넣어야 하나요?", a: "넣어야 합니다. 이 사례는 이탈하지 않은 것이 아니라 관측이 끝난 시점까지 유지된 중도절단 사례이며, 이를 빼면 생존율이 과소추정됩니다." },
        { q: "세그먼트 생존율이 다르면 원인이라고 볼 수 있나요?", a: "아닙니다. 채널·플랜·프로모션에 따른 고객 구성과 시점이 함께 다를 수 있으므로 관측된 차이로만 해석하고, 개입 효과는 실험으로 확인해야 합니다." },
        { q: "개체별 이탈 확률을 예측하나요?", a: "아닙니다. 이 도구는 시간별 생존과 위험을 집계해 요약합니다. 개체별 스코어링이나 분류 모델은 별도 문제입니다." },
      ],
    },
    en: {
      eyebrow: "Tool guide",
      title: "Find when a key action drops off and which group to inspect first",
      question: "How long does a defined key action persist, and when does drop-off risk rise?",
      answer: "Estimate survival, hazard, and observed recurring value from censored action episodes.",
      outputs: ["Survival curve with 95% interval", "Drop-off or exit risk by period", "Observed-horizon RMST and recurring value"],
      lead: "Action survival and drop-off analysis uses one row per entity action-observation episode and keeps cases still active at the observation end as censored observations. Subscription is only one input example. It does not predict individual drop-off or establish why segments differ.",
      detailsLabel: "See the calculation conditions and limits",
      sections: [
        ["When to use it", "Use it to determine when an activation, return, repeat purchase, or key-feature action drops off and where to place an intervention. Each row must be one entity episode from action start until exit or observation end."],
        ["What it calculates", "It calculates Kaplan–Meier survival, Greenwood 95% intervals, interval hazard, median survival, and restricted mean survival time within the observed horizon. Periodic recurring value and gross margin optionally produce observed-horizon value."],
        ["How to act", "Form an onboarding, reminder, product, or pricing hypothesis before the risk interval, then validate it with an A/B test or holdout. Segment differences are associations, and neither survival nor value is extrapolated beyond observed data."],
      ],
      faq: [
        { q: "Should cases still active at observation end be included?", a: "Yes. They are censored observations, not cases that never exit. Excluding them biases survival downward." },
        ...
        { q: "Does this predict drop-off for individual entities?", a: "No. It summarizes time-based survival and hazard for action episodes. Individual scoring is a separate modeling problem." },
      ],
    },
  },
  "5-26": {

 ...
 *
 * 2026-09-21부터 7일이며 **시작 시점과 무관하게 모든 계정에 같은 길이**를
 * 적용한다(소급 포함). 한때 기준일 이전 체험만 14일로 두는 분기가 있었으나
 * 운영 결정으로 없앴다 — 길이가 계정마다 갈리면 화면·메일·결제 기산일이
 * 각각 다른 날짜를 말하게 되고, 그 차이를 설명할 자리가 제품에 없다.
 *
...
/** 그 체험에 적용되는 밀리초 길이. 지금은 전 계정 공통이다. */
export function trialMsFor() {
  return PRO_TRIAL_MS;
}
...
export function trialUntilSql(alias = "") {
  return `(${alias ? `${alias}.` : ""}trial_started_at+INTERVAL '${PRO_TRIAL_DAYS} days')`;
}

// 남은 기간 구간 — GA에 원본 날짜를 싣지 않기 위한 범주형이다.
// 길이가 7일 하나뿐이라 3~7일이 상한 구간이다.
export function trialRemainingBucket(start, now = Date.now()) {
  ...
  return days <= 0 ? "expired" : days < 3 ? "under_3d" : "3_7d";
}

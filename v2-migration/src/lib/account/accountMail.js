...
export async function dispatchAccountMail() {
  ...
  await db.query(`INSERT INTO gop_account_mail(id,account_id,kind,reference,due_at) SELECT 'expiry:'||a.id::text||':'||x.until::text,a.id,'expiry',x.until::text,x.until-INTERVAL '7 days' FROM gop_accounts a CROSS JOIN LATERAL (SELECT GREATEST(a.trial_started_at+INTERVAL '14 days',(SELECT max(p.expires_at) FROM gop_payment_orders p WHERE p.account_id=a.id AND p.status='paid' AND p.mode='live')) AS until) x WHERE a.service_reminders AND x.until>NOW() ON CONFLICT(id) DO NOTHING`);
  ...
  for (let index = 0; index < 20; index++) {
    ...
    if (!job) break;
    try {
      const owner = (await db.query("SELECT a.*,(SELECT max(expires_at) FROM gop_payment_orders WHERE account_id=a.id AND status='paid' AND mode='live') AS paid_until FROM gop_accounts a WHERE a.id=$1", [job.account_id])).rows[0];
      ...
      if (job.kind === "receipt") {
        ...
        details = { ...details, amount: order?.amount, expiresAt: order?.expires_at ? new Date(order.expires_at).toISOString().slice(0, 10) : "" };
      } else {
        ...
        if (job.kind === "review") {
          ...
          const current = (await db.query("SELECT GREATEST(a.trial_started_at+INTERVAL '14 days',(SELECT max(expires_at) FROM gop_payment_orders WHERE account_id=a.id AND status='paid' AND mode='live')) AS until FROM gop_accounts a WHERE a.id=$1", [job.account_id])).rows[0]?.until;

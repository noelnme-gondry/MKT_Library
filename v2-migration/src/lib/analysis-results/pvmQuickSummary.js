...
export function buildPvmQuickSummary({ csvData, dashboardFilter, denomBasis = "installs" } = {}) {
  ...
  const rows = getMonFilteredRows(csvData, dashboardFilter).map((row) => ({ ...row, spend: Number(row.spend ?? row.cost) || 0, _t: new Date(`${row.date}T00:00:00Z`).getTime() })).filter((row) => !Number.isNaN(row._t));
  ...
  const keys = { ch: "channel", cmp: mapped.has("campaign_id") ? "campaign_id" : null, cr: mapped.has("creative_id") ? "creative_id" : null, resultField };

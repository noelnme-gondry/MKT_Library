...
export function buildPvmQuickSummary({ csvData, dashboardFilter, denomBasis = "installs" } = {}) {
  ...
  const rows = getMonFilteredRows(csvData, dashboardFilter || {}).map((row) => ({ ...row, spend: Number(row.spend ?? row.cost) || 0, campaign_id: row.campaign_id ?? row.campaign_name, _t: new Date(`${row.date}T00:00:00Z`).getTime() })).filter((row) => !Number.isNaN(row._t));

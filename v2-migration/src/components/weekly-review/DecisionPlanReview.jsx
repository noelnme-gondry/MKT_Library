...
export default function DecisionPlanReview({ record, locale = "ko", onChange, onEvidenceChange }) {
  ...
  return <section className="decision-evidence"><details><summary>{en ? "Saved operating target and design" : "저장한 운영 목표와 설계"}</summary><dl>{decisionPlanRows(plan, locale).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl></details>

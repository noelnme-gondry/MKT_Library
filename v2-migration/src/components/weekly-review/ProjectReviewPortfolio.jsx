...
export default function ProjectReviewPortfolio({ locale = "ko" }) {
  ...
  return <AnalysisExportProvider value={exportContext}><section className="project-review-portfolio wr-card" aria-labelledby="project-review-portfolio-title">
    ...
    <section data-information-section="" className="project-review-report-scope"><header data-information-heading="">{en ? "Choose report scope" : "보고서에 담을 결정 선택"}</header>
    ...
    {reportRecords.length > 0 ? <DownloadHub toolId="weekly-review" locale={locale} label={en ? "Share / download project" : "프로젝트 공유·다운로드"} /> : <p>{en ? "No matching decisions. Adjust the filters to export." : "조건에 맞는 결정이 없습니다. 필터를 조정한 뒤 내보내세요."}</p>}
    ...
    <section data-information-section=""><header data-information-heading="">{en ? "Browse decisions by tool" : "도구별 결정 찾아보기"}</header><div className="project-review-portfolio__tools">{groups.map(group => {

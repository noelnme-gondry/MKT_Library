...
export default function DecisionEvidence({ record, locale = "ko", title, expanded = false }) {
  const en = locale === "en";
  ...
  return <details className="decision-evidence" open={expanded || undefined}>
    <summary>{title || (en ? "Evidence at decision time" : "결정 당시 분석 근거")}{evidence.capturedAt && ` · ${evidence.capturedAt.slice(0, 10)}`}</summary>

...
export default function WeeklyProjectSetup({ locale, name, setName, target, setTarget, metric, currency, canSave, targetInvalid, status, persistenceEnabled, proActive = false, onSave, children }) {
  const en = locale === "en";
  ...
  return <section className="wr-project" id="wr-project" aria-labelledby="wr-project-title">
    <div className="wr-section-heading"><div><h2 id="wr-project-title">{name || (en ? "My weekly project" : "내 주간 프로젝트")}</h2><p>{metric.toUpperCase()} · {unit} · {en ? "Review criteria" : "리뷰 기준"}</p></div></div>
    <div id="wr-project-fields">
      ...
      {children}
      <div className="wr-project-actions"><button type="button" className="btn primary" disabled={!canSave} onClick={onSave}>{en ? "Save setup for next week" : "다음 주를 위해 설정 저장"}</button><Link href={en ? "/en/storage" : "/storage"}>{en ? "Storage settings" : "저장소 설정"}</Link></div>
      <p className="wr-note">{!proActive ? (en ? "Analysis is free. Saving settings and period history requires active Pro. Changes apply to this session." : "분석은 무료입니다. 설정과 기간별 기록 저장에는 유효한 Pro가 필요하며, 변경 사항은 현재 세션에 적용됩니다.") : persistenceEnabled ? (en ? "Settings and campaign aggregates stay in this browser for up to 90 days. Mapping is remembered by the uploader. Delete them in Storage." : "설정과 캠페인 집계는 이 브라우저에 최대 90일 보관합니다. 매핑은 업로더가 기억하며, 저장소에서 삭제할 수 있습니다.") : (en ? "Device storage is off. You can review now, but settings will not survive this session." : "기기 저장이 꺼져 있습니다. 지금 분석할 수 있지만 설정은 현재 세션만 유지됩니다.")}</p>
    </div>

...
export default function WeeklyProjectSetup({ locale, name, setName, target, setTarget, metric, currency, canSave, targetInvalid, status, persistenceEnabled, proActive = false, onSave, hasResult, children }) {
  ...
  return <section className="wr-project" id="wr-project" aria-labelledby="wr-project-title">
    <div className="wr-section-heading"><div><h2 id="wr-project-title">{name || (en ? "My weekly project" : "내 주간 프로젝트")}</h2><p>{metric.toUpperCase()} · {unit} · {en ? "Review criteria" : "리뷰 기준"}</p></div><button type="button" className="btn" aria-expanded={editing} aria-controls="wr-project-fields" onClick={() => setEditing(!editing)}>{editing ? (en ? "Close settings" : "설정 접기") : (en ? "Edit project settings" : "프로젝트 기준 편집")}</button></div>

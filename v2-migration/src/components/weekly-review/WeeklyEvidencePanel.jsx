...
export default function WeeklyEvidencePanel({ evidence, review, locale = "ko", onChooseCampaign }) {
  ...
  return <section className="wr-card wr-evidence" aria-labelledby="wr-evidence-title">
    ...
    <div className="wr-tablewrap" role="region" aria-label={en ? "Campaign comparison, scroll horizontally" : "캠페인 비교표, 가로 스크롤"} tabIndex={0}>
      <table className="wr-campaign-table"><caption>{en ? "Observed campaign performance · current spend order" : "캠페인별 관측 성과 · 이번 지출 순"}</caption><thead><tr>

...
export default function DecisionFollowUp({ record, locale = "ko", requestOpen = 0 }) {
  const en = locale === "en";
  ...
  const expanded = open || requestOpen > dismissedRequest;
  ...
  useReviewDraftGuard(expanded && Boolean(action.trim() || serializeDecisionPlan(plan) || selectedRails.length || question !== (record.reviewQuestion || "")));
  // Only committed observations may become the basis for another decision.
  const observed = Boolean(record.actual?.trim() || record.learning?.trim() || record.closureReason);
  return <section className="decision-follow-up" aria-label={en ? "Decision feedback loop" : "결정 피드백 루프"}>
    {parent && <p>{en ? "Previous decision" : "이전 결정"}: <a href={decisionReviewHref(locale, parent.id)}>{parent.action}</a></p>}
    {record.parentDecisionId && !parent && <p>{en ? "The previous decision is not available in this project." : "이전 결정은 현재 프로젝트에 없습니다."}</p>}
    {children.length > 0 && <><strong>{en ? "Next decisions" : "이어진 결정"}</strong><ul>{children.map(child => <li key={child.id}><a href={decisionReviewHref(locale, child.id)}>{child.action}</a> · {child.reviewDate || (en ? "No review date" : "검토일 미정")}</li>)}</ul></>}
    {observed && !expanded && <button className="btn" onClick={() => setOpen(true)}>{en ? "Turn this learning into the next decision" : "배운 점으로 다음 결정 만들기"}</button>}
    {expanded && <div className="decision-follow-up__form">
      ...
      <section data-information-section=""><header data-information-heading="">{en ? "Set a numeric target or experiment design" : "수치 목표·실험 설계 설정"}</header><DecisionPlanFields locale={locale} value={plan} onChange={setPlan} /></section>
      <section data-information-section=""><header data-information-heading="">{en ? "Guardrails and review question (optional)" : "유지 조건·검토 질문 (선택)"}</header>      {rails.length > 0 && <fieldset className="decision-plan-fields"><legend>{en ? "Guardrails for the next decision" : "다음 결정에서 지킬 조건"}</legend>{rails.map(rail => <label key={rail.key}>{rail.label} {rail.op === "gte" ? "≥" : "≤"}<input inputMode="decimal" value={railValues[rail.key] || ""} onChange={event => setRailValues(values => ({ ...values, [rail.key]: event.target.value }))} /></label>)}<p>{en ? "Set fresh thresholds; previous thresholds are not copied." : "이번 결정의 기준을 입력하세요. 이전 결정의 임계값을 자동 복사하지 않습니다."}</p>{invalidRails && <p role="status">{en ? "Enter numeric guardrail thresholds." : "유지할 조건의 기준값은 숫자로 입력하세요."}</p>}</fieldset>}
      ...
      <button className="btn" onClick={() => { setOpen(false); setDismissedRequest(requestOpen); setAction(""); setPlan({}); setRailValues({}); }}>{en ? "Cancel" : "취소"}</button>
    </div>}
    {pending && <ReviewSaveDialog locale={locale} record={pending} onClose={() => setPending(null)} onSaved={() => { setOpen(false); setDismissedRequest(requestOpen); setAction(""); setPlan({}); setRailValues({}); trackProductEvent("decision_follow_up_saved", { tool_id: record.toolId, locale, source: "project_review" }); }} />}
  </section>;
}

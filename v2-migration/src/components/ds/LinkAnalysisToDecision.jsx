...
export default function LinkAnalysisToDecision({ evidence, toolId, metric, locale = "ko" }) {
  ...
  return <details className="decision-evidence"><summary>{en ? "Link this result to an existing decision" : "이 결과를 기존 결정에 연결"}</summary>
    ...
    <label>{en ? "Decision to review" : "결과를 검토할 결정"}<select value={selected} onChange={event => { setSelected(event.target.value); setConfirmed(false); setSaved(""); }}><option value="">{en ? "Choose a decision" : "결정을 선택하세요"}</option>{candidates.map(({ record, match }) => <option key={record.id} value={record.id}>{match.recommended ? (en ? "Matching metric · " : "지표 일치 · ") : ""}{record.action} · {record.reviewDate}</option>)}</select></label>
    {chosen && <><EvidenceCompatibility match={chosen.match} locale={locale} /><label><input type="checkbox" checked={confirmed} onChange={event => setConfirmed(event.target.checked)} />{en ? "I checked the metric, treatment direction, population and period, including differences or missing details." : "지표·처치 방향·집단·기간과 다르거나 미확인인 조건을 확인했습니다."}</label><button className="btn" disabled={!confirmed} onClick={() => setPending({ id: selected, evidence: serializeReviewEvidence({ ...evidence, capturedAt: new Date().toISOString() }) })}>{en ? "Save linked result" : "연결한 결과 저장"}</button></>}

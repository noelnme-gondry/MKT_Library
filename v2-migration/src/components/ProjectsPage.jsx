...
export default function ProjectsPage({ locale = "ko" }) {
  ...
  return <div className="projects-page">
    ...
    {backup && <section className="project-import-preview"><h2>{en ? "Check before restoring" : "복원 전 확인"}</h2><p>{backup.project.name || (en ? "Unnamed project" : "이름 없는 프로젝트")} · {backup.files.length} {en ? "files" : "파일"} · {bytesLabel(backup.bytes)} · {backup.project.decisions.length} {en ? "decisions" : "결정"}</p><p>{en ? "Includes source files, mappings, settings, period snapshots and decisions. Import stays in this browser and does not run analysis automatically." : "원본 파일·매핑·설정·기간 집계·결정 기록을 포함합니다. 이 브라우저에서만 가져오며 분석은 자동 실행하지 않습니다."}</p><button className="btn" disabled={busy} onClick={() => restore(false)}>{en ? "Restore as new project" : "새 프로젝트로 복원"}</button>{active && <button className="btn" disabled={busy} onClick={() => restore(true)}>{en ? "Replace current project" : "현재 프로젝트에 복원"}</button>}<button className="btn ghost" onClick={() => setBackup(null)}>{en ? "Cancel" : "취소"}</button></section>}
    <div className="project-list">
      ...
      {[...projects].sort((a, b) => (nextReview(a) || "9999").localeCompare(nextReview(b) || "9999")).map(project => {
        ...
        return <article className="project-card" key={project.id}>
          ...
          <div className="project-actions"><button className="btn primary" disabled={busy || switching} onClick={() => open(project.id)}>{en ? "Open review" : "리뷰 열기"}</button><button className="btn" disabled={busy} onClick={() => exportOne(project.id)}>{en ? "Export backup" : "백업 내보내기"}</button><button className="btn ghost" disabled={busy} onClick={() => run(async () => { if (window.confirm(en ? "Delete this project and its stored files and records?" : "프로젝트와 저장된 파일·기록을 삭제할까요?")) await useAppStore.getState().deleteProject(project.id); })}>{en ? "Delete" : "삭제"}</button></div>
        </article>;
      })}
    </div>

...
const COPY = {
  ko: {
    ...
    ambiguous_date_format: "03/04/2026처럼 월·일 순서를 가릴 수 없는 날짜가 있습니다. 월/일(미국식)로 읽었습니다 — 일/월 형식이라면 YYYY-MM-DD로 바꿔 다시 올리세요.",
    duplicate_mappings: "한 항목에 두 컬럼이 매핑됐습니다. 뒤쪽 컬럼만 쓰이고 앞쪽 값은 버려지므로 합계가 줄어듭니다 — 하나를 해제하세요.",
  ...
  en: {
    ...
    ambiguous_date_format: "Some dates such as 03/04/2026 do not say which part is the month. They were read as month/day (US order) — if your file is day/month, re-export as YYYY-MM-DD.",
    duplicate_mappings: "Two columns are mapped to the same field. Only the later column is used and the earlier values are dropped, so totals come out low — unmap one of them.",

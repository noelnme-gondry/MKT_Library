...
export default function SubscriptionPurchasePrompt() {
  ...
  return <ModalDialog open={Boolean(prompt)} onClose={close} ariaLabel={en ? "Take your analysis into your next meeting" : "분석을 다음 회의에서 바로 쓰세요"} overlayClassName="purchase-dialog-overlay" panelClassName="purchase-dialog">
    ...
    <h2>{en ? "Reports you can use and edit" : "회의에 바로 가져갈 분석 보고서"}</h2>
    <p>{en ? "Keep the conclusion, supporting evidence, and next actions together." : "결론과 근거, 다음 행동을 한 번에 정리해 가져가세요."}</p>
    <p>{en ? `Analysis downloads require an active purchased pass. Sign-in and the ${PRO_TRIAL_DAYS}-day trial do not unlock downloads.` : `분석자료 다운로드는 유효한 구매 이용권이 필요합니다. 로그인·${PRO_TRIAL_DAYS}일 체험만으로는 다운로드할 수 없습니다.`}</p>
    ...
    <section data-information-section=""><header data-information-heading="">{en ? "Not ready to purchase? (optional)" : "구매를 보류하는 이유가 있나요? (선택)"}</header><div className="purchase-reasons">{["price", "identity", "trust", "refund", "later"].map((reason, index) => <button className="btn ghost" type="button" key={reason} disabled={reasonPrompt === prompt} onClick={() => { trackProductEvent("subscription_gate_reason", { tool_id: prompt?.toolId, locale: en ? "en" : "ko", gate_reason: reason, source: "purchase_prompt" }); setReasonPrompt(prompt); }}>{(en ? ["Price", "Account requirements", "Need more confidence", "Refund terms", "Not now"] : ["가격", "계정 요구", "신뢰·효용 확인", "환불 조건", "지금은 아님"])[index]}</button>)}</div>{reasonPrompt === prompt && <p role="status">{en ? "Thank you for your feedback. You can close this window and continue your analysis." : "의견 감사합니다. 이 창을 닫고 분석을 계속할 수 있습니다."}</p>}</section>

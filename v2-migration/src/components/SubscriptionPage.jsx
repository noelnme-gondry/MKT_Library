...
export default function SubscriptionPage({ locale = "ko" }) {
  ...
  return <div className="projects-page subscription-page">
    ...
    <section id="purchase" className="subscription-purchase" aria-labelledby="purchase-title">
      <div className="purchase-summary"><span className="purchase-plan-badge">Pro</span><h2 id="purchase-title">{en ? "Pro for your next decision" : "분석을 다음 결정까지 연결하세요"}</h2><dl><div><dt>{en ? "Period" : "이용 기간"}</dt><dd>{en ? "1 calendar month" : "1개월"}</dd></div><div><dt>{en ? "Renewal" : "갱신 방식"}</dt><dd>{en ? "No automatic renewal" : "자동 갱신 없음"}</dd></div><div className="purchase-total"><dt>{en ? "Payment amount" : "결제 금액"}</dt><dd>{SUBSCRIPTION.monthlyKrw.toLocaleString(en ? "en-US" : "ko-KR")}{en ? " KRW" : "원"}</dd></div></dl><ul className="purchase-deliverables"><li><strong>{en ? "Share the evidence" : "근거를 보고서로"}</strong><span>{en ? "Download conclusions, charts and limitations in Word and Excel." : "결론·차트·해석 한계를 Word와 Excel로 다운로드"}</span></li><li><strong>{en ? "Record the decision" : "결정을 기록으로"}</strong><span>{en ? "Save the action, target and review date with its evidence." : "실행할 행동·목표·검토일을 분석 근거와 함께 저장"}</span></li><li><strong>{en ? "Review what happened" : "실행 결과를 다음 행동으로"}</strong><span>{en ? "Record observations and connect the next decision." : "관측 결과와 배운 점을 남기고 후속 결정으로 연결"}</span></li></ul><a href="#plans">{en ? "Review plan details" : "플랜 구성 다시 보기"}</a></div>
      <div className="purchase-payment"><h3>{en ? "Complete your purchase" : "Pro 구매"}</h3><SubscriptionCheckout locale={locale} /><p className="purchase-policy-links"><Link href={en ? "/en/terms" : "/terms"}>{en ? "Terms" : "이용약관"}</Link><a href="#refund-policy">{en ? "Refund policy" : "환불정책"}</a><Link href={en ? "/en/contact" : "/contact"}>{en ? "Contact support" : "고객센터 문의"}</Link></p></div>
    </section>

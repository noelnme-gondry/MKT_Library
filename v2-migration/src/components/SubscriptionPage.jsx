...
export default function SubscriptionPage({ locale = "ko" }) {
  ...
  return <div className="projects-page subscription-page">
    ...
    <section id="purchase" className="subscription-purchase" aria-labelledby="purchase-title">
      <div className="purchase-summary"><span className="purchase-plan-badge">Pro</span><h2 id="purchase-title">{en ? "Your report pass" : "보고서 이용권 구매"}</h2><p>{en ? "Word and Excel reports, multiple projects, batch reports and branding." : "Word·Excel 보고서, 여러 프로젝트, 일괄 보고서와 브랜딩."}</p><dl><div><dt>{en ? "Period" : "이용 기간"}</dt><dd>{en ? "1 calendar month" : "1개월"}</dd></div><div><dt>{en ? "Renewal" : "갱신 방식"}</dt><dd>{en ? "No automatic renewal" : "자동 갱신 없음"}</dd></div><div className="purchase-total"><dt>{en ? "Payment amount" : "결제 금액"}</dt><dd>{SUBSCRIPTION.monthlyKrw.toLocaleString(en ? "en-US" : "ko-KR")}{en ? " KRW" : "원"}</dd></div></dl><a href="#plans">{en ? "Review plan details" : "플랜 구성 다시 보기"}</a></div>
      <div className="purchase-payment"><h3>{en ? "Payment and access" : "결제·이용권 확인"}</h3><SubscriptionCheckout locale={locale} /><p className="purchase-policy-links"><Link href={en ? "/en/terms" : "/terms"}>{en ? "Terms" : "이용약관"}</Link><a href="#refund-policy">{en ? "Refund policy" : "환불정책"}</a><Link href={en ? "/en/contact" : "/contact"}>{en ? "Contact support" : "고객센터 문의"}</Link></p></div>
    </section>

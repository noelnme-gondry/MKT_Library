...
const COPY = {
  ko: {
    ...
    failed: "예측 모델 비교를 완료하지 못했습니다. 기존 WLS 결과와 Shapley R² 분해에는 영향이 없습니다.", retry: "예측 모델 다시 실행",
  ...
  en: {
    ...
    failed: "The predictive comparison did not complete. Existing WLS results and Shapley R² are unchanged.", retry: "Retry predictive models",
...
export default function CreativePredictiveModelPanel({ metrics = [], attributes = [], metric = "ctr", mappedKeys = new Set(), signature, locale = "ko", source = "csv" }) {
  ...
  const [run, setRun] = useState({ signature: "", status: "idle", randomForest: null, svm: null });
  ...
  const execute = useCallback(async () => {
    ...
    setRun({ signature: runSignature, status: "loading", randomForest: null, svm: null });
    ...
    try {
      ...
      setRun({ signature: runSignature, status: "complete", randomForest, svm });
      ...
      if (requestId !== requestRef.current) return;
      setRun({ signature: runSignature, status: "failed", randomForest: null, svm: null });
  ...
  const visible = run.signature === runSignature ? run : { status: "idle", randomForest: null, svm: null };
  ...
  return <section className="block creative-model-router" id="s-creative-predictive-models">
    ...
    {visible.status === "failed" && <div className="required-banner"><p>{C.failed}</p><button type="button" className="ab-button" onClick={execute}>{C.retry}</button></div>}

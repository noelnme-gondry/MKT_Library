              ...
              {health && (
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                    ...
                    {mmm.run.seasonalitySelection?.enabled && <div className="stat-card"><div className="lbl">{tx("계절성 자동선택", "Seasonality selection")}</div><div className="val">{mmm.run.seasonalitySelection.selected.id}</div></div>}
                  ...
                  {(mmm.run.baselineSelection?.enabled || mmm.run.seasonalitySelection?.enabled || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>

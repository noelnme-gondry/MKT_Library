                ...
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  {(mmm.run.baselineSelection?.enabled || Array.isArray(mmm.run.seasonalityPeriods) || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>
                    ...
                    {mmm.run.seasonalitySelection?.enabled ? ` ${mmm.run.seasonalitySelection.evidence?.detected
                      ? tx(`계절성은 최근 12주 예측과 분리했습니다. 전체 ${mmm.run.seasonalitySelection.evidence.observedWeeks}주에서 Brand/Performance 집계 매체·이벤트·추세를 통제한 뒤, 연간 파형 후보의 BIC가 ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)} 개선되고 52주 간 파형 상관이 ${Number(mmm.run.seasonalitySelection.evidence.seasonalLagCorrelation).toFixed(2)}로 유지됐습니다. 외부시장 지표를 포함·제외한 검증에서도 같은 후보인지 확인했고, 3개 rolling 구간에서 무계절 모델 대비 WMAPE 악화가 허용범위(${Number(mmm.run.seasonalitySelection.evidence.rollingMaxDegradation || 0).toFixed(1)}%p) 안인지 확인한 뒤 ${mmm.run.seasonalitySelection.selected.id}를 채택했습니다.`, `Seasonality is separated from recent 12-week forecasting. Across all ${mmm.run.seasonalitySelection.evidence.observedWeeks} weeks, after controlling for aggregated Brand/Performance media, events, and trend, the selected annual-shape candidate improved BIC by ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)} and retained ${Number(mmm.run.seasonalitySelection.evidence.seasonalLagCorrelation).toFixed(2)} 52-week shape correlation. We also checked that the same candidate survives with and without industry controls, then used three rolling folds and required WMAPE degradation versus the no-seasonality model to stay within ${Number(mmm.run.seasonalitySelection.evidence.rollingMaxDegradation || 0).toFixed(1)} percentage points before selecting ${mmm.run.seasonalitySelection.selected.id}.`)
                      : tx(`전체 ${mmm.run.seasonalitySelection.evidence?.observedWeeks || mmm.panel.week.length}주에서 매체·이벤트·추세를 통제한 연간 파형의 개선과 반복 안정성이 기준에 못 미쳐 계절성을 미사용으로 판정했습니다. 최근 12주 예측 오차는 이 판정에 사용하지 않았습니다.`, `Across all ${mmm.run.seasonalitySelection.evidence?.observedWeeks || mmm.panel.week.length} weeks, the annual shape did not meet the improvement and repeatability thresholds after controlling for media, events, and trend. Recent 12-week forecast error was not used for this decision.`)}` : mmm.run.seasonalityPeriods?.length ? ` ${tx(`현재 ${mmm.panel.week.length}주 이력은 계절성 자동 판정 최소 96주에 못 미쳐, 연간 ${mmm.run.seasonalityPeriods.length}차 계절성을 기본값으로 유지했습니다. 최근 12주 오차로 계절성을 제거하지 않습니다.`, `The current ${mmm.panel.week.length}-week history is shorter than the 96-week minimum for automatic seasonality selection, so the annual ${mmm.run.seasonalityPeriods.length}-harmonic baseline remains active. Recent 12-week error does not remove seasonality.`)}` : ` ${tx("계절성 자동 판정이 비활성화되어 현재 설정을 유지했습니다.", "Automatic seasonality assessment is disabled, so the current configuration was retained.")}`}

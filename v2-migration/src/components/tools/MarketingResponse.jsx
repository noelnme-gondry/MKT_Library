...
function downloadMmmWorkbook({ mmm, cannib, decomp, trend, forecast, csvData, colMap, locale, currency }) {
  ...
  add("04_Model", [
    ...
    ["baseline_selection", JSON.stringify(run.baselineSelection || null)], ["media_penalty_selection", JSON.stringify(run.mediaPenaltySelection || null)], ["joint_transform_check", JSON.stringify(run.jointTransform || null)],
            ...
            <>
              ...
              {health && (
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  {(mmm.run.baselineSelection?.enabled || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>

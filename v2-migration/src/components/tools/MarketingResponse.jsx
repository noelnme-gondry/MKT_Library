              ...
              {health && (
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  {(mmm.run.baselineSelection?.enabled || Array.isArray(mmm.run.seasonalityPeriods) || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>
                    ...
                    {mmm.run.seasonalitySelection?.enabled ? ` ${tx(`계절성은 이벤트·step을 별도 제어한 상태에서 연간 조화파 0~4개 후보를 최근 ${mmm.run.seasonalitySelection.selected.folds}개 12주 구간으로 검증해 ${mmm.run.seasonalitySelection.selected.id}를 선택했습니다. 오차가 사실상 같으면 더 매끄러운 계절성을 유지합니다.`, `Seasonality controls events and steps separately, then selects ${mmm.run.seasonalitySelection.selected.id} from 0–4 annual harmonic candidates using ${mmm.run.seasonalitySelection.selected.folds} recent 12-week windows. Near ties retain the smoother seasonal shape.`)}` : mmm.run.seasonalityPeriods?.length ? ` ${tx(`현재 이력은 자동 후보 선택 기준(78주)에 못 미쳐, 연간 ${mmm.run.seasonalityPeriods.length}차 계절성을 기본값으로 유지했습니다. 기여가 0에 가까우면 계절성을 끈 것이 아니라 이벤트·step·추세를 통제한 뒤 남은 반복 신호가 약하다는 뜻입니다.`, `The history is shorter than the 78-week automatic-selection threshold, so the annual ${mmm.run.seasonalityPeriods.length}-harmonic baseline remains active. A near-zero contribution means the repeating signal is weak after controlling for events, steps, and trend; it is not disabled.`)}` : ` ${tx("시간순 검증에서 계절성 미사용이 선택되었습니다. 이벤트·step을 계절성으로 돌려 설명하지 않기 위한 결과입니다.", "Time-ordered validation selected no seasonality, rather than attributing events or steps to a repeating pattern.")}`}

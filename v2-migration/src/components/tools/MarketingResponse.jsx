                ...
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  {(mmm.run.baselineSelection?.enabled || Array.isArray(mmm.run.seasonalityPeriods) || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>
                    ...
                    {mmm.run.seasonalitySelection?.enabled ? ` ${mmm.run.seasonalitySelection.evidence?.detected
                      ? tx(`계절성은 최근 12주 예측과 분리했습니다. 전체 ${mmm.run.seasonalitySelection.evidence.observedWeeks}주에서 이벤트·step·추세·매체를 통제한 뒤 52주 반복 상관 ${Number(mmm.run.seasonalitySelection.evidence.lagCorrelation).toFixed(2)}, annual-1의 BIC 개선 ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)}을 확인해 ${mmm.run.seasonalitySelection.selected.id}를 유지했습니다.`, `Seasonality is separated from recent 12-week forecasting. Across all ${mmm.run.seasonalitySelection.evidence.observedWeeks} weeks, after controlling for events, steps, trend, and media, the 52-week recurrence correlation was ${Number(mmm.run.seasonalitySelection.evidence.lagCorrelation).toFixed(2)} and annual-1 improved BIC by ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)}; ${mmm.run.seasonalitySelection.selected.id} was retained.`)
                      : tx(`전체 ${mmm.run.seasonalitySelection.evidence?.observedWeeks || mmm.panel.week.length}주에서 이벤트·step·추세·매체를 통제한 52주 반복 증거가 기준에 못 미쳐 계절성을 미사용으로 판정했습니다. 최근 12주 예측 오차는 이 판정에 사용하지 않았습니다.`, `Across all ${mmm.run.seasonalitySelection.evidence?.observedWeeks || mmm.panel.week.length} weeks, 52-week recurrence after controlling for events, steps, trend, and media did not meet the evidence threshold. Recent 12-week forecast error was not used for this decision.`)}` : mmm.run.seasonalityPeriods?.length ? ` ${tx(`현재 ${mmm.panel.week.length}주 이력은 연간 반복성을 자동 판정하기 위한 104주에 못 미쳐, 연간 ${mmm.run.seasonalityPeriods.length}차 계절성을 기본값으로 유지했습니다. 최근 12주 오차로 계절성을 제거하지 않습니다.`, `The current ${mmm.panel.week.length}-week history is shorter than the 104 weeks required to assess annual recurrence, so the annual ${mmm.run.seasonalityPeriods.length}-harmonic baseline remains active. Recent 12-week error does not remove seasonality.`)}` : ` ${tx("계절성 자동 판정이 비활성화되어 현재 설정을 유지했습니다.", "Automatic seasonality assessment is disabled, so the current configuration was retained.")}`}

...
import {
  ...
  mmmResolveAbsorb,
  _mmmChans,
} from "@/utils/mmmMathPr401";
import { mmmForecastScenarioEligibility } from "@/utils/mmmMath";
...
import { buildMmmCollinearityGroupedPerformance } from "@/utils/mmmWeeklyPerformance";
import { buildMmmWeeklyPerformance as buildMmmPr401WeeklyPerformance } from "@/utils/mmmWeeklyPerformancePr401";
...
function downloadMmmWorkbook({ mmm, cannib, decomp, trend, forecast, csvData, colMap, locale, currency }) {
  ...
  add("04_Model", [
    ...
    ["pr401_provenance", JSON.stringify(run.pr401Provenance || null)], ["baseline_selection", JSON.stringify(run.baselineSelection || null)], ["seasonality_selection", JSON.stringify(run.seasonalitySelection || null)], ["media_penalty_selection", JSON.stringify(run.mediaPenaltySelection || null)], ["joint_transform_check", JSON.stringify(run.jointTransform || null)],
  ...
  add("05b_ChannelAttribution", [
    ...
    ...buildMmmPr401WeeklyPerformance(mmm.panel, run.saturationByChannel).map((contribution) => {
      const channel = (mmm.panel.channels || []).find((item) => item.key === contribution.key);
      const cost = contribution.avgWeeklySpend * mmm.panel.week.length;
      const value = contribution.avgWeeklyPredicted * mmm.panel.week.length;
      return [
...
export default function MarketingResponse({ locale = "ko" }) {
  ...
  const mmmBundle = useMemo(() => {
    ...
    try {
      ...
      // PR #401의 단일 채널 MMM을 그대로 실행한다. 날짜·플랫폼·컬럼 필터는
      // 현재 UI에서 패널을 만든 뒤 적용되지만, 이후 도입된 집계 Classic,
      // Bayesian-like, blackout prior, trend-freeze 엔진은 호출하지 않는다.
      ...
      if (!run) throw new Error("PR #401 Bayesian posterior estimate failed");
      run.modelVariant = "pr401-channel-mmm";
      run.methodLabel = "PR #401 channel MMM (frozen e6e40cf)";
      run.pr401Provenance = {
        commit: "e6e40cf",
  ...
  // PR #401에는 고상관 채널 그룹 재적합이 없었다. 화면의 기간·그룹 보기
  // 컨트롤은 유지하되 모델 기여를 후대 로직으로 다시 배분하지 않는다.
  ...
  const weeklyChannelPerformance = useMemo(() => {
    ...
    return buildMmmPr401WeeklyPerformance(mmm.panel, mmm.run.saturationByChannel);
  }, [mmm, stage]);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
        ...
        {stage === "mmm" && mmm && !mmm.empty && (
          <div className="ab-pillgroup" style={{ margin: 0 }}>
            ...
            <span className="ab-pill active">{tx("PR #401 MMM", "PR #401 MMM")}</span>
            <span
              title={tx(
                "계산 엔진은 PR #401(e6e40cf)의 채널별 Empirical-Bayes MMM으로 동결했습니다. 현재 날짜·플랫폼·컬럼 필터는 그대로 적용됩니다.",
                "The calculation engine is frozen to the per-channel empirical-Bayes MMM from PR #401 (e6e40cf). Current date, platform, and column filters remain active.",
              )}
            ...
            <span style={{ color: MUTED, fontSize: "10.5px" }}>
              {tx("엔진 v1.2.0 · 채널별 공동 적합", "Engine v1.2.0 · joint per-channel fit")}
            </span>
          </div>
        )}
        {mmm && !mmm.empty && (
      ...
      {panelEmpty ? (
        ...
        <>
          ...
          {stage === "mmm" && (() => {
            ...
            const viewedWeeklyChannelPerformance = buildMmmPr401WeeklyPerformance(
              viewedPanel,
              viewedRun.saturationByChannel,
            );
              ...
              {displayedWeeklyChannelPerformance.length > 0 && (
                <section className="block mmm-weekly-performance" id="s-mmm-weekly-performance">
                      ...
                      <p>{tx(
                        weeklyPerformanceView === "grouped" && hasCollinearityGroups
                          ...
                          : (displayedCollinearityGroupRefit?.enabled
                            ...
                            : "PR #401 방식으로 집행 주 Cost를 채널 반응곡선에 대입한 별도 표시값입니다."),
                        weeklyPerformanceView === "grouped" && hasCollinearityGroups
                          ? (displayedCollinearityGroupRefit?.enabled
                          ...
                          : (displayedCollinearityGroupRefit?.enabled
                            ...
                            : "This is the separate PR #401 display calculation that evaluates active-week Cost on each channel response curve."),
                      )}</p>
                    </div>
                        ...
                        {displayedWeeklyChannelPerformance.map((row) => {
                          ...
                          return (
                            <tr key={row.key} style={row.posteriorPositive != null && row.posteriorPositive < 0.8 ? { opacity: 0.62 } : undefined}>
                              ...
                              <td>
                                <span className="mmm-data-table__tag is-media">
                                  {tx("PR401 추정", "PR401 estimate")}
                                </span>
                              </td>
                            </tr>
                          );
                  ...
                  <p className="mmm-weekly-performance__foot">{tx(
                    "PR #401 방식대로 각 집행 주 Cost를 해당 채널의 responseAt 곡선에 대입해 채널별 평균 성과와 CPA/ROAS를 표시합니다. 이 채널 표는 당시 Decomp와 별도 표시 계산이므로 합계가 Decomp와 일치하도록 강제하지 않습니다.",
                    "Following PR #401, each active week's Cost is evaluated on that channel's responseAt curve to display average channel performance and CPA/ROAS. This historical channel table is a separate display calculation and is not forced to reconcile with Decomp.",
                  )}</p>

                ...
                <section className="block" aria-label={tx("모델 건강 진단", "Model health diagnostics")}>
                  ...
                  {(mmm.run.baselineSelection?.enabled || Array.isArray(mmm.run.seasonalityPeriods) || mmm.run.mediaPenaltySelection?.enabled || mmm.run.jointTransform?.enabled) && <p className="muted" style={{ fontSize: "11px", lineHeight: 1.5, margin: "8px 0 0" }}>
                    ...
                    {mmm.run.seasonalitySelection?.enabled ? ` ${mmm.run.seasonalitySelection.evidence?.detected
                      ? tx(`계절성은 최근 12주 예측과 분리했습니다. 전체 ${mmm.run.seasonalitySelection.evidence.observedWeeks}주에서 Brand/Performance 집계 매체·이벤트·추세를 통제한 뒤, 연간 파형 후보의 BIC가 ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)} 개선되고 52주 간 파형 상관이 ${Number(mmm.run.seasonalitySelection.evidence.seasonalLagCorrelation).toFixed(2)}로 유지됐습니다. 실제 연도 반복성은 ${Number(mmm.run.seasonalitySelection.evidence.observedRecurrenceScale).toFixed(2)}배 강도로 보수 조정했고, ${mmm.run.seasonalitySelection.selected.id === "business-harmonic-shrink" ? "큰 연간 흐름은 유지하면서 짧은 파동을 더 강하게 누르는 비즈니스 계절성" : mmm.run.seasonalitySelection.selected.id}을 채택했습니다.`, `Seasonality is separated from recent 12-week forecasting. Across all ${mmm.run.seasonalitySelection.evidence.observedWeeks} weeks, after controlling for aggregated Brand/Performance media, events, and trend, the annual-shape candidate improved BIC by ${Number(mmm.run.seasonalitySelection.evidence.bicImprovement).toFixed(1)} and retained ${Number(mmm.run.seasonalitySelection.evidence.seasonalLagCorrelation).toFixed(2)} 52-week shape correlation. We conservatively adjusted strength to ${Number(mmm.run.seasonalitySelection.evidence.observedRecurrenceScale).toFixed(2)} from observed recurrence and selected ${mmm.run.seasonalitySelection.selected.id === "business-harmonic-shrink" ? "business seasonality that preserves the broad annual shape while shrinking shorter waves more strongly" : mmm.run.seasonalitySelection.selected.id}.`)

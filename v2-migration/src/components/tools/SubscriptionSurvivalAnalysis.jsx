...
export default function SubscriptionSurvivalAnalysis({ locale = "ko", rows: rowsOverride, analyzed: analyzedOverride } = {}) {
  ...
  return <ToolPageShell toolId={TOOL_ID} titleLevel={0} title={tx(locale, "핵심 액션 생존·이탈 분석", "Action Survival & Drop-off Analysis")} locale={locale} summary={<p>{tx(locale, "특정 핵심 액션을 계속 유지하는지, 언제 이탈·종료하는지 중도절단을 반영해 봅니다. 구독은 하나의 입력 예시일 뿐이며, 개별 이탈을 예측하거나 원인을 확정하지는 않습니다.", "Measure whether an entity keeps a defined key action and when it exits or drops off with censoring handled. Subscription is only one input example; this does not predict individual drop-off or establish causes.")}</p>} toc={[{ id: "subscription-survival-result", title: tx(locale, "결론", "Conclusion") }, { id: "subscription-survival-curve", title: tx(locale, "생존곡선", "Survival") }, { id: "subscription-hazard", title: tx(locale, "위험 구간", "Risk") }]}>
    ...
    {hasRows && gateOpen && <section className="block" aria-label={tx(locale, "분석 설정", "Analysis settings")}>
      {/* 도구 안 필터는 `.analysis-local-controls`가 공용 패턴이다 — `.form-row`는
          globals.css에 정의가 없어 스타일이 붙지 않는다. */}
      <div className="analysis-local-controls"><div className="analysis-local-controls__inner">
        <span className="analysis-local-controls__label">{tx(locale, "분석 설정", "Settings")}</span>
        <label>{tx(locale, "입력 방식", "Input mode")}<select className="map-select" value={draft.inputMode} onChange={(event) => setDraft((value) => ({ ...value, inputMode: event.target.value }))}><option value="periods">{tx(locale, "기간 + 이벤트 여부", "Duration + event flag")}</option><option value="dates">{tx(locale, "시작·이벤트·관측 종료일", "Start, event, and observation dates")}</option></select></label>
        <label>{tx(locale, "시간 단위", "Time unit")}<select className="map-select" value={draft.timeUnit} onChange={(event) => setDraft((value) => ({ ...value, timeUnit: event.target.value }))}><option value="day">{tx(locale, "일", "Day")}</option><option value="week">{tx(locale, "주", "Week")}</option><option value="month">{tx(locale, "월", "Month")}</option></select></label>
        ...
        <label>{tx(locale, "세그먼트", "Segment")}<select className="map-select" value={draft.segmentKey} onChange={(event) => setDraft((value) => ({ ...value, segmentKey: event.target.value }))}><option value="">{tx(locale, "전체만", "Overall only")}</option>{availableSegments.map((key) => <option key={key} value={key}>{key}</option>)}</select></label>

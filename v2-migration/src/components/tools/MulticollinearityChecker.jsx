...
export default function MulticollinearityChecker({ locale = "ko" } = {}) {
  ...
  const maxVif = vifRows.reduce((highest, row) => Math.max(highest, row.vif), Number.NEGATIVE_INFINITY);
  const formattedMaxVif = Number.isFinite(maxVif) ? maxVif.toFixed(2) : "∞";
  const decisionPrefill = result?.vif && !isDemo ? {
  ...
  return <ToolPageShell toolId="5-25" locale={locale} titleLevel={0} title={tr("VIF 다중공선성 점검", "VIF Multicollinearity Check")} summary={<p>{tr("MMM을 돌리기 전에 채널별 지출이 서로 너무 같이 움직였는지 확인합니다. 숫자는 진단 신호이며, 공선성을 해결한 인과 추정은 아닙니다.", "Check whether channel spend moved too tightly together before running MMM. This is a diagnostic signal, not a causal fix.")}</p>}>
    ...
    {analyzed && <>
      <section className={`block diagnostic-tool__verdict diagnostic-tool__verdict--${verdict || "unknown"}`}><h2 className="section-title">{tr("판정", "Verdict")}</h2><strong>{verdict === "ok" ? tr("MMM 진행 가능 · 해석은 신중하게", "MMM can proceed · interpret cautiously") : verdict === "warn" ? tr("주의 · 채널 분리 변동 권장", "Warning · create independent variation") : verdict === "severe" ? tr("중단 · 현재 데이터로 기여도 분리 금지", "Stop · do not separate contribution") : tr("데이터 추가 필요", "Need more data")}</strong><p>{copy}</p></section>

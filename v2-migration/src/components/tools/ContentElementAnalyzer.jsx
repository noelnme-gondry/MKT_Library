...
export default function ContentElementAnalyzer() {
  ...
  useEffect(() => {
    ...
    chartInstance.current = new Chart(chartRef.current, {
      ...
      options: {
        ...
        scales: {
          x: {
            title: { display: true, text: `${fit.outcome} 변화 (계수)`, color: CHART_THEME.muted },
          ...
          tooltip: {
            callbacks: {
              label: (ctx) => {
                ...
                if (!r) return "";
                return `${r.name}: 계수 ${r.coef.toFixed(3)} · 95%CI [${r.ciLo.toFixed(3)}, ${r.ciHi.toFixed(3)}] · p=${r.p.toFixed(3)}${r.sig ? " (유의)" : " (무유의)"}`;
              },
            },
          },
        },
  ...
  return (
    <div className="tab-pane active">
      ...
      {/* ── §0 매핑 ── */}
      <section className="block">
        <h2 className="section-title"><span className="ix">§0</span>컬럼 지정</h2>
        <div className="csv-loaded-bar">
          <div className="csv-loaded-info">
            <span className="dot" style={{ background: isDemo ? "#f59e0b" : "#22c55e" }}></span>
            {isDemo ? <strong>샘플 데이터로 미리보기 중</strong> : <strong>{fileName}</strong>}
            <span className="csv-loaded-stats tnum">{csvData.raw.length.toLocaleString()}행 · 숫자 컬럼 {numericCols.length}개</span>
          </div>
        ...
        <div style={{ marginTop: "12px", display: "grid", gap: "12px" }}>
          <div>
            <label style={{ fontSize: "12.5px", fontWeight: 600, color: "var(--text-1)" }}>{C.outcomeLabel}</label>
            <div style={{ fontSize: "11px", color: MUTED, margin: "2px 0 6px" }}>설명하고 싶은 성과 숫자 1개를 고르세요.</div>
          ...
          <div>
            <label style={{ fontSize: "12.5px", fontWeight: 600, color: "var(--text-1)" }}>{C.featureLabel}</label>
            <div style={{ fontSize: "11px", color: MUTED, margin: "2px 0 6px" }}>성과에 영향을 줬을 만한 요소들을 고르세요(있음/없음은 0/1, 길이·개수는 숫자).</div>
        ...
        {!canAnalyze ? (
          <div className="required-banner" style={{ marginTop: "12px" }}>
            <strong>⚠ 성과 1개 + 요소 1개 이상을 지정하세요</strong>
          </div>
        ) : analyzed ? (
          <div style={{ marginTop: "12px", display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            <span style={{ color: "#22c55e", fontSize: "12px", fontWeight: 600 }}>✓ 분석 완료</span>
            <span style={{ color: MUTED, fontSize: "11px" }}>컬럼을 바꾸면 결과가 숨겨지고 다시 분석해야 합니다.</span>
            <button className="ab-pill" style={{ marginLeft: "auto" }} onClick={() => requestAd(() => setAnalyzedSig(analyzeSig(outcome, features, fileName)))}>↻ 다시 분석</button>
          </div>
        ) : (
          <div style={{ marginTop: "12px", background: "linear-gradient(135deg,rgba(122,162,247,0.12),rgba(122,162,247,0.03))", border: "1px solid rgba(122,162,247,0.3)", borderRadius: "10px", padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
            <div style={{ fontSize: "12.5px", color: "var(--text-1)" }}>✅ 지정 완료. <strong>분석을 실행하세요.</strong></div>
            <button className="ab-pill" style={{ background: "#7aa2f7", color: "#0b0d12", fontWeight: 700, borderColor: "#7aa2f7", fontSize: "13px", padding: "8px 18px" }} onClick={() => requestAd(() => setAnalyzedSig(analyzeSig(outcome, features, fileName)))}>▶ 분석하기</button>
          </div>
        )}
      </section>
        ...
        <section className="block">
          <div className="required-banner">
            ...
            <p style={{ margin: ".35rem 0 0", fontSize: "12.5px" }}>
              {fit.error === "no-variance" && "고른 요소가 전부 같은 값이라 성과와의 관계를 추정할 수 없습니다. 값이 변하는 요소를 넣어주세요."}
              {fit.error === "too-few-rows" && `데이터 행이 요소 수 대비 너무 적습니다(행 ${fit.n || 0} · 변수 ${fit.k || 0}). 콘텐츠 편수를 늘리거나 요소 수를 줄이세요.`}
              {fit.error === "singular" && "요소들이 서로 너무 비슷(공선성)해 각 요소의 몫을 분리할 수 없습니다. 겹치는 요소를 하나만 남겨보세요."}
            </p>
          </div>
        </section>
      )}
      ...
      {analyzed && fit && !fit.error && (
        <>
          ...
          <section className="block" style={{ background: "linear-gradient(135deg, rgba(122,162,247,0.12), rgba(192,132,252,0.05))", border: "1px solid rgba(122,162,247,0.25)", borderRadius: "14px", padding: "18px 20px" }}>
            ...
            <p className="muted" style={{ fontSize: "12px", marginTop: "-4px", marginBottom: "14px" }}>{C.heroSub}</p>
            {topSig ? (
              <div style={{ background: "var(--surface-container-low)", border: "1px solid rgba(34,197,94,0.4)", borderRadius: "10px", padding: "12px 14px", marginBottom: "12px" }}>
                <div style={{ fontSize: "12px", color: MUTED, marginBottom: "4px" }}>🏆 가장 강한 요소</div>
            ...
            <div className="callout warn" style={{ margin: 0 }}>
              ...
              <div className="body">
                <strong>연관이지 인과 아님</strong>
                <p style={{ margin: ".25rem 0 0" }} dangerouslySetInnerHTML={{ __html: C.causationBody }} />
              </div>
            </div>
          </section>
          ...
          <section className="block">
            <h2 className="section-title"><span className="ix">§1</span>어떤 요소가 성과와 연관됐나</h2>
          ...
          <details className="block">
            <summary style={{ cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "var(--primary, #adc6ff)", padding: "4px 0" }}>
              📊 전문가 뷰 — 계수·표준오차·p값 전체 표, 모델 적합도
            </summary>
            <div style={{ marginTop: "12px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
                <div style={{ fontSize: "12.5px", color: MUTED }}>
                  R² {(fit.R2).toFixed(3)} · adj.R² {(fit.adjR2).toFixed(3)} · n {fit.n.toLocaleString()} · 절편 {fit.intercept.toFixed(3)}
                  {fit.dropped.length ? ` · 무분산 제외: ${fit.dropped.join(", ")}` : ""}
                </div>
              ...
              <div className="table-wrap" style={{ marginTop: "8px" }}>
                <table className="data" style={{ fontSize: "12.5px" }}>
                  <thead>
                    <tr>
                      <th style={{ textAlign: "left" }}>요소</th>
                      <th style={{ textAlign: "right" }} title="추정 효과(계수)">계수</th>
                      <th style={{ textAlign: "right" }} title="표준오차">SE</th>
                      <th style={{ textAlign: "right" }} title="t = 계수/SE (연관 강도)">t</th>
                      <th style={{ textAlign: "right" }} title="two-sided p-value">p</th>
                      <th style={{ textAlign: "right" }} title="95% 신뢰구간">95% CI</th>
                  ...
                  <tbody>
                    {fit.rows.map((r) => (
                      <tr key={r.name}>
                        ...
                        <td style={{ textAlign: "center" }}>{r.sig ? (r.coef >= 0 ? "유의 ↑" : "유의 ↓") : "무유의"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="muted" style={{ fontSize: "11px", marginTop: "8px" }}>
                중요도 순서는 연관 강도(|t|) 기준입니다(단위가 다른 요소도 공정 비교). 계수는 각 요소의 원단위 효과 — 있음/없음(0/1) 요소는 &quot;있을 때 vs 없을 때&quot;, 숫자 요소는 &quot;1 증가 시&quot; 성과 변화입니다.
              </p>
            </div>
          </details>
        </>

  ...
  return <div className="tab-pane active" id="tab-brand-incrementality">
    ...
    {result?.ok && <section className="block" id="brand-its-result">
      ...
      <div className="callout"><div className="body"><strong>{result.ci95[0] > 0
        ? result.diagnostics.smallSampleHac
          ? tx(locale, `증가 신호는 있으나 사전 ${result.prePeriods}기간은 HAC가 안정적인 ${result.diagnostics.hacReliablePrePeriods}기간보다 짧습니다. 탐색적 결과로만 읽으세요.`, `There is a lift signal, but ${result.prePeriods} pre-periods is below the ${result.diagnostics.hacReliablePrePeriods} periods where HAC is more stable. Treat this as exploratory.`)
          : tx(locale, "현재 HAC 모델의 95% 구간은 변화 없음과 겹치지 않습니다. 다만 관찰 연구의 인과 증명은 아닙니다.", "The current HAC-model 95% interval does not include no change. This is still not causal proof from an observational study.")
        : tx(locale, "증가 신호는 있지만, HAC 보정 불확실성을 넘었다고 말하기 어렵습니다.", "There is a lift signal, but it does not clearly exceed HAC-adjusted uncertainty.")}</strong><p>{tx(locale, `캠페인 시작일 ${result.campaignStartDate} 이후 실제 성과와 사전 추세 기반 반사실을 비교했습니다. 대조군이 없으므로 계절성·PR·프로모션 영향은 분리되지 않습니다.`, `We compare actual outcomes after ${result.campaignStartDate} with a pre-trend counterfactual. Without a control, seasonality, PR, and promotions are not separated.`)}</p></div></div>
      <div className="chart-container" style={{ height: "320px", marginTop: "16px" }}><canvas ref={chartRef} /></div>
      <details style={{ marginTop: "14px" }}><summary>{tx(locale, "근거·한계 확인", "Review evidence and limitations")}</summary><ul><li>{tx(locale, `사전 ${result.prePeriods}기간 · 집행 ${result.postPeriods}기간 · 사전 추세 일평균 변화 ${result.trend.slopePerDay.toFixed(2)}`, `${result.prePeriods} pre periods · ${result.postPeriods} campaign periods · pre-trend daily change ${result.trend.slopePerDay.toFixed(2)}`)}</li><li>{tx(locale, `사전 추세 적합도 R² ${result.trend.rSquared == null ? "추정 불가" : result.trend.rSquared.toFixed(2)}`, `Pre-trend R² ${result.trend.rSquared == null ? "not estimable" : result.trend.rSquared.toFixed(2)}`)}</li><li>{tx(locale, `잔차 AR(1) ${result.diagnostics.residualAr1 == null ? "추정 불가" : result.diagnostics.residualAr1.toFixed(2)}에 맞춘 Andrews 대역폭(랙 ${result.diagnostics.hacLag})과 자유도 보정 HAC·t 분포로 95% CI를 계산했습니다. iid 표준오차 ${formatValue(result.iidStandardError, locale)} 대신 ${formatValue(result.standardError, locale)}를 사용합니다.`, `The 95% CI uses an Andrews bandwidth (lag ${result.diagnostics.hacLag}) based on residual AR(1) ${result.diagnostics.residualAr1 == null ? "not estimable" : result.diagnostics.residualAr1.toFixed(2)}, finite-sample-corrected HAC, and a t distribution: ${formatValue(result.standardError, locale)} rather than iid ${formatValue(result.iidStandardError, locale)}.`)}</li>{result.diagnostics.smallSampleHac && <li>{tx(locale, `사전 기간 ${result.prePeriods}개는 HAC가 안정적인 기준 ${result.diagnostics.hacReliablePrePeriods}개보다 짧습니다. 이 CI는 과소평가될 수 있어 증분 확정 근거로 쓰지 마세요.`, `The ${result.prePeriods} pre-periods are below the ${result.diagnostics.hacReliablePrePeriods} periods where HAC is more stable. This CI can still be too narrow; do not use it to confirm incrementality.`)}</li>}<li>{result.diagnostics.hasPeriodGaps ? tx(locale, `${result.diagnostics.grain === "week" ? "주" : result.diagnostics.grain === "month" ? "월" : "일"} 단위 누락 기간이 ${result.diagnostics.missingPeriods}개 있습니다. 가장 긴 간격은 ${result.diagnostics.maxGapDays}일입니다.`, `There are ${result.diagnostics.missingPeriods} missing ${result.diagnostics.grain} period(s); the longest gap is ${result.diagnostics.maxGapDays} days.`) : tx(locale, `${result.diagnostics.grain === "week" ? "주간" : result.diagnostics.grain === "month" ? "월간" : "일별"} cadence에서 누락 기간은 발견되지 않았습니다.`, `No missing periods were detected for the ${result.diagnostics.grain} cadence.`)}</li>{result.diagnostics.invalidRows > 0 && <li>{tx(locale, `날짜·성과·집행 여부가 유효하지 않은 ${result.diagnostics.invalidRows}행은 제외했습니다.`, `${result.diagnostics.invalidRows} row(s) with invalid date, outcome, or campaign status were excluded.`)}</li>}<li>{tx(locale, "HAC는 잔차 자기상관으로 과소평가되던 불확실성을 보정하지만, 대조군 없는 관찰 연구의 계절성·PR·프로모션 교란은 제거하지 못합니다.", "HAC corrects uncertainty that serially correlated residuals can understate, but it cannot remove seasonality, PR, or promotion confounding without a control.")}</li><li>{tx(locale, "다음 캠페인에서는 비집행 지역·오디언스를 남겨 홀드아웃/DiD로 인과 근거를 강화하세요.", "For the next campaign, retain an unexposed region or audience to strengthen causal evidence with holdout / DiD.")}</li></ul></details>
    </section>}
  </div>;
}

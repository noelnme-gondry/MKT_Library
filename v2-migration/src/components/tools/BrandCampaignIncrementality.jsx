...
const isNumericColumn = (rows, header) => rows.slice(0, 100).filter((row) => Number.isFinite(numeric(row?.[header]))).length >= Math.min(3, rows.length);
...
export default function BrandCampaignIncrementality({ locale = "ko" }) {
  ...
  useEffect(() => {
    ...
    chartInstance.current = new Chart(chartRef.current, {
      ...
      options: {
        ...
        scales: { x: { ticks: { color: theme.muted, maxTicksLimit: 8 }, grid: { color: theme.grid } }, y: { ticks: { color: theme.muted }, grid: { color: theme.grid } } },
      },
    });
  ...
  const readiness = [
    ...
    { id: "its", icon: "↗", title: tx(locale, "날짜별 성과와 ON/OFF 시점이 있다", "I have dated outcomes and a clear ON/OFF date"), body: tx(locale, "최소 집행 전 21기간·집행 후 7기간으로 ITS 반사실 추정을 실행합니다.", "Run an ITS counterfactual with at least 21 pre and 7 post periods."), cta: tx(locale, "ITS 분석 준비", "Set up ITS") },
  ...
  return <div className="tab-pane active" id="tab-brand-incrementality">
    ...
    {dataPath === "its" && <section className="block" id="brand-its-setup">
      ...
      <p className="muted">{tx(locale, "한 행은 하루 또는 한 주입니다. 브랜드 검색·직접 유입·가입 중 하나를 성과 지표로 쓰고, 캠페인 집행 여부는 집행 전 OFF → 집행 후 ON인 한 번의 연속 구간이어야 합니다.", "One row is a day or week. Choose brand search, direct traffic, or signups as the outcome. Campaign status must be one continuous OFF-before / ON-after window.")}</p>
      {!hasData ? <div className="csv-uploader">
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "10px" }}><button type="button" className="ab-pill" onClick={downloadTemplate}>{tx(locale, "CSV 양식 다운로드", "Download CSV template")}</button><button type="button" className="ab-pill" onClick={() => loadRows(brandDemo(), "demo_brand_campaign_its.csv")}>{tx(locale, "예시 데이터 보기", "View example data")}</button></div>
      </div> : <>
    ...
    {result && !result.ok && <section className="block"><div className="callout warn"><div className="body"><strong>{tx(locale, "아직 정직한 ITS 추정을 만들 수 없습니다", "ITS is not yet identifiable")}</strong><p>{result.reason === "multiple_campaign_windows" ? tx(locale, "ON/OFF 구간이 여러 번입니다. 이번 버전은 한 번의 연속 캠페인 구간만 분석합니다. 구간 하나만 남기거나 통제군 설계를 사용하세요.", "There are multiple ON/OFF windows. This version analyzes one continuous campaign window; isolate one window or use a control-group design.") : result.reason === "insufficient_pre_periods" ? tx(locale, `집행 전 기간이 ${result.prePeriods}개입니다. 최소 ${result.minPrePeriods}개 기간이 필요합니다.`, `There are ${result.prePeriods} pre periods; at least ${result.minPrePeriods} are required.`) : result.reason === "insufficient_post_periods" ? tx(locale, `집행 후 기간이 ${result.postPeriods}개입니다. 최소 ${result.minPostPeriods}개 기간이 필요합니다.`, `There are ${result.postPeriods} post periods; at least ${result.minPostPeriods} are required.`) : tx(locale, "날짜·성과·집행 여부를 다시 확인하세요.", "Check date, outcome, and campaign-status columns.")}</p></div></div></section>}
    ...
    {result?.ok && <section className="block" id="brand-its-result">
      ...
      <div className="callout"><div className="body"><strong>{result.ci95[0] > 0 ? tx(locale, "관찰된 증가분은 기준선 불확실성 범위보다 큽니다.", "Observed lift exceeds the estimated baseline uncertainty.") : tx(locale, "증가 신호는 있지만, 기준선 불확실성을 넘었다고 말하기 어렵습니다.", "There is a lift signal, but it does not clearly exceed baseline uncertainty.")}</strong><p>{tx(locale, `캠페인 시작일 ${result.campaignStartDate} 이후 실제 성과와 사전 추세 기반 반사실을 비교했습니다. 대조군이 없으므로 계절성·PR·프로모션 영향은 분리되지 않습니다.`, `We compare actual outcomes after ${result.campaignStartDate} with a pre-trend counterfactual. Without a control, seasonality, PR, and promotions are not separated.`)}</p></div></div>
      <div className="chart-container" style={{ height: "320px", marginTop: "16px" }}><canvas ref={chartRef} /></div>
      <details style={{ marginTop: "14px" }}><summary>{tx(locale, "근거·한계 확인", "Review evidence and limitations")}</summary><ul><li>{tx(locale, `사전 ${result.prePeriods}기간 · 집행 ${result.postPeriods}기간 · 사전 추세 일평균 변화 ${result.trend.slopePerDay.toFixed(2)}`, `${result.prePeriods} pre periods · ${result.postPeriods} campaign periods · pre-trend daily change ${result.trend.slopePerDay.toFixed(2)}`)}</li><li>{tx(locale, `사전 추세 적합도 R² ${result.trend.rSquared == null ? "추정 불가" : result.trend.rSquared.toFixed(2)}`, `Pre-trend R² ${result.trend.rSquared == null ? "not estimable" : result.trend.rSquared.toFixed(2)}`)}</li><li>{result.diagnostics.hasDateGaps ? tx(locale, `최대 ${result.diagnostics.maxGapDays}일의 날짜 공백이 있습니다. 누락 구간은 반사실의 신뢰도를 낮춥니다.`, `There is a maximum ${result.diagnostics.maxGapDays}-day date gap, which lowers counterfactual confidence.`) : tx(locale, "날짜 공백은 발견되지 않았습니다.", "No date gaps were detected.")}</li><li>{tx(locale, "다음 캠페인에서는 비집행 지역·오디언스를 남겨 홀드아웃/DiD로 인과 근거를 강화하세요.", "For the next campaign, retain an unexposed region or audience to strengthen causal evidence with holdout / DiD.")}</li></ul></details>
    </section>}
  </div>;
}

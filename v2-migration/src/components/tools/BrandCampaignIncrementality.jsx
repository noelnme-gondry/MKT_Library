...
export default function BrandCampaignIncrementality({ locale = "ko" }) {
  ...
  return <div className="tab-pane active" id="tab-brand-incrementality">
    ...
    {result?.ok && <section className="block" id="brand-its-result">
      ...
      <div className="callout"><div className="body"><strong>{result.ci95[0] > 0
        ? result.diagnostics.ar1EvidenceTier === "exploratory"
          ? tx(locale, `AR(1) 가정 아래 증가 신호가 있으나 사전 ${result.prePeriods}기간 결과는 탐색적으로만 읽으세요. 증분 확정 근거가 아닙니다.`, `There is a lift signal under the AR(1) assumption, but ${result.prePeriods} pre-periods are exploratory only. This does not confirm incrementality.`)
          : result.diagnostics.ar1EvidenceTier === "assumption_sensitive"
            ? tx(locale, "AR(1) 가정 아래 95% 구간은 변화 없음과 겹치지 않지만, 사전기간과 잔차 가정에 민감한 관찰 증거입니다.", "Under the AR(1) assumption, the 95% interval excludes no change, but this observational evidence remains sensitive to history length and the residual assumption.")
            : tx(locale, "AR(1) 가정 아래 95% 구간은 변화 없음과 겹치지 않습니다. 그래도 관찰 연구의 인과 증명은 아닙니다.", "Under the AR(1) assumption, the 95% interval excludes no change. This is still not causal proof from an observational study.")
        : tx(locale, "증가 신호는 있지만, AR(1) 모형의 불확실성을 넘었다고 말하기 어렵습니다.", "There is a lift signal, but it does not clearly exceed uncertainty under the AR(1) model.")}</strong><p>{tx(locale, `캠페인 시작일 ${result.campaignStartDate} 이후 실제 성과와 사전 추세 기반 반사실을 비교했습니다. 대조군이 없으므로 계절성·PR·프로모션 영향은 분리되지 않습니다.`, `We compare actual outcomes after ${result.campaignStartDate} with a pre-trend counterfactual. Without a control, seasonality, PR, and promotions are not separated.`)}</p></div></div>
      <div className="chart-container" style={{ height: "320px", marginTop: "16px" }}><canvas ref={chartRef} /></div>
      <details style={{ marginTop: "14px" }}><summary>{tx(locale, "근거·한계 확인", "Review evidence and limitations")}</summary><ul><li>{tx(locale, `사전 ${result.prePeriods}기간 · 집행 ${result.postPeriods}기간 · AR(1) 사전 추세 일평균 변화 ${result.trend.slopePerDay.toFixed(2)}`, `${result.prePeriods} pre periods · ${result.postPeriods} campaign periods · AR(1) pre-trend daily change ${result.trend.slopePerDay.toFixed(2)}`)}</li><li>{tx(locale, `사전 추세 적합도 R² ${result.trend.rSquared == null ? "추정 불가" : result.trend.rSquared.toFixed(2)}`, `Pre-trend R² ${result.trend.rSquared == null ? "not estimable" : result.trend.rSquared.toFixed(2)}`)}</li><li>{tx(locale, `95% CI는 잔차 AR(1) ${result.diagnostics.residualAr1 == null ? "추정 불가" : result.diagnostics.residualAr1.toFixed(2)}를 가정한 Prais–Winsten 추세와 미래 잔차합 분산으로 계산했습니다. 모형 표준오차 ${formatValue(result.standardError, locale)}를 사용합니다.`, `The 95% CI uses a Prais–Winsten trend and future residual-sum variance under residual AR(1) ${result.diagnostics.residualAr1 == null ? "not estimable" : result.diagnostics.residualAr1.toFixed(2)}. Model SE: ${formatValue(result.standardError, locale)}.`)}</li><li>{result.diagnostics.ar1EvidenceTier === "exploratory" ? tx(locale, `사전 ${result.prePeriods}기간은 탐색적 구간입니다. AR(1) 가정이 맞지 않으면 CI가 달라질 수 있으므로 증분 확정에 쓰지 마세요.`, `The ${result.prePeriods} pre-periods are exploratory. If the AR(1) assumption is wrong, the CI can differ; do not use this to confirm incrementality.`) : result.diagnostics.ar1EvidenceTier === "assumption_sensitive" ? tx(locale, `사전 ${result.prePeriods}기간은 AR(1) 가정에 민감한 구간입니다. 더 긴 사전기간 또는 통제군으로 확인하세요.`, `The ${result.prePeriods} pre-periods remain sensitive to the AR(1) assumption. Confirm with longer history or a control group.`) : tx(locale, `사전 ${result.prePeriods}기간은 참고 가능한 길이지만, AR(1) 가정과 통제군 부재 한계는 남습니다.`, `The ${result.prePeriods} pre-periods are usable as reference, but the AR(1) assumption and lack of a control remain limitations.`)}</li><li>{result.diagnostics.hacReferenceStandardError == null ? tx(locale, "HAC 참고 표준오차는 추정하지 못했습니다. 결과 CI에는 사용하지 않습니다.", "The HAC reference SE could not be estimated and is not used for the result CI.") : tx(locale, `비교용 HAC 표준오차는 ${formatValue(result.diagnostics.hacReferenceStandardError, locale)} (랙 ${result.diagnostics.hacLag})입니다. 결과 CI에는 사용하지 않습니다.`, `Reference-only HAC SE is ${formatValue(result.diagnostics.hacReferenceStandardError, locale)} (lag ${result.diagnostics.hacLag}); it is not used for the result CI.`)}</li><li>{result.diagnostics.hasPeriodGaps ? tx(locale, `${result.diagnostics.grain === "week" ? "주" : result.diagnostics.grain === "month" ? "월" : "일"} 단위 누락 기간이 ${result.diagnostics.missingPeriods}개 있습니다. 가장 긴 간격은 ${result.diagnostics.maxGapDays}일입니다.`, `There are ${result.diagnostics.missingPeriods} missing ${result.diagnostics.grain} period(s); the longest gap is ${result.diagnostics.maxGapDays} days.`) : tx(locale, `${result.diagnostics.grain === "week" ? "주간" : result.diagnostics.grain === "month" ? "월간" : "일별"} cadence에서 누락 기간은 발견되지 않았습니다.`, `No missing periods were detected for the ${result.diagnostics.grain} cadence.`)}</li>{result.diagnostics.invalidRows > 0 && <li>{tx(locale, `날짜·성과·집행 여부가 유효하지 않은 ${result.diagnostics.invalidRows}행은 제외했습니다.`, `${result.diagnostics.invalidRows} row(s) with invalid date, outcome, or campaign status were excluded.`)}</li>}<li>{tx(locale, "AR(1) 모형은 잔차가 한 기간 전 충격을 일정 비율로 이어간다고 가정합니다. 이 가정은 계절성·PR·프로모션 교란을 제거하지 못합니다.", "The AR(1) model assumes a residual shock carries into the next period at a stable rate. It does not remove seasonality, PR, or promotion confounding.")}</li><li>{tx(locale, "다음 캠페인에서는 비집행 지역·오디언스를 남겨 홀드아웃/DiD로 인과 근거를 강화하세요.", "For the next campaign, retain an unexposed region or audience to strengthen causal evidence with holdout / DiD.")}</li></ul></details>
    </section>}
  </div>;
}

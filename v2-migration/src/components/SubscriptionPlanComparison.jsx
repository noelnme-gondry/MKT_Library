...
export default function SubscriptionPlanComparison({ locale = "ko", paid = false, trialEndsAt = null, now }) {
  ...
  const rows = en ? [
    ...
    ["On-device reviews & decisions", "Save and revisit after sign-in", "Save and revisit after sign-in"],
    ["Project creation", `${SUBSCRIPTION.freeProjects} project · free after the trial`, "No project-count cap*"],
    ...
    ["기기 내 리뷰·결정 기록", "로그인 후 저장·다음 결과 검토", "로그인 후 저장·다음 결과 검토"],
    ["프로젝트 생성", `${SUBSCRIPTION.freeProjects}개 · 체험 종료 후에도 무료`, "개수 제한 없음*"],
  ...
  return <div id="plans" className="plan-comparison">
    <div className="plan-comparison-grid">
      {[false, true].map(pro => <article key={String(pro)} className={`plan-card${pro ? " plan-card--pro" : ""}`} aria-labelledby={`plan-${pro ? "pro" : "free"}-title`}>
        ...
        <div className="plan-card-header">
          ...
          <p className="plan-billing">{pro ? (en ? "1 calendar month · no automatic renewal" : "1개월 이용권 · 자동 갱신 없음") : (en ? "Analyze without signup · sign in to save" : "분석은 가입 없이 · 저장은 로그인 후")}</p>
        </div>
    ...
    <p className="plan-footnote">{en ? "The 14-day trial applies to Pro features such as creating additional projects and saving account memos. On-device reviews remain free after the trial. The trial starts with your first successful account memo save; account memos do not sync your entire project." : "14일 체험은 추가 프로젝트 생성·계정 메모 저장 등 Pro 기능에 적용됩니다. 기기 내 리뷰는 체험 종료 후에도 무료입니다. 체험은 첫 계정 메모 저장이 성공한 날 시작하며, 계정 메모가 프로젝트 전체를 동기화하지는 않습니다."}</p>

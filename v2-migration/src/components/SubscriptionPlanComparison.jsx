...
export default function SubscriptionPlanComparison({ locale = "ko", paid = false, trialEndsAt = null, now }) {
  ...
  const rows = en ? [
    ["Analysis tools", "All tools included"],
    ["Reviews & decision records", "Save and review next results"],
    ["Project creation", "No project-count cap*"],
    ["Analysis downloads", "Word, Excel, plus available CSV/PNG"],
    ["Multi-project reports", "Batch weekly reports"],
    ["Batch report branding", "Logo, company name and footer"],
    ...
    ["프로젝트 생성", "개수 제한 없음*"],
    ["분석 결과 다운로드", "Word·Excel + 제공되는 CSV·PNG"],
    ["여러 프로젝트 보고서", "주간 보고서 일괄 모으기"],
    ["일괄 보고서 브랜딩", "로고·회사명·푸터 설정"],
    ["내 프로젝트 백업", "내보내기·복원"],
    ["계정 결정 메모", "새 메모 저장·수정"],
  ];
  return <div id="plans" className="plan-comparison">
    <div className="plan-comparison-grid">
      {[false, true].map(pro => <article key={String(pro)} className={`plan-card${pro ? " plan-card--pro" : ""}`} aria-labelledby={`plan-${pro ? "pro" : "free"}-title`}>
        ...
        <dl className="plan-features">{(pro ? rows : [rows[0], en ? ["Analysis results", "View on the website"] : ["분석 결과", "사이트에서 확인"]]).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>

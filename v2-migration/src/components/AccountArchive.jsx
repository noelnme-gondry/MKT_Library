...
import AccountMemoPreview from "./account/AccountMemoPreview";
...
export default function AccountArchive({ locale = "ko", record = null, profile = false, compact = false, anchorId, onSession, hideIdentity = false }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (event.data?.type === "gop-account-ready") { trackLoginCompleted(event, { locale, source: "account_archive" }); refresh(); }
    };
  ...
  return <section id={anchorId} className="account-archive block" aria-label={profile ? (en ? "My account" : "마이페이지") : (en ? "Account decision archive" : "계정 결정 보관함")}>
    ...
    {!profile && <p>{en ? `Analysis needs no signup. Sign in to save decision memos. One ${PRO_TRIAL_DAYS}-day Pro trial per Google account starts when you create your first project; it does not renew automatically.` : `분석은 가입 없이. 결정 메모 보관은 로그인으로. Google 계정당 1회, 첫 프로젝트를 만든 날부터 ${PRO_TRIAL_DAYS}일간 Pro를 체험하며 자동 결제되지 않습니다.`}</p>}
    ...
    {!session.account ? <button className={profile ? "btn primary account-profile-login" : "btn"} disabled={busy} onClick={() => {
      ...
      run(async () => { try { const result = await accountRequest("login", { method: "POST" }); popup.location.replace(result.url); trackLoginStarted("google", { locale, source: "account_archive" }); } catch (error) { popup.close(); throw error; } });
    ...
    {!session.account && session.mailEnabled && <section className="account-link-login" aria-labelledby="account-link-title">
      ...
      <form onSubmit={event => { event.preventDefault(); run(async () => { await accountRequest("email-login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, locale }) }); trackLoginStarted("email_link", { locale, source: "account_archive" }); setMessage(en ? "If this email identifies an existing account, check your inbox for a link." : "기존 계정에 등록된 이메일이라면 받은편지함에서 링크를 확인해 주세요."); }); }}>

...
export default function AccountArchive({ locale = "ko", record = null }) {
  const en = locale === "en";
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (next.account && !record) { const result = await accountRequest("memos"); if (active && currentVersion === version) setMemos(result.memos); }
    ...
    return () => { active = false; window.removeEventListener("message", onMessage); window.removeEventListener("focus", refresh); };
  }, [en, locale, record]);
  if (!session?.enabled) return message ? <p role="status">{message}</p> : null;
  ...
  return <section className="account-archive block" aria-label={en ? "Account decision archive" : "계정 결정 보관함"}>
    <h3>{en ? "Keep this decision across devices" : "다른 기기에서도 이 결정 이어보기"}</h3>
    <p>{en ? "Analysis needs no signup. Sign in to save decision memos. One 14-day Pro trial per Google account starts with your first save; it does not renew automatically." : "분석은 가입 없이. 결정 메모 보관은 로그인으로. Google 계정당 1회, 첫 저장일부터 14일간 Pro를 체험하며 자동 결제되지 않습니다."}</p>
    <p>{en ? "Source CSVs are never sent to our server. Only the decision memo you choose is stored in your account. Memos may contain campaign names and figures you wrote." : "CSV 원본은 서버에 보내지 않습니다. 저장하기로 선택한 결정 메모만 계정에 보관됩니다. 메모에는 작성한 캠페인명·수치가 포함될 수 있습니다."}</p>
    {!session.account ? <button className="btn" disabled={busy} onClick={() => {
      ...
      {!record && <label>{en ? "Choose a local decision to save" : "계정에 보관할 로컬 결정 선택"}<select value={selectedId} onChange={event => { setSelectedId(event.target.value); setConsent(false); }}><option value="">{en ? "Choose a decision" : "결정 선택"}</option>{localRecords.map(item => <option key={item.id} value={item.id}>{item.action}</option>)}</select></label>}

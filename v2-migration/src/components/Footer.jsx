...
export default function Footer() {
  ...
  return <footer className={`site-footer${hasAppShell ? " site-footer--app" : ""}`}>
    <div className="site-footer__inner">
      ...
      <nav className="site-footer__column" aria-label={copy.learn}><strong>{copy.learn}</strong><Link href={`${p}/weekly-review`}>{copy.review}</Link><Link href={`${p}/weekly-report`}>{copy.report}</Link><Link href={`${p}/templates`}>{copy.templates}</Link><Link href={`${p}/manuals`}>{copy.manuals}</Link><Link href={`${p}/blog`}>{copy.blog}</Link><Link href={`${p}/glossary`}>{copy.glossary}</Link><Link href={`${p}/guide`}>{copy.guide}</Link></nav>

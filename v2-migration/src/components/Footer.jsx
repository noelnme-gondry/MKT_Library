...
export default function Footer() {
  ...
  const p = isEn ? "/en" : "";
  const copy = isEn
    ? { note: "Free, browser-only performance marketing analysis.", tools: "Analyze", learn: "Learn", legal: "Legal", dashboard: "Operations dashboard", variance: "Performance variance", fatigue: "Creative fatigue", budget: "Budget allocation", report: "Weekly report", blog: "Blog", glossary: "Glossary", guide: "Operating guides", templates: "Templates", calculators: "No-CSV calculators", diagnose: "Diagnose a problem", privacy: "Privacy", terms: "Terms", contact: "Contact" }
    : { note: "원본 데이터를 서버에 보내지 않는 무료 퍼포먼스 마케팅 분석 도구.", tools: "분석 도구", learn: "실무 자료", legal: "정책", dashboard: "운영 대시보드", variance: "성과 변동 분석", fatigue: "소재 피로도", budget: "예산 배분", report: "주간 보고서", blog: "블로그", glossary: "용어사전", guide: "운영 가이드", templates: "템플릿·체크리스트", calculators: "무CSV 계산기", diagnose: "성과 문제 진단", privacy: "개인정보처리방침", terms: "이용약관", contact: "문의하기" };
  return <footer className={`site-footer${hasAppShell ? " site-footer--app" : ""}`}>
    <div className="site-footer__inner">
      ...
      <nav className="site-footer__column" aria-label={copy.learn}><strong>{copy.learn}</strong><Link href={`${p}/weekly-report`}>{copy.report}</Link><Link href={`${p}/calculator`}>{copy.calculators}</Link><Link href={`${p}/diagnose`}>{copy.diagnose}</Link><Link href={`${p}/templates`}>{copy.templates}</Link><Link href={`${p}/blog`}>{copy.blog}</Link><Link href={`${p}/glossary`}>{copy.glossary}</Link><Link href={`${p}/guide`}>{copy.guide}</Link></nav>

  ...
  return <>
    ...
    <div id="cmdk" className="cmdk-overlay" hidden={!isCmdkOpen} onMouseDown={(event) => { if (event.target === event.currentTarget) setCmdkOpen(false); }}>
      <div ref={panelRef} className="cmdk-panel" role="dialog" aria-modal="true" aria-label={T.placeholder} onKeyDown={onDialogKeyDown}>
        <div className="cmdk-inputwrap"><span aria-hidden>⌕</span><input ref={inputRef} id="cmdk-input" type="search" role="combobox" aria-autocomplete="list" aria-expanded="true" aria-controls="cmdk-results" aria-activedescendant={results[selected] ? `cmdk-option-${results[selected].key}` : undefined} value={query} onChange={(event) => { setQuery(event.target.value); setSelected(0); }} onKeyDown={onInputKey} placeholder={T.placeholder} autoComplete="off" spellCheck="false" /><button type="button" onClick={() => setCmdkOpen(false)} aria-label={T.close}>ESC</button></div>

...
export default function GlobalModals({ locale = "ko" }) {
  ...
  const entries = useMemo(() => {
    ...
    const workspace = [
      ...
      { key: "review", head: true, title: workspaceNavItem("review", locale).name, group: locale === "en" ? "Decision workspace" : "의사결정 워크스페이스", href: locale === "en" ? "/en/weekly-review" : "/weekly-review", kind: locale === "en" ? "Review" : "검토", hint: workspaceNavItem("review", locale).desc, searchText: locale === "en" ? "past decisions review inbox weekly outcome check what happened" : "주간 리뷰 지난 결정 결정 검토함 주간 검토 결과 확인 회고" },
      { key: "storage", title: locale === "en" ? "Stored on this device" : "이 기기에 저장된 것", group: locale === "en" ? "Decision workspace" : "의사결정 워크스페이스", href: locale === "en" ? "/en/storage" : "/storage", kind: locale === "en" ? "Privacy" : "개인정보", hint: locale === "en" ? "Review or remove files saved in this browser" : "이 브라우저에 저장된 파일 확인·삭제", searchText: locale === "en" ? "device storage saved CSV data remove privacy local browser" : "이 기기 저장 파일 CSV 데이터 삭제 사생활 브라우저" },

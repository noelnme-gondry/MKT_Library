...
it("계정·구독·프로젝트를 한 화면에서 보여준다", async () => {
  ...
  expect(screen.getByRole("link", { name: "프로젝트 열기" }).getAttribute("href")).toBe("/weekly-review");
});
...
it("직접 입력으로 저장한 매핑이 이 기기에 남는다", async () => {
  ...
  await waitFor(async () => expect(await listMappingMemory()).toHaveLength(1));
  const [record] = await listMappingMemory();
...
it("저장한 매핑을 한 줄만 지울 수 있다", async () => {
  ...
  expect((await listMappingMemory())[0].canonicalKey).toBe("media_spend");

  // 전체 삭제만 있으면 한 줄을 고치려고 나머지를 전부 버려야 한다.
  fireEvent.click(screen.getByRole("button", { name: "삭제" }));
  await waitFor(async () => expect(await listMappingMemory()).toHaveLength(0));
});
...
it("매핑 사용 토글은 규칙을 지우지 않고 적용만 멈춘다", async () => {
  ...
  window.localStorage.setItem(MAPPING_MEMORY_ENABLED_KEY, "true");
  render(<MyAccountPage locale="ko" />);
  const toggle = await screen.findByLabelText("저장한 매핑 사용");
  await waitFor(() => expect(toggle.checked).toBe(true));
  fireEvent.click(toggle);
  await waitFor(() => expect(window.localStorage.getItem(MAPPING_MEMORY_ENABLED_KEY)).toBe("false"));
});

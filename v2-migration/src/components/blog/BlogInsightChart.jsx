...
export default function BlogInsightChart({ visual, locale }) {
  ...
  return <figure><figcaption>{series.label}</figcaption><div className="chart-container"><canvas ref={ref} role="img" aria-label={series.label} /></div><section data-information-section=""><header data-information-heading="">{locale === "en" ? "Chart values" : "차트 수치"}</header><table><tbody>{series.labels.map((label, index) => <tr key={index}><th scope="row">{label}</th><td>{series.values[index] == null ? (locale === "en" ? "Not computable" : "계산 불가") : typeof series.values[index] === "object" ? `${series.values[index].x}, ${series.values[index].y}` : series.values[index]}</td></tr>)}</tbody></table></section></figure>;
}

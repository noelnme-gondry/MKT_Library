...
export default function SubscriptionTrialOffer({ locale = "ko", onNavigate }) {
  ...
  return <aside className="subscription-trial-offer">
    <div><strong>{en ? "Try Pro with your first project" : "첫 프로젝트로 Pro를 체험하세요"}</strong><p>{en ? "One 14-day trial per Google account, starting when you create your first project—not at sign-in. No automatic payment." : "Google 계정당 1회, 로그인일이 아닌 첫 프로젝트를 만든 날부터 14일입니다. 자동 결제되지 않습니다."}</p></div>
    <Link className="btn" href={en ? "/en/weekly-review#wr-next" : "/weekly-review#wr-next"} onClick={onNavigate}>{en ? "Save a decision to try Pro" : "결정 저장하고 Pro 체험하기"}</Link>

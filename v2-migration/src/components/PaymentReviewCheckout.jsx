...
export default function PaymentReviewCheckout({ clientKey, product, locale = "ko" }) {
  ...
  return <section className="checkout-review" aria-labelledby="checkout-review-title">
    ...
    <button type="button" className="btn" disabled={busy} onClick={open}>{busy ? (en ? "Opening…" : "여는 중…") : ready ? (en ? `Open KRW ${product.amount.toLocaleString("en-US")} test checkout` : `${product.amount.toLocaleString("ko-KR")}원 테스트 결제창 열기`) : (en ? "Show test payment methods" : "테스트 결제수단 확인")}</button>

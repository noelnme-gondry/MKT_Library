...
export default function AccountDecisionLibrary({ locale, signedIn, memos, loading, records, busy, message, onCopy, onExport, onCalendar, onDelete, renderSave, children, anchorId }) {
  const en = locale === "en";
    ...
    {!signedIn ? <div className="account-library__empty"><p>{en ? "Sign in to see your saved decisions." : "로그인하면 계정에 저장한 결정을 볼 수 있습니다."}</p>{accountControls}</div> : <>
      {loading ? <p role="status">{en ? "Loading saved decisions…" : "저장한 결정을 불러오는 중…"}</p> : memos.length ? <div className="account-library__records">{memos.map(memo => {
        ...
        return <article className="account-library__record" key={memo.id}>
          ...
          <section data-information-section="" className="account-library__details"><header data-information-heading="">{en ? "Decision details" : "결정 내용 보기"}</header>{memo.conclusion && <p>{memo.conclusion}</p>}{memo.learning && <p>{memo.learning}</p>}<div className="account-library__actions"><button className="btn" disabled={!memo.reviewDate} onClick={() => onCalendar(memo)}>{en ? "Add review to calendar" : "검토일 캘린더에 추가"}</button><button className="btn ghost" disabled={busy} onClick={() => onDelete(memo)}>{en ? "Delete account copy" : "계정 사본 삭제"}</button></div></section>
        </article>;
      ...
      <footer className="account-library__footer">{memos.length > 0 && <button className="btn ghost" onClick={onExport}>{en ? "Export existing memos · free" : "기존 메모 내보내기 · 무료"}</button>}<Link className="btn ghost" href={en ? "/en/account" : "/account"}>{en ? "Account settings" : "계정 설정"}</Link></footer>
    </>}

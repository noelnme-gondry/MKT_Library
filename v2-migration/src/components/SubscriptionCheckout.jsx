...
export default function SubscriptionCheckout({ locale = "ko" }) {
  ...
  return <div className="subscription-checkout">
    {hasPaidAccess(entitlement) ? <p role="status">{en ? "Your report pass is active until" : "보고서 이용권 사용 중 · 만료일"} {new Date(entitlement.expiresAt).toLocaleDateString(en ? "en-US" : "ko-KR")}</p> : config?.enabled ? <><p>{en ? "One-time payment. No automatic renewal. By purchasing, you agree to the terms and refund policy below." : "자동 갱신 없는 1회 결제입니다. 구매 시 아래 이용약관과 환불정책에 동의합니다."}</p>{config.mode === "test" && <p>{en ? "Test checkout · no actual charge" : "테스트 결제 · 실제 청구되지 않음"}</p>}</> : <p>{en ? "Contact customer service for purchase availability." : "이용권 구매 가능 여부는 고객센터로 문의해 주세요."}</p>}
    <div id="toss-payment-methods" /><div id="toss-payment-agreement" />
    {!hasPaidAccess(entitlement) && config?.enabled && <button className="btn primary" type="button" disabled={busy} onClick={ready ? pay : prepare}>{busy ? (en ? "Processing…" : "처리 중…") : ready ? (en ? "Pay KRW 5,900" : "5,900원 결제하기") : (en ? "Choose payment method" : "결제수단 선택")}</button>}

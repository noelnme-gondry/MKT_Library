...
export const TOOL_GUIDE = {
  ...
  "5-28": {
    when: "고객이 어느 갱신 구간까지 남는지와 이탈 위험이 높아지는 시점을 중도절단까지 반영해 확인합니다.",
    grain: "1행 = 한 번의 구독 시작부터 이탈 또는 관측 종료까지의 구독 에피소드 1건",
    needs: [
      { col: "tenure_periods", label: "구독 기간", why: "구독 시작부터 이탈 또는 관측 종료까지의 기간", required: true },
      { col: "event_observed (0/1)", label: "이탈 관측 여부", why: "1=이탈 관측, 0=관측 종료 시 활성", required: true },
      { col: "entry_period", label: "관측 진입 기간", why: "서로 다른 관측 진입 시점 보정(선택)", required: false },
      { col: "channel·plan·promo", label: "세그먼트", why: "획득 채널·플랜·프로모션별 관측 생존 비교", required: false },
      { col: "cac", label: "고객 획득비용", why: "관측기간 LTV와 함께 확인(선택)", required: false },
    ],
    prep: ["활성 구독도 0(중도절단)으로 포함하세요. 빼면 생존율이 과소추정됩니다.", "기간 단위(일·주·월)를 파일 전체에서 통일하고, 이탈 정의를 결과에 함께 기록하세요.", "세그먼트 차이는 관측된 연관이며 원인은 아닙니다. 개입 효과는 A/B 또는 홀드아웃으로 확인하세요."],
    example: "tenure_periods,event_observed,channel,plan,promo,cac\n3,1,Meta,monthly,1,40000\n14,0,Organic,annual,0,8000\n7,1,ASA,monthly,0,52000",
  },
  "5-25": {
...
export const TOOL_GUIDE_EN = {
  ...
  "5-28": {
    when: "Measure how long customers remain and when churn risk rises while retaining censored active subscriptions.",
    grain: "1 row = one subscription episode from start until churn or observation end",
    needs: [
      { col: "tenure_periods", label: "Subscription duration", why: "Periods from subscription start to churn or observation end", required: true },
      { col: "event_observed (0/1)", label: "Churn observed", why: "1 = observed churn; 0 = active at observation end", required: true },
      { col: "entry_period", label: "Observation entry", why: "Optional correction for different observation starts", required: false },
      { col: "channel · plan · promo", label: "Segment", why: "Observed survival comparison by channel, plan, or promotion", required: false },
      { col: "cac", label: "Customer acquisition cost", why: "Optional comparison with observed-horizon LTV", required: false },
    ],
    prep: ["Include active subscriptions as 0 (censored); excluding them biases survival downward.", "Use one time unit—day, week, or month—throughout the file and state the churn definition with the result.", "Segment differences are observed associations. Validate interventions with an A/B test or holdout."],
    example: "tenure_periods,event_observed,channel,plan,promo,cac\n3,1,Meta,monthly,1,40000\n14,0,Organic,annual,0,8000\n7,1,ASA,monthly,0,52000",
  },
  "5-25": {

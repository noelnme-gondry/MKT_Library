...
export const TOOL_GUIDE = {
  ...
  "5-18": {
    ...
    needs: [
      ...
      { col: "market_index / price_index / search_volume", label: "연속형 컨트롤", why: "시장 수요·가격 등 광고 밖의 동시 변화를 관측 통제", required: false },
    ...
    prep: [
      ...
      "연속형 컨트롤은 타깃과 같은 기간·주기로 빈칸 없이 넣습니다. 0/1 사건은 더미/이벤트, 변경 후 유지되는 상태는 구조변화 step으로 구분하세요.",
      "이 앱의 보수적 식별 기준은 최소 52주이며, 가능하면 104주 이상을 권장합니다. 52주 미만이면 예산 추천을 보류합니다.",
    ],
    example: "country,date,revenue,google_spend,meta_spend,market_index,event_dummy,regime_step\nKR,2024-01-01,8400000,4200000,3100000,98,0,0\nKR,2024-01-08,8950000,4400000,3000000,101,1,0\nKR,2024-01-15,8100000,3900000,3200000,99,0,1",
  },
  "5-20": {
  ...
  "5-27": {
    ...
    prep: [
      "App Store Connect는 '소스 유형', Google Play는 '트래픽 소스'로 내보내면 됩니다.",
  ...
  "5-28": {
    ...
    needs: [
      { col: "tenure_periods", label: "생존 관측 기간", why: "액션 시작부터 이탈·종료 또는 관측 종료까지의 기간", required: true },
      { col: "event_observed (0/1)", label: "이탈·종료 관측 여부", why: "1=이탈·종료 이벤트 관측, 0=관측 종료 시 유지", required: true },
      { col: "entry_period", label: "관측 진입 기간", why: "서로 다른 관측 진입 시점 보정(선택)", required: false },
      { col: "channel·event_type·campaign_name", label: "세그먼트", why: "채널·액션 유형·캠페인별 관측 생존 비교", required: false },
      { col: "cac", label: "획득·유지비", why: "관측기간 반복 가치와 함께 확인(선택)", required: false },
    ],
    prep: ["관측 종료까지 액션을 유지한 사례도 0(중도절단)으로 포함하세요. 빼면 생존율이 과소추정됩니다.", "기간 단위(일·주·월)를 파일 전체에서 통일하고, 이탈·종료 이벤트 정의를 결과에 함께 기록하세요.", "세그먼트 차이는 관측된 연관이며 원인은 아닙니다. 개입 효과는 A/B 또는 홀드아웃으로 확인하세요."],
    example: "tenure_periods,event_observed,channel,event_type,cac\n3,1,Meta,14일 미실행,40000\n14,0,Organic,관측 종료 시 유지,8000\n7,1,ASA,구매 중단,52000",
  },
  "5-25": {
...
export const TOOL_GUIDE_EN = {
  ...
  "5-18": {
    ...
    needs: [
      ...
      { col: "market_index / price_index / search_volume", label: "Continuous controls", why: "Adjust observed associations for concurrent non-media movement such as market demand or price", required: false },
    ...
    prep: [
      ...
      "Continuous controls must cover the same periods and cadence as the target with no gaps. Use Dummy/event for 0/1 events and Structural step for a persistent post-change state.",
  ...
  "5-27": {
    ...
    prep: [
      "Export by Source Type in App Store Connect, or Traffic Source in Google Play Console.",
  ...
  "5-28": {
    ...
    grain: "1 row = one entity action-observation episode from action start until exit or observation end",
    needs: [
      { col: "tenure_periods", label: "Survival observation duration", why: "Periods from action start to exit or observation end", required: true },
      { col: "event_observed (0/1)", label: "Drop-off or exit observed", why: "1 = observed exit; 0 = still active at observation end", required: true },
      { col: "entry_period", label: "Observation entry", why: "Optional correction for different observation starts", required: false },
      { col: "channel · event_type · campaign_name", label: "Segment", why: "Observed survival comparison by channel, action type, or campaign", required: false },
      { col: "cac", label: "Acquisition or maintenance cost", why: "Optional comparison with observed-horizon recurring value", required: false },
    ],
    prep: ["Include cases still active at observation end as 0 (censored); excluding them biases survival downward.", "Use one time unit—day, week, or month—throughout the file and state the exit definition with the result.", "Segment differences are observed associations. Validate interventions with an A/B test or holdout."],
    example: "tenure_periods,event_observed,channel,event_type,cac\n3,1,Meta,no action for 14 days,40000\n14,0,Organic,active at cutoff,8000\n7,1,ASA,purchase stopped,52000",
  },
  "5-25": {

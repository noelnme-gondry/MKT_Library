...
import { normalizeDateLabel, normalizeNumericValue } from "@/lib/data-import/normalizeValues";
import { NUMERIC_STANDARD_KEYS, STANDARD_FIELDS } from "./csvConstants";
...
// 오독된다.
function normalizeMappedValue(value, standardKey) {
  if (NUMERIC_STANDARD_KEYS.has(standardKey)) {
    const normalized = normalizeNumericValue(value);
    if (!normalized) return value;
    // PapaParse 문자열 계약은 유지하되 XLSX가 이미 숫자로 준 셀은 타입을 바꾸지 않는다.
    return typeof value === "string" ? String(normalized.value) : value;
  }
  if (STANDARD_FIELDS[standardKey]?.type === "date") {
    // 유효한 일자는 ISO로 통일한다. 월/주 라벨은 cadence 판별에 필요하므로 grain을
    // 보존하며, 형식이 잘못된 값은 import 분석행 필터가 진단할 수 있게 원본을 남긴다.
    return normalizeDateLabel(value) || value;
  }
  return value;
}
...
export function mapRowsToStandard(raw = [], mapping = {}) {
  return raw.map((row) => {
    ...
    for (const [origKey, value] of Object.entries(row || {})) {
      ...
      mapped[standardKey] = normalizeMappedValue(value, standardKey);
    }

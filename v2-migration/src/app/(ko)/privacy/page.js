      ...
      sections={[
        {
          ...
          content: <p>메일 기능이 제공되면 계정에 연결된 실결제의 주문번호·금액·이용기간 안내와 일회용 로그인 링크를 등록 이메일로 보냅니다. 검토일·만료 안내는 별도 선택 동의 후 발송하며 보관함에서 언제든 해제할 수 있습니다. 결정 메모 내용과 CSV는 메일에 포함하지 않습니다. 발송 대기 정보에는 계정·주문 또는 메모 식별자·일정·발송 상태만 보관하며 계정 유지 기간 동안 중복 방지에 사용합니다. 로그인 링크 정보는 해시로 저장하고 10분간 유효합니다. 실제 메일 제공자·처리 지역은 운영 활성화 전에 고지합니다. 결제 버튼을 누르면 현재 작업을 이 브라우저에만 임시 보관하고, 복원 후 삭제하거나 24시간 뒤 다음 정리 때 삭제합니다. 기기 저장 화면의 전체 삭제·저장 끄기도 임시본을 삭제합니다.</p>,
        },
        {
          title: "계정 결정 보관 (기능 제공 시)",
          content: <p>Google 로그인을 선택하면 Google의 고유 계정 식별자와 검증된 이메일을 계정 식별·이용권 연결·계정당 1회 14일 체험 관리에 사용합니다. 첫 메모 저장 때 체험이 시작되며 자동 결제되지 않습니다. 저장 전에 확인하고 선택한 결정 메모(행동·결론·가설·검토일·검토 결과 등)만 서버에 보관합니다. 메모에는 직접 작성한 캠페인명이나 수치가 포함될 수 있습니다. CSV 원본·매핑·분석 데이터셋·로컬 스냅샷은 전송하지 않습니다. 계정 메모는 기기 저장의 90일 만료와 별개이며 이용권 만료만으로 삭제하지 않습니다. 보관함에서 개별 삭제하거나 문의 페이지에서 계정 삭제를 요청할 수 있습니다. 계정 식별자·이메일·체험 시작일은 계정 유지 기간 동안 보관하며, 로그인 세션은 30일, 로그인 대기 정보는 10분간 유효합니다. 마케팅 수신 동의는 이메일 구독에서 별도로 받습니다. Google의 로그인 서비스와 운영 데이터베이스를 사용하며, 기능 활성화 전 처리위탁·이전 지역 등 운영 정보를 고지합니다.</p>,
        },
        {
          title: "이용권 결제와 복원",

...
export async function GET(request) {
  try { const account = await readAccount(request); return accountResponse({ enabled: accountsEnabled(), mailEnabled: mailEnabled(), account: account ? { id: account.id, email: account.email, trialStartedAt: account.trial_started_at, serviceReminders: account.service_reminders === true } : null, entitlement: accountEntitlement(account) }); }

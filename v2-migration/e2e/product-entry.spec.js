...
for (const locale of ["ko", "en"]) {
  test(`mobile sample entry and readable evidence (${locale})`, async ({ page }) => {
    ...
    await expect(mobile.locator(".mobile-quick-start__sample")).toBeVisible();
    ...
    for (const light of [true, false]) {
      ...
      await expect(mobile.locator(".mobile-quick-start__sample")).toHaveCSS("font-size", "14px");
      expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1)).toBe(true);
    }
    await mobile.locator(".mobile-quick-start__sample").click();

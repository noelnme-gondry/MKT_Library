...
faq:
  - q: "If ATT opt-in is low, did performance actually get worse?"
    a: "No. Losing measurement is not the same as losing performance. Without consent, IDFA-based cross-app tracking is restricted. Report differences alone cannot establish actual performance or the amount missing."
  - q: "How do I design the SKAN conversion value?"
    a: "Each SKAN window has limited value resolution, so the metrics tied most directly to your business KPI belong at the top of the priority order. Packing in too many events buries the signal that matters."
...
- Methods like [MMM](/tools/mmm-contribution), which estimate channel contribution from aggregate data without user identity, rose in value.

Aggregate analysis also needs a valid design and identification. An on/off before–after difference alone is not causal evidence; check controls, seasonality and concurrent changes. Detailed responses are in the [iOS privacy · ATT · SKAN guide](/guide/ios-privacy-att-skan).

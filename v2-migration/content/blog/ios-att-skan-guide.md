...
faq:
  - q: "ATT 동의율이 낮으면 성과가 실제로 나빠진 건가요?"
    a: "아닙니다. 측정이 끊긴 것과 성과가 나빠진 것은 다릅니다. 동의하지 않으면 IDFA 기반 교차 앱 추적이 제한됩니다. 보고값의 차이만으로 실제 성과나 누락 크기를 확정할 수는 없습니다."
  - q: "SKAN 컨버전 값은 어떻게 설계하나요?"
    a: "SKAN의 창별 반환 값은 해상도가 제한되므로 비즈니스 KPI에 직접 연결되는 지표를 우선순위 위쪽에 배치합니다. 욕심내서 많은 이벤트를 넣으면 정작 중요한 신호가 묻힙니다."
  - q: "SKAN 4는 이전 버전과 무엇이 다른가요?"
    a: "전환 측정 창이 하나에서 셋으로 늘어 설치 후 35일까지의 행동을 측정할 수 있고(수신에는 추가 지연이 있음), 정밀한 fine 값 외에 low·medium·high의 coarse 값이 추가됐습니다. 캠페인 식별자도 계층형으로 넓어졌지만, 얼마나 세밀하게 받을지는 개인정보 임계가 결정합니다."
  - q: "SKAN의 개인정보 임계(crowd anonymity)가 무엇인가요?"
    a: "재식별 위험을 낮추기 위해 반환 정보량을 제한하는 Apple의 체계입니다. 반환되는 값과 소스 ID의 상세도는 티어와 창에 따라 달라집니다. 캠페인 분할 뒤 정보가 줄었다면 반환 조건과 실제 분포를 확인해야 하며, 분할만으로 티어 하락을 확정할 수는 없습니다."
  - q: "SKAN 데이터로 ROAS를 계산할 수 있나요?"
    a: "제한적으로만 가능합니다. 매출을 컨버전 값 구간으로 인코딩해 근사할 수는 있지만, 값의 해상도가 낮고 측정 창을 넘어선 매출은 잡히지 않습니다. SKAN ROAS는 제한된 추정치입니다. 캠페인별 누락과 성숙도가 다르면 순위도 왜곡될 수 있습니다."
...
- [MMM](/tools/mmm-contribution)처럼 유저 식별 없이 집계 데이터로 채널 기여를 보는 방법의 가치가 커졌어요.

집계 분석도 설계와 식별 조건이 필요합니다. 단순 켜기·끄기 전후 차이를 인과 효과로 확정하지 말고, 비교군·계절성·동시 변경을 점검하세요. 세부 대응은 [iOS 프라이버시·ATT·SKAN 가이드](/guide/ios-privacy-att-skan)에 있어요.
...
원본 보고서의 기간·창·매핑 버전을 메모한 뒤 [SKAN과 MMP 비교](/blog/skan-vs-mmp-attribution) 순서로 차이를 좁히세요. Apple의 [복수 전환 창](https://developer.apple.com/documentation/storekit/receiving-postbacks-in-multiple-conversion-windows)과 [AdAttributionKit 상호 운용 문서](https://developer.apple.com/documentation/adattributionkit/adattributionkit-skadnetwork-interoperability)는 반환 조건을 확인하는 기준입니다. 이 절차만으로 관측되지 않은 매출이나 인과 효과가 복원되지는 않습니다.

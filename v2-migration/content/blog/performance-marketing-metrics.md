...
faq:
  ...
  - q: "ROAS와 LTV:CAC는 뭐가 다른가요?"
    a: "ROAS는 정한 창의 매출을 광고비로 나눕니다. LTV:CAC는 생애가치를 획득비용으로 나누며, 마진·비용 범위·고객군 정의도 다를 수 있어 단순히 기간만 긴 ROAS는 아닙니다."
  - q: "CPA는 낮은데 ROAS가 안 나오면 뭐가 문제인가요?"
    a: "객단가와 구매 전환을 살펴보되, 전환 정의·매출 창·지연·유입 구성·추적 누락도 확인하세요. 이 패턴만으로 광고 앞단의 문제를 배제할 수 없습니다."
  - q: "CPA·ROAS는 좋은데 LTV:CAC가 나쁘면요?"
    a: "재구매·리텐션과 함께 고객군·마진·획득비용 범위·생애가치 추정 가정을 확인하세요. 리텐션 문제라는 확정 판정은 아닙니다."
  - q: "지표들을 어떤 순서로 봐야 하나요?"
    a: "광고비 → CPI(설치) → CPA(행동) → ROAS(단기 매출) → LTV:CAC(장기 가치) 순으로 사슬을 세워두고, 어느 칸이 '빨간불'인지부터 찾으세요. 그 칸에서 점검할 가설을 좁히세요. 분해만으로 원인을 확정할 수는 없습니다."
...
이게 중요한 이유는, **CPM·CTR·CVR은 CPA의 산술적 구성 요소**이기 때문이에요. CPA가 높을 때 CPM이 높으면 매체 경쟁·타겟 단가 문제, CTR이 낮으면 소재·타겟 문제, 전환율이 낮으면 랜딩·[퍼널](/blog/funnel-dropoff-analysis) 문제예요. 결과만 보고는 소재를 갈지 타겟을 바꿀지 못 정해요.
...
무료 [운영 대시보드](/dashboard)에 CSV를 올려 매핑을 확인하고 분석을 실행해 보세요. 입력에 있는 비용·전환·매출 지표를 함께 비교할 수 있습니다. 공개 Google Sheets는 기능이 활성화된 환경에서 불러올 수 있으며, 최신 데이터는 버튼으로 다시 가져옵니다. 비공개 시트 연결이나 자동 동기화가 아닙니다. 업로드한 CSV는 브라우저에서 처리되고 원본 행은 서버로 전송·저장하지 않습니다. 기기 내 저장 설정도 확인하세요.

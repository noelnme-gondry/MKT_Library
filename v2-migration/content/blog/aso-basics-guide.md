...
faq:
  ...
  - q: "스토어 전환율은 어느 단계를 말하나요?"
    a: "두 개로 나눠야 진단이 됩니다. 목록 전환율은 페이지 조회 ÷ 노출로 아이콘·앱 이름이 좌우하고, 페이지 전환율은 설치 ÷ 페이지 조회로 스크린샷·평점이 좌우합니다. 이들은 운영용 비율이며 Apple 공식 전환율과 다릅니다. 노출 100,000·페이지 조회 12,000·설치 3,600이면 각각 12%와 30%입니다. 합쳐서 3.6% 하나로만 보면 아이콘을 고쳐야 하는지 스크린샷을 고쳐야 하는지 알 수 없습니다."
...
이 글의 두 비율은 운영용 파생 지표입니다. Apple의 공식 전환율은 고유 기기 노출 대비 다운로드·사전 주문이며, Impressions에는 제품 페이지 조회도 포함됩니다. 조회와 다운로드는 동일 사용자의 순차 이동으로 연결된 값이 아니므로, 설치 ÷ 페이지 조회를 페이지 방문자의 설치 확률로 읽으면 안 됩니다. 고유 기기와 횟수, 재다운로드 포함 여부를 맞춰 비교하세요. [Apple 지표 정의](https://developer.apple.com/help/app-store-connect-analytics/reference/metrics-definitions).
...
1. **효율 하락** — 소스별 전환율이 실제로 내려갔다. 페이지·유입 의도·경쟁·측정 변화 등이 후보입니다.
...
1. 스토어 페이지 전환(아이콘·스크린샷·평점) — 변경안 실험
2. 키워드(이름·부제) — 노출 확대
3. 유료와 연계한 총 설치·증분 검토

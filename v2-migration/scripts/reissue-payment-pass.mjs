...
import { readNicepayPayment, nicepayConfigured } from "../src/lib/subscription/nicepayServer.js";
...
const mode = process.env.PAYMENTS_PROVIDER === "nicepay" && nicepayConfigured() ? process.env.NICEPAY_MODE : secret.startsWith("live_gsk_") ? "live" : secret.startsWith("test_gsk_") ? "test" : null;
...
try {
  const result = await reissuePass({ db, orderId: value("--order"), verifiedCase: value("--verified-case"), mode,
    fetchPayment: async (key, order) => {
      if (order.provider === "nicepay") return readNicepayPayment(order);
      if (!secret || (order.provider && order.provider !== "toss")) throw new Error("PROVIDER_UNAVAILABLE");

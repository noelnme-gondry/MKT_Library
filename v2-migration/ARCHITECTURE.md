...
| (데이터 임포트) | `lib/data-import/*` + `csvConstants.js` | V1 프로파일·정규화·**도구 스코프 매핑 후보/충돌**·xlsx·wide→long·헤더행 탐지. V2 기준선=`data-import/schema/*`(전역 canonical registry·legacy migration·파생 도구 인벤토리) + `semantic-mapper/mapDataset.js`(아직 업로드 경로 미연결) |

...
| SegmentCompositionChange.jsx (5-29) | `segmentCompositionMath.js` ← `lib/segment-composition/segmentPanel.js`(SegmentPanelV1 정규화)·`profileSegmentCandidates.js`·`mappingSignature.js` + `data-import/SegmentRoleMapper.jsx` | 세그먼트 축을 표준키로 고정하지 않고 **사용자가 원본 컬럼에 역할을 선언**한다. long/wide/rate 세 shape을 한 계약으로 정규화한 뒤 분포 TVD·멤버 %p·Mix/Rate/Interaction 무잔차 분해·순증 구간을 계산. 관측 분해이며 인과가 아니고, 표본 미달·비배타 축은 숫자 대신 사유 코드를 돌려준다 |
| └ 자동 선언 (5-29) | `lib/segment-composition/autoDeclare.js` | 업로드 즉시 기간·인원수·비용·단위·범위·축을 잡고 **남은 구분 컬럼 전부**를 축으로 돌린다. 애매한 컬럼(0/1·연속형·다중값)은 확인 목록으로 남기고, 사용자가 매퍼를 건드리면 멈춘다 |
| └ 운영 지문·인과 확인 (5-29) | `segmentOpsMath.js` · `segmentCausalMath.js` ← `regMath.js`(REG_STATS.ols) | 비용 이동·CPA×볼륨 2×2·기간 스캔·반복성은 **가설을 좁히는 관측 신호**. 인과는 대조 범위가 선언됐을 때만 열리고(단위·기간 FE + 단위 군집 견고 SE), 사전 추세 위반·위약 실패면 결과를 닫는다. 매개 경로는 식별 불가로 제공하지 않음 |
| MarketingResponse.jsx + marketingResponseModel.jsx | **엔진이 두 벌이다** — 기본·예측 경로는 `mmmMath.js`(기여분해+`mmmForecast`), `mmmMode`가 `classic`·`prism`이면 `mmmMathPr416.js`(PR #416 시점 모델의 동결 스냅샷, `MMM_PRISM_MODEL_CONFIG` 보유). 두 파일은 module-level 심볼 708개를 **같은 이름·같은 본문**으로 공유하므로 한쪽만 고치면 다른 모드에 반영되지 않는다 — 통합하지 말 것(핵심 알고리즘 17개는 의도적으로 갈려 있고 classic 재현성이 그 이유다). 대신 `mmmEngineDivergence.test.js`가 공유 심볼의 분기를 감지한다. 그 외 `regMath.js`·`regForecastMath.js`·`responseCannibRank.js`·`mmmPriorMath.js`·`mmmBusinessSeasonality.js` + `lib/analysis/webr/mmmElasticNet.js` | UI/상태와 모델·차트·export 분리. WebR glmnet은 동일 시간창의 **예측 챌린저**일 뿐 기여·인과 모델을 자동 대체하지 않음 |
...
| (결정 검토) | `lib/decisionReview.js`·`decisionComparableActual.js`·`decisionComparisonScope.js`·`forecastReview.js` | 결정 기록 스키마(v11: 관측 이력 `episodes` — 한 결정은 여러 번 관측되고 `actual`·`learning`은 최신 관측의 미러다, §12.24b / v10: 가드레일 목록 `guardrails`)·기준일+N일 비교 후보·데이터 범위 스코프 |
...
- **유입 경로 서베이**: `assistant/SourceSurveyPopup`을 `GlobalModals`가 렌더하므로 어느 페이지로 처음 들어와도(홈·블로그·용어·가이드·도구) 같은 질문을 한 번 받는다. 주관식 한 칸이고 화면 가운데 뜨되 뒤를 덮는 백드롭이 없다(바깥은 계속 조작 가능). 판정·정규화 SSOT `lib/survey/sourceSurvey.js`(영구 차단 localStorage / 세션 반복 차단 sessionStorage), 다른 안내와의 겹침 방지 `lib/assistant/overlayPresence.js`. 저장은 `POST /api/survey/source` → `lib/survey/sourceSurveyServer.js` → `gop_source_survey`(답변·언어·분류된 진입면만, IP·계정·URL 없음). 스키마 `v2-migration/scripts/source-survey-schema.sql`은 앱 시작 마이그레이션에서 멱등 적용하며, 조회는 `v2-migration/scripts/source-survey-report.mjs`로 한다. 운영 DB 적용 여부는 배포 후 별도로 확인한다.
...
- 최종 결과는 `ds/ResultActionCard`(결론·근거·다음 행동) 공용 계약. 기간 표시는 `ds/ComparisonPeriods`로 날짜를 나누지 않고 두 행에 둔다. 핵심 그림은 `coreFigure` 슬롯으로 행동·저장보다 먼저 렌더한다. 예시 데이터면 그 뒤에 `ds/DecisionReviewPreview`(저장 안 함·조건만). 분석 후 매핑 수정은 `ds/MappingEditorDialog`(+`AnalyzedDataLine` 한 줄) — `CsvUploader`(`collapseWhenAnalyzed`)·5-20·5-24·5-29가 공유. `store.lastToolRouteId`(휘발)는 빈 프로젝트 화면의 '방금 본 분석 다시 열기'. 확인할 점 표시는 `ds/IssueMark`(`AnalysisBasisBar` tooltip·`AnalysisDetails`·`AnalysisScopeEvidence`·`EvidenceStatusBadge`가 모두 이것으로 그린다 — 문제 없으면 null). 세그먼트 컨트롤은 `ds/PillGroup`(radiogroup+Arrow/Home/End) — `.ab-pillgroup` 생마크업 신규 추가 금지. 접근성: 실제 `h1/h2`·`tablist/tab/tabpanel`·Cmd-K combobox·CSV live semantics·`:focus-visible`. 라우트별 error boundary + `global-error.js`.

## 5.1 콘텐츠 SEO·전환 경로
- **사이트 전체 여정**: `ds/JourneyProgress`(준비→근거→결정→리뷰, 가는 막대 네 칸 · 완료는 호출부가 아는 단계만 `completed`)를 홈·StartGate·도치 결과·ToolPageShell·주간 리뷰에서 공유. 홈은 `LandingPage` children 슬롯으로 실제 DochiAssistant를 도구 목록 앞에 렌더한다. `lib/siteJourney`는 범주형 최초 유입면을 탭 세션에 보관해 `analytics` 이벤트에 연결한다. 도치의 실제 실행·결과 노출은 `AssistantWorkspace`(도치 결과 화면은 `summaryHead`/`summaryFoot` 슬롯으로 결과 요약 면 하나를 만든다) — 요약→성과 변동 상세의 실제 기간·지표는 adapter manifest의 `comparison` → `handoffCsvToRoute` → 휘발성 `analysisHandoff`로 전달하며 새 CSV에서는 폐기한다. 분석별 핵심 그림은 `assistant/ResultCharts.jsx`(어댑터 `options.variant`로 선택, 사양은 `lib/assistant/coreFigures.js` 한 곳 — 도구 화면 5-21·5-3·5-25·9-6도 `ToolCoreFigure`로 같은 그림, 모든 차트의 PNG 받기는 `ds/FigurePngButton`(캔버스·HTML 공용, HTML은 `utils/figureImage`)), 5-3 모드 B는 `currentSpends`를 받으면 그리디와 `improveAllocationFromCurrent`(지금 배분 출발 이동 탐색) 중 예상 성과가 많은 쪽, 수동 결정은 공용 `DecisionReview`(자동 대조 범위 없음)에서 처리한다.
...
- **주간 리뷰 엔진 → `src/lib/weekly-review/`** (순수 함수, 골든): `period.js` 기간 판정(요일 맞춤) ·
  ...
  `snapshotStore.historySeriesFor`는 기간 중복을 제외한 이력을 만들고 같은 결과를 판정 배열과 `WeeklyHistoryEvidence`의 실제 기간·값 표에 연결한다. 예측 입력은 `tools/ForecastBudgetControls`에서 원본 CSV 통화로 받아 고정 환율이 제약에 들어가지 않게 한다. `scripts/check-public-readiness.mjs`는 공개 계정·결제 설정만 GET으로 재확인하며 실거래·메일 성공을 뜻하지 않는다.
  ...
  홈은 `#dochi-upload`와 `/weekly-review`를 주요 진입점으로 둔다. `WeeklyReviewScreen`의 `ReviewLoop`가 비교·결정·다음 검토를 연결하며, 도구의 결정 기록 링크는 `#wr-history`를 열어 저장한 결정으로 바로 이동한다. 리뷰 업로더는 `showToolGuide={false}`로 대시보드 전용 안내를 숨긴다.

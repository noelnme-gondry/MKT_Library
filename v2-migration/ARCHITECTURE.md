...
- **유입 경로 서베이**: `assistant/SourceSurveyPopup`을 `GlobalModals`가 렌더하므로 어느 페이지로 처음 들어와도(홈·블로그·용어·가이드·도구) 같은 질문을 한 번 받는다. 주관식 한 칸이고 전면 모달이 아니라 구석 카드다(≤720px 하단 시트). 판정·정규화 SSOT `lib/survey/sourceSurvey.js`(영구 차단 localStorage / 세션 반복 차단 sessionStorage / 체류 예산 25초), 도치 인사와의 겹침 방지 `lib/assistant/overlayPresence.js`. 저장은 `POST /api/survey/source` → `lib/survey/sourceSurveyServer.js` → `gop_source_survey`(답변·언어·분류된 진입면만, IP·계정·URL 없음). 스키마 `scripts/source-survey-schema.sql`(손으로 적용), 조회 `v2-migration/scripts/source-survey-report.mjs`.

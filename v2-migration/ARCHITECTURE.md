...
| (커스텀 지표) | `metrics/customMetric.js` (CUSTOM_OPS·customMetricCompute·customMetricToDescriptor·isValidCustomMetricDef) | 유저가 실제 컬럼 2개+연산으로 조립(eval 없음, 순수·결정론). UI=`ds/CustomMetricBuilder.jsx`(드롭다운 피커+라이브 미리보기+삭제, body portal). 소비: `dashboard/VizTab.jsx` KPI 카드 |
...
- **persist(Phase B/C)**: `persist` 미들웨어로 **`viewConfig`+`customMetrics`만** localStorage 저장(`partialize=persistPartialize`, name `mkt_view_config`). 원본 CSV·필터 Set은 **절대 저장 X**(§2.2). 서버/테스트엔 `noopStorage` 폴백. `customMetrics`={scope→조립정의[]}, `addCustomMetric/removeCustomMetric`.

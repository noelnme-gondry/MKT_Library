...
## 5.1 콘텐츠 SEO·전환 경로
- **사이트 전체 여정**: `ds/JourneyProgress`(준비→근거→결정→리뷰, 가는 막대 네 칸 · 완료는 호출부가 아는 단계만 `completed`)를 홈·StartGate·도치 결과·ToolPageShell·주간 리뷰에서 공유. 홈은 `LandingPage` children 슬롯으로 실제 DochiAssistant를 도구 목록 앞에 렌더한다. `lib/siteJourney`는 범주형 최초 유입면을 탭 세션에 보관해 `analytics` 이벤트에 연결한다. 도치의 실제 실행·결과 노출은 `AssistantWorkspace`(도치 결과 화면은 `summaryHead`/`summaryFoot` 슬롯으로 결과 요약 면 하나를 만든다) — 분석별 핵심 그림은 `assistant/ResultCharts.jsx`(어댑터 `options.variant`로 선택), 5-3 결과는 `budgetAllocTool.improveAllocationFromCurrent`(지금 배분 출발 이동 탐색)와 모드 B 그리디 중 예상 성과가 많은 쪽, 수동 결정은 공용 `DecisionReview`(자동 대조 범위 없음)에서 처리한다.

...
### 구현 완료 (2026-06, 전 Phase)
- Phase 1~7 전부 구현·검증 완료. `runRegForecastTests` 8종 추가(21/21). 주입 harness로
  render-throw 0·브리지 번역·계절성·OS분리·degenerate 검증. 핵심 함수: `REG_FORECAST`
  (`run`/`buildTimeFeatures`/`_transformCombined`/`_invDep`), `renderRegForecast`,
  `regForecastChart`, `regLabFromMmm`(MMM→회귀 브리지). `REG_STATS.ols`에 `XtXi` additive 노출.

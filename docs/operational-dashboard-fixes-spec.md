...
- **design-only 항목(#3·#4·#8)은 사용자 결정 전 구현 착수 금지** — 옵션·트레이드오프만 제시.
...
| 항목 | 사유 |
|---|---|
| #3·#4·#8 코드 작성 | design-only — 사용자가 옵션 A/B/C 중 택1 + 가드 임계값 결정해야 진행 |
...
| 3 | 스코어카드 탭형 드릴다운(D30 제거+일별 차트) | 설계만 | 중 (60~90줄) | 차트형태·cost클릭·부족가드 결정 |
| 4 | 페이싱 요일 계절성 예측 + 액션 정의 토글 | 설계만 | 중~대 | 추정구간·산식·액션분리 결정 |
| 8 | 퍼널 CVR 요일 계절성 보정 | 설계만 | 소~중 (10~60줄) | A/B/C·가드임계값 결정 |
...
**상태**: 설계만 (차트형태·cost클릭·데이터부족 가드 3건 결정 선행)

> **⛔ 사용자 결정 전 코드 작성 금지 — 아래 옵션·골격은 결정 후 참고용 설계일 뿐. 사용자가 '생각만' 요청한 항목.**
...
**목표**
(A) 요일 계절성 보정 예측 (B) 메트릭별(설치/액션/매출) 적용 — 이미 pill로 부분 구현, 예측 로직만 메트릭 무관 동작하면 자동 충족 (C) 액션=가입/구매 선택.
...
**수용 기준**
- [ ] (결정 후) 추정구간·산식·액션범위 확정
- [ ] 요일보정 예측 토글, 데이터부족 시 잠금+선형 폴백
- [ ] pacingCacheKey에 MON_FILTER_KEY 포함(기존 버그 수정)
- [ ] metric별(installs/actions/revenue) 동작
...
**구현 골격 (옵션 B/C 채택 시)**
1. `17699` — FUNNEL_STATE에 `weekdayAdj:false`.
2. `17704` — funnelCacheKey return에 `|wadj:${FUNNEL_STATE.weekdayAdj}`. (MON_FILTER_KEY는 이미 포함.)
3. `17776-17787` 직후 — daily 각 x에 `x.w=new Date(x.date).getDay()`. 7-bucket `wdMean[0..6]`(유효 cvr만, weekdayDetrend 산출 패턴)+요일별 표본수 `wdN[0..6]`. 가드 `weekdayAdjOk = wdN.filter(n=>n>0).length>=2 && 활성요일 최소 wdN>=3`. OK일 때만 `x.cvrAdj/x.devPctAdj/x.lowAdj`(`x.cvr - wdMean[x.w] + dailyMean` 기준 −1σ). FUNNEL_CACHE에 `weekdayProfile`·`weekdayAdjOk` 저장.
4. `17851-17861`·`17920-17935` — `FUNNEL_STATE.weekdayAdj && c.weekdayAdjOk`면 보정값 사용+차트 요일보정 라인/색.
5. `17845-17849` — 요일 효과 제거 ON/OFF pill(data-funnel-wadj), `!c.weekdayAdjOk`면 disabled+'요일 표본 부족 🔒'(DETREND_STATE.weekdayOn UI 8205-8207 패턴).
6. `17936-17941` — `[data-funnel-wadj]` 핸들러: `FUNNEL_STATE.weekdayAdj=...; FUNNEL_CACHE.key=null; navigate('5-11');`.
7. `17942` — T4(합성 평일 낮음/주말 높음 → 보정 후 평일 거짓 −1σ 사라짐).
...
**검증**
- Node inject(`/tmp/validate_funnel2.js` 패턴 — **존재 여부 재확인 후, 없으면 신규 작성**): `_demoEfficiencyData()`로 CSV_STATE 세팅 → buildFunnelCache 후 (B면) `c.weekdayProfile` 길이 7·`c.weekdayAdjOk` boolean. 토글 ON/OFF로 `x.low` 수 변화(ON에서 평일 거짓 급락 감소). 데모 90일+이라 요일당 ≥12 → weekdayAdjOk=true.
...
**수용 기준**
- [ ] (결정 후) A/B/C·보정단위·가드임계값 확정
- [ ] (B/C 시) 요일보정 토글, 데이터부족 시 잠금+⊘
- [ ] funnelCacheKey에 토글 sig 포함
...
| 6 | **#3 스코어카드 드릴다운** | design-only — 차트형태·cost클릭·부족가드 3건 결정 후. #1의 sticky 필터·차트 패턴 의존 |
| 7 | **#8 퍼널 요일 보정** | design-only — A/B/C·가드임계값 결정 후. weekdayDetrend 재사용, #4와 요일 프로파일 로직 공유 가능 |
| 8 | **#4 페이싱 요일 예측** | design-only — 추정구간·산식·액션범위 결정 후. 가장 분량 큼(옵션 Y면 광범위). #8의 요일 프로파일 패턴 재사용 |

**병렬화 가능**: #6·#7은 다른 항목과 독립이라 언제든. #4·#8은 요일 프로파일(`CANNIBAL_STATS.weekdayDetrend` 재사용) 로직을 공유하므로 함께 결정·구현하면 효율적.

**결정 게이트** (착수 전 사용자 확인 필수): #3(3건)·#4(3건+추가)·#8(5건)·#5 작업A(4건). 이 결정들이 묶이면 PR 1건씩 진행 가능. 나머지(#1·#2·#5 B·C·#6·#7)는 결정 없이 즉시 착수 가능.

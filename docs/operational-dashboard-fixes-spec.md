...
**상태**: 구현 (✅ 결정 완료 — 작업 A = 비율법 우선 + 곡선 폴백, COHORT_MATURATION 재사용 / 작업 B·C 즉시 가능)
...
**작업 A — 예측 모델 (✅ 결정 완료 2026-06-22)**

> **✅ 결정 확정: 비율법 우선 + 곡선 폴백 (= 옵션 C 기반, COHORT_MATURATION 재사용)**
> - `MATURATION_MATH.empiricalRatios`(성숙 과거 코호트의 `Dn/Dk 누적 ARPU 비율` 평균)를 **재사용**해 미마감 구간을 `arpu[base] × ratio[d]`로 예측 → 같은 페이지의 ROAS 성숙도와 **방법론 통일**(친숙·검증됨).
> - **비율 산출 불가 시(성숙 코호트 부족) power-law 곡선 적합(`fitCumArpu`)으로 폴백** — 옵션 B의 곡선을 2차 경로로. 어느 경로든 ★SSOT 단일 함수 `ltvPredict(unit, day)`로 진입(표=차트 byte 일치).
> - `buildLtvCacCache`에서 rev0/7/14만 집계하던 부분을 `ALL_DNS=[0,7,14,30,60,90,180,360]` 전체로 확장(17410-17416 패턴 복제, hasN 플래그) + 단위별 관측 Dn 집합·마감 여부 저장. horizon pills(17469)에 **360 추가**.
> - 골든: 비율법 경로가 기본이라 **T5(predict)·T2(payback) expected 재계산** + 비율법 전제 골든(T6 관측마감 판정·T7 표=차트 SSOT) 추가. 폴백(곡선) 경로도 별도 케이스로.
...
**결정 게이트**: ~~#3·#4·#8 설계 결정 + #5 작업A~~ → **2026-06-22 전부 완료**. **8개 항목 모두 구현 착수 가능** — 미해결 설계 결정 없음. #5 작업A = 비율법 우선 + 곡선 폴백(COHORT_MATURATION 재사용)으로 확정.

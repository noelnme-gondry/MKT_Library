...
| D6 | **중첩 분해**: 단계별 독립 분해 폐기 → **최소 grain 전역 분해 후 롤업** | 사용자 요구 "모든 값 더하면 절대 변화량과 일치 + 드릴다운 정합" 만족 (§4 상세) |
...
## 6. 유의성 임계값 (내가 제공 — config 상수, §8 결정론)

```js
const PVM_SIG_RULES = {
  overallFlatPct: 0.02,     // |Δ지표_total| < 2%·CPA1 → 전체 "큰 변화 없음"
  entityAbsFloorPct: 0.03,  // |기여| ≥ 3%·CPA2  (절대 중요도 바닥)
  entityShareOfTotal: 0.15, // |기여| ≥ 15%·|Δ지표_total| (변동 설명력)
};
// 엔티티 "유의 변동" 판정:
//   |contribution| ≥ max(entityAbsFloorPct*CPA2, entityShareOfTotal*|deltaCpa|)
//   (max → Δ가 작을 때 floor가 지배 → 거의 안 잡힘 → "큰 변화 없음"이 자연 발생)
// 노이즈 가드(작은 엔티티)는 표시 단계 result 합<PVM_NOISE_THRESHOLD를 "기타"로 묶어 별도 처리(§4.3).
// 방향(지표별):
//   CPI/CPA: contribution>0 = 악화(빨강), <0 = 개선(초록).
//   (ROAS 후속: 부호 반전 — display-invert 패턴 PR#37/#43 참고.)
// 헤드라인 "기여%": |contribution| / |deltaCpa| (전체 변동에서 차지하는 몫). |deltaCpa|≈0이면 % 생략하고 절대액만.
```
임계값은 전부 `PVM_SIG_RULES`로 분리(LLM은 서술만, 판정은 하드 규칙) → 결정론·튜닝 가능.

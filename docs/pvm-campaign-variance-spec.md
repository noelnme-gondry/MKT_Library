...
| D8 | **기간 선택 2종 버튼** ✓**사용자 승인(2026-06-26)**: ① **비교 lookback**(직전주/2주전/3주전 — 현재=최신주 고정, **기준만** N주 뒤로) ② **주 정의**(마감주 월~일[기본] / 최근 7일 롤링) | 2%씩 연속 상승=누적 트렌드 탐지. 수학(§4)·골든 불변, P1/P2 행만 다름 (§3 상세) |
...
| `buildPvmCache` (L17758) | 마지막 **14일**을 7일씩 둘로 split | **파라메트릭**: weekBasis(마감주/최근7일) × lookback(1/2/3주전), 현재 고정·기준만 뒤로 (§3·D8) |
...
## 3. 기간 정의 — 파라메트릭 (주 정의 × lookback, `buildPvmCache` 재작성) (D8)

**현재(P2)는 항상 최신 기간으로 고정. lookback 버튼은 기준(P1)만 N주 뒤로 민다.**
state: `PVM_STATE.weekBasis ∈ {"calendar"(기본),"rolling7"}`, `PVM_STATE.lookback ∈ {1(기본),2,3}`.

```
공통: rows = getMappedRows(); dates = 고유 날짜 오름차순; maxDate, minDate.
getMonday(d): JS getUTCDay() 기준 offset=(day===0?6:day-1), monday=d−offset*86400000 (UTC 고정).

[A. weekBasis="calendar" — 마감 월~일 주]
  thisWeekMon = getMonday(maxDate). 단 일요일(thisWeekMon+6) > maxDate (미마감) → thisWeekMon −= 7일.
  P2(현재) = [thisWeekMon, thisWeekMon+6]                    ← lookback 무관, 항상 최신 마감주(W0).
  P1(기준) = [thisWeekMon − 7*lookback일, +6일]              ← W-lookback (lookback=2면 W-1 건너뜀).
  rowsP2/rowsP1 = 각 행 date의 getMonday가 P2/P1 월요일과 일치.

[B. weekBasis="rolling7" — 최근 7일 롤링]
  P2(현재) = [maxDate−6, maxDate]                            ← 항상 최신 7일(달력 정렬 무시·완전성 불요).
  P1(기준) = [maxDate−6 − 7*lookback, maxDate − 7*lookback]  ← 7N일 뒤로(lookback=2면 14일 뒤로, 사이 7일 건너뜀).
  rowsP2/rowsP1 = 행 date가 각 [시작,끝] 범위 내(inclusive).

가드(공통): rowsP1 또는 rowsP2 비면 insufficientData.
  - calendar: 완전 마감 주가 (lookback+1)개 미만이면 그 lookback 버튼 잠금(🔒, 캠페인 pill 패턴).
  - rolling7: (maxDate − 7*(lookback+1) + 1) < minDate (= 데이터 span < 7*(lookback+1)일) 이면 그 버튼 잠금.
  - 활성 lookback이 잠기면 가장 큰 가능한 lookback으로 폴백(없으면 insufficientData,
    메시지: "최소 2주치 데이터 필요").
표시용: p1Range/p2Range = [시작,끝] 날짜 문자열 → 캡션에 항상 실제 범위 노출(라벨만으론 헷갈림).
캐시: buildPvmCache는 weekBasis·lookback을 입력으로 받아 재계산(토글 시 navigate 재렌더 → 재호출). 무거운 메모 없음.
```

⚠ calendar: 한 주 내 일부 요일 누락 허용(있는 행만 합산). P2가 7일 미만이면 캡션 "현재 N/7일" 경고.
⚠ rolling7: P2가 maxDate 기준 최근 7일이라 진행 중 주의 부분 요일도 포함(최신성↑·달력정렬↓ — 의도된 트레이드오프).
⚠ 14일 split(현재 코드) 완전 폐기.
⚠ lookback은 **기준만** 뒤로(현재 고정) — "2%씩 연속 상승=누적 트렌드" 탐지 목적(W0 vs W-3로 3주 누적 변화).
...
> State: `PVM_STATE = { metric:"cpa"|"cpi", currency:"krw"|"usd", weekBasis:"calendar", lookback:1, drillChannel:null, crMode:"full"|"creativeOnly", crChannel,crCampaign, showNew:true }`.
...
- **기간 선택 2종**(D8, 헤더 또는 §0 상단 — 변경 시 navigate 재렌더):
  - **주 정의** pill: `마감주(월~일)`(기본) / `최근 7일` (`data-pvm-weekbasis`, `PVM_STATE.weekBasis`).
  - **비교 기준** pill: `직전주`(기본) / `2주전` / `3주전` (`data-pvm-lookback="1|2|3"`, `PVM_STATE.lookback`). 데이터 부족 버튼은 🔒 disabled(§3 가드).
  - 두 pill 아래 캡션에 **실제 날짜 범위** 노출: `현재 {p2시작~p2끝} vs 기준 {p1시작~p1끝}` (라벨만으론 헷갈림 방지).
- **헤드라인 문구**(유의성 §6 기준):
  - 전체: `현재 {지표} {0.0}{단위} → {0.0}{단위} ({▲/▼} {Δ절대값}{통화}, {±%}, {기준라벨} 대비)` / 변동 미미하면 "큰 변화 없음". (기준라벨 = lookback에 따라 "직전주/2주전/3주전".)
...
### 5.1 §1 절대 변화량 (스코어카드)
- 2×2 stat 카드: [기준 Cost, 현재 Cost, Δ Cost] · [기준 {지표}, 현재 {지표}, Δ {지표}]. (기준=P1, 현재=P2)
- Δ는 **절대값 + 통화** (예: `Cost +1,250,000원`, `CPA +1,840원`). 부호·색(악화 빨강/개선 초록, 지표 방향 §6).
- 캡션: `기준 {p1시작}~{p1끝}({기준라벨}) vs 현재 {p2시작}~{p2끝}`. P2<7일이면 경고. weekBasis·lookback 반영.
...
- 구조(중첩·드릴다운·New·CTR 전부 시연되게):
  ...
  - **≥5주치 일별**(35일+) → 완전 마감 월~일 주 ≥4개(lookback 3·rolling7 3주전 전부 잠금해제). 마지막 날짜를 **일요일로** 맞춰 thisWeek 완전 마감.
  - **의도된 WoW 시그널**: ① 한 채널 CPA 악화(rate↑·소재 피로), ② 한 채널 볼륨 확대(mix↑), ③ 최신주에 **신규 소재 1개 진입**(New 배지 시연), ④ 한 소재 효율 개선(개선 초록), ⑤ **2%씩 여러 주 연속 상승하는 채널 1개**(lookback 2·3 버튼에서 누적 트렌드 드러나게 — D8 시연).
...
2. **기간**: `buildPvmCache` 파라메트릭(weekBasis × lookback, §3) 교체 + 버튼 잠금/폴백 + insufficientData 메시지.
3. **통화/지표 state** + `pvmFmtMoney(v,cur)` + `PVM_SIG_RULES`.
4. **렌더**: §0~§4 + 정렬(`data-sortval` 보강). 핸들러 바인딩(`data-pvm-metric/currency/weekbasis/lookback/drill-channel/cr-mode/cr-channel/cr-campaign/show-new`) — `bindCSVPageHandlers` 위임 블록에 5-21 분기 추가(navigate 재렌더).
...
- [ ] **렌더 throw 가드(headless)** — `validate_pvm.js`(주입식 `code+inject`, §12.22 패턴): 데모 패널 주입 →
      ① 게이트 화면, ② 분석 후 §0~§4 전부 render throw 없음(채널 0개·1개·다채널·creative 미매핑·campaign 미매핑 케이스), ③ 통화 ₩/$ 양쪽, ④ weekBasis calendar/rolling7 × lookback 1/2/3 전 조합.
- [ ] **기간 선택 정확성**(D8, 데모): calendar lookback=1/2/3이 P1을 W-1/W-2/W-3로, rolling7이 maxDate 기준 7N일 뒤로 잡는지 + P2는 lookback 무관 고정 확인. 데이터 부족 lookback 버튼 잠금·폴백 확인.
- [ ] **중첩 정합 수치 확인**(데모): §2 각 채널 기여 = §3 그 채널 캠페인 기여 합 = §4 그 캠페인 소재 기여 합 (1e-6). (모든 lookback에서 Σ기여=ΔCPA 유지)
...
- §12에 recipe 1줄: "12.x 5-21 PVM 중첩 분해 — 최소 grain 전역 Bennet 후 롤업으로 채널→캠페인→소재 드릴다운 정합(단계 독립 분해 폐기), 기간 선택(마감주/최근7일 × lookback 1/2/3, 현재 고정·기준만 뒤로), ₩/$ 토글, 정렬 `data-sortval` 보강."

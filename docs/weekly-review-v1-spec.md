...
> **상태**: V1 구현 완료(엔진 9모듈 + 화면 + 저장소). 남은 것은 §13.2. 이 문서 하나로 개발 범위가 떨어지는 것을 목표로 한다.
...
| 6 | Decision 스키마 | v8 → v9, 목표·가드레일 7필드 추가 | 옛 레코드는 "판정 불가" |
...
- **임계값 튜닝** — 실제 캠페인 데이터로 오탐/미탐을 재본다. 특히 `MIN_Z`는 스냅샷이 8주
  쌓이기 전에는 발동하지 않으므로 초기 사용자에게는 사실상 크기+볼륨 게이트만 작동한다.

...
> **상태**: 엔진 7모듈 + 화면 1차 구현 완료. 남은 것은 §13.2. 이 문서 하나로 개발 범위가 떨어지는 것을 목표로 한다.
...
### 13.2 아직 붙지 않은 것 (구현 현황)

| | 상태 |
|---|---|
| 순수 모듈 7개 + 골든 | ✅ |
| 화면(결론·원인·지난결정·행동·보고서) · 라우트 교체 · 도치 안내 | ✅ |
| Decision 스키마 v9 필드 | ✅ (저장 폼은 대상만 — 목표·가드레일 입력 UI는 미구현) |
| **스냅샷 영속화**(IndexedDB) | ❌ — 그래서 지금은 **2주치 업로드 경로만** 동작한다. 이력이 없어 z 판정은 크기+표본만으로 내려간다 |
| **지난 결정 자동 연결** | ❌ — `scoreDecision`은 완성됐지만 화면이 넘겨줄 저장된 결정이 아직 없다(`lastDecision: null`) |
| 사용자 날짜 필터 UI | ❌ — `resolveComparisonPeriods`는 `custom`을 받지만 화면에 컨트롤이 없다 |
| 프로젝트/KPI 설정 화면 | ❌ — 지금은 CPA·actions 고정 |

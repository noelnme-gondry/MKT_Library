...
### 결론
"탭 클릭"과 "분석 run"을 두 개의 구분되는 GA 신호로 보고 싶다는 요청은, **이미 `tool_view`(도구 진입)+`run_analysis`(분석 실행)로 사실상 충족됨** — GA4 컨벤션상 "도구별 이벤트 4개를 따로 만든다"보다 "이벤트 2종 + tool 파라미터로 구분"이 표준적이고 집계도 더 쉬움(GA 탐색 보고서에서 `tool` 파라미터로 바로 분할 가능). 5-3은 내부 탭이 없는 도구라 "탭 클릭"이라는 개념 자체가 없고, 그 경우 `tool_view`가 "그 도구를 열었다"는 의미로 자연스럽게 대응됨.

...
- 무료 분석 도구 정책은 유지한다. 수익화 준비의 범위는 **주간 반복 사용 가치 검증**이며, 가격·구매·라이선스는 아직 제공하지 않는다.
- CSV 업로드·분석 실행 → 기간과 KPI 확인 → 지난 결정 검토 → 사용자가 다음 결정 저장 → 보고서 복사/인쇄 → 다음 기간 CSV로 재검토한다. 프로젝트는 이 기기에 1개이며 서버 동기화는 없다.
...
- 공개 진단 글의 KO/EN CTA는 주간 리뷰로 연결할 수 있다. 집계 이벤트는 완료·저장·복사/인쇄·비교 데이터 출처 범주만 전달하며 파일명·캠페인명·프로젝트명·실제 지표값은 보내지 않는다.

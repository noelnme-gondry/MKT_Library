...
- 결제 화면 표시: 상품·기간·금액을 요약하고 현재 가능한 결제 행동을 먼저 둔다. 심사 경로가 열려 있어도 구매 불가 사유와 무료 샘플 대안은 결제 칼럼 맨 위에 유지한다(2026-09-14 결정). 심사 경로 때문에 그 고지를 감추면 운영 환경의 일반 방문자가 구매 경로도 사유도 없이 테스트 결제 버튼만 보게 된다. 중복으로 접는 것은 로그인 필수 안내뿐이며, 심사용 결제 버튼은 보조 버튼으로 둔다. KO: “로그인 없이 결제창을 확인할 수 있습니다. 실제 청구·결제 승인·이용권 발급은 진행되지 않습니다.” / EN: “Preview the checkout without signing in. No actual charge, payment approval or pass is issued.” 갱신·분석 임시 보관·계정 복원 설명은 “구매·이용 안내 / Purchase details”에 접어 보존한다. 자동 갱신 없음·금액·환불정책 링크와 테스트 여부는 접지 않는다.

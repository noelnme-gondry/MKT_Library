...
- 결제 화면 표시: 상품·기간·금액을 요약하고 현재 가능한 결제 행동을 먼저 둔다. 심사 경로에서는 로그인 필수 안내와 일반 구매 불가 배너를 중복 노출하지 않는다. KO: “로그인 없이 결제창을 확인할 수 있습니다. 실제 청구·결제 승인·이용권 발급은 진행되지 않습니다.” / EN: “Preview the checkout without signing in. No actual charge, payment approval or pass is issued.” 갱신·분석 임시 보관·계정 복원 설명은 “구매·이용 안내 / Purchase details”에 접어 보존한다. 자동 갱신 없음·금액·환불정책 링크와 테스트 여부는 접지 않는다.
- 심사 결제창 확인(2026-09-14): 운영 환경에 해당 상점의 테스트 클라이언트 키가 설정돼 있으면 같은 구독 페이지에서 로그인 없이 신형 결제위젯과 선택 결제수단의 테스트 창을 열 수 있다. 상품명·금액은 실제 이용권 상품과 같다. 이 경로는 실결제 주문·승인 API·Pro 발급·구매 분석 이벤트에 연결하지 않는다. 테스트 창 복귀는 승인 성공으로 표현하지 않으며 제공자 결제 식별자를 공개 페이지로 전달하지 않는다. KO: 심사용 테스트 결제창 · 실제 청구·결제 승인·Pro 권한 발급 없음. / EN: Test checkout for integration review · No actual charge, payment approval or Pro access. 일반 가입 제한과 실구매의 로그인 필수 정책은 유지한다.

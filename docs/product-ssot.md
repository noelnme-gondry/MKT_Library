...
- 계정 기능 일반 공개 후 새 구매는 로그인을 거쳐 검증된 이메일의 계정에 연결한다. 운영자 허용 목록을 설정한 검증 기간에는 기존 익명 구매를 유지하고 로그인한 운영자 구매만 계정에 연결한다. 로그인 전 분석은 무료로 유지하고, 기존 익명 구매의 코드 복원도 계속 지원한다. 새 구매 때 마케팅 동의는 요구하지 않는다.

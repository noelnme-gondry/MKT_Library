...
| 상세 | 근거 차트·표·가정·제약 | 결론을 받치는 차트·표는 하단에 둔다. **분석 범위·분모·신뢰도·방법·실행 정보는 화면에 늘어놓지 않는다**(2026-09-24 사용자 결정) — 확인할 점이 있을 때만 결론 옆 빨간 `!` 하나(`ds/IssueMark`)가 어느 기간·열·칸이 문제인지 말하고, 전체 기록은 상세 워크북(XLSX)의 `00_README`·`04_SCOPE`에 남긴다 |

- 순서: 머리(결론 + 확인할 점 `!`) → 수치 → 다음 행동 → (4층) → 보조 버튼 → 결론을 받치는 차트·표. 신뢰도 라벨(`주의해서 해석` 등)은 쓰지 않는다 — 문제가 없으면 아무 표시도 없다.
- `/start`에서 분석할 수 있는 도구는 품질 검사·승인 단계 없이 `분석 열기` 한 번으로 연다. 확인할 점은 도구 결과의 `!`가 말한다.
- 예시 데이터 결과에서는 4층 자리에 **저장 미리보기**(`ds/DecisionReviewPreview`)를 둔다: 저장하지 않는다는 사실, 저장되는 것(결론·핵심 수치·다시 볼 날짜), 조건(로그인 + Pro, 7일 체험 포함), `내 데이터로 분석하기`·`Pro 안내`. 입력 폼은 두지 않는다(가짜 결정 금지).
...
| 결과 머리·결론을 배경에 흩어 두기 | 결론보다 약한 하단 안내가 먼저 보인다 | 결과 요약은 흰 면 하나: 제목·입력 한 줄 → 결론·수치 3칸·버튼 → 공통 설정 한 줄. 다음 프로젝트 안내도 같은 흰 면·보조 버튼 |

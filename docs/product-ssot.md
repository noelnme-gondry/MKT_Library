...
- 결제 개인정보 계약: 주문번호·상품·금액·결제 상태·이용 기간·복원 코드 해시만 서버에 보관한다. 카드 입력은 토스 결제 화면에서 처리한다. CSV·분석 수치·프로젝트명·파일명은 결제 서버에 보내지 않는다. 결제 복원은 프로젝트 동기화가 아니다. 결제 기록 보관은 법정 의무에 따라 별도 운영하며 브라우저 90일 정책과 구분한다.

### 2026-09-10 감사 보완: 구매와 작업 복귀

- 결제 미구성/조회 실패 시 유료 범위를 무료로 해제하지 않는다. 구매 준비 상태와 고객센터 경로를 표시하며 분석을 계속할 수 있게 한다.
...
- 구매 진입 전 도구의 공개 라우트만 세션에 보관하여 승인 후 돌아가는 링크를 제공한다. 원본 CSV 자동 저장이나 자동 복원은 약속하지 않는다.
...
- 분석 설정 저장은 프로젝트별 최대 20개, 컬럼 매핑과 공통 필터에 한정한다. 도구별 모델 입력·분석 결과·이용권은 포함하지 않는다. 기기 저장이 켜진 프로젝트에서만 저장하며 프로젝트 백업에 포함한다.
- 복원은 헤더가 일치할 때만 적용한다. 현재 기간 유지와 저장된 기간 적용을 구분하고, 계산은 다시 실행해야 한다.
- 구독 페이지의 무료 Word·Excel 샘플은 내장 데모 데이터만 사용한다. 실제 내보내기 생성기로 만들며, 사용자 데이터나 결제 권한을 참조하지 않는다. 샘플에는 차트 이미지가 포함되지 않는다.

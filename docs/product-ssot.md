...
- 무료/체험 구분: 프로젝트 1개 생성과 기기 내 주간 리뷰·결정 기록은 체험 종료 후에도 무료이며 저장 시 로그인·기기 보관 설정이 필요하다. 추가 프로젝트 생성과 계정 메모 새 저장·수정은 유효 Pro(14일 체험 포함)가 필요하다. 계정 메모는 프로젝트 전체의 동기화가 아니다. 요금표에서 `기기 내 리뷰·결정 기록 / On-device reviews & decisions`과 `계정 결정 메모 / Account decision memos`를 분리하고, 이용 요금과 브라우저 보관 기간을 혼동시키지 않는다. 기존 프로젝트 보존·읽기·백업 계약 및 구매 전용 다운로드 조건은 유지한다. / Creating one project and saving/revisiting on-device reviews remain free after the trial; saving requires sign-in and device storage. Creating additional projects and writing account memos require active Pro, including a trial. Account memos do not sync the entire project. Distinguish free access from device retention and preserve the existing backup and purchased-download rules.
...
- 구매 진입 계약: 다운로드 게이트와 요금제에서 아직 체험을 시작하지 않은 계정에 `결정 저장하고 Pro 체험하기 / Save a decision to try Pro`를 제공한다. 비로그인 상태에서는 자격을 확정하지 않고 Google 계정당 1회·첫 계정 저장일부터 14일이라는 조건을 안내한다. 체험 중은 `Pro 체험 중 / Pro trial`과 남은 일수·종료일을 표시하고 구매한 Pro와 구분한다. 요금제 상단은 플랜·가격·체험 진입에 집중하고 계정 메모 목록을 넣지 않는다. 복원·기기 이전은 `이미 구매한 이용권 복원 / Restore an existing purchase` 아래로 접되 결제 성공 후 복귀·복원 코드 보관은 바로 제공한다. Pro 비교 항목의 `계정 결정 메모 / Account decision memos`는 무료에서 기존 메모 읽기·내보내기, Pro에서 새 메모 저장·수정이다.

...
판매자 표시 정보(2026-09-09 사용자 제공, 2026-09-14 제공 이미지의 주소 원문 반영): 상호 공드리공작소 · 대표자 신기훈 · 사업자등록번호 856-07-03210 · 주소 서울특별시 동대문구 천호대로41길 26, 402호(용두동, 천우네오젠) · 고객센터 010-8829-9034 · 이메일 gondry.montauk@gmail.com. 주소는 사용자가 제공한 이미지의 원문을 전사했다. 통신판매업 신고번호는 미제공이며 임의 생성하지 않는다. KO/EN 모두 법적 이름과 주소 원문을 보존한다. 코드 사본은 `src/lib/sellerFacts.js`가 소유한다.

**25단어 이하**

| 언어 | 승인 문구 |
|---|---|
| KO | 무료 캠페인 CSV 분석과 Pro의 결정 기록·다음 주 결과 검토를 연결하는 마케팅 워크스페이스입니다. |
| EN | A marketing workspace connecting free campaign CSV analysis with Pro decision records and next-week outcome reviews. |
...
|---|---|---|---|
| F-01 | 모든 분석 도구는 무료입니다. | 분석 실행·화면 결과 확인 무료. 프로젝트·리뷰 저장은 Pro(7일 체험 포함). 분석 결과 다운로드는 5,900원 1개월 구매 이용권 대상. 기존 기록 읽기·백업 내보내기는 만료 후에도 유지. "영구 무료"는 쓰지 않는다. | ✅ `BRAND_FACTS.price` 일치 |
| F-02 | 분석은 가입이나 로그인 없이 시작합니다. | 계정 없이 CSV 분석 가능. 리뷰·결정 기록 저장은 로그인과 유효 Pro가 필요하다. 공개 Google Sheets 접근은 **공개 URL 전제**이며 광고 계정 연동을 뜻하지 않는다. | `account` 일치 |
...
| F-04 | 원본 파일은 이 기기에서 처리합니다. | 원본 파일은 서버로 보내지 않는다. 유효 Pro와 기기 저장 설정이 있어야 새 파일을 보관한다. 기기 저장본은 마지막 사용 후 90일 만료 대상이다. §5.1.3에 따라 계정 보관을 제공할 때 사용자가 선택한 결정 메모만 별도로 서버에 저장하며 직접 삭제할 수 있다. | `persistence` 일치 |
...
- 요금제 표시 이름은 Free(무료) / Pro이며, Pro는 기존 5,900원 1개월 보고서 이용권의 표시명이다. 새로운 등급·자동 갱신 상품을 추가한 것이 아니다. 무료는 분석 실행·화면 결과 확인이며, Pro(7일 체험 포함)는 첫 프로젝트부터 생성·리뷰와 결정 기록 저장을 제공한다. 일괄 보고서 보기·브랜딩 저장도 유효 Pro 범위이며, 분석 보고서 다운로드·인쇄는 별도로 유효한 구매 이용권이 필요하다. 기존 기록의 열람·백업 내보내기는 만료 후에도 유지한다. 저장 한도는 두 플랜에 동일하게 적용한다. 화면의 파일 구성 예시는 실제 분석 결과나 고객 사용 실적으로 표시하지 않는다.
...
- 결제 화면 표시: 상품·기간·금액을 요약하고 현재 가능한 결제 행동을 먼저 둔다. 심사 경로가 열려 있어도 구매 불가 사유와 무료 샘플 대안은 결제 칼럼 맨 위에 유지한다(2026-09-14 결정). 심사 경로 때문에 그 고지를 감추면 운영 환경의 일반 방문자가 구매 경로도 사유도 없이 테스트 결제 버튼만 보게 된다. 로그인 필수 안내는 결제 행동 옆에 바로 표시하며, 심사용 결제 버튼은 보조 버튼으로 둔다. KO: “로그인 없이 결제창을 확인할 수 있습니다. 실제 청구·결제 승인·이용권 발급은 진행되지 않습니다.” / EN: “Preview the checkout without signing in. No actual charge, payment approval or pass is issued.” 갱신·분석 임시 보관·계정 복원 설명은 “구매·이용 안내 / Purchase details”에 바로 표시한다. 자동 갱신 없음·금액·환불정책 링크와 테스트 여부는 접지 않는다.
- 구매 흐름 보완(2026-09-20): Pro 구매는 보고서 다운로드 → 근거를 포함한 결정 저장 → 관측과 후속 결정이라는 사용 흐름을 함께 안내한다. 로그인 계정 이메일은 결제 내역 식별을 위해 NICEPAY에 전달하며 결제 전에 고지한다. 이름·전화번호는 수집하거나 임의 생성하지 않는다. 결제창 종료는 결제 실패로 단정하지 않고 상태 확인을 먼저 제공한다. / Pro checkout explains report delivery, evidence-backed decisions and follow-up observations. The signed-in email is sent to NICEPAY for payment identification, disclosed before checkout; names and phone numbers are not invented or collected. Closing the payment window prompts a status check rather than an assertion of failure.
- PG 전환(2026-09-20): 신규 구매 중단은 기존 이용권 복원·취소 통보 처리를 막지 않는다. 기존 주문은 발급 당시 PG로 확인한다. 화면을 연 뒤 PG/모드가 바뀌면 결제를 제출하지 않고 새로고침을 안내한다. KO: “결제 설정이 변경됐습니다. 페이지를 새로고침한 뒤 결제해 주세요. 결제는 요청하지 않았습니다.” / EN: “Checkout settings changed. Refresh this page before paying. No payment was submitted.”
- 심사 결제창 확인(2026-09-14): 운영 환경에 해당 상점의 테스트 클라이언트 키가 설정돼 있으면 같은 구독 페이지에서 로그인 없이 신형 결제위젯과 선택 결제수단의 테스트 창을 열 수 있다. 상품명·금액은 실제 이용권 상품과 같다. 이 경로는 실결제 주문·승인 API·Pro 발급·구매 분석 이벤트에 연결하지 않는다. 테스트 창 복귀는 승인 성공으로 표현하지 않으며 제공자 결제 식별자를 공개 페이지로 전달하지 않는다. 화면 정본 문구는 §2.1의 결제 화면 표시 항목 하나가 갖는다(KO “…실제 청구·결제 승인·이용권 발급은 진행되지 않습니다.” / EN “…No actual charge, payment approval or pass is issued.”) — 같은 문장을 여기에 다시 적으면 두 정의가 갈린다. 일반 가입 제한과 실구매의 로그인 필수 정책은 유지한다.
- 심사용 아이디·비밀번호 로그인(2026-09-14): 카드사 심사는 소셜 로그인 대신 일반 아이디·비밀번호를 요구한다. 아이디는 계정에 등록된 이메일이며 Google로 만든 계정은 그 Google 이메일이 곧 아이디다. 비밀번호 로그인은 `ACCOUNT_PASSWORD_LOGINS`에 나열된 계정에만 열리고, 값이 없으면 기능과 화면이 함께 닫힌다 — 공개 비밀번호 가입이 아니므로 F-02의 "분석은 가입·로그인 없이"는 그대로다. 비밀번호는 scrypt로 저장하고 저장소·로그·화면에 남기지 않으며, 계정 생성은 `scripts/provision-review-account.mjs`가 환경변수로만 받는다. 심사 계정은 체험·구매 없이 무료 상태로 두어 심사자가 무료 화면과 구매 유도를 그대로 본다. 심사가 끝나면 환경변수를 비우는 것으로 닫는다.
- CSV 업로드·분석 실행 → 기간과 KPI 확인 → 지난 결정 검토 → 사용자가 다음 결정 저장 → 보고서 복사/인쇄 → 다음 기간 CSV로 재검토한다. 유효 Pro(7일 체험 포함)로 프로젝트를 만들 수 있으며 프로젝트마다 파일·집계·결정을 분리한다. 기존 프로젝트 열기·내보내기는 권한 만료로 회수하지 않는다. CSV·프로젝트 전체의 서버 동기화는 없다. 선택한 결정 메모의 계정 보관은 §5.1.3의 별도 활성화 조건을 따른다.
...
- Pro가 유효하고 기기 저장이 켜지면 캠페인별 집계·프로젝트 설정을 기존 IndexedDB에 보관한다. 90일 만료 항목은 다음 저장소 접근 때 삭제하며, 저장소의 전체 삭제에 포함한다. 저장 실패 시 다음에는 두 기간 CSV가 필요함을 알린다. 원본 파일 보관은 기존 F-04 계약을 따른다.
...
- Sheets 도움말은 공개 탭 URL(`gid` 포함) 불러오기·수동 재조회, 비공개 탭 CSV 내려받기, 사용자 환경의 BigQuery 예약 쿼리→Connected Sheets 추출·예약 갱신을 구분한다. BigQuery/Sheets 준비 자동화와 사이트의 수동 가져오기·분석하기는 별개이며 자동 동기화·비공개 OAuth 연결을 약속하지 않는다. Google 권한·과금·추출 행수 및 최신 갱신 확인을 안내한다. / Sheets help distinguishes public-tab import and manual refresh, private-tab CSV upload, and user-managed BigQuery schedules with Connected Sheets extracts and refresh schedules. Preparing data automatically does not automate importing or analyzing it here; private OAuth access is not supported.
...
발행 글은 `blogInsightRegistry`에서 한·영의 관련 H2 섹션 또는 CSV 부적합 사유를 명시한다. CSV로 확인할 수 있는 글은 그 자리에 **예시 결과**(실제 엔진을 공개 예시 데이터에 돌린 결과 한 줄 + 막대, 캡션에 "예시 데이터" 명시)를 먼저 보이고 "내 CSV로 같은 분석 보기"를 둔다. 부적합한 글은 본문 문장만으로 만든 **두 문항 30초 점검**을, 모든 글 끝에는 **상황 확인**(세 갈래 → 한 줄 답 + 도구·글 링크)을 둔다. 예시 결과에서 도구로 넘어가면 도구 첫 줄이 출처("「글」의 예시 데이터로 연 결과")와 두 갈래(내 CSV로 바꾸기·글로 돌아가기)만 말하고 데모 안내 모달·유입 설문은 띄우지 않는다. **이 내용을 내 CSV로 확인 / Check this with your CSV**은 기존 분석 엔진 결과와 차트 하나를 보여 준다. 별도 실험 설계가 필요한 글의 **이 문단의 데이터 먼저 살펴보기 / Inspect the data behind this section**은 열을 선택한 기술적 집계(합계·합계의 비율)이며 모형 결과로 부르지 않는다. 퍼널 집계는 동일 사용자의 이탈률로 해석하지 않는다. 빈칸·비숫자를 0으로 바꾸지 않는다. 5MB·2만 행 이하 CSV를 브라우저 메모리에서 처리하고, 상세 이동 때 같은 파일·전체 도구 매핑을 전달하되 분석 게이트는 유지한다. 기존 도구 데이터가 있으면 교체를 확인받는다. 계측에는 공개 글 식별자·도구·상태만 보내며 원본·파일명·열 이름은 보내지 않는다.

### 5.1.3 계정 결정 보관과 Pro 체험 (2026-09-11 구현, 운영 활성화 전)

- 구매 전 보고서 확인: 실제 분석 결과의 결론·수치·범위·한계를 브라우저 화면에서 무료로 미리 본다. 요약 미리보기이며 Word/Excel 전체 파일이나 재계산 실행 결과로 표현하지 않는다. 원본·미리보기는 서버 또는 분석 이벤트에 보내지 않는다. 파일 다운로드의 구매 조건은 유지한다. / Before purchasing, users can preview their actual conclusion, metrics, scope and limitations in the browser for free. This is a summary preview, not the full Word/Excel file or a recalculation. Source data and preview content are never sent to the server or analytics. File downloads still require an active purchased pass.
- 검토일이 있는 메모는 저장 시 검토일·Pro 만료 안내에 한 번의 명시적 선택으로 동의할 수 있다. 기본값은 미선택이며 메모 저장과 서비스 알림 설정은 같은 트랜잭션으로 반영한다. 실패 시 둘 다 취소한다. 기존 마이페이지의 수신 해제와 Pro 이용기간 조건을 유지한다. / A dated memo can enroll review-date and Pro-expiry reminders with one explicit, unchecked-by-default choice. Memo persistence and service-reminder enrollment commit together and both roll back on failure. My account opt-out and active-Pro conditions remain unchanged.
- 유입·입력 안내: 기존 템플릿 목록에서 작업별 사례 → 입력 조건 → 분석으로 연결한다. 민감한 Google Sheets는 공개하지 않고 CSV로 내려받아 업로드한다. 실습이 있는 글의 상단 요약에는 도구 CTA를 중복 배치하지 않으며, 별도 준비 안내와 본문 실습을 연결한다. / Task-based examples connect to input requirements and analysis through the existing template collection. Confidential Sheets are downloaded as CSV without making them public. Articles with an inline exercise use a separate preparation block instead of repeating a tool CTA inside the opening summary.

- 계정 보관함은 `계정에 저장한 결정 / Saved account decisions` 목록과 `검토 이어하기 / Continue review`가 중심이다. 빈 목록은 `아직 저장한 결정이 없습니다 / No saved decisions yet`와 첫 결정 기록 진입을 제공한다. `계정에 보관하기 / Save to account`에서만 로컬 결정 선택·보낼 메모·동의를 펼친다. CSV 미동기화 고지는 유지하며 알림·로그아웃은 접힌 `계정 설정 / Account settings`에 둔다. 불러오는 중에는 빈 목록으로 단정하지 않는다. 현재 리뷰의 단계는 다른 기록 수나 방문 라우트로 완료 처리하지 않으며 진행 표시의 다음 리뷰를 별도 활성 링크처럼 강조하지 않는다.

- 헤더 요금제 링크는 `Pro 안내 / About Pro`로 표시하고 가격은 요금제·구매 화면에서 안내한다. 계정 이메일과 이용권 상태는 별도 행으로 구분한다. 알림 선택은 `이메일 알림 받기 / Email reminders` 아래 `선택한 검토일과 Pro 만료일을 알려드려요. / Get reminders for selected review dates and Pro expiry.`와 `선택 사항 · 마케팅 메일 아님 / Optional · No marketing emails`을 보조 문구로 제공한다. 체크박스와 본문을 별도 열에 두어 다음 줄도 본문 시작점에 정렬하며 이메일만 필요할 때 강제 줄바꿈한다.

- 프로젝트 리뷰 흐름(2026-09-20): 기본 진입은 모든 도구의 결정 목록이며 CSV 없이 결과 검토를 시작할 수 있다. 주간 성과 비교는 명시적으로 선택한다. 결정 행에서 관측·효과 근거·다음 행동을 같은 상세 화면에서 다룬다. 분석 결과를 기존 결정에 직접 연결하며 새 결정 수를 늘리지 않는다. 연결 시 지표·기간·대상 일치 여부를 확인하고 미확인을 일치로 표시하지 않는다. 후속 결정은 행동·목표·검토일부터 입력하고 수치 목표·실험 설계·유지 조건은 펼쳐 설정한다. 예정일 전 중단은 종료 사유로 기록하며 효과 판정과 구분한다. 보고서는 사용자가 도구·검토 기간·결정 흐름을 선택하고 이전 결정 포함 범위와 제외 수를 확인한다. / Project review opens the cross-tool decision agenda without requiring a CSV. Weekly performance comparison is an explicit task. A single decision detail contains observations, effect evidence and follow-up actions. Link analysis evidence directly without creating another decision; show metric, period and population compatibility, including unknowns. Follow-up starts with action, goal and date; numeric targets, experiment design and guardrails are expandable. Early closure records a reason, not an effect verdict. Reports allow tool, review-date and decision-thread selection with ancestor context and explicit coverage.
- 프로젝트 진입(2026-09-12): `내 프로젝트 / My projects`는 목록이며 프로젝트를 열면 그 이름을 H1으로 사용한다. 리뷰와 관리를 동급 탭으로 두지 않는다. 프로젝트가 없으면 `첫 리뷰 시작하기 / Start your first review`로 분석부터 시작하며, 첫 명시적 저장에서 프로젝트 이름을 정한다. 개별 도구의 결정도 저장 시 기존 프로젝트 또는 새 프로젝트를 선택한다. 취소·실패는 작성 중인 입력을 초기화하지 않는다. CSV를 다른 프로젝트로 자동 이동하지 않으며 저장 위치 변경을 파일 동기화로 표현하지 않는다. 진행 표시는 업로드 전에는 데이터 준비, 분석 결과에서는 근거 확인, 결정 저장 후에는 다음 검토로 실제 상태를 따른다.

- 작업 화면 계약: 프로젝트가 주간 리뷰의 소유 단위다. `/weekly-review`는 현재 프로젝트 선택·이번 주 리뷰·프로젝트 관리를 한 화면에서 제공하고, 기존 `/projects`도 같은 작업 화면의 관리 진입으로 유지한다. 프로젝트 전환은 기존 파일·설정·결정 복원 절차를 거치며 관리 화면을 열었다 닫아도 현재 리뷰 입력은 유지한다. CSV와 프로젝트 전체는 계속 로컬이다.
- 작업 화면은 보이는 H1 하나만 사용한다. 목록은 `내 프로젝트 / My projects`, 프로젝트 안에서는 프로젝트 이름, 아직 프로젝트가 없으면 `새 리뷰 / New review`다. 중복된 영문 상단 라벨·주간 리뷰 대제목을 붙이지 않는다. 마이페이지는 닫기 버튼, 명확한 로그인 버튼, 아이콘과 이름이 있는 관리 링크를 제공하며 기본 밑줄 링크 나열로 표현하지 않는다. 로그인 복구와 개인정보 링크는 보조 위계로 유지한다.
- 공통 헤더 오른쪽 `마이페이지 / My account`는 Google 로그인·이메일 복원, 계정·Pro 상태, `내 프로젝트 / My projects`, `저장·백업 설정 / Storage & backup settings`, 요금제·로그아웃을 제공한다. 계정 저장·로그아웃·알림 변경 후 같은 화면의 모든 계정 표시를 갱신한다. 계정 전체 메모와 현재 로컬 프로젝트의 기록은 동일한 저장소인 것처럼 표시하지 않는다.
- 모든 명시적 리뷰 저장은 로그인 필수다(개별 분석의 결정·주간 결정·주간 보고서·검토 결과 수정/완료·리뷰 가져오기). 분석·작성은 익명으로 가능하고, 저장 직전에 세션을 다시 확인한다. 로그인 취소·실패 시 초안을 유지하며 입력만으로 기존 기록을 변경하지 않는다. CSV·전체 보고서는 계속 기기에만 저장하고, 결정 메모의 계정 보관은 별도 선택·동의를 받는다. / Every explicit review save requires sign-in; analysis and drafting remain anonymous. Source CSVs and full reports stay on this device. Account memo storage requires separate selection and consent.

- B안: CSV 원본은 로컬 처리한다. 사용자가 미리 확인하고 선택한 결정 메모만 계정에 보관한다. 분석 입력·결과 데이터셋·파일명·매핑·스냅샷은 계정 저장 API의 허용 필드에 포함하지 않는다.
- Google 계정당 1회 7일 Pro 체험. **시작 시점과 무관하게 전 계정 동일**하다(2026-09-21 소급 적용 결정). 길이가 계정마다 갈리면 화면·만료 알림·결제 기산일이 각각 다른 날짜를 말하게 되고 그 차이를 설명할 자리가 제품에 없다. 길이는 `archiveContract.js`의 `PRO_TRIAL_DAYS`가 소유하고 SQL은 `trialUntilSql()`로만 만든다. 가입일이 아닌 첫 프로젝트 체험 시작 요청의 승인 시각을 서버에 기록하고, 동시 저장·기기 변경·재로그인으로 재시작하지 않는다. 체험은 자동 결제되지 않는다.
- 무료/체험 구분(2026-09-14): 무료는 분석 실행·화면 결과 확인이다. 첫 프로젝트를 포함한 프로젝트 생성, 기기 내 리뷰·결정 기록 저장·수정, 파일·분석 설정·기간 집계 보관 및 백업 복원은 유효 Pro(7일 체험 포함)가 필요하다. 만료 후에는 기존 기록의 열람·내보내기·백업·삭제만 유지하며 새 저장·수정은 막는다. 첫 체험은 프로젝트 생성·첫 리뷰 저장 흐름의 명시적 체험 시작 요청으로 활성화한다. 계정 메모 보관은 체험 시작 조건이 아니며 별도 선택이다. 체험 활성 확인 후 로컬 프로젝트 저장을 별도로 진행하므로 원본·전체 보고서를 계정에 보내지 않는다. 체험 시작과 기기 저장 성공을 구분하고 실패·취소 시 초안을 유지한다. / Free includes analysis and on-screen results. Creating any project, saving or updating device reviews and decisions, retaining files, analysis setups and period snapshots, and restoring backups require active Pro, including the 7-day trial. Existing records remain readable, exportable and deletable after expiry. The first trial starts through an explicit trial request in project creation or the first review save. Account memo storage is optional and is not a trial prerequisite. Account memo storage never syncs source files or full reports; confirm local saving separately and preserve drafts on failure or cancellation.
- 분석자료 다운로드는 현재 유효한 구매 이용권이 있어야 한다. 7일 체험·로그인만으로는 허용하지 않으며 구매 이력이 있어도 만료된 이용권은 거절한다. 구매 권한이 없으면 파일을 생성·전달하지 않고 구독 구매 안내를 연다. Word·Excel·분석 CSV/PNG·보고서 PDF/인쇄에 같은 조건을 적용한다. 기존 결정 기록 내보내기·프로젝트 백업·캘린더·무료 샘플은 별도 계약대로 유지한다. / Analysis downloads require an active purchased pass, not just sign-in or the 7-day trial. Expired purchases do not qualify. Existing record exports, project backups, calendars and free samples remain available under their existing rules.
- 구매 진입 계약: 다운로드 게이트와 요금제에서 아직 체험을 시작하지 않은 계정에 `프로젝트 만들고 Pro 체험하기 / Create a project to try Pro`를 제공한다. 비로그인 상태에서는 자격을 확정하지 않고 Google 계정당 1회·첫 프로젝트 체험 시작일부터 7일이라는 조건을 안내한다. 체험 중은 `Pro 체험 중 / Pro trial`과 남은 일수·종료일을 표시하고 구매한 Pro와 구분한다. 요금제 상단은 플랜·가격·체험 진입에 집중하고 계정 메모 목록을 넣지 않는다. 복원·기기 이전은 `이미 구매한 이용권 복원 / Restore an existing purchase` 아래로 접되 결제 성공 후 복귀·복원 코드 보관은 바로 제공한다. 무료 카드에는 분석과 화면 결과 확인만 나열한다. Pro 카드에는 프로젝트·리뷰와 결정 기록·계정 메모 저장을 명시하고, 만료 후 기존 기록 읽기·내보내기는 별도 안내한다.
- 만료 후 계정의 기존 메모 열람·내보내기·삭제는 무료다. 새 계정 저장과 변경은 Pro 권한을 확인한다. 계정당 메모 상한은 2,000개다. 로컬 기록은 자동 업로드하지 않고 개별 선택으로 이전한다.
- 구매 안전성(2026-09-12): 운영 빌드에서는 테스트 키로 신규 주문·승인·이용권 발급을 허용하지 않는다. 결제 준비와 구매 의도를 분리하며 `begin_checkout`은 사용자가 결제를 진행하는 동작에서만 기록한다. 로그인 실패는 KR/EN 안내와 원래 화면 복귀 경로로 처리하고, 허용목록 제한·다른 브라우저의 이메일 링크·구매 이용권 없음은 체험 만료와 구분한다. 구매 보류 응답은 모달을 강제로 닫지 않고 접수 확인을 표시한다. 계정 권한의 오프라인 검증 유예는 결제 이용권과 같은 최대 72시간이며 실제 이용기간 만료를 넘지 않는다. 이는 이용기간 연장이 아니며 계정 저장은 서버에서 매번 권한을 확인한다.
...
- 서비스 메일 위탁 제공자는 Resend(Plus Five Five, Inc.)다. 발송 요청 시 TLS HTTPS로 수신 이메일·발신 주소·제목·본문(주문번호·금액·이용기간 또는 일회용 로그인 링크)과 발송 식별자를 전달한다. CSV와 결정 메모 내용은 제외한다. 발송 경로는 일본 도쿄(ap-northeast-1), 메일 본문·배달 로그·계정 기록 저장은 미국으로 서로 구분한다. 일반 플랜의 이메일·로그는 30일, 백업은 7일 보관하며 Resend 서비스 계약 종료 후 잔여 고객 데이터는 90일 이내 삭제한다. 선택 알림은 보관함에서 해제하고, 메일 처리 중단·삭제 요청은 고객센터로 받는다. 메일 처리를 원하지 않으면 이메일 로그인·알림을 이용할 수 없으며 계정 삭제를 요청할 수 있다. 익명 CSV 분석은 계속 이용할 수 있다. 이 고지는 계정 DB의 처리 지역 확인을 대체하지 않는다. 근거: [Resend GDPR](https://resend.com/security/gdpr), [Privacy Policy](https://resend.com/legal/privacy-policy), [Sending regions](https://resend.com/docs/dashboard/domains/regions) (2026-09-11 확인).
- `ACCOUNTS_ENABLED`의 일반 공개는 OAuth 설정·DB 마이그레이션·운영 개인정보 고지 및 실계정 스모크 검증 뒤에만 한다. 검증용 선행 활성화는 서버의 `ACCOUNT_ALLOWED_EMAILS`에 운영자가 승인한 이메일만 지정한 경우에 한한다. Google 테스트 사용자 설정만으로 접근 제한을 보장하지 않는다. 허용 목록은 Google 검증 이메일·이메일 복원·기존 세션에 적용하며 빈 목록은 모두 거부한다. 목록 제거는 별도 일반 공개 승인 뒤에만 한다. 설정 미완료 상태를 로그인 가능·메일 발송 완료로 표시하지 않는다. 기존 익명 구매와 복원 코드 계약을 유지한다.
- 계정 저장 호스팅은 Railway이며 운영 PostgreSQL 리전은 네덜란드 암스테르담(EU West, `europe-west4-drams3a`)이다. 로그인·계정 저장 요청 시 계정 식별자·이메일·체험/세션 정보와 사용자가 선택한 메모를 처리한다. 보관 기간과 삭제 방법은 위 계정 계약을 따른다. Google 로그인은 Google이 처리하며 Google의 전 세계 서버 처리·보관 정책을 따른다. Google에는 CSV·결정 메모를 보내지 않으며 요청 권한은 openid·email뿐이다. 계정 처리를 원하지 않으면 로그인·계정 저장을 이용하지 않고 익명 분석은 계속 이용할 수 있다. 로컬 저장을 포함한 리뷰 저장에는 로그인이 필요하다. 근거: [Railway Regions](https://docs.railway.com/deployments/regions), [Google Privacy](https://policies.google.com/privacy) (2026-09-11 확인). 운영 콘솔의 리전 경고를 이유로 저장 국가를 임의 변경하지 않는다.
- 신규 결제는 반드시 로그인한 검증 이메일 계정에 연결한다. 계정 기능이 꺼지면 신규 결제도 차단하며 운영 변수로 익명 결제를 우회하지 않는다. 계정 허용 목록은 이 정책을 바꾸지 않는다. 로그인 전 분석은 무료이며 기존 익명 실구매의 코드 복원은 지원한다. 마케팅 동의는 요구하지 않는다.
- 재구매는 현재 이용기간(진행 중 체험 포함)의 만료일 뒤에 1개월을 더한다. 이미 만료됐다면 승인 시점부터 1개월이다. 달력 월 기준이며 다음 달에 같은 날짜가 없으면 말일로 맞춘다. 주문별 이용기간은 최초 승인 반영 때 한 번만 배정하고 재조회·재시도로 늘리지 않는다. 취소한 이전 주문의 기간을 뒤의 주문이 무료로 메우지 않는다. / Renewing adds one calendar month after your current access expires, including an active trial. After expiry, the month starts at payment approval. Periods are allocated once per order. A later purchase does not fill a cancelled earlier period for free.
- 이용기간 만료 즉시 Pro 기능을 잠그되 만료를 이유로 데이터를 삭제하지 않는다. 기존 기록 열람·내보내기·백업은 유지하며 나중에 재구매하면 다시 이어 쓴다. 7일 후 자동 삭제 정책은 채택하지 않는다. 브라우저 저장소의 별도 90일 미사용 정리·사용자 직접 삭제는 이 계약과 구분해 안내한다. / Pro features stop at expiry, but expiry does not delete your records. Existing records remain readable and exportable; renew later to resume. Separate local-browser retention and user-initiated deletion still apply.
- 지연 입금은 `입금 확인을 기다리고 있습니다 / Waiting for your deposit`로 표시하고 입금 전에는 Pro를 발급하지 않는다. 주문번호를 표시하며 `입금 상태 확인 / Check deposit status`로 재조회한다. 입금 대기 소유권 쿠키는 짧은 승인 대기 쿠키와 분리해 유지한다. 계정에 연결된 주문은 결제 쿠키 없이도 해당 계정으로 승인·복원할 수 있다. 연락처가 없는 기존 익명 주문에서 쿠키와 코드가 모두 소실된 경우 자동 복구를 보장하지 않는다. 가상계좌 실입금·메일 전달 실측 전 일반 판매를 열지 않는다.
- 결제 원천 재조회는 최근 5분 검증을 재사용한다. 공급자 장애 때만 마지막 성공 검증 시점으로부터 최대 72시간을 허용하며 새 조회로 유예를 연장하지 않는다. 실제 만료·확인된 취소는 유예 대상이 아니다.
- 운영 내부 메일 작업은 기동 후와 15분마다 영수증·옵트인 알림을 확인한다. 계정·메일 설정이 활성일 때만 실행하며 개발·빌드에서는 보내지 않는다. 앱 중단·제공자 장애로 지연될 수 있고 API 수락을 실제 배달로 표현하지 않는다. / The production server checks receipts and opted-in reminders on startup and every 15 minutes when account mail is enabled. Outages can delay delivery; provider acceptance is not proof of inbox delivery.
...
| 7 | 근거·방법론 | 얼마나 믿고 어떻게 검증할까 (본문 하단 또는 도움말 팝오버) |
...
| 상세 | 근거 차트·표·가정·제약 | 주요 그림은 결론 바로 뒤에, 추가 차트·표는 하단에 둔다. **분석 범위·분모·신뢰도·방법·실행 정보는 화면에 늘어놓지 않는다**(2026-09-24 사용자 결정) — 확인할 점이 있을 때만 결론 옆 빨간 `!` 하나(`ds/IssueMark`)가 어느 기간·열·칸이 문제인지 말하고, 전체 기록은 상세 워크북(XLSX)의 `00_README`·`04_SCOPE`에 남긴다 |

- 순서: 머리(결론 + 확인할 점 `!`) → 핵심 수치·그림 → 다음 행동 → 검토·저장 → 보조 버튼 → 추가 근거. 핵심 그림은 결론과 같은 면에 두고, 그림이 이미 보여 주는 수치는 별도 타일로 반복하지 않는다. 신뢰도 라벨(`주의해서 해석` 등)은 쓰지 않는다 — 문제가 없으면 아무 표시도 없다. / Read the conclusion, key figures/visual, next action, review/save, then secondary actions and supporting detail. Keep the core visual on the same surface and avoid repeating its numbers in tiles.
- 요약에서 상세로 이동하면 실제 계산에 쓴 기간·지표·필터를 전달한다. 다른 비교 조건으로 조용히 전환하지 않는다. 날짜는 완전한 날짜 단위로 묶고 분석/비교 기간은 별도 행으로 표시한다. 결론과 행동은 의미상 다른 문단이며 화면 폭에 맞춘 강제 줄바꿈은 넣지 않는다. / Preserve calculated periods, metric and filters on drill-down. Keep dates intact, separate current/prior period rows, and use semantic paragraphs for conclusion and action.
- 핵심 그림을 일부만 보여 주면 전체/표시 개수와 전체보기 동작을 제공한다. VIF는 심각도순이며 미계산 값을 정상으로 간주하지 않는다. PNG에도 표시 범위가 남는다. / Partial figures disclose shown/total counts and provide a full view; rank VIF by severity, keep unknowns explicit, and retain coverage in PNGs.
- `/start`에서 분석할 수 있는 도구는 품질 검사·승인 단계 없이 `분석 열기` 한 번으로 연다. 확인할 점은 도구 결과의 `!`가 말한다.
- 예시 데이터 결과에서는 핵심 그림과 다음 행동 뒤에 **다음 기간 결과와 비교하기 / Review this decision next period**(`ds/DecisionReviewPreview`)를 둔다. 문구: **예시 결과는 저장되지 않습니다. 내 데이터로 분석하면 결론·핵심 수치·검토일을 저장할 수 있습니다. / Sample results are not saved. Analyze your own data to save the conclusion, key figures and review date.** 조건: **저장은 로그인과 Pro(7일 체험 포함)가 필요합니다. 분석은 무료입니다. / Saving requires sign-in and Pro, including the 7-day trial. Analysis is free.** `내 데이터로 분석하기`·`Pro 안내`를 제공하며 입력 폼은 두지 않는다(가짜 결정 금지).
- 분석이 끝난 도구의 업로드·매핑은 **한 줄 요약 + `데이터·매핑 편집` 대화상자**(`ds/MappingEditorDialog`)로 고친다. 접기·펼치기가 아니라 이름 있는 편집 화면이다(§4). 결론 카드 위에 매핑 선택상자가 보이면 안 된다.
- 결과 아래 '이어서 볼 수 있는 분석'은 지금 되는 것 3개 + `전체 분석 보기` 링크.
...
| 도움말 | `HelpTip` | `InfoPopover` 기반 `ⓘ` — 마우스 올리기·키보드 Enter/Escape·터치로 이용. 설명을 `title`에만 두지 않는다 |
...
| 상자 안 상자 (2026-09-24) | 위계가 흐려지고 가장자리가 어긋난다 | 바깥 상자 하나, 안쪽은 선으로 구분 |
| 세로로 쌓인 형제 상자의 좌우 가장자리 불일치 | 어디는 튀어나오고 어디는 좁아 보인다 | 같은 폭·같은 가장자리 |
| 제목 바로 위 12px 라벨(eyebrow·kicker) | 폰에서 안 읽히고 제목과 경쟁한다 | 제목 한 줄에 합치거나 제목 아래 본문으로 |
| 굵기 400·600·700 밖의 값 | 사이값은 구분되지 않고 위계만 흐린다 | 본문 400 · 라벨·버튼 600 · 제목 700 |
| 왼쪽 굵은 색 막대 + 바탕 안내 상자 | 전형적인 생성형 UI 모양, 모든 안내가 경고처럼 보인다 | 얇은 네 변 테두리 또는 본문 글자 |
| 본문 위에 떠 있는 버튼(고정 위치) | 스크롤 위치마다 결과·CTA를 덮는다 | 헤더·본문 흐름 안에 둔다(읽기 바처럼 대상이 보이면 숨는 예외만) |
| 본문(14~16px)보다 작은 제목 | 위계가 뒤집힌다(12px 소제목이 실제로 있었다) | 글자 네 단: h1 26 · h2 20 · h3/h4 16 · 본문 14~16. 폰에서는 12·13px 단이 14px로 오른다 |
| 매핑 칸 이름을 용어로(전체 모수·세그먼트 축 선언·멤버) | 무엇을 고르라는지 모른다 | "이 파일에서 ~는 어느 열인가요?" 질문 + 선택 목록은 `추천`(값으로 판단)과 `기타(파일 순서)` 두 묶음. 설계 확인은 예/아니오로 답하는 문장, 기본값은 `아직 모름` |
| 페이지 가로 넘침 | 폰에서 화면이 좌우로 밀린다 | 표는 스크롤 래퍼, 화면낭독용 표는 `div.sr-only`로 감싼다(표는 1px 폭을 무시한다) |
| 신뢰도·방법·분석 범위·실행 정보 블록, 신뢰도 배지 | 쓸 곳이 없는 정보가 결과보다 길다 | 확인할 점이 있을 때만 결론 옆 빨간 `!`(`ds/IssueMark`) — 누르면 기간·열·칸 |
| 분석 전 품질 검사·승인 단계 | 분석할 수 있는데 허들이 된다 | 바로 결과로. 확인할 점은 결과의 `!` |
| 그림 이모지(💡🧪📁🎯 등) · CSS 대문자 라벨 · 숫자 고정폭 글꼴 | 생성형 UI 티, 결과가 터미널처럼 보인다 | 상태 기호(✓ ⚠ ↗ ✕)만 허용, 대소문자 그대로, 숫자는 본문 글꼴 + tabular-nums |
| 고정된 표면의 그림자 · 장식 그라디언트 · 올라오는 hover (2026-09-24) | 모든 카드가 떠 보이고 누를 수 있는 것처럼 보인다 | 그림자는 떠 있는 층(메뉴·팝오버·대화상자)만, 바탕은 단색, hover는 색 변화만 |
| 누를 수 없는 글자에 알약 모양(바탕+둥근 모서리) | 버튼과 구분되지 않는다 | 색 글자 또는 "·"로 이은 본문. 알약은 누르는 컨트롤만 |
| 글자색에 토큰 밖 색(hex 리터럴) | 테마마다 대비가 갈리고 색이 늘어난다 | `--text-*`·`--primary`·`--danger`·`--success`·`--warning` 토큰. 차트 데이터셋만 `CHART_THEME` |
| 같은 목적지 CTA를 한 화면에 두 번 | 어느 쪽을 눌러야 하는지 고민하게 된다 | 목적지 하나에 버튼 하나(홈 샘플·진단은 각 한 곳) |
| 빈 보관함에 도구·설명을 늘어놓기 | 처음 온 사람이 할 일이 안 보인다 | 빈 `내 프로젝트`는 한 문장 + `분석 시작` 버튼 하나 |
| 이 파일로 계산하지 않은 분석을 "지금 이 파일로 되는 분석"에 세기 (2026-09-25) | 샘플에서 20개가 되는 척했지만 숫자는 1개만 나왔다 | 되는 분석 = 이 파일로 실제 계산된 것만. 나머지는 아래 묶음에 줄마다 필요한 값 한 문장(`lib/toolDataNeeds.js`) 또는 돌렸지만 판정 못 한 사유 |
| 안 되는 분석 줄에 예시 결과 문장 | 이 파일의 결과로 읽힌다 | 필요한 값 한 문장만. 컬럼 이름(전환수 (분자))이 아니라 들어 있어야 할 값("실험군·대조군별 방문자 수와 전환 수") |
| 결과 머리·결론을 배경에 흩어 두기 | 결론보다 약한 하단 안내가 먼저 보인다 | 결과 요약은 흰 면 하나: 제목·입력 요약 → 결론·핵심 수치/그림·행동 → 공통 설정. 수치는 가용 폭에 맞춰 4·2·1열로 배치하고 날짜/금액 중간 줄바꿈과 잘림을 피한다. 다음 프로젝트 안내도 같은 흰 면·보조 버튼 |
| 진행 단계를 탭 모양(현재만 상자·밑줄 링크)으로 | 눌러서 이동하는 메뉴로 읽힌다 | 가는 막대 네 칸, 완료(✓)는 실제로 끝난 단계만, 폰은 "2 / 4 분석 근거 · 다음: 결정 저장" 한 줄 |
| 분석마다 같은 막대·꺾은선으로 결과 그리기 (2026-09-25) | 무엇을 답하는 분석인지 그림에서 사라진다 | 분석별 핵심 그림: 성과 변동=직전→비중→효율→최근 다리+채널별 두 성분, 포화=평균↔한계 단가, 예산=지금↔바꾼 안, 채널 중복=VIF 기준선, 소재=상태 띠, 생존=계단 곡선+95% 띠 |
...
| 굵기 3단 | `src/app/fontWeightScale.test.js` | CSS·인라인 굵기는 400·600·700만(`@font-face` 범위 제외) |
| 렌더 디자인 규칙 | `e2e/design-rules.spec.js` | 겹침·고정 요소 가림·상자 안 상자·가장자리 불일치·제목 위 라벨·굵기·색 막대 상자·**본문보다 작은 제목·가로 넘침**을 3개 폭·정적/예시 결과 화면에서 측정, 예시 결과는 **결론 카드 위 매핑 선택상자 0개**(예외는 `data-design-exempt="규칙: 사유"`) |
...
- KO: “분석 도구는 무료입니다. 프로젝트·리뷰와 결정 기록 저장·보고서 다운로드·브랜딩은 5,900원 1개월 이용권으로 제공합니다. 자동 갱신은 없습니다.” EN: “Analysis tools are free. Projects, saved reviews and decisions, report downloads and branding are included in a KRW 5,900 one-month pass without automatic renewal.”
...
- 기본 무료 범위는 분석 도구 전체·화면 결과 확인. Pro 범위는 모든 프로젝트 생성·리뷰와 결정 기록 저장·수정·파일과 설정 보관·백업 복원·일괄 보고서·브랜딩이며 분석 결과 다운로드는 구매 전용이다. 만료 후 기존 기록 읽기·내보내기·백업·삭제는 유지한다. 개수 제한 해제는 저장 공간 무제한을 뜻하지 않는다.
...
- 리뷰 수정·완료는 기기 보관이 성공한 뒤에만 완료로 표시한다. 실패하면 입력 초안을 유지하고 재시도할 수 있어야 한다. 기기 저장이 꺼져 있으면 같은 저장창에서 명시적으로 켤 수 있다.
- 미저장 리뷰 입력이 있으면 프로젝트를 바꾸기 전에 확인한다. 이미 보관한 파일만 있는 경우에는 매번 확인하지 않는다. 전환 취소는 오류로 표시하지 않는다.
- 계정 메모 조회 실패는 빈 목록과 구분하고 다시 불러오기를 제공한다. 계정 보관 성공 후 갱신 실패를 저장 실패로 바꾸지 않는다. 같은 ID의 기기/계정 내용이 다르면 두 내용을 비교한다. 계정 내용을 기기에 가져올 때는 별도 기록으로 저장하여 기기에만 있는 상세 검토 이력을 보존한다. 기기 내용으로 계정 메모를 갱신하는 동작은 명시적으로 선택한다. 계정 메모는 원본 파일·전체 프로젝트 자동 동기화가 아니다.
- 주간 자동 검토는 도구 종류와 무관하게 저장한 기준 기간·전환 기준·통화·필터 범위를 확인한다. 집계로 재현할 수 없는 범위는 판정을 보류한다. 아직 검토일이 오지 않은 기록이나 조건이 없는 최신 기록 때문에 비교 가능한 결정을 숨기지 않는다.
- CSV 없이 모델 입력만 저장한 설정은 파일 업로드·CSV 비교를 요구하지 않는다. ‘확인한 설정 적용 / Apply checked setup’ 후 복원된 입력을 확인하고 분석한다.
- 관심 이벤트는 가격 제시 후의 관심이지 결제율이나 반복 매출이 아니다. 게이트 조회 source=project_limit은 Pro 없이 프로젝트 생성·저장을 시도한 경우로 해석한다. 이름·파일명·CSV·금액 지표·키·로고·결제키·복원 코드는 GA에 보내지 않는다. 테스트와 실결제 활성화 이벤트를 구분한다.
- 결제 개인정보 계약: 주문번호·상품·금액·결제 상태·이용 기간·복원 코드 해시를 서버에 보관한다. 계정 보관 활성화 시 로그인한 구매 또는 명시적인 기존 이용권 연결에 계정 ID를 추가한다. 카드 입력은 선택된 결제대행사(나이스페이 또는 기존 토스페이먼츠)의 결제 화면에서 처리한다. 신규 결제대행사는 서버 설정으로 선택하며 기존 주문은 원래 결제대행사에서 조회한다. 나이스페이는 서버 승인·Basic 인증 모델의 카드 1회 결제를 준비하며, 계약·테스트 검증 전 실결제를 활성화하지 않는다. CSV·분석 수치·프로젝트명·파일명은 결제 서버에 보내지 않는다. 결제 복원은 프로젝트 동기화가 아니다. 결제 기록 보관은 법정 의무에 따라 별도 운영하며 브라우저 90일 정책과 구분한다.
...
- 홈 샘플 미리보기는 내장 효율 데모의 실제 기간·채널·비용·전환 계산을 표시한다. 대표 채널은 CPA 상승률 기준이며 전체 성과로 일반화하지 않는다. KO/EN 모두 체험용 데이터임을 명시한다.
- 모바일 홈은 CSV 없이 샘플 결과를 체험하는 동선을 우선 제공한다. 계산기·진단은 바로 열고, 템플릿·PC 링크 복사는 보조 경로로 둔다. PC 링크는 사용자 CSV를 전달하지 않는다.
- 홈 첫 화면은 메시지와 제품 미리보기의 두 영역으로 구성한다. 주요 행동은 ‘샘플로 체험하기 / Explore a sample’와 CSV 업로드이며, 질문·학습·모바일 보조 경로는 다음 위계에 둔다. 샘플은 실제 제품 화면 전체가 아닌 계산 결과 미리보기라고 명시하고 도치는 브랜드 이미지로 사용한다.
- 홈 결과 설명과 공통 콘텐츠 버튼은 14px 이상, 입력 필드는 16px, 보조 메타 정보는 12px 이상을 기준으로 한다. 차트의 실제 수치와 기간은 줄바꿈·축약으로 숨기지 않는다.
...
### 첫 결과와 반복 검토 (2026-09-12)

- 홈·진입 화면의 ‘샘플로 체험하기 / Explore a sample’는 내장 데이터에서 CPA 상승률이 가장 큰 유료 채널과 최근 7일·직전 7일을 선택한다. 미리보기·결과·주간 리뷰는 동일 원본 범위와 actions 기준으로 시작한다. 실제 업로드는 기존 컬럼 확인·분석 게이트를 유지한다.
- ‘샘플 데이터 / Sample data’와 채널·비교 기간을 결과에 표시한다. 샘플 주간 리뷰에는 실제 프로젝트의 저장 설정·집계·결정을 혼합하지 않는다. 샘플 설정은 실제 프로젝트에 저장하지 않는다.
- 결과는 핵심 판단을 먼저 보여주고, 분석 준비 설명과 실행 상태는 보조 위계에 둔다. 품질 한계·보류 사유·추가 분석에 계속 접근할 수 있어야 한다.
- 예시는 곧장 결과다(2026-09-24): 도구의 ‘예시 데이터로 결과 바로 보기’와 홈 샘플은 매핑 확인·분석하기 클릭·안내 창 없이 결론 카드까지 연다. 샘플 CSV 하나로 모든 도구를 돌릴 수는 없으므로(도구마다 행의 뜻이 다르다), 샘플 결과 화면에서는 모든 분석을 ‘가능’으로 두고 샘플이 못 채우는 도구는 그 도구의 예시 데이터로 연다. 예시가 사람의 선언을 대신해야 하는 입력(5-23 차단 기간·전환 시점, 5-28 이탈 이벤트 정의)은 예시 데이터가 함께 싣고, 내 데이터는 계속 직접 지정한다. 데모 안내 창은 없앴다. `e2e/example-straight-to-result.spec.js`가 발행 도구 전체에서 파생해 강제한다.
- 원본 데이터 통화는 묻지 않고 채운다(2026-09-24): 로그인 계정에 기억한 선택 → 이 브라우저의 마지막 선택 → 화면 언어(한국어 원, 그 밖은 달러). 선언은 환산이 아니며, 업로드 화면의 통화 버튼으로 바꾸면 그 값이 다음 기본값이 된다. 로그인 상태면 선택 하나(KRW|USD)만 계정에 저장한다.
- 이름(2026-09-24): 저장한 결정을 모아 보는 곳은 **내 프로젝트 / My projects** 하나다(구 주간 리뷰·지난 결정·결정 검토함·프로젝트 리뷰). 결과를 프로젝트로 만드는 모든 버튼은 **다음 마케팅 프로젝트로 만들기 / Make it my next marketing project**다(구 리뷰 저장·다음 검토 약속 만들기·다음 검토로 저장·프로젝트로 넘기기·주간 리뷰 만들기). 기존 결정 갱신은 ‘내 프로젝트에 저장’, 필드 이름(다음 검토일 등)은 그대로다.
- 주간 리뷰 결론 옆 ‘요약 복사 / Copy summary’는 기간·KPI·판단·저장한 결정과 해석 한계를 복사한다. Slack·Notion으로 자동 전송하지 않는다. 전체 보고서 복사도 무료이며 유료 다운로드 계약은 유지한다.
- 결정 기록은 대상·행동·검토일을 먼저, 목표·가드레일은 선택적으로 펼친다. 가드레일 미설정 시 자동 판정 불가를 계속 알린다. 다음 CSV 진입은 업로드 영역을 실제로 펼치며 기존 프로젝트·매핑 복원과 새 데이터 검증을 재사용한다.
...
- 결정 저장 시 화면에서 확인한 결론·핵심 수치·근거·비교 범위·분석 상태를 작은 선택한 근거 기록으로 함께 보존한다. 원본 행·파일명은 근거 기록과 공유 링크에 넣지 않는다. 과거 기록에 근거가 없으면 미보관으로 표시한다. 계정 보관을 선택하면 검토 설계·선택한 근거 요약·관측 이력도 메모와 함께 보관한다. 원본 행·파일명·데이터셋 스냅샷은 보내지 않는다.

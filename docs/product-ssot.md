...
|---|---|---|---|
| F-01 | 모든 분석 도구는 무료입니다. | 분석 도구·프로젝트 1개·단일 보고서 무료. 확장 기능은 구독 관심 확인 단계이며 현재 결제 없음. "영구 무료"는 쓰지 않는다. | ✅ `BRAND_FACTS.price` 일치 |
...
| F-04 | 분석 데이터는 이 기기에만 저장되고, 마지막 사용 후 90일이 지나면 자동으로 지워집니다. | 서버로 전송·보관하지 않음. 사용자는 언제든 파일을 지우거나 저장을 끌 수 있음. | ✅ `persistence` 일치 |
...
- 무료 분석 도구 정책은 유지한다. 수익화 준비의 범위는 **주간 반복 사용 가치 검증**이며, 월 5,900원 예정 요금을 제시하고 관심만 확인한다. 결제·자동 갱신·구독 신청은 이루어지지 않는다. 라이선스 검증은 환경 설정이 있는 경우에만 노출하며 실제 발급 운영은 미개시다.
- CSV 업로드·분석 실행 → 기간과 KPI 확인 → 지난 결정 검토 → 사용자가 다음 결정 저장 → 보고서 복사/인쇄 → 다음 기간 CSV로 재검토한다. 무료로 새 프로젝트 1개를 만들 수 있으며 프로젝트마다 파일·집계·결정을 분리한다. 기존 프로젝트 열기·내보내기는 권한 만료로 회수하지 않는다. 서버 동기화는 없다.
...
### 프로젝트·구독 관심 검증 (2026-09-09)
- KO: “월 5,900원 구독에 대한 관심을 확인하고 있습니다. 지금은 결제·자동 갱신·구독 신청이 이루어지지 않습니다.” EN: “We are checking interest in a KRW 5,900/month plan. No payments, automatic renewals, or subscriptions are created on this page.”
- `/projects` 및 `/en/projects`: 프로젝트별 IndexedDB 파일·매핑·주간 집계·결정·보고서·브랜딩. `/subscription` 및 `/en/subscription`: 무료 범위, 예정 요금, 관심 버튼, 실제 기기 저장량 안내. 현재는 preview/noindex이며 sitemap/RSS에서 제외한다.
- 기본 무료 범위는 분석 도구 전체·프로젝트 1개·단일 보고서. 확장 범위는 여러 프로젝트·일괄 보고서·브랜딩. 개수 제한 해제는 저장 공간 무제한을 뜻하지 않는다.
...
- 관심 이벤트는 가격 제시 후의 관심이지 결제율이나 반복 매출이 아니다. 게이트 조회는 무료 한도 시도 source=project_limit만 별도 해석한다. 이름·파일명·CSV·금액 지표·키·로고는 GA에 보내지 않는다.

...
|---|---|---|---|
| F-01 | 모든 분석 도구는 무료입니다. | 분석 실행·화면 결과 확인 무료. 프로젝트·리뷰 저장은 Pro(14일 체험 포함). 분석 결과 다운로드는 5,900원 1개월 구매 이용권 대상. 기존 기록 읽기·백업 내보내기는 만료 후에도 유지. "영구 무료"는 쓰지 않는다. | ✅ `BRAND_FACTS.price` 일치 |
...
- CSV 업로드·분석 실행 → 기간과 KPI 확인 → 지난 결정 검토 → 사용자가 다음 결정 저장 → 보고서 복사/인쇄 → 다음 기간 CSV로 재검토한다. 유효 Pro(14일 체험 포함)로 프로젝트를 만들 수 있으며 프로젝트마다 파일·집계·결정을 분리한다. 기존 프로젝트 열기·내보내기는 권한 만료로 회수하지 않는다. CSV·프로젝트 전체의 서버 동기화는 없다. 선택한 결정 메모의 계정 보관은 §5.1.3의 별도 활성화 조건을 따른다.
...
- Google 계정당 1회 14일 Pro 체험. 가입일이 아닌 첫 프로젝트 체험 시작 요청의 승인 시각을 서버에 기록하고, 동시 저장·기기 변경·재로그인으로 재시작하지 않는다. 체험은 자동 결제되지 않는다.
- 무료/체험 구분(2026-09-14): 무료는 분석 실행·화면 결과 확인이다. 첫 프로젝트를 포함한 프로젝트 생성, 기기 내 리뷰·결정 기록 저장·수정, 파일·분석 설정·기간 집계 보관 및 백업 복원은 유효 Pro(14일 체험 포함)가 필요하다. 만료 후에는 기존 기록의 열람·내보내기·백업·삭제만 유지하며 새 저장·수정은 막는다. 첫 체험은 프로젝트 생성·첫 리뷰 저장 흐름의 명시적 체험 시작 요청으로 활성화한다. 계정 메모 보관은 체험 시작 조건이 아니며 별도 선택이다. 체험 활성 확인 후 로컬 프로젝트 저장을 별도로 진행하므로 원본·전체 보고서를 계정에 보내지 않는다. 체험 시작과 기기 저장 성공을 구분하고 실패·취소 시 초안을 유지한다. / Free includes analysis and on-screen results. Creating any project, saving or updating device reviews and decisions, retaining files, analysis setups and period snapshots, and restoring backups require active Pro, including the 14-day trial. Existing records remain readable, exportable and deletable after expiry. The first trial starts through an explicit trial request in project creation or the first review save. Account memo storage is optional and is not a trial prerequisite. Account memo storage never syncs source files or full reports; confirm local saving separately and preserve drafts on failure or cancellation.
- 분석자료 다운로드는 현재 유효한 구매 이용권이 있어야 한다. 14일 체험·로그인만으로는 허용하지 않으며 구매 이력이 있어도 만료된 이용권은 거절한다. 구매 권한이 없으면 파일을 생성·전달하지 않고 구독 구매 안내를 연다. Word·Excel·분석 CSV/PNG·보고서 PDF/인쇄에 같은 조건을 적용한다. 기존 결정 기록 내보내기·프로젝트 백업·캘린더·무료 샘플은 별도 계약대로 유지한다. / Analysis downloads require an active purchased pass, not just sign-in or the 14-day trial. Expired purchases do not qualify. Existing record exports, project backups, calendars and free samples remain available under their existing rules.
- 구매 진입 계약: 다운로드 게이트와 요금제에서 아직 체험을 시작하지 않은 계정에 `프로젝트 만들고 Pro 체험하기 / Create a project to try Pro`를 제공한다. 비로그인 상태에서는 자격을 확정하지 않고 Google 계정당 1회·첫 프로젝트 체험 시작일부터 14일이라는 조건을 안내한다. 체험 중은 `Pro 체험 중 / Pro trial`과 남은 일수·종료일을 표시하고 구매한 Pro와 구분한다. 요금제 상단은 플랜·가격·체험 진입에 집중하고 계정 메모 목록을 넣지 않는다. 복원·기기 이전은 `이미 구매한 이용권 복원 / Restore an existing purchase` 아래로 접되 결제 성공 후 복귀·복원 코드 보관은 바로 제공한다. 무료 카드에는 분석과 화면 결과 확인만 나열한다. Pro 카드에는 프로젝트·리뷰와 결정 기록·계정 메모 저장을 명시하고, 만료 후 기존 기록 읽기·내보내기는 별도 안내한다.

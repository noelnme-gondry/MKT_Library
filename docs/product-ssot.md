...
- 무료/체험 구분(2026-09-14): 무료는 분석 실행·화면 결과 확인이다. 첫 프로젝트를 포함한 프로젝트 생성, 기기 내 리뷰·결정 기록 저장·수정, 파일·분석 설정·기간 집계 보관 및 백업 복원은 유효 Pro(14일 체험 포함)가 필요하다. 만료 후에는 기존 기록의 열람·내보내기·백업·삭제만 유지하며 새 저장·수정은 막는다. 첫 체험은 저장 전 초안의 선택한 메모를 사용자가 확인·동의해 계정에 처음 보관하면 시작한다. 체험 활성 확인 후 로컬 프로젝트 저장을 별도로 진행하므로 원본·전체 보고서를 계정에 보내지 않는다. 계정 저장 성공과 기기 저장 성공을 구분하고 실패·취소 시 초안을 유지한다. / Free includes analysis and on-screen results. Creating any project, saving or updating device reviews and decisions, retaining files, analysis setups and period snapshots, and restoring backups require active Pro, including the 14-day trial. Existing records remain readable, exportable and deletable after expiry. The first trial can start by explicitly selecting, reviewing and consenting to save a draft memo to the account before saving its local project. Account memo storage never syncs source files or full reports; confirm local saving separately and preserve drafts on failure or cancellation.
...
- 계정 메모 조회 실패는 빈 목록과 구분하고 다시 불러오기를 제공한다. 계정 보관 성공 후 갱신 실패를 저장 실패로 바꾸지 않는다. 같은 ID의 기기/계정 내용이 다르면 두 내용을 비교하고 어느 쪽을 갱신할지 사용자가 선택한다. 계정 메모는 원본 파일·전체 프로젝트 자동 동기화가 아니다.

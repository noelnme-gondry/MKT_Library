...
| 🟡 **D-19** | 전 도구 상세 XLSX | 공개 분석 도구 19개 모두 `ResultActionCard`를 쓰며 공통 워크북 액션 예외 0. 도구별 수식 시트는 5-2·5-21·5-24·5-26·5-27의 집계·PVM·AR(1) 하이브리드·ASA 규칙·ASO 분해 5종을 먼저 배선 | **1차 완료(2026-08-25)**: 전 도구에 원본 전체·매핑·범위·화면 결론·엔진 경계·후속 수식을 같은 구조로 제공하고 워커에서 생성한다. **남음**: 나머지 14개 도구의 상세 결과표를 공통 엔진 출력 연결보다 깊은 도구별 계산 시트로 옮긴다. 복잡 모델은 재학습 수식이 아니라 §5.5.1의 하이브리드 경계를 유지한다 |

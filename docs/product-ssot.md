...
- Google 계정당 1회 7일 Pro 체험. 2026-09-21 이전에 시작한 체험은 종전 14일을 그대로 지킨다 — 정책을 소급하면 이미 쓰고 있는 사람의 남은 기간이 말없이 줄어든다. 길이는 `archiveContract.js`의 `PRO_TRIAL_DAYS`가 소유하고 SQL은 `trialUntilSql()`로만 만든다. 가입일이 아닌 첫 프로젝트 체험 시작 요청의 승인 시각을 서버에 기록하고, 동시 저장·기기 변경·재로그인으로 재시작하지 않는다. 체험은 자동 결제되지 않는다.

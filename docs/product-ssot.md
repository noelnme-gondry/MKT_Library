...
- 작업 화면은 보이는 H1 하나만 사용한다. 리뷰 제목은 `프로젝트 리뷰 / Project review`이며 중복된 영문 상단 라벨·주간 리뷰 대제목을 붙이지 않는다. 프로젝트 선택과 화면 전환은 같은 높이의 도구 모음으로 정렬한다. 마이페이지는 닫기 버튼, 명확한 로그인 버튼, 아이콘과 이름이 있는 관리 링크를 제공하며 기본 밑줄 링크 나열로 표현하지 않는다. 로그인 복구와 개인정보 링크는 보조 위계로 유지한다.
- 공통 헤더 오른쪽 `마이페이지 / My account`는 Google 로그인·이메일 복원, 계정·Pro 상태, `내 프로젝트 관리 / Manage my projects`, `현재 프로젝트 리뷰 / Current project review`, `저장·백업 설정 / Storage & backup settings`, 로그아웃을 제공한다. 계정 저장·로그아웃·알림 변경 후 같은 화면의 모든 계정 표시를 갱신한다. 계정 전체 메모와 현재 로컬 프로젝트의 기록은 동일한 저장소인 것처럼 표시하지 않는다.

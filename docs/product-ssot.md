...
- 요금제 표시 이름은 Free(무료) / Pro이며, Pro는 기존 5,900원 1개월 보고서 이용권의 표시명이다. 새로운 등급·자동 갱신 상품을 추가한 것이 아니다. 무료는 분석 실행·화면 결과 확인이며, Pro(14일 체험 포함)는 첫 프로젝트부터 생성·리뷰와 결정 기록 저장을 제공한다. 보고서 다운로드·일괄 보고서·브랜딩은 유효한 구매 이용권이 필요하다. 기존 기록의 열람·백업 내보내기는 만료 후에도 유지한다. 저장 한도는 두 플랜에 동일하게 적용한다. 화면의 파일 구성 예시는 실제 분석 결과나 고객 사용 실적으로 표시하지 않는다.

...
| F-03 | 업로드한 CSV는 브라우저에서 처리됩니다. | 파싱·계산이 클라이언트에서 실행되고 원본 행을 서버로 전송·저장하지 않는다는 의미. 브라우저 확장·사용자 네트워크·외부 공개 URL까지 포괄하는 절대 보안 주장은 하지 않는다. | ⚠️ 현재 코드 문구는 "데이터가 브라우저를 **벗어나지 않습니다**" — 절대 표현. §10 D-03 |
...
| F-06 | **추정 결과는 불확실성과 함께 해석해야 합니다.** | 신뢰구간·식별 실패 사유를 함께 보여주는 것이 원칙. **"모든 숫자에 95% CI가 있다"는 보편 문장은 금지** — PVM 분해·Aha 그리드·ASA 키워드처럼 CI가 정의되지 않는 출력이 있다. | ❌ 현재 코드가 보편 주장. §10 **D-01(P0)** |
...
| 금지 패턴 | 대체 |
|---|---|
| 첫 화면에 동급 CTA 3개 이상 | primary 1개 + 필요할 때만 보이는 보조 행동 |
...
| `brandClaimScope.test.js` | F-06처럼 보편 주장이 `llms.txt`에 나가지 않는지 |
...
| ID | 항목 | 실측 | 완료 정의 |
|---|---|---|---|
| **D-01** | `BRAND_FACTS.uncertainty`가 "추정값에는 95% 신뢰구간을 함께 표시합니다"를 **보편 주장**으로 `llms.txt`에 내보냄. CI가 정의되지 않는 출력(PVM 분해·Aha 그리드·ASA 키워드)이 존재 | `llms.txt/route.js:140`이 전 사실을 무조건 방출 | 문구를 F-06(조건부)으로 교체 + `brandClaimScope` 가드 |

### P1 — 접근성·일관성

| ID | 항목 | 실측 | 완료 정의 |
|---|---|---|---|
| **D-02** | 브랜드명 2표기 공존 | `Growth Opt Playbook` 80곳 vs `Growth Optimization Playbook` 2곳(`brandFacts.js`). `BRAND`/`getBrand()`는 **소비처 0** | `BRAND.name`을 확정값으로 교체, KO shortName 정리, 미사용이면 소비처 배선 또는 제거 판단 |
| **D-03** | F-03 문구가 절대 표현("브라우저를 벗어나지 않습니다") | `brandFacts.js` privacy | "브라우저에서 처리되며 원본 행을 서버로 전송·저장하지 않습니다"로 완화 |
...
| ID | 항목 | 완료 정의 |
|---|---|---|
| D-09 | 탭·라디오 ARIA 소유 관계 전수 확인 | `role="tablist"` 8곳·`role="radiogroup"` 7곳이 §6.1 계약 충족 |
...
| D-12 | 문서 드리프트 | `ARCHITECTURE.md`·`worklog.md`의 구 SPA 서술 정리 |

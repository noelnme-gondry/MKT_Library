...
| 4 | U1 판단 지점에 결정 검토 진입점 복구 | M | **제품 판단 대기** — 레일 재마운트와 결과 카드 내 통합 중 선택 필요 |
| 5 | U2 `NEXT_TOOL_IDS` 상단을 같은 그룹 우선으로 재정렬하고 R1 테스트화 | M | 미착수 |
| 6 | U3 자체 드롭존 6경로를 공용 파일 가드 경유로 전환 | M | 미착수 — 파싱 흐름별 분할 필요 |
| 7 | U5 무거운 도구에 오버레이·더블 rAF 디퍼 적용 | L | 미착수 — 5-3 → 5-21 → 9-6 순 후보 |
| 8 | U7 자격 엔진을 하나로 파생 또는 통합 | M | **제품 판단 대기** — 과도기 예외인지, 계약 통합인지 결정 필요 |
| 9 | U9 결정 기록과 U11 소개 문단 누락 해소 | S | 미착수 |
| — | U10 죽은 `AnalysisEligibilityList` 처리 | S | **제품 판단 대기** — 11개 카피를 `AssistantWorkspace`로 옮길지 제거할지 선택 필요 |

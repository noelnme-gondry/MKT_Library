...
> **상태 (2026-09-17 최종)**: P0·P1·P3 수정 완료. P2는 2건 수정 + 1건 진단 정정 + 1건 오탐.
> C(브라우저 저장 migration) 감사·수정 완료 — 아래 §C. 저장 레코드 세대 관리도 배선 완료.
>
> ⚠ **2026-09-17 자기 정정 2건**: P2-001(중복 매핑)은 **오탐**이었고, P2-003(모드 C)은
> **진단이 절반 틀려** 그 위에 쓴 화면 문구가 잘못 배포됐다(수정 완료). 둘 다 원인이
> 같다 — **엔진만 읽고 그 옆의 배선을 안 봤다**. 각 항목에 경위를 남겼다.
...
| **P2 Medium** | 4 | 음수 분모 · 통화 파싱 → **수정됨** · 모드 C → **진단 일부 오류, 고지 수정** · 중복 매핑 → **오탐** |
...
### [P2-001] 중복 매핑 — **오탐이었다 (2026-09-17 정정)**

원래 보고: "두 컬럼이 같은 표준키로 매핑되면 뒤쪽이 앞쪽을 덮는데 화면이 아무 말도 안 한다."

**이 진단은 틀렸다.** 주 업로드 경로에는 이미 게이트가 있다:

```js
// CsvUploader.jsx:875,887
...
const analysisBlocked = ... || mappingBlocked ...;
```

`analysisBlocked`면 **분석하기 CTA가 렌더 자체가 안 되고**(`CsvUploader.jsx:1284`의
`&& !analysisBlocked`), 매핑 그리드에는 헤더별 `conflict` 칩이 뜬다.

**왜 틀렸나**: 재현 스크립트가 `mapRowsToStandard`를 직접 호출해 **UI 게이트를 우회**했다.
§7이 "스모크 `beforeEach`의 상태 주입이 진입 경로를 우회한다"고 적어 둔 그 함정에
그대로 걸렸다 — 엔진만 부르고 "화면이 말하지 않는다"고 단정한 것이다.

**남긴 것**: 추가했던 `duplicate_mappings` 품질 이슈는 되돌리지 않았다. 틀린 경고는
아니고(중복이 실제로 있을 때만 뜬다), `AnalysisBasisBar`는 복원된 데이터셋·assistant
경로에서도 렌더되므로 주 경로 게이트가 닿지 않는 자리를 덮는다. **다만 이것이 1차
방어라고 읽으면 안 된다** — 1차 방어는 위의 `analysisBlocked`다.
...
### [P2-003] 예산 배분 모드 C — **진단 일부 오류 (2026-09-17 정정), 고지는 수정 후 유지**

원래 보고: "곡선이 '여기 더 넣지 마라'고 말하는 채널에 평균 효율만 보고 예산을 넣는다."

**절반이 틀렸다.** `calculateAllocationModeC`만 읽고 `maxSpends`가 어디서 오는지를 안 봤다.
실제로는 곡선이 **채널별 상한**으로 배분에 반영된다:

```js
// budgetAllocTool.js:540 getAllocationEvidenceLimits
let cap = xMax;                                   // 관측 최대 지출
if (shape?.shape === "bell" && shape.vertex > 0)
  cap = Math.min(cap, shape.vertex);              // ∩형이면 꼭짓점에서 자름
```

이 값이 `evidenceLimits.maxSpends`로 모드 C에 전달된다(`BudgetAllocation.jsx:995`).
즉 "더 쓰면 나빠지는" 지점 너머로는 배분되지 않는다.

**맞는 부분**: 상한 *안에서* 나누는 비율은 최근 평균 CPR의 역수이고 한계효율이 아니다.
한계효율이 떨어지는 구간에서도 평균이 좋으면 계속 배분된다.

**여파**: 이 틀린 진단으로 쓴 화면 문구("곡선은 결과 예측에만 쓰이고 배분에는 반영되지
않으므로")가 그대로 배포됐다. §8 정직성 위반이라 2026-09-17에 상한 반영 사실을 담아
KR/EN 모두 고쳤다. **기본값(모드 C)은 바꾸지 않았다** — 한계효율은 곡선 적합 품질에
민감해 채널당 데이터가 적으면 모드 B가 더 불안정하고, 관측 평균 기반인 C가 견고하다.
...
4. ~~**변형검사 전반**~~ — **2026-09-17 해소**. 5-21·5-27·5-29에 복제·스케일·순서·
   리네임 불변 + 무잔차 유지를 걸었다(`engineMetamorphic.test.js` 14건).
5. ~~**Simpson's paradox**~~ — **2026-09-17 해소**. 아래 §S 참조.
...
## S. Simpson's paradox 실측 (2026-09-17)

QA 요청서 §5가 직접 지목한 항목 — "mix shift 때문에 전체 성과가 변했는데 시스템이
개별 campaign 성과 악화라고 잘못 설명하지 않는가". 1차 감사에서 **수학만 보고
넘어갔던** 자리라 이번에 픽스처로 실측했다.

### 픽스처 (5-21)

| | 기간 1 | 기간 2 |
|---|---|---|
| 캠페인 A | 비용 900 / 결과 180 → **CPA 5** | 비용 200 / 결과 50 → **CPA 4** (개선) |
| 캠페인 B | 비용 100 / 결과 2 → **CPA 50** | 비용 1,800 / 결과 40 → **CPA 45** (개선) |
| **전체** | **CPA 5.49** | **CPA 22.22** (악화) |

전 캠페인이 개선됐는데 전체는 4배 악화 — 원인은 전적으로 구성 이동이다.

### 결과: 분해는 정확했다

```
A: 비중 98.9%→55.6%  mix +4.06  rate −0.77
B: 비중  1.1%→44.4%  mix +14.58 rate −1.14
Σmix +18.64 · Σrate −1.91 · Σ 16.73 = Δ 16.73  ✓
```

**믹스를 범인으로, 단위 효율을 개선으로 정확히 귀속한다.** 5-29(3항 분해)도 같은
시나리오로 확인했고 동일하게 올바르다. 화면(5-21 표)도 mix/rate를 중립적인 숫자와
툴팁으로 보여주므로 거짓 서술이 없다.

### 다만 죽은 코드에 거짓 서술이 있었다

`PVM_MATH.classifyNarrative`(소비처 **0곳** — 전수 grep 확인)가 A를 이렇게 설명했다:

> "A의 비중이 98.9%→55.6%로 **늘었고** 평균보다 **비싼 편**(CPA 4원)이라"

둘 다 사실과 반대다 — A는 비중이 43%p **줄었고** CPA 4원은 전체 평균 13.86원보다
**가장 싸다**. 원인은 `mixWorse = mix >= 0` 하나로 문장을 고른 것이다. mix가 양수가
되는 경로는 둘인데(`mix = (cpāᵢ − C̄)·Δs`) 한쪽만 문장으로 굳혀 놨다:

| 경로 | 옛 문장 | 사실 |
|---|---|---|
| 비싼 것 × 비중↑ | "늘었고 비싼 편" | ✓ |
| **싼 것 × 비중↓** | "늘었고 비싼 편" | ✗ **방향·가격 둘 다 반대** |

**심각도**: 화면에 도달하지 않으므로 사용자 피해는 없었다. 다만 누가 배선하는 순간
거짓 문장이 배포되는 잠복 함정이라, 두 축(비중 방향 · 평균 대비 위치)에서 파생하도록
고쳤다. `C̄`를 새 인자로 넘기지 않고 `mix`와 `Δs`의 부호에서 되찾으므로 시그니처는
그대로다. 수학(mix·rate·contribution)은 한 자리도 안 건드렸다(§11).

고친 뒤 같은 픽스처: *"A의 비중이 98.9%→55.6%로 **줄었고** 평균보다 **저렴한** 편
(CPA 5→4원)이라 전체를 +4원 끌어올림"* — 싼 채널이 줄어 전체 CPA가 올랐다는,
실무적으로 가장 흔한 경로를 이제 정확히 말한다.
...
### 5차 — 저장 레코드 세대 관리 + 자기 정정 (2026-09-17)
- `WORKSPACE_DATASET_SCHEMA_VERSION` 신설. 복원 시 구세대면 **파일은 살리고 매핑만**
  현재 규칙으로 다시 인식하고(`buildMappingContract`), 그 사실을 `/storage`가 KR/EN으로
  말한다. 재인식이 아무것도 못 잡으면 저장본을 유지한다(빈 매핑보다 옛 매핑이 낫다).
  의미 바인딩(V2)은 새 매핑과 짝이 안 맞으므로 함께 비운다.
- 모드 C 화면 문구를 사실에 맞게 수정(곡선은 상한으로 반영된다).
- 위 P2-001·P2-003 진단 정정.
- `datasetSchemaVersion.test.js` 5건 · `workspaceRestoreSchema.test.js` 3건(복원 배선).

### 6차 — Simpson 픽스처 + 형제 엔진 변형검사 (2026-09-17)
- `simpsonParadox.test.js` 14건 — 5-21·5-29가 구성 변화와 단위 내부 변화를 반대로
  귀속하지 않는지. 서술은 네 사분면을 전부 돌고 **반대말이 들어가면 실패**시킨다.
- `engineMetamorphic.test.js` 14건 — 5-21·5-27·5-29의 복제·스케일·순서·리네임 불변과
  변형 후에도 무잔차가 유지되는지.
- `classifyNarrative`의 거짓 서술 수정(위 §S).

### 남은 것 (의도적으로 안 함)
- **예산 배분 모드 C의 배분 비율** — 상한은 곡선을 반영하고, 그 안에서 평균 효율로
  나누는 것은 선언된 동작이다. 기본값을 B로 올릴지는 제품 결정이고 수학 변경은 §11.
- **중복 매핑의 덮어쓰기 동작** — 애초에 분석이 차단되므로 하류까지 도달하지 않는다.

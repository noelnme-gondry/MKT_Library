...
- **수정 후**: `test:all` **4,015 passed / 0 failed** · `lint` 0 · `build` 성공.

> **상태 (2026-09-16 갱신)**: P0-001 · P1-001 · P1-002 · P1-003 **수정 완료**(커밋 `eff9e14`).
> C(브라우저 저장 migration) 감사 완료 — 아래 §C. 남은 P2/P3는 미착수.
...
| **P2 Medium** | 4 | 중복 매핑 무경고 · 음수 분모 · 모드 C 평균효율 배분 · (통화 파싱은 P1-003 수정에 포함돼 해소) |
...
### 남은 것 (미착수)
P2 3건(중복 매핑 무경고 · 음수 분모 · 모드 C 평균효율 배분) · P3 1건(모호 날짜 무고지) ·
저장 데이터셋 스키마 버전 필드(읽기 정책이 제품 결정이라 보고만 함).

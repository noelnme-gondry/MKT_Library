...
- **기준선**: `npm run test:all` = **3,951 passed / 2 skipped / 0 failed** (178s). 기존 스위트는
  깨끗하다. 즉 아래 결함은 **현재 가드가 보지 않는 자리**에 있다.
...
| 등급 | 건수 | 요지 |
|---|---:|---|
| **P0 Critical** | 1 | A/B 대량 검정(CSV 경로)이 근사 p로 "유의" 판정 — 같은 도구의 수동 경로와 판정이 반대로 갈린다 |
| **P1 High** | 3 | 같은 화면·같은 라벨의 KPI가 두 값 (CVR·ROAS) · 같은 CSV 셀이 도구마다 다른 숫자로 파싱 |
| **P2 Medium** | 4 | 예산 도구 통화 파싱 누락 · 중복 매핑 무경고 · 음수 분모 · 모드 C 평균효율 배분 |
| **P3 Low** | 1 | 모호 날짜(`03/04/2026`) 무고지 미국식 확정 |
...
### [P2-001] 중복 매핑이 경고 없이 마지막 컬럼만 남긴다
...
→ 매핑 계약 검증에서 중복 표준키를 감지해 사용자에게 알릴 것.

### [P2-002] 음수 분모가 falsy 가드를 통과해 음수 효율을 만든다
`src/utils/metrics/metricRegistry.js:22` `ratio = (num, den) => (den ? ... : null)`.
재현: `impressions: -50` → `CPM = -2000`, `denom: -2` → `CPI = -50`. `den > 0` 가드가 맞다
(노출·클릭·설치는 정의상 음수가 될 수 없다). 매체 리포트의 환불·조정 행에서 실제로 발생 가능.

### [P2-003] 예산 배분 모드 C가 평균 CPR로 배분하면서 결과는 포화 곡선으로 보고한다
...
모드 B(한계효용 그리디)가 별도로 존재하므로 모드 C는 선언된 동작이지만, 화면이 이 차이를
명시하지 않으면 §12.14의 "효율차<1.2면 억지 처방 대신 정직" 원칙과 같은 종류의 문제가 된다.
→ 모드 C 결과에 "평균 효율 기준 배분 — 포화 곡선을 반영하지 않음" 고지 권장.

### [P2-004] `allocParseNum`이 통화기호를 못 읽는다
위 P1-003 표 참조. `parseFloat("₩1000")` → `NaN` → `null`. 해당 채널이 배분에서 제외되는데
화면에는 사유가 없다.
...
### [P3-001] 모호한 숫자형 날짜를 고지 없이 미국식으로 확정
...
**어느 규칙으로 읽었는지 화면이 말하지 않는다.** 유럽식 export에서 3월 4일 ↔ 4월 3일이 뒤집힌다.
→ 모호한 패턴(`A/B/YYYY`, A·B 둘 다 ≤12)이 감지되면 매핑 화면에 "M/D로 읽었습니다" 한 줄.
...
**9. 브라우저 저장 데이터 migration 문제가 있는가?** 결정 스키마는 v9→v10→v11로
구세대 폴백을 목록 함수 안에 두는 패턴(§12.24b)이 확립돼 있고 골든이 있다. 이번 QA에서
migration 결함은 재현하지 못했다 — 다만 **깊이 있게 보지 못한 영역**이므로 "문제 없음"이
아니라 "미검증"으로 보고한다.
...
- `src/utils/__fixtures__/qaFixtures.js` — 재사용 QA 픽스처(손계산 정답·비대칭 가중·지저분한 숫자·14일 효율 CSV·A/B 저전환)
- `src/utils/kpiInvariants.test.js` — **21건 통과**. 골든 2 · 집계 불변식 5 · 퇴화 입력 2 · 숫자 표기 계약 12

**결함 수정은 하지 않았다.** `AGENTS.md` §6.2(감사 흐름)와 §2.7(모호한 결정 임의 확정 금지)에
따라 발견을 재현 가능한 형태로 보고하는 데서 멈췄다. 수정 착수 여부·순서는 사용자 결정 사항이며,
착수 시 위 "Regression test" 블록을 같은 PR에 넣는다.

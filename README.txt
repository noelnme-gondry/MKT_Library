...
================================================================
  ...
  가이드 본문이 index.html 내부 함수로 하드코딩되어 있던 구조를
  외부 JSON 파일로 분리하는 작업을 진행 중입니다.

  ── 디렉토리 구조 ──
    content/
      schema.md            데이터 스키마 정의 (필독)
      pages/
        1-1.json           ✓ 마이그레이션 완료
        ...                미체크 페이지는 index.html 함수 형태로 유지

  ── 신규 페이지 추가 ──
    1) content/pages/{id}.json 파일 생성
       (schema.md 형식 참조)
    ...
    3) 기존 page_{id}() 함수는 fallback으로 둘 수도 있고
       완전히 제거해도 됨 (데이터 로드 실패 시 fallback 사용)

  ── 로컬 테스트 주의 ──
    JSON 비동기 로드는 file:// 프로토콜에서는 CORS로 차단됩니다.
    로컬 검증 시 npm start (또는 npx serve .) 로 서빙해서 확인하세요.
    Railway/Netlify 등 정적 호스팅 환경에서는 정상 작동.
